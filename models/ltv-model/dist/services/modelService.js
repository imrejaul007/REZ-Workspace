...
export class LTVModelService {
     ...
     */
    calculateTimeframePredictions(baseLTV, features) {
     ...
     */
    determineTier(ltv) {
        if (ltv >= TIER_THRESHOLDS.platinum)
            return 'platinum';
        if (ltv >= TIER_THRESHOLDS.gold)
            return 'gold';
        if (ltv >= TIER_THRESHOLDS.silver)
            return 'silver';
        return 'bronze';
    }
    /**
     ...
     */
    calculateFeatureContributions(features, baseLTV) {
        const contributions = [];
    ...
    calculateConfidence(features) {
        ...
        if (features.orderCount > 10)
            confidence += 0.08;
        if (features.orderCount > 50)
            confidence += 0.05;
        // Boost for reasonable activity period
        if (features.daysActive > 30)
            confidence += 0.05;
        if (features.daysActive > 180)
            confidence += 0.05;
        // Boost for consistent retention
        if (features.retentionRate > 0.3)
            confidence += 0.05;
        // Reduce confidence for edge cases
        if (features.totalRevenue === 0 && features.orderCount === 0)
            confidence -= 0.20;
        if (features.daysActive < 7)
            confidence -= 0.15;
        if (features.retentionRate < 0.1)
            confidence -= 0.10;
