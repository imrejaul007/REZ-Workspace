...
exports.getTrendingRecommendations = getTrendingRecommendations;
exports.getSimilarItems = getSimilarItems;
exports.getFrequentlyBoughtRecommendations = getFrequentlyBoughtRecommendations;
exports.getRecommendationsByType = getRecommendationsByType;
exports.recordUserPurchase = recordUserPurchase;
...
async function getPersonalizedRecommendations(userId, limit = DEFAULT_LIMIT) {
    ...
    logger_js_1.logger.info(`Getting personalized recommendations for user: ${userId}`);
    // Get user's purchase history
    const purchasedProductIds = (0, dataStore_js_1.getUserPurchasedProductIds)(userId);
    // Get all products
    const allProducts = (0, dataStore_js_1.getAllProducts)();
    if (purchasedProductIds.size === 0) {
        // Cold start: Return trending items
        logger_js_1.logger.info(`Cold start for user ${userId}, returning trending items`);
        return getTrendingRecommendations(normalizedLimit);
    }
    // Build user preference vector from purchase history
    const purchasedProducts = (0, dataStore_js_1.getProductsByIds)(Array.from(purchasedProductIds));
    const userPreferenceEmbedding = computeAverageEmbedding(purchasedProducts.map(p => p.embedding));
    // Score all products
    ...
    for (const product of allProducts) {
        ...
        const similarityScore = (0, embedding_js_1.cosineSimilarity)(userPreferenceEmbedding, product.embedding);
        // Get trending score (normalized)
        const trendingData = (0, dataStore_js_1.getTrendingItems)(7, allProducts.length);
 ...
 */
async function getTrendingRecommendations(limit = DEFAULT_LIMIT, category) {
    const normalizedLimit = normalizeLimit(limit);
    logger_js_1.logger.info(`Getting trending recommendations, limit: ${normalizedLimit}, category: ${category ?? 'all'}`);
    const trendingData = (0, dataStore_js_1.getTrendingItems)(7, normalizedLimit * 2);
    const maxVelocity = Math.max(...trendingData.map(t => t.velocity), 1);
    const recommendations = [];
    for (const item of trendingData) {
        const product = (0, dataStore_js_1.getProductById)(item.productId);
        if (!product)
            continue;
 ...
 */
async function getSimilarItems(productId, limit = DEFAULT_LIMIT) {
    const normalizedLimit = normalizeLimit(limit);
    logger_js_1.logger.info(`Getting similar items for product: ${productId}`);
    const sourceProduct = (0, dataStore_js_1.getProductById)(productId);
    if (!sourceProduct) {
        logger_js_1.logger.warn(`Product not found: ${productId}`);
        return [];
    }
    const allProducts = (0, dataStore_js_1.getAllProducts)();
    // Find similar products using embedding similarity
    const similarProducts = (0, embedding_js_1.findMostSimilar)(sourceProduct.embedding, allProducts.filter(p => p.id !== productId).map(p => ({
        id: p.id,
        name: p.name,
        embedding: p.embedding,
    })), normalizedLimit);
    const recommendations = similarProducts.map(similar => ({
        id: similar.id,
        name: similar.name,
 ...
 */
async function getFrequentlyBoughtRecommendations(productId, limit = DEFAULT_LIMIT) {
    const normalizedLimit = normalizeLimit(limit);
    logger_js_1.logger.info(`Getting frequently bought together for product: ${productId}`);
    const sourceProduct = (0, dataStore_js_1.getProductById)(productId);
    if (!sourceProduct) {
        logger_js_1.logger.warn(`Product not found: ${productId}`);
        return [];
    }
    const coPurchases = (0, dataStore_js_1.getFrequentlyBoughtTogether)(productId, normalizedLimit);
    const recommendations = [];
    const maxFrequency = Math.max(...coPurchases.map(c => c.frequency), 1);
    for (const cp of coPurchases) {
        const product = (0, dataStore_js_1.getProductById)(cp.productId);
        if (!product)
            continue;
        recommendations.push({
            id: product.id,
 ...
 */
async function getRecommendationsByType(type, userId, productId, limit) {
    const normalizedLimit = normalizeLimit(limit);
    switch (type) {
        case 'personalized':
            if (!userId) {
        ...
        case 'similar':
            if (!productId) {
                logger_js_1.logger.warn('productId required for similar recommendations');
                return [];
            }
            return getSimilarItems(productId, normalizedLimit);
        case 'frequently-bought':
            if (!productId) {
                logger_js_1.logger.warn('productId required for frequently-bought recommendations');
                return [];
            }
            return getFrequentlyBoughtRecommendations(productId, normalizedLimit);
        default:
            logger_js_1.logger.warn(`Unknown recommendation type: ${type}`);
            return [];
    }
}
/**
 ...
 */
function recordUserPurchase(userId, productId, quantity = 1) {
    (0, dataStore_js_1.recordPurchase)(userId, productId, quantity);
    logger_js_1.logger.info(`Recorded purchase: user=${userId}, product=${productId}, qty=${quantity}`);
}
// Helper functions
/**
 ...
 */
function computeCategoryAffinity(category, purchasedProducts) {
 ...
 */
function buildPersonalizationReason(similarity, trending, categoryAffinity) {
    const reasons = [];
    if (similarity > 0.7) {
