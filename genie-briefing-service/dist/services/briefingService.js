...
import { BriefingModel } from '../models/index.js';
 ...
 */
function documentToBriefing(doc) {
    return {
        id: doc.id,
        user_id: doc.user_id,
        type: doc.type,
 ...
 */
function generateSampleItems(sectionType) {
    const items = [];
    switch (sectionType) {
        case 'calendar':
            items.push({
 ...
 */
function generateBriefingSections(briefingType, includeSections) {
    const allSections = ['calendar', 'tasks', 'followups', 'weather', 'insights', 'reminders'];
    const sectionsToInclude = includeSections || allSections;
    // Morning briefings focus on planning, evening on reflection
    const morningSections = ['calendar', 'tasks', 'weather', 'insights'];
    const eveningSections = ['tasks', 'followups', 'insights', 'reminders'];
 ...
 */
function generateSummary(type, sections) {
    const highPriorityCount = sections.reduce((count, section) => count + section.items.filter((item) => item.priority === 'high').length, 0);
 ...
 */
export async function getBriefingByDate(userId, date, briefingType) {
    logger.info('get_briefing_by_date', { userId, date, briefingType });
    const query = { user_id: userId, date };
    if (briefingType) {
 ...
 */
export async function getMorningBriefing(userId, date) {
 ...
 */
export async function getEveningBriefing(userId, date) {
    const targetDate = date || getTodayDate();
 ...
 */
export async function listBriefings(userId, query) {
 ...
 */
export async function createBriefing(userId, input) {
 ...
 * Update a briefing
 */
export async function updateBriefing(id, userId, input) {
 ...
 * Delete a briefing
 */
export async function deleteBriefing(id, userId) {
 ...
 */
export async function updateBriefingItem(briefingId, userId, sectionType, itemId, updates) {
    ...
    if (updates.action_url !== undefined)
        item.action_url = updates.action_url;
    doc.updated_at = new Date();
    await doc.save();
