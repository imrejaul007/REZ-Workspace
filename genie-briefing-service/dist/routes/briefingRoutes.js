...
import { CreateBriefingSchema, UpdateBriefingSchema, GenerateBriefingSchema, GetBriefingQuerySchema, ListBriefingsQuerySchema, } from '../types.js';
...
// ============================================================================
router.get('/evening', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const tenantId = req.tenantContext.tenant_id;
    // Parse optional date query param
    const queryResult = GetBriefingQuerySchema.safeParse(req.query);
...
// ============================================================================
router.patch('/:id/items/:sectionType/:itemId', asyncHandler(async (req, res) => {
    const userId = req.userId;
    const tenantId = req.tenantContext.tenant_id;
    const briefingId = req.params.id;
    ...
    const validSectionTypes = ['calendar', 'tasks', 'followups', 'weather', 'insights', 'reminders'];
    ...
    try {
        const briefing = await briefingService.updateBriefingItem(briefingId, userId, sectionType, itemId, updates);
        if (!briefing) {
            res.status(404).json(createResponse(false, undefined, {
                code: 'BRIEFING_NOT_FOUND',
                message: 'Briefing not found',
