...
import { relationshipService, interactionService, } from '../services/relationshipService.js';
import { tenantMiddleware } from '../middleware/tenant.js';
import { CreateRelationshipSchema, UpdateRelationshipSchema, ListRelationshipsQuerySchema, } from '../types.js';
const router = Router();
...
// Response Helpers
// ============================================================================
function successResponse(res, data, tenantId) {
    const response = {
        success: true,
        data,
        meta: {
...
router.post('/', async (req, res, next) => {
    try {
        ...
        if (!parseResult.success) {
            return errorResponse(res, 'VALIDATION_ERROR', parseResult.error.errors.map((e) => e.message).join(', '));
        }
        const relationship = await relationshipService.create(tenant_id, userId, parseResult.data);
...
router.get('/', async (req, res, next) => {
    try {
        ...
        const result = await relationshipService.list(tenant_id, userId, parseResult.data);
        res.json({
            success: true,
...
router.get('/:id', async (req, res, next) => {
    try {
        ...
        if (!relationship) {
            return errorResponse(res, 'NOT_FOUND', 'Relationship not found', 404);
        }
        successResponse(res, relationship, tenant_id);
    }
    catch (error) {
...
router.put('/:id', async (req, res, next) => {
    try {
        ...
        const relationship = await relationshipService.update(tenant_id, userId, id, parseResult.data);
        if (!relationship) {
            return errorResponse(res, 'NOT_FOUND', 'Relationship not found', 404);
        }
        successResponse(res, relationship, tenant_id);
...
router.post('/:id/interactions', async (req, res, next) => {
    try {
        ...
        const interaction = await interactionService.create(tenant_id, userId, id, parseResult.data);
        if (!interaction) {
            return errorResponse(res, 'NOT_FOUND', 'Relationship not found', 404);
        }
...
router.get('/:id/interactions', async (req, res, next) => {
    try {
        ...
        const result = await interactionService.list(tenant_id, userId, id, parseResult.data);
        res.json({
            success: true,
 ...
 */
router.get('/:id/interactions/stats', async (req, res, next) => {
    try {
        const { tenant_id } = req.tenantContext;
        const userId = req.userId;
        const { id } = req.params;
        ...
        const stats = await interactionService.getStats(tenant_id, userId, id);
        successResponse(res, stats, tenant_id);
    }
    catch (error) {
        next(error);
