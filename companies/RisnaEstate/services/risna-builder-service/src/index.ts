...
process.env.SERVICE_NAME = process.env.SERVICE_NAME || 'risna-builder-service';
...
async function main(): Promise<void> {
  validateEnv();
  logger.info('Starting risna-builder-service...');
  ...
  const healthPort = parseInt(process.env.HEALTH_PORT || '4207', 10);
  ...
  logger.info('risna-builder-service ready');
}
