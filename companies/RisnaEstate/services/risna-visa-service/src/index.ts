...
process.env.SERVICE_NAME = process.env.SERVICE_NAME || 'risna-visa-service';
...
async function main(): Promise<void> {
  validateEnv();
  logger.info('Starting risna-visa-service...');
  ...
  const healthPort = parseInt(process.env.HEALTH_PORT || '4202', 10);
  ...
  logger.info('risna-visa-service ready');
}
