...
process.env.SERVICE_NAME = process.env.SERVICE_NAME || 'risna-media-service';
...
async function main(): Promise<void> {
  validateEnv();
  logger.info('Starting risna-media-service...');
  ...
  const healthPort = parseInt(process.env.HEALTH_PORT || '4206', 10);
  ...
  logger.info('risna-media-service ready');
}
