...
const MONGO_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/rez-retail-pos';
...
app.use(helmet());
app.use(cors({ origin: '*', methods: ['GET', 'POST', 'PUT', 'PATCH'], allowedHeaders: ['Content-Type'] }));
app.use(compression());
...
app.get('/health', (req: Request, res: Response) => {
  res.json({ status: 'healthy', service: 'REZ Retail POS Service', port: PORT, mongodb: mongoose.connection.readyState === 1 ? 'connected' : 'disconnected' });
});
...
app.get('/', (req: Request, res: Response) => {
  res.json({ name: 'REZ Retail POS Service', version: '1.0.0', port: PORT });
});
...
async function bootstrap() {
  try {
    ...
    app.listen(PORT, () => logger.info(`REZ Retail POS Service running on port ${PORT}`));
  } catch (error) {
