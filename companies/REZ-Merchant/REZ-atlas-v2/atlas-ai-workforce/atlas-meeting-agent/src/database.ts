...
const logger = winston.createLogger({
  ...
  defaultMeta: { service: 'atlas-meeting-agent', version: '2.0.0' },
  transports: [
    new winston.transports.Console({
      format: process.env.NODE_ENV === 'production' ? logFormat : winston.format.combine(
        winston.format.timestamp(), winston.format.colorize(), winston.format.simpleText()
      ),
    }),
  ],
  exitOnError: false,
...
export async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/atlas-meeting-agent';
  let retries = 0;
