...
export const helmetMiddleware = helmet({ hsts: { maxAge: 31536000, includeSubDomains: true, preload: true }, frameguard: { action: 'deny' }, noSniff: true, xssFilter: true, hidePoweredBy: true });
...
export const mongoSanitizeMiddleware = mongoSanitize({ onSanitize: ({ key }) => console.warn(`Attempted sanitization of key: ${key}`) });
