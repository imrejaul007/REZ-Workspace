...
async function testGetSyncHistory() {
  ...
  logger.info(`Sync History: ${history.length} records`);
}
...
async function testGetReviewStats() {
  ...
  try {
    ...
    if (products.products && products.products.length > 0) {
      ...
      logger.info('⚠️ Skipping review stats test - no products found');
    }
  } catch (error) {
    logger.info('⚠️ Skipping review stats test - error getting products');
  }
}
...
async function testGetProductReviews() {
  ...
  try {
    ...
    if (products.products && products.products.length > 0) {
      ...
      logger.info('⚠️ Skipping product reviews test - no products found');
    }
  } catch (error) {
    logger.info('⚠️ Skipping product reviews test - error getting products');
  }
}
...
async function testGetProductVariants() {
  try {
    ...
    if (products.products && products.products.length > 0) {
      ...
      logger.info('⚠️ Skipping variants test - no products found');
    }
  } catch (error) {
    logger.info('⚠️ Skipping variants test - error getting products');
  }
}
...
export async function quickTest() {
  ...
  try {
    // Test Sync
    logger.info('1️⃣ Testing Sync Service...');
