...
export const logger = {
  ...
  /**
   * Alias for `info` — added so call-sites ported from raw `console.log`
   * by the codebase-wide console→logger transform (commit 0571cac) don't
   * need to be rewritten. Same behaviour as `info`: dev-only, stripped
   * by `installProductionConsoleGuard` in production.
   */
  ...
  /**
   * Alias for `info` used by call-sites ported from raw `console.debug`.
   * Same dev-only behaviour.
   */
