 ...
 *
 * NOTE on shared-types integration:
 * The canonical product schemas from @rez/shared-types/schemas use
 * `pricing.original` and `pricing.selling` (nested). The local schemas
 * validate flat UI fields (price, compareAtPrice) that map to the canonical
 * nested shape in the service layer. See the comment at the bottom of this
 * file for the current mapping.
 */
...
// Base product shape — canonical shape matching @rez/shared-types CreateProductSchema
// Flat UI field names (price, compareAtPrice) map to canonical pricing.selling, pricing.original
const baseProductShape = {
  ...
  compareAtPrice: z
    ...
    .max(999999.99, 'Original price exceeds maximum limit'),
  sku: z
    .string()
...
// MRP refine — selling (price) must not exceed original (compareAtPrice)
const productMrpRefine = (data: { price: number; compareAtPrice: number }) =>
  data.price <= data.compareAtPrice;

export const createProductSchema = z.object(baseProductShape).refine(productMrpRefine, {
  message: 'Selling price must not exceed original price',
  path: ['price'],
});

export type CreateProductFormData = z.infer<typeof createProductSchema>;

// editProductSchema extends the base object, then re-applies the refine
export const editProductSchema = z
  .object({ ...baseProductShape, id: z.string().min(1, 'Product ID is required') })
  .refine(productMrpRefine, {
    message: 'Selling price must not exceed original price',
    path: ['price'],
  });
...
export const parseValidationErrors = (
  errors: Record<string, { _errors?: string[] } | unknown>
...
// ─── Canonical schemas from @rez/shared-types ────────────────────────────────
//
// Field mapping (local UI ↔ canonical API):
//   formData.price → pricing.selling   (selling price)
//   formData.compareAtPrice → pricing.original  (original/MRP)
//   formData.storeId → store (ObjectId)
//
// The service layer (services/api/products.ts) sends the canonical payload shape:
//   pricing: { selling: number; original: number }
//   store: string
//
// Local schemas above validate flat UI fields; canonical schemas are re-exported
// for type-level usage at the service boundary.
import {
  CreateProductSchema as CanonicalCreateProductSchema,
  UpdateProductSchema as CanonicalUpdateProductSchema,
} from '@rez/shared-types';
export {
  CanonicalCreateProductSchema as CreateProductSchema,
  CanonicalUpdateProductSchema as UpdateProductSchema,
};
