...
export const handleWebImageUpload = (): Promise<{ uri: string; file?: File }[]> => {
  ...
  return new Promise((resolve) => {
    ...
    if (typeof window === 'undefined' || typeof document === 'undefined') {
      logger.warn('Platform: Web image upload not available in this environment');
      resolve([]);
      return;
    }
    ...
    try {
      ...
      input.onchange = (event) => {
        try {
          ...
          logger.error('Platform: Error processing selected files', { error });
          resolve([]);
        }
      };
      ...
      input.onerror = () => {
        logger.error('Platform: Error with file input');
        resolve([]);
      };
      ...
      logger.error('Platform: Error creating file input', { error });
      resolve([]);
    }
  });
};

export const getWebSafeStyles = (styles) => {
  if (isWeb) {
