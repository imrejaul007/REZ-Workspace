  ...
  try {
    if (Platform.OS === 'web') {
      ...
      await WebBrowser.openBrowserAsync(url, {
        presentationStyle: (options?.presentationStyle || 'pageSheet') as unknown,
        toolbarColor: '#3b82f6',
    ...
    logger.error('PDF: Error previewing PDF', { url, error });
    Alert.alert(
      'Preview Error',
...
export const downloadPDF = async (
  ...
  try {
    ...
    logger.error('PDF: Error downloading PDF', { url, filename, error });
...
const downloadPDFWeb = async (url: string, filename: string): Promise<string> => {
  try {
    ...
    logger.error('PDF: Error downloading PDF on web', { url, filename, error });
    throw error;
  }
};
