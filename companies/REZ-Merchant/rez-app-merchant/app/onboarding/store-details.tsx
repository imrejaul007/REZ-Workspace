...
const storeDetailsSchema = z.object({
  ...
  street: z.string().min(1, 'Street address is required').min(5, 'Please enter a complete address'),
    ...
    <View style={styles.selectorContainer}>
      ...
      <View style={styles.optionsRow}>
        {storeTypes.map((type) => (
          <TouchableOpacity
            key={type.value}
            style={[styles.typeCard, selectedStoreType === type.value && styles.typeCardSelected]}
            onPress={() => {
              setSelectedStoreType(type.value);
              setValue('storeType', type.value as 'online' | 'offline' | 'both');
            }}
          >
            <Ionicons
              ...
              size={28}
              color={
                selectedStoreType === type.value ? Colors.light.primary : Colors.light.textSecondary
              }
            />
            <Text
              style={[styles.typeText, selectedStoreType === type.value && styles.typeTextSelected]}
            >
              {type.label}
            </Text>
          </TouchableOpacity>
      ...
      {errors.storeType && <Text style={styles.errorText}>{errors.storeType.message}</Text>}
    </View>
  );
