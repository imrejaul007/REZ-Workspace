...
const bankDetailsSchema = z
  .object({
    ...
    accountType: z.enum(
      ['savings', 'current', 'business'] as const,
...
export default function BankDetailsScreen() {
  ...
  const [ifscDetails, setIfscDetails] = useState<unknown>(null);
      ...
      if (status.data.bankDetails) {
        ...
        Object.keys(bankDetails).forEach((key) => {
          if (key !== 'confirmAccountNumber') {
            setValue(key as keyof BankDetailsFormData, (bankDetails as unknown as Record<string, unknown>)[key] as BankDetailsFormData[keyof BankDetailsFormData]);
          }
        });
  ...
  const validateIFSCCode = async (ifsc: string) => {
    try {
      ...
      logger.error('IFSC validation error:', error);
    } finally {
  ...
  const onSubmit = async (data: BankDetailsFormData) => {
    try {
      ...
      showAlert('Success', 'Bank details saved successfully! Proceed to document upload.', [
    ...
    <View style={styles.selectorContainer}>
      ...
      <View style={styles.optionsRow}>
        {accountTypes.map((type) => (
          <TouchableOpacity
            key={type.value}
            style={[styles.typeCard, selectedAccountType === type.value && styles.typeCardSelected]}
            onPress={() => {
              setSelectedAccountType(type.value);
              setValue('accountType', type.value as 'savings' | 'current' | 'business');
            }}
          >
            <Ionicons
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.pageDescription}>Provide your bank details for receiving payments</Text>
        ...
        <View style={styles.gstSwitch}>
          <View style={styles.switchLabel}>
            ...
            <View style={styles.switchTextContainer}>
              ...
              <Text style={styles.switchDescription}>Is your business registered under GST?</Text>
            </View>
          </View>
          <Switch
            value={gstRegistered}
          ...
          <View style={styles.optionsRow}>
            {taxFilingOptions.map((option) => (
              <TouchableOpacity
                ...
                onPress={() => setValue('taxFilingFrequency', option.value as 'quarterly' | 'monthly' | 'annually')}
              >
                <Text
                  style={[
