...
export default function PendingApprovalScreen() {
  ...
  const updateTimelineFromStatus = (status) => {
    if (!status) return;
  ...
  useEffect(() => {
    ...
    onboardingService
      ...
      .catch((e) => console.warn('[PendingApproval] Failed to fetch onboarding status:', e));
  ...
  const renderSupportOption = (option: (typeof SUPPORT_OPTIONS)[0], index: number) => (
    ...
    >
      <View style={styles.supportIcon}>
        <Ionicons name={option.icon as unknown as keyof typeof Ionicons.glyphMap} size={24} color="#3B82F6" />
      </View>
      ...
      >
        ...
        <LinearGradient colors={['#3B82F6', '#2563EB']} style={styles.successHeader}>
          ...
          <Animatable.View animation="fadeInUp" delay={700} style={styles.applicationIdCard}>
            ...
            <Text style={styles.applicationIdValue}>#{uuidv4().substring(0, 8).toUpperCase()}</Text>
          </Animatable.View>
        </LinearGradient>
