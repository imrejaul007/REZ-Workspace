...
export default function AddEditServiceScreen() {
  ...
  const storePickerItems: PickerItem[] = stores.map((s) => ({
    id: s._id,
    label: s.name,
    subtitle: (s as unknown as { location?: { city?: string; area?: string } }).location?.city || (s as unknown as { location?: { city?: string; area?: string } }).location?.area || undefined,
  }));
  ...
  const loadConsultationForms = async () => {
    try {
      const response = await apiClient.get<unknown>('merchant/consultation-forms');
      if (response.success && response.data) {
        setConsultationForms(response.data.forms || response.data);
      }
    } catch (e) {
      logger.error('Failed to load consultation forms:', e);
    }
  };
  ...
  const loadTeamForPricing = async () => {
    ...
    try {
      ...
      const res = await apiClient.get<unknown>(`merchant/team?storeId=${storeId}&limit=50`);
  ...
  const populateForm = (service: MerchantService) => {
    ...
    setBufferTimeAfter((service.serviceDetails as unknown as { bufferTimeAfter?: number })?.bufferTimeAfter?.toString() || '0');
    ...
    setFreeCancellationHours((service as unknown as { cancellationPolicy?: { freeCancellationHours?: number } }).cancellationPolicy?.freeCancellationHours || 24);
    setLateCancellationFee(((service as unknown as { cancellationPolicy?: { lateCancellationFee?: string } }).cancellationPolicy?.lateCancellationFee as 'none' | 'partial' | 'full') || 'none');
    ...
    const specs = (service as unknown as { specifications?: Array<{ key: string; value: string }> }).specifications || [];
    const getSpec = (key: string) => specs.find((s) => s.key === key)?.value || '';
