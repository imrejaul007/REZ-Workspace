...
import {
  ...
  TableBookingStats,
  Pagination,
} from '@/services/api/tableBookings';
...
export default function AllTableBookingsScreen() {
  ...
  const loadBookings = async (page: number = 1, append: boolean = false) => {
    try {
      ...
      const params: unknown = { page, limit: 50 };
    ...
    <View>
      ...
      <View style={styles.dateFilterRow}>
        ...
        <TouchableOpacity
          style={[
            styles.dateChip,
            selectedDate === new Date().toISOString().split('T')[0] && styles.dateChipActive,
          ]}
        ...
        >
          <Text
            style={[
              styles.dateChipText,
              selectedDate === new Date().toISOString().split('T')[0] && styles.dateChipTextActive,
            ]}
          >
            Today
          </Text>
        ...
        >
          <Text
            style={[
              styles.dateChipText,
              selectedDate === getTomorrowDate() && styles.dateChipTextActive,
            ]}
          >
            Tomorrow
          </Text>
  ...
  const actionLabels = {
    ...
    no_show: {
      title: 'Mark No Show',
      message: 'Mark this customer as a no-show? They did not arrive for their booking.',
    },
  };
    ...
    <View style={styles.container}>
      ...
      {actionModal.action && (
        <ConfirmModal
          ...
          cancelText="Go Back"
          type={
            actionModal.action === 'cancelled' || actionModal.action === 'no_show'
              ? 'danger'
              : 'default'
          }
          ...
          onConfirm={() =>
            actionModal.booking &&
            actionModal.action &&
            handleUpdateStatus(actionModal.booking, actionModal.action)
          }
