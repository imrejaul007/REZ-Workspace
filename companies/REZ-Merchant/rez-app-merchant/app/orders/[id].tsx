...
const StatusUpdateModal = ({
  ...
  // Reset selectedStatus and notes when the modal opens so stale
...
const OrderTimeline = ({ order }: { order: LiveOrder }) => {
  const timelineEvents = [
    ...
    { status: 'ready', timestamp: order.readyAt, label: 'Ready for Pickup/Delivery' },
    { status: 'dispatched', timestamp: null, label: 'Dispatched' },
...
export default function OrderDetailsScreen() {
  ...
  const fetchOrder = useCallback(async () => {
    ...
    try {
      // Use orderService to fetch order details from real order service
      const orderData = await orderService.getOrderById(id);
      setOrder(orderData);
    } catch (error) {
      if (__DEV__) console.error('Error fetching order:', error);
      showAlert('Error', error.message || 'Failed to fetch order details');
      router.back();
    } finally {
      setLoading(false);
    }
  ...
  const updateOrderStatus = async (newStatus: OrderStatus, notes?: string) => {
    ...
    try {
      const updatedOrder = await orderService.updateOrderStatus(order._id || order.id, {
        status: newStatus,
        notes,
        notifyCustomer: true,
      });
  ...
  const handleCancelOrder = async () => {
    ...
    const orderNumber = order.orderNumber || order.orderId?.slice(-6) || orderId.slice(-6);

    // Confirm cancellation
    showConfirm(
      'Cancel Order',
      `Are you sure you want to cancel order #${orderNumber}? This action may not be reversible.`,
      async () => {
  ...
  const formatAddress = (address) => {
    ...
    // Handle fullAddress from LiveOrder
  ...
  // Derive order number from various sources
        ...
        <View style={styles.orderSummary}>
          <View style={styles.summaryRow}>
            ...
            <ThemedText style={styles.summaryValue}>
              ₹{(order.subtotal ?? totalAmount).toFixed(2)}
            </ThemedText>
          </View>
          {(order.discount ?? 0) > 0 && (
            <View style={styles.summaryRow}>
        ...
        <View style={styles.paymentInfo}>
          <View style={styles.paymentRow}>
            ...
            <ThemedText style={styles.paymentValue}>
              {(order.paymentMethod || order.payment?.method || 'unknown').replace('_', ' ').toUpperCase()}
            </ThemedText>
          </View>
          ...
          <View style={styles.paymentRow}>
            ...
            <ThemedText style={styles.paymentValue}>
              {(order.paymentStatus || order.payment?.status || 'pending').toUpperCase()}
            </ThemedText>
          </View>
        </View>
      </View>
          ...
          <View style={styles.refundModalSheet}>
            ...
            <View style={styles.refundOrderInfo}>
              <ThemedText style={styles.refundOrderLabel}>
                Order #{order?.orderNumber || order?.orderId?.slice(-6) || order?._id?.slice(-6) || ''}
              </ThemedText>
