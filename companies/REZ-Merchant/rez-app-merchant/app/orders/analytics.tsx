...
const MetricCard = ({ title, value, icon, color, growth, subtitle }: MetricCardProps) => (
  <View style={[styles.metricCard, { borderLeftColor: color }]}>
    ...
    {growth && (
      <ThemedText
        style={[
          styles.metricGrowth,
          {
            color: growth.startsWith('+') ? '#4CAF50' : growth.startsWith('-') ? '#F44336' : color,
          },
        ]}
      >
        {growth}
    ...
    {subtitle && <ThemedText style={styles.metricSubtitle}>{subtitle}</ThemedText>}
  </View>
);
...
export default function OrderAnalyticsScreen() {
  ...
  const fetchAnalytics = useCallback(async () => {
    try {
      if (__DEV__) logger.log('Fetching order analytics...');
      ...
      if (__DEV__) logger.log('Analytics received', analyticsData);
      setAnalytics(analyticsData);
    } catch (error) {
      if (__DEV__) logger.error('Error fetching analytics', error);
    } finally {
      setLoading(false);
      setRefreshing(false);
    }
