...
export default function BulkActionsScreen() {
  ...
  useEffect(() => {
    if (!hasPermission('products:edit')) {
      showAlert('Permission Denied', 'You do not have permission to perform bulk actions.', [
  ...
  const selectAll = () => {
    setSelectedProducts(new Set(products.map((p) => (p as unknown as { _id?: string })._id || p.id)));
  };
  ...
  const handleUndo = async (historyItemId: string) => {
    showAlert('Undo Action', 'Are you sure you want to undo this action?', [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Undo',
        onPress: async () => {
          try {
            ...
            setActionHistory((prev) =>
              prev.map((item) => (item.id === historyItemId ? { ...item, canUndo: false } : item))
            );
      ...
      <View style={styles.actionGrid}>
        ...
        >
          <Ionicons
            ...
            size={24}
            color={
              bulkAction.type === 'update_price' ? Colors.light.primary : Colors.light.textSecondary
            }
          />
        ...
        <TouchableOpacity
          style={[
            ...
            {
              borderColor:
                bulkAction.type === 'delete' ? Colors.light.destructive : Colors.light.border,
            },
          ]}
        ...
        >
          <Ionicons
            ...
            size={24}
            color={
              bulkAction.type === 'delete' ? Colors.light.destructive : Colors.light.textSecondary
            }
          />
          <ThemedText
            style={[
      ...
      <View style={styles.section}>
        ...
        {bulkAction.type === 'update_price' && (
          <View>
            ...
            <ThemedText style={styles.label}>
              {bulkAction.priceType === 'percentage'
        ...
        {bulkAction.type === 'apply_discount' && (
          <View>
            ...
            <ThemedText style={styles.label}>
              {bulkAction.discountType === 'percentage'
                ? 'Discount Percentage (%)'
                : 'Discount Amount (₹)'}
            </ThemedText>
            <TextInput
  ...
  const renderProductItem = useCallback(
    ({ item }: { item: Product }) => {
      const itemId = (item as unknown as { _id?: string })._id || item.id;
      ...
      return (
        ...
        >
          <View style={styles.productCheckbox}>
            {isSelected && <Ionicons name="checkmark" size={16} color={Colors.light.background} />}
          </View>
          ...
          <View style={styles.productInfo}>
            ...
            <View style={styles.productMeta}>
              ...
              <ThemedText style={styles.productMetaText}>
                Stock: {(item.inventory as unknown as { stock?: number })?.stock || item.inventory?.quantity || 0}
              </ThemedText>
            </View>
          </View>
          ...
          <View
            style={[
              ...
              {
                backgroundColor:
                  (item as unknown as { status?: string }).status === 'active' || item.isActive
                    ? Colors.light.success
                    : Colors.light.warning,
              },
            ]}
          >
            <ThemedText style={styles.productStatusText}>
              {(item as unknown as { status?: string }).status || (item.isActive ? 'active' : 'inactive')}
            </ThemedText>
          </View>
        </TouchableOpacity>
      );
      ...
      <View style={styles.section}>
        ...
        {actionHistory.map((item) => (
          <View key={item.id} style={styles.historyItem}>
            ...
            {item.canUndo && (
              <TouchableOpacity style={styles.undoButton} onPress={() => handleUndo(item.id)}>
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.searchSection}>
        <FormInput name="search" control={control} placeholder="Search products..." icon="search" />

        <View style={styles.filters}>
          <View style={styles.filterItem}>
            <FormInput name="category" control={control} placeholder="Category" />
          </View>
          <View style={styles.filterItem}>
            <FormInput name="status" control={control} placeholder="Status" />
          </View>
        </View>
      </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {/* Product List */}
        <View style={styles.section}>
          <ThemedText style={styles.sectionTitle}>Products ({products.length})</ThemedText>
          ...
          {loading ? (
            ...
            <FlashList
              {...({
                ...
                keyExtractor: (item: Product) => (item as unknown as { _id?: string })._id || item.id,
                scrollEnabled: false,
        ...
        {selectedProducts.size > 0 && bulkAction.type && !isProcessing && (
          <View style={styles.section}>
            <TouchableOpacity
              style={[styles.applyButton, bulkAction.type === 'delete' && styles.applyButtonDelete]}
