...
export default function ProductDetailScreen() {
  ...
  const fetchProduct = async () => {
    ...
    try {
      ...
      // Try catalog service first (real backend), fall back to merchant API
      let data;
      try {
        // Use catalog service with retry logic
        const catalogProduct = await catalogService.getProduct(id);
        data = {
          success: true,
          data: catalogProduct,
        };
        logger.info('📦 [PRODUCT] Product loaded from catalog service:', {
          id: catalogProduct.id || catalogProduct._id,
          name: catalogProduct.name,
        });
      } catch (catalogError) {
        // Fall back to merchant API
        devWarn('[PRODUCT] Catalog service failed, using merchant API:', catalogError);
        data = await apiClient.get(`merchant/products/${id}`);
      }
  ...
  const handleDeleteConfirmed = async (productId: string) => {
    ...
    try {
      logger.info('🗑️ [DELETE] Calling catalogService.deleteProduct with retry logic...');

      // Use catalog service with retry logic, fall back to merchant API
      try {
        await catalogService.deleteProduct(productId, product?.merchantId || '');
        logger.info('✅ [DELETE] Product deleted successfully via catalog service');
      } catch (catalogError) {
        // Fall back to merchant API
        devWarn('[DELETE] Catalog service delete failed, using merchant API:', catalogError);
        await productsService.deleteProduct(productId);
        logger.info('✅ [DELETE] Product deleted successfully via merchant API');
      }
  ...
  const renderImageGallery = () => {
    ...
    if (validImages.length === 0 && singleImage) {
      return (
        <Animated.View entering={FadeIn.duration(300)} style={styles.imageContainer}>
          <Image source={{ uri: singleImage }} style={styles.productImage} resizeMode="cover" />
        </Animated.View>
      );
    }
      ...
      <Animated.View entering={FadeIn.duration(300)} style={styles.imageContainer}>
        ...
        >
          {validImages.map((image, index) => {
            ...
            const imageUrl =
              typeof image === 'string' ? debugImageUrl(image) : debugImageUrl(image.url || '');
            return (
              <Image
                key={index}
                source={{ uri: imageUrl }}
            ...
            <View style={styles.productHeader}>
              ...
              <View style={styles.priceContainer}>
                <ThemedText style={styles.price}>
                  ₹
                  {(
                    Number(product.pricing?.selling) ||
              ...
              <View style={styles.inventoryGrid}>
                <View style={styles.inventoryItem}>
                  ...
                  <ThemedText style={styles.inventoryValue}>
                    {product.inventory.stock ?? product.inventory.quantity ?? 0}
                  </ThemedText>
                </View>
                <View style={styles.inventoryItem}>
          ...
          <Animated.View entering={FadeInUp.delay(400).springify()} style={styles.section}>
            ...
            <View style={styles.detailRow}>
              ...
              <ThemedText style={styles.detailValue}>
                {(product.category as { name?: string })?.name ||
                  (typeof product.category === 'string' ? 'N/A' : 'N/A')}
              </ThemedText>
            </View>
            {product.subcategory && (
              <View style={styles.detailRow}>
            ...
            {product.productType && (
              <View style={styles.detailRow}>
                ...
                <ThemedText style={styles.detailValue}>{product.productType}</ThemedText>
              </View>
            )}
          </Animated.View>
