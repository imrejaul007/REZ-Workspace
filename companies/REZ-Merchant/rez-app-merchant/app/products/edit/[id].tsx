...
export default function EditProductScreen() {
  ...
  const loadProduct = async () => {
    try {
      ...
      // Note: getProduct returns ProductSummary; cast to unknown first, then Partial<Product>
      const product = (await productsService.getProduct(id as string)) as unknown as Partial<Product>;
      logger.info('✏️ [EDIT PAGE] Product loaded:', product);
      logger.info('✏️ [EDIT PAGE] Product inventory:', product.inventory);
      logger.info('✏️ [EDIT PAGE] Stock:', {
        stock: product.inventory?.quantity,
        lowThreshold: product.inventory?.lowStockThreshold,
      });

      if (product) {
        const extra = product as unknown as Record<string, unknown>;
        const extraPricing = extra.pricing as Record<string, unknown> | undefined;
        const extraInventory = extra.inventory as Record<string, unknown> | undefined;
        const extraSeo = extra.seo as Record<string, unknown> | undefined;
        const extraPrice = extra.price as Record<string, unknown> | undefined;
        const extraCashback = extra.cashback as Record<string, unknown> | undefined;
        const extraStore = extra.store as string | Record<string, unknown> | undefined;

        setFormData({
          name: product.name || '',
          description: product.description || '',
          shortDescription: (extra.shortDescription as string) || '',
          sku: (extra.sku as string) || '',
          barcode: (extra.barcode as string) || '',
          category: typeof product.category === 'string' ? product.category : '',
          subcategory: (extra.subcategory as string) || '',
          brand: (extra.brand as string) || '',
          price:
            product.pricing?.selling?.toString() ||
            (extraPrice?.current as number)?.toString() ||
            '',
          costPrice:
            product.pricing?.cost?.toString() || (extra.costPrice as string) || '',
          compareAtPrice:
            (extraPricing?.original as number)?.toString() ||
            (extraPrice?.original as number)?.toString() ||
            '',
          currency: product.pricing?.currency || 'INR',
          stock: product.inventory?.quantity?.toString() || '0',
          lowStockThreshold: product.inventory?.lowStockThreshold?.toString() || '5',
          trackInventory: extraInventory?.isAvailable !== false,
          allowBackorders: extraInventory?.allowBackorder === true,
          weight: (extra.weight as number)?.toString() || '',
          dimensions: {
            length: (extra.dimensions as Record<string, number>)?.length?.toString() || '',
            width: (extra.dimensions as Record<string, number>)?.width?.toString() || '',
            height: (extra.dimensions as Record<string, number>)?.height?.toString() || '',
            unit: (extra.dimensions as Record<string, string>)?.unit as unknown as 'cm' | 'inch' || 'cm',
          },
          metaTitle: (extraSeo?.title as string) || (extra.metaTitle as string) || '',
          metaDescription: (extraSeo?.description as string) || (extra.metaDescription as string) || '',
          searchKeywords:
            product.seo?.keywords?.join(', ') ||
            ((extraSeo?.keywords as string[])?.join(', ')) ||
            ((extra.searchKeywords as string[])?.join(', ')) ||
            '',
          tags: product.tags?.join(', ') || '',
          status: extra.isActive ? 'active' : 'inactive',
          visibility: product.visibility?.featured ? 'featured' : 'public',
          cashbackPercentage: extraCashback?.percentage?.toString() || '0',
          cashbackMaxAmount: extraCashback?.maxAmount?.toString() || '',
          cashbackActive: extraCashback?.percentage !== undefined && (extraCashback.percentage as number) > 0,
          hsnCode: (extraPricing?.gst as Record<string, unknown>)?.hsnCode as string || '',
          sacCode: (extraPricing?.gst as Record<string, unknown>)?.sacCode as string || '',
          gstRate: (extraPricing?.gst as Record<string, number>)?.gstRate?.toString() || '',
          taxSlab: (extraPricing?.gst as Record<string, unknown>)?.taxSlab as unknown as ProductFormData['taxSlab'] || '',
        });
        ...
        logger.info('✏️ [EDIT PAGE] Form data populated:', {
          ...
          isFeatured: product.visibility?.featured,
        ...
        if (product.images && Array.isArray(product.images) && product.images.length > 0) {
          type ApiImage = { url: string; altText?: string; isPrimary?: boolean; order?: number };
          const productImages: ProductImage[] = (product.images as Array<string | ApiImage>).map((img: string | ApiImage, index: number) => ({
            uri: typeof img === 'string' ? img : img.url,
            url: typeof img === 'string' ? img : img.url,
            altText: typeof img === 'object' ? (img as ApiImage).altText ?? '' : '',
            isMain: typeof img === 'object' ? (img as ApiImage).isPrimary ?? index === 0 : index === 0,
            sortOrder: typeof img === 'object' ? (img as ApiImage).order ?? index : index,
            uploading: false,
          }));
          setImages(productImages);
        } else if (extra.image) {
          const fallbackImage = extra.image as string;
          setImages([
            {
              uri: fallbackImage,
              url: fallbackImage,
        ...
        // Populate videos
        const videos = extra.videos as Array<Record<string, unknown>> | undefined;
        if (videos && Array.isArray(videos)) {
          const productVideos: ProductVideo[] = videos.map(
            (video, index) => ({
              ...
              sortOrder: (video.sortOrder as number) ?? index,
              uploading: false,
            })
          );
        ...
        if (extra.storeId) {
          setSelectedStoreId(extra.storeId as string);
        } else if (extraStore) {
          const storeId =
            typeof extraStore === 'string'
              ? extraStore
              : ((extraStore as Record<string, unknown>)?._id || (extraStore as Record<string, unknown>)?.id) as string | undefined;
          if (storeId) {
  ...
  const validateForm = (): boolean => {
    ...
    if (
      formData.costPrice &&
      (isNaN(Number(formData.costPrice)) || Number(formData.costPrice) < 0)
    ) {
    ...
    if (
      ...
      (isNaN(Number(formData.compareAtPrice)) || Number(formData.compareAtPrice) < 0)
    ) {
    ...
    if (
      ...
      (isNaN(Number(formData.dimensions.length)) || Number(formData.dimensions.length) < 0)
    ) {
    ...
    if (
      ...
      (isNaN(Number(formData.dimensions.width)) || Number(formData.dimensions.width) < 0)
    ) {
    ...
    if (
      ...
      (isNaN(Number(formData.dimensions.height)) || Number(formData.dimensions.height) < 0)
    ) {
    ...
    if (
      ...
      (isNaN(Number(formData.cashbackMaxAmount)) || Number(formData.cashbackMaxAmount) < 0)
    ) {
    ...
    try {
      const productRequest: unknown = {
        ...
        pricing: {
          ...
          original: formData.compareAtPrice
          ...
          // GST fields
          gst:
            formData.hsnCode || formData.sacCode || formData.gstRate || formData.taxSlab
              ? {
  ...
  const basicInfoErrors = getSectionErrors([
    'name',
    'description',
    'shortDescription',
    'sku',
  ...
  const pricingErrors = getSectionErrors([
    'price',
    'costPrice',
    'compareAtPrice',
    'currency',
  ...
  const physicalErrors = getSectionErrors([
    'weight',
    'dimensions.length',
    'dimensions.width',
    'dimensions.height',
      ...
      >
        ...
        >
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              ...
              textAlignVertical="top"
            />
            <ThemedText style={styles.charCount}>{formData.shortDescription.length}/300</ThemedText>
            {errors.shortDescription && (
              <ThemedText style={styles.errorText}>{errors.shortDescription}</ThemedText>
            )}
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            />
            {errors.subcategory && (
              <ThemedText style={styles.errorText}>{errors.subcategory}</ThemedText>
            )}
          </View>
        ...
        >
          ...
          <View style={styles.formGroup}>
            ...
            />
            {errors.costPrice && (
              <ThemedText style={styles.errorText}>{errors.costPrice}</ThemedText>
            )}
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            {errors.compareAtPrice && (
              <ThemedText style={styles.errorText}>{errors.compareAtPrice}</ThemedText>
            )}
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            {errors.cashbackPercentage && (
              <ThemedText style={styles.errorText}>{errors.cashbackPercentage}</ThemedText>
            )}
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              ...
              keyboardType="decimal-pad"
            />
            {errors.cashbackMaxAmount && (
              <ThemedText style={styles.errorText}>{errors.cashbackMaxAmount}</ThemedText>
            )}
          </View>
        ...
        >
          ...
          <View style={styles.formGroup}>
            ...
            {errors.lowStockThreshold && (
              <ThemedText style={styles.errorText}>{errors.lowStockThreshold}</ThemedText>
            )}
          </View>
        ...
        >
          <View style={styles.formGroup}>
            ...
            <ThemedText style={styles.helpText}>
              Optimized for search engines (max 60 chars)
            </ThemedText>
            <TextInput
              ...
              maxLength={60}
            />
            <ThemedText style={styles.charCount}>{formData.metaTitle.length}/60</ThemedText>
            {errors.metaTitle && (
              <ThemedText style={styles.errorText}>{errors.metaTitle}</ThemedText>
            )}
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            <TextInput
              ...
              textAlignVertical="top"
            />
            <ThemedText style={styles.charCount}>{formData.metaDescription.length}/160</ThemedText>
            {errors.metaDescription && (
              <ThemedText style={styles.errorText}>{errors.metaDescription}</ThemedText>
            )}
          </View>
          ...
          <View style={styles.formGroup}>
            ...
            {errors.searchKeywords && (
              <ThemedText style={styles.errorText}>{errors.searchKeywords}</ThemedText>
            )}
          </View>
        ...
        <CollapsibleSection title="Visibility & Status" icon="eye" defaultExpanded={true} required>
          <View style={styles.formGroup}>
            <FormSelect
              label="Status"
              value={formData.status}
              onValueChange={(value) => updateFormData('status', value as 'draft' | 'active' | 'inactive' | 'archived')}
              options={[
          ...
          <View style={styles.formGroup}>
            <FormSelect
              ...
              onValueChange={(value) => updateFormData('visibility', value as 'public' | 'hidden' | 'featured')}
              options={[
        ...
        {/* Section: GST / Tax */}
        <CollapsibleSection title="GST & Tax" icon="receipt" defaultExpanded={false}>
          <View style={styles.formGroup}>
          ...
          <View style={styles.formGroup}>
            <FormSelect
              ...
              onValueChange={(value) => {
                updateFormData('taxSlab', value as '0' | '5' | '12' | '18' | '28' | 'exempt' | '');
