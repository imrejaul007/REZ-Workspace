...
export default function StoreReviewsScreen() {
  ...
  const [filter, setFilter] = useState<
    'all' | 'with_images' | 'verified' | '5' | '4' | '3' | '2' | '1'
  >('all');
  const [sort, setSort] = useState<'newest' | 'oldest' | 'rating_high' | 'rating_low' | 'helpful'>(
    'newest'
  );
  ...
  const loadReviews = async () => {
    try {
      ...
      if (response.reviews) {
        setReviews((response.reviews || []) as unknown as Review[]);
        setStats((response.stats || null) as unknown as ReviewStats);
      } else {
  ...
  const handleApproveReview = async (reviewId: string) => {
    setConfirmModal({
      ...
      message:
        'Are you sure you want to approve this review? The user will receive 10 rezcoins as a reward.',
      type: 'default',
      onConfirm: async () => {
              ...
              {stats.totalReviews > 0 && (
                <View style={styles.breakdown}>
                  ...
                  {[5, 4, 3, 2, 1].map((rating) => {
                    const count =
                      stats.ratingBreakdown[rating as keyof typeof stats.ratingBreakdown] || 0;
                    const percentage =
          ...
          <View style={styles.filtersSection}>
            ...
            >
              {(['newest', 'oldest', 'rating_high', 'rating_low'] as const).map((s) => (
                ...
                >
                  <ThemedText
                    style={[styles.filterPillText, sort === s && styles.filterPillTextActive]}
                  >
        ...
        {error && (
          <Animated.View entering={FadeInUp} style={styles.errorContainer}>
            <Card style={styles.errorCard} padding="md">
              ...
              >
                <BodyText
                  style={{ color: Colors.text.inverse, fontWeight: Typography.fontWeight.semiBold }}
                >
                  Retry
                </BodyText>
              </TouchableOpacity>
          ...
          reviews.map((review, index) => (
            ...
            >
              <Card style={styles.reviewCard} padding="lg">
                ...
                {/* Review Content */}
                {review.title && <Heading3 style={styles.reviewTitle}>{review.title}</Heading3>}
