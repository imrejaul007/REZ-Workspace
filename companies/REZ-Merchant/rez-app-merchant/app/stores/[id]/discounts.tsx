...
export default function StoreDiscountsScreen() {
  ...
  const createButtonText = isCardMode
    ? 'Create Your First Card Offer'
      ...
      <SafeAreaView style={styles.container}>
        ...
        <View style={styles.loadingContainer}>
          ...
          <Text style={styles.loadingText}>
            Loading {isCardMode ? 'card offers' : 'discounts'}...
          </Text>
        </View>
      </SafeAreaView>
    );
      ...
      >
        ...
        {!error && discounts.length === 0 && (
          <View style={styles.emptyContainer}>
            <Ionicons
              name={isCardMode ? 'card-outline' : 'pricetag-outline'}
              size={64}
              color="#9CA3AF"
            />
          ...
          discounts.map((discount) => {
            ...
            return (
              <View key={discount._id} style={styles.discountCard}>
                ...
                {(discount as MerchantDiscount).paymentMethod && (discount as MerchantDiscount).paymentMethod !== 'all' && (
                  <View style={styles.paymentMethodBadge}>
                    <Ionicons
                      name={((discount as MerchantDiscount).paymentMethod === 'card' ? 'card' : 'flash') as unknown as keyof typeof Ionicons.glyphMap}
                      size={14}
                      color="#8B5CF6"
                    />
                    <Text style={styles.paymentMethodText}>
                      {(discount as MerchantDiscount).paymentMethod === 'card' ? 'Card Offer' : 'UPI Offer'}
                    </Text>
                  </View>
                )}
                ...
                <View style={styles.discountDetails}>
                  ...
                  <View style={styles.discountDetailRow}>
                    ...
                    <Text style={styles.discountDetailText}>
                      {discount.usageLimitPerUser} use{discount.usageLimitPerUser > 1 ? 's' : ''}{' '}
                      per user
