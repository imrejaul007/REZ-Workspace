...
const dealSchema = z.object({
  ...
  cashbackPercentage: z
    ...
    .refine((val) => parseFloat(val) >= 0 && parseFloat(val) <= 100, 'Must be between 0 and 100'),
...
export default function EditDealScreen() {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        reset({
          ...
          type: dealData.type as 'cashback' | 'discount' | 'voucher' | 'combo' | 'special' | 'walk_in',
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets[0]) {
        ...
        try {
          ...
          const uploadResult = await uploadsService.uploadImage(
            asset.uri,
            undefined,
            'general',
            fileObject
  ...
  const onSubmit = async (data: DealFormData) => {
    try {
      ...
      const updateData: Partial<unknown> = {
