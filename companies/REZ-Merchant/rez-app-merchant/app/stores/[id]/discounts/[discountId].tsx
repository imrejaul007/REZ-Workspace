...
export default function EditDiscountScreen() {
  ...
  const loadDiscount = async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        let paymentMethodValue = (discountData as unknown as Record<string, unknown>).paymentMethod as string || 'all';
        ...
        reset({
          ...
          notValidAboveStoreDiscount:
            discountData.restrictions?.notValidAboveStoreDiscount || false,
          ...
          paymentMethod: paymentMethodValue as 'upi' | 'card' | 'all',
          cardType: ((discountData as unknown as Record<string, unknown>).cardType as 'credit' | 'debit' | 'all' | undefined) || 'all',
          bankNames: ((discountData as unknown as Record<string, unknown>).bankNames as string[] | undefined)?.join(', ') || '',
          cardBins: ((discountData as unknown as Record<string, unknown>).cardBins as string[] | undefined)?.join(', ') || '',
        });
      } else {
        setAlertModal({
          visible: true,
  ...
  const onSubmit = async (data: DiscountFormData) => {
    try {
      ...
      let applicableOnValue: 'bill_payment' | 'card_payment' =
        discount?.applicableOn || 'bill_payment';
        ...
        <View style={styles.storeInfo}>
          ...
          <View style={styles.statusContainer}>
            <View
              style={[
                styles.statusBadge,
                isCurrentlyValid() ? styles.statusActive : styles.statusInactive,
              ]}
            >
              <Ionicons
              ...
              />
              <Text
                style={[styles.statusText, { color: isCurrentlyValid() ? '#10B981' : '#EF4444' }]}
              >
