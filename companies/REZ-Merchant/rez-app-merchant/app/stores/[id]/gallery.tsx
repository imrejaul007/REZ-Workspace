...
const showSuccess = (title: string, message: string, setSuccess, setShow) => {
  setSuccess({ title, message });
  setShow(true);
};

const showError = (title: string, message: string, setError, setShow) => {
  setError({ title, message });
  setShow(true);
};

// Conditional import for drag functionality
let DraggableFlatList: unknown = null;
let RenderItemParams: unknown = null;
...
if (Platform.OS !== 'web') {
  try {
    ...
    if (__DEV__) logger.log('Draggable FlatList not available for this platform');
  }
}
...
export default function StoreGalleryScreen() {
  ...
  const handleBulkUpload = async (assets: unknown[]) => {
    try {
      setUploading(true);
  ...
  const handleSetCover = async (item: GalleryItem) => {
    try {
      ...
      showError(
        'Error',
        error.message || 'Failed to set cover image',
        setErrorMessage,
        setShowErrorModal
      );
    } finally {
  ...
  const handleDragEnd = async ({ data }: { data: GalleryItem[] }) => {
    try {
      ...
      showError(
        'Error',
        'Failed to reorder items. Reloading...',
        setErrorMessage,
        setShowErrorModal
      );
      await loadGallery();
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      showSuccess(
        'Success',
        `${itemIds.length} items deleted successfully!`,
        setSuccessMessage,
        setShowSuccessModal
      );
    } catch (error) {
  ...
  const handleBulkVisibilityToggle = async (isVisible: boolean) => {
    ...
    try {
      ...
      showSuccess(
        'Success',
        `${selectedItems.size} items ${isVisible ? 'shown' : 'hidden'} successfully!`,
        setSuccessMessage,
        setShowSuccessModal
      );
    } catch (error) {
  ...
  const handleQuickVisibilityToggle = async (item: GalleryItem) => {
    try {
      ...
      showError(
        'Error',
        error.message || 'Failed to toggle visibility',
        setErrorMessage,
        setShowErrorModal
      );
    }
  ...
  const calculateAnalytics = () => {
    ...
    const categoryCounts = galleryItems.reduce(
      (acc, item) => {
  ...
  const handleCreateCategory = async () => {
    ...
    if (
      GALLERY_CATEGORIES.includes(categoryName) ||
    ...
    showSuccess(
      'Success',
      `Category "${categoryName}" will be created when you upload the first item to it.`,
      setSuccessMessage,
      setShowSuccessModal
    );
  };
      ...
      >
        ...
        {!isSelectionMode && !dragEnabled && (
          <View style={styles.quickActionsOverlay}>
            ...
            >
              <Ionicons name={item.isVisible ? 'eye' : 'eye-off'} size={16} color="#FFF" />
            </TouchableOpacity>
            <TouchableOpacity style={styles.quickActionButton} onPress={() => handleSetCover(item)}>
              <Ionicons name={item.isCover ? 'star' : 'star-outline'} size={16} color="#FFF" />
            </TouchableOpacity>
            <TouchableOpacity
    ...
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      {galleryItems.length === 0 && !loading ? (
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity style={styles.emptyButton} onPress={() => setShowUploadModal(true)}>
        ...
        <>
          {Platform.OS === 'web' || !DraggableFlatList || !dragEnabled ? (
            <FlatList
              data={galleryItems}
              renderItem={({ item }) =>
                renderGalleryItem({ item, drag: undefined, isActive: false })
              }
                      ...
                      <View style={styles.analyticsSection}>
                        ...
                        <View style={styles.analyticsItemCard}>
                          <Image
                            source={{
                              uri:
                                analytics.mostViewed.type === 'video'
                                  ? analytics.mostViewed.thumbnail || analytics.mostViewed.url
                                  : analytics.mostViewed.url,
                            }}
