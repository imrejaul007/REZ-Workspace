...
function deleteDirectory(dirPath, retries = 3) {
  ...
  for (let i = 0; i < retries; i++) {
    try {
      ...
      logger.info('Metro cache cleared successfully!');
      return;
    } catch (error) {
      if (i === retries - 1) {
        logger.warn(`Warning: Could not fully clear cache at ${dirPath}`);
        logger.warn('You may need to manually delete it or close any processes using it.');
      }
    }
  }
}
...
if (fs.existsSync(expoCachePath)) {
  logger.info('Clearing .expo cache...');
