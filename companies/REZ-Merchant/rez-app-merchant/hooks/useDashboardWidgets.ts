...
export const useDashboardWidgets = () => {
  ...
  const loadLayouts = useCallback(async () => {
    try {
      ...
      if (layoutsData) {
        try {
          parsedLayouts = JSON.parse(layoutsData).map((layout) => ({
            ...layout,
            createdAt: new Date(layout.createdAt),
            updatedAt: new Date(layout.updatedAt),
          }));
        } catch (e) {
          logger.error('[DashboardWidgets] Failed to parse layouts from storage:', e);
          parsedLayouts = [];
        }
      }
      ...
      logger.error('Error loading dashboard layouts:', error);
  ...
  const saveLayouts = async (layoutsToSave: DashboardLayout[]) => {
    try {
      ...
      logger.error('Error saving dashboard layouts:', error);
    }
  };
  ...
  const saveActiveLayoutId = async (layoutId: string) => {
    try {
      ...
      logger.error('Error saving active layout ID:', error);
    }
  };
  ...
  const importLayout = useCallback(
    async (config, name?: string) => {
      try {
        ...
        logger.error('Error importing layout:', error);
