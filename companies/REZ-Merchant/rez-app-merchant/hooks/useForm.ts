 ...
 */
export const useForm = <T extends FieldValues = unknown>({
  schema,
  onSubmit,
  onError,
  ...config
  ...
  const form = useReactHookForm<T>({
    resolver: schema ? (zodResolver as unknown)(schema) : undefined,
  ...
  const clearFieldError = useCallback(
    (fieldName: keyof T) => {
      clearErrors(fieldName as unknown);
    },
    [clearErrors]
  );
...
export const useFormValidation = () => {
   ...
   */
  const validateData = useCallback(async (data, schema: ZodSchema) => {
    try {
   ...
   */
  const validateField = useCallback(async (fieldName: string, value, schema: ZodSchema) => {
    try {
      await (schema as unknown).pick?.({ [fieldName]: true })?.parseAsync?.({ [fieldName]: value });
...
export const useAsyncFormSubmit = <T extends FieldValues>(onSubmit: (data: T) => Promise<void>) => {
  ...
  return {
    ...form,
    onSubmit: handleSubmit(submitAsync as unknown),
  };
};
