...
export default function ImageUploader({
  ...
  const pickImages = async () => {
    try {
      ...
      if (__DEV__) logger.error('Error picking images', error);
  ...
  const uploadImages = async (imagesToUpload: ProductImage[], allImages: ProductImage[]) => {
    ...
    for (const image of imagesToUpload) {
      try {
        if (__DEV__) logger.log('Uploading image', { uri: image.uri });
        ...
        if (__DEV__) logger.log('Upload result', { result });

        if (result && result.length > 0 && result[0].url) {
          // Update with URL
          if (__DEV__) logger.log('Image uploaded successfully', { url: result[0].url });
          updateImageUrl(image.uri, result[0].url, false);
        } else {
          if (__DEV__) logger.error('Invalid upload result', { result });
          updateImageError(image.uri, 'Upload completed but no URL received');
        }
      } catch (error) {
        if (__DEV__) logger.error('Upload error', error);
  ...
  const updateImageUrl = (uri: string, url: string, uploading: boolean) => {
    if (__DEV__) logger.log('Updating image URL', { uri, url, uploading });
    const updated = images.map(img =>
      img.uri === uri ? { ...img, url, uploading, uploadProgress: 100, error: undefined } : img
    );
    if (__DEV__) logger.log('Updated images array', { count: updated.length });
    onImagesChange(updated);
  };

  const updateImageError = (uri: string, error: string) => {
    if (__DEV__) logger.error('Image upload error', { uri, error });
    onImagesChange(
      images.map(img =>
  ...
  const removeImage = (index: number) => {
    if (index < 0 || index >= images.length) {
      if (__DEV__) logger.error('[IMAGE] Invalid index for removal', { index });
      return;
    }
    ...
    showConfirm(
      ...
      () => {
        if (__DEV__) logger.log('[IMAGE] Removing image', { index, uri: imageToRemove.uri, url: imageToRemove.url });
        if (__DEV__) logger.log('[IMAGE] Images before removal', { count: images.length });

        // Remove image by index
        let updatedImages = images.filter((_, i) => i !== index);

        if (__DEV__) logger.log('[IMAGE] Images after removal', { count: updatedImages.length });
          ...
          <View key={`${image.uri || image.url || index}-${index}`} style={styles.imageCard}>
            ...
            <View style={styles.imageContainer}>
              ...
              <TouchableOpacity
                ...
                onPress={() => {
                  if (__DEV__) logger.log('[IMAGE] Remove button clicked', { index, uri: image.uri, url: image.url });
