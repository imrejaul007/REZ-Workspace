...
const VariantForm: React.FC<VariantFormProps> = ({
  ...
  const handleImageSelect = async () => {
    try {
      ...
      if (__DEV__) logger.error('Error picking image', error);
  ...
  const uploadImage = async (imageUri: string) => {
    try {
      ...
      if (__DEV__) logger.log('Uploading variant image', { uri: imageUri });
      ...
      if (__DEV__) logger.log('Variant image uploaded successfully', { url: result.url });

      setUploadedImageUrl(result.url);
      showAlert('Success', 'Image uploaded successfully');
    } catch (error) {
      if (__DEV__) logger.error('Failed to upload variant image', error);
