...
export class ErrorBoundary extends React.Component<ErrorBoundaryProps, ErrorBoundaryState> {
  ...
  handleRetry = () => {
    ...
    if (errorCount < this.maxRetries) {
      ...
      logError(
        parseError(new Error('Max retries exceeded')) as unknown,
...
export const useErrorBoundary = (): UseErrorBoundaryReturn => {
  ...
  const handleError = React.useCallback((err: Error) => {
    logError(parseError(err) as unknown, 'useErrorBoundary');
    setError(err);
  }, []);
...
export const ErrorBoundaryProvider = ({ children }: ErrorBoundaryProviderProps) => {
  ...
  const handleSetError = (err: Error) => {
    logError(parseError(err) as unknown, 'ErrorBoundaryProvider');
    setError(err);
  };
...
export const AsyncErrorBoundary = ({ children, onError }: AsyncErrorBoundaryProps) => {
  ...
  const handleAsyncError = React.useCallback(
    (error: Error) => {
      logError(parseError(error) as unknown, 'AsyncErrorBoundary');
      onError?.(error);
      setError(error);
    },
    [setError, onError]
