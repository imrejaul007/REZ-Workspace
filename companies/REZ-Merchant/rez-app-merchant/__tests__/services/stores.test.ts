...
beforeAll(async () => {
  const mod = require('../../services/api/stores');
  storeService = mod.default;
});
...
describe('StoreService', () => {
  ...
  it('activateStore activates', async () => {
    mockPost.mockResolvedValue({ success: true, data: { ...MOCK_STORE, isActive: true } });
    await storeService.activateStore('s1');
    expect(mockPost).toHaveBeenCalled();
  });

  it('deactivateStore deactivates', async () => {
    mockPost.mockResolvedValue({ success: true, data: { ...MOCK_STORE, isActive: false } });
