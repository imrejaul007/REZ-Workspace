...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ success: true, data: { service: 'REZ Competitive Intelligence', version: '1.0.0', status: 'healthy', timestamp: new Date().toISOString() } });
});

app.get('/ready', async (_req: Request, res: Response) => {
  if (mongoose.connection.readyState === 1) res.json({ success: true, data: { status: 'ready' } });
...
async function startServer(): Promise<void> {
  try {
    ...
    app.listen(config.port, '0.0.0.0', () => {
      logger.info(`Competitive Intelligence Service started on port ${config.port}`);
    });
