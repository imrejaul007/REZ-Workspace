#!/bin/bash
# =============================================================================
# REZ-Merchant Deployment Script
# Deploys all merchant-facing services to Render
# =============================================================================
...
LOG_FILE="${SCRIPT_DIR}/logs/deploy-rez-merchant_${TIMESTAMP}.log"
...
INDUSTRY_SERVICES=(
    "industry-os/restauranthub"
    "industry-os/rez-hotel-service"
)

ALL_SERVICES=("${MERCHANT_SERVICES[@]}" "${INDUSTRY_SERVICES[@]}")
...
check_prerequisites() {
    log_header "REZ-MERCHANT PRE-DEPLOYMENT CHECKS"
...
main() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                    REZ-MERCHANT DEPLOYMENT                     ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    log_header "DEPLOYING ${total_services} MERCHANT SERVICES"
    ...
    # Print summary
    log_header "DEPLOYMENT SUMMARY - REZ-MERCHANT"
    ...
    if [ $FAILED_COUNT -eq 0 ]; then
        log_success "All REZ-Merchant services deployed successfully!"
    else
