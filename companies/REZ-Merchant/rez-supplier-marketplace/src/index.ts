...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ success: true, data: { service: 'REZ Supplier Marketplace', version: '1.0.0', status: 'healthy', timestamp: new Date().toISOString() } });
});
...
async function startServer(): Promise<void> {
  try {
    ...
    app.listen(config.port, '0.0.0.0', () => {
      logger.info(`Supplier Marketplace Service started on port ${config.port}`);
    });
