...
export class RestaurantWhatsAppService {
  ...
  async sendNewOrderAlert(
    ...
    if (order.specialInstructions) {
      message += `\n\n📝 Note: ${sanitizeInput(order.specialInstructions)}`;
    }
  ...
  /**
   * Send new review alert to manager
   */
  async sendReviewAlert(
    ...
    let message = `${isNegative ? '⚠️' : '📢'} *NEW REVIEW* ${stars}\n\n`;
    message += `👤 Customer: ${sanitizeInput(review.customerName)}\n`;
    message += `🏪 Branch: ${review.branchName}\n`;
    message += `📍 Source: ${review.source}\n`;

    if (review.comment) {
      message += `\n💬 "${truncate(sanitizeInput(review.comment), 100)}"`;
    }
    ...
    return this.sendWhatsAppMessage(
      ...
      message,
      'staff_review_alert',
      { rating: review.rating, source: review.source },
      'staff'
    );
  }
  ...
  async sendWhatsAppMessage(
    ...
    try {
      ...
      await this.logStore.logs.get(logEntry.id) && Object.assign(
        this.logStore.logs.get(logEntry.id)!,
        { error: errorMessage }
      );
   ...
   * WhatsApp Business API requires customer to initiate conversation within 24 hours
   */
  isWithinSessionWindow(lastCustomerMessage?: Date): boolean {
    if (!lastCustomerMessage) return true; // No restriction for template messages
