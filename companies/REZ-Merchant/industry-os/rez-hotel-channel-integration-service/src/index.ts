...
const config = {
  ...
  mongoUrl: process.env.MONGO_URL || 'mongodb://localhost:27017/rez_hotel_channels',
  internalToken: process.env.INTERNAL_SERVICE_TOKEN || '',

  // Channel API endpoints
  BOOKING_COM_API: process.env.BOOKING_COM_API_URL || 'https://supply-xml.booking.com',
  MMT_API: process.env.MMT_API_URL || 'https://api.makemytrip.com',
  GOIBIBO_API: process.env.GOIBIBO_API_URL || 'https://api.goibibo.com',
  EXPEDIA_API: process.env.EXPEDIA_API_URL || 'https://api.expedia.com',
  AIRBNB_API: process.env.AIRBNB_API_URL || 'https://api.airbnb.com',
  GOOGLE_HOTEL_API: process.env.GOOGLE_HOTEL_API_URL || 'https://hotelads.googleapis.com',
};
  ...
  async syncInventory(connection: any, inventory: any[]) {
    ...
    try {
      const ariPayload = {
        ...
        hotel_id: credentials.propertyId || credentials.hotelId,
...
const mmtProvider: ChannelProvider = {
  ...
  async syncInventory(connection: any, inventory: any[]) {
    ...
    try {
      const payload = {
        partner_id: credentials.partnerId || credentials.apiKey,
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        synced = response.data?.updated_count || 0;
      } else {
    ...
    try {
      const payload = {
        ...
        rate_plans: rates.map((r) => ({
          ...
          base_rate: r.baseRate,
          currency: r.currency || 'INR',
          min_los: r.restrictions?.minStay || 1,
        })),
      };
  ...
  async testConnection(credentials: any) {
    if (!credentials.apiKey || !credentials.hotelId) {
...
const goibiboProvider: ChannelProvider = {
  ...
  async syncInventory(connection: any, inventory: any[]) {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        synced = response.data?.inventory_updated || 0;
      } else {
    ...
    try {
      const payload = {
        ...
        rate_plans: rates.map((r) => ({
          ...
          currency: r.currency || 'INR',
          min_nights: r.restrictions?.minStay || 1,
        })),
      };
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        synced = response.data?.rates_updated || 0;
      } else {
  ...
  async testConnection(credentials: any) {
    if (!credentials.clientId || !credentials.clientSecret || !credentials.hotelId) {
...
const expediaProvider: ChannelProvider = {
  ...
  async syncInventory(connection: any, inventory: any[]) {
    ...
    try {
      const payload = {
        propertyId: credentials.propertyId || credentials.hotelId,
        inventory: inventory.map((inv) => ({
      ...
      if (process.env.NODE_ENV === 'production') {
        ...
        synced = response.data?.updated || 0;
      } else {
    ...
    try {
      const payload = {
        ...
        rates: rates.map((r) => ({
          ...
          minLOS: r.restrictions?.minStay || 1,
        })),
      };
...
const airbnbProvider: ChannelProvider = {
  ...
  async syncInventory(connection: any, inventory: any[]) {
    ...
    try {
      const payload = {
        listing_id: credentials.listingId || credentials.propertyId,
  ...
  async updateBooking(connection: any, bookingId: string, status: string) {
    ...
    try {
      if (process.env.NODE_ENV === 'production') {
        await axios.patch(`${config.AIRBNB_API}/reservations/${bookingId}`, { reservation_state: statusMap[status] || status }, {
          headers: { 'Authorization': `Bearer ${credentials.accessToken}` },
          timeout: 15000,
        });
      } else {
  ...
  async testConnection(credentials: any) {
    if (!credentials.accessToken || !credentials.listingId) {
...
const googleHotelProvider: ChannelProvider = {
    ...
    try {
      const payload = {
        ...
        rate_updates: rates.map((r) => ({
          ...
          base_rate: { amount_micros: Math.round(r.baseRate * 1000000), currency: r.currency || 'INR' },
        })),
      };
  ...
  async testConnection(credentials: any) {
    if (!credentials.accessToken || !credentials.partnerId || !credentials.propertyId) {
...
app.post('/api/connections', async (req: res, next) => {
  try {
    ...
    const connection = new ChannelConnection({ id: uuidv4(), hotelId: data.hotelId, channel: data.channel, credentials: data.credentials, settings: data.settings || {}, status: 'active' });
        ...
        for (const booking of channelBookings) {
          ...
          if (!existing) {
            await ChannelBooking.create({
              ...
              roomId: booking.roomId || '', guestName: booking.guestName, guestEmail: booking.guestEmail,
              guestPhone: booking.guestPhone, checkinDate: new Date(booking.checkinDate),
              checkoutDate: new Date(booking.checkoutDate), rooms: booking.rooms || 1,
              guests: booking.guests || 1, totalAmount: booking.totalAmount,
              commission: booking.commission || 0, netAmount: booking.netAmount || booking.totalAmount,
              currency: booking.currency || 'INR', status: 'confirmed', paymentStatus: booking.paymentStatus || 'paid',
