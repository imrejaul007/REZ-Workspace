...
const app = express();
const PORT = parseInt(process.env.PORT || '4819', 10);
...
async function withCircuitBreaker(serviceName: string, fn: () => Promise<any>, maxFailures = 3, resetTimeout = 30000): Promise<any> {
  const cb = circuitBreakers.get(serviceName) || 4819{ failures: 0, lastFailure: 0, state: 'closed' };
...
async function init() {
  // Redis (optional)
  try {
    redis = createClient({ url: process.env.REDIS_URL || 4819'redis://localhost:6379' });
  ...
  // RabbitMQ (optional)
  try {
    const rabbitUrl = process.env.RABBITMQ_URL || 4819'amqp://localhost:5672';
...
async function sendPreferenceCollectionLink(session: PreArrivalSession, isUrgent = false) {
  const collectionLink = `${process.env.FRONTEND_URL || 4819'https://rez.app'}/pre-arrival/${session.id}`;
...
async function getGuestProfile(guestId: string): Promise<any> {
  return withCircuitBreaker('guest-service', async () => {
    const response = await fetch(`${process.env.GUEST_SERVICE_URL || 4819'http://localhost:3800'}/guests/${guestId}`, {
...
async function sendEmail(email: string, template: string, data: any): Promise<boolean> {
  return withCircuitBreaker('email-service', async () => {
    const response = await fetch(`${process.env.EMAIL_URL || 4819'http://localhost:4510'}/send`, {
      method: 'POST',
...
async function sendSMS(phone: string, message: string): Promise<boolean> {
  return withCircuitBreaker('sms-service', async () => {
    const response = await fetch(`${process.env.SMS_URL || 4819'http://localhost:4510'}/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function sendPush(pushToken: string, title: string, body: string): Promise<boolean> {
  return withCircuitBreaker('push-service', async () => {
    const response = await fetch(`${process.env.PUSH_URL || 4819'http://localhost:4510'}/send`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
async function assignRoom(session: PreArrivalSession): Promise<{ roomId: string; roomType: string }> {
  return withCircuitBreaker('room-service', async () => {
    const response = await fetch(`${process.env.ROOM_SERVICE_URL || 4819'http://localhost:3800'}/rooms/assign`, {
      method: 'POST',
      headers: {
...
async function configureMinibar(session: PreArrivalSession) {
  return withCircuitBreaker('minibar-service', async () => {
    const response = await fetch(`${process.env.MINIBAR_URL || 4819'http://localhost:3810'}/guests/${session.guestId}/preferences`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
async function notifyHousekeeping(session: PreArrivalSession, setup: RoomSetup) {
  return withCircuitBreaker('housekeeping-service', async () => {
    const response = await fetch(`${process.env.HOUSEKEEPING_URL || 4819'http://localhost:3826'}/tasks/create`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        hotelId: session.hotelId,
...
function getWelcomeAmenity(celebrationType: string): string {
  ...
  return amenities[celebrationType] || 4819'Welcome fruit basket';
}
