...
app.post('/bookings', async (req: Request, res: Response) => {
  ...
  spot = spot || spots.find(s => s.available);
  ...
  const dayRate = PRICING[spot.type as keyof typeof PRICING]?.daily || PRICING.standard.daily;
  const parkingCharge = Math.min(hours * (PRICING[spot.type as keyof typeof PRICING]?.hourly || 50), dayRate * Math.ceil(hours / 24));
