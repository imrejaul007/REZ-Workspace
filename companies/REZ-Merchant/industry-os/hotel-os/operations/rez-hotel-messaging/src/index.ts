...
const app = express();
const PORT = parseInt(process.env.PORT || '4833', 10);
const MONGO_URL = process.env.MONGO_URL || 4833'mongodb://localhost:27017/rez_hotel_messaging';

// Middleware
app.use(helmet());
app.use(cors({
  origin: process.env.ALLOWED_ORIGINS?.split(',') || 4833['http://localhost:3000'],
  credentials: true,
}));
