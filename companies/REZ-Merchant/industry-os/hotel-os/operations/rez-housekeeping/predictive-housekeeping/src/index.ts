...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/predictive-housekeeping';
const REDIS_URL = process.env.REDIS_URL || 'redis://localhost:6379';
const JWT_SECRET = process.env.JWT_SECRET || 'predictive-housekeeping-secret';
const PMS_URL = process.env.PMS_URL || 'http://localhost:4031';
const HOUSEKEEPING_URL = process.env.HOUSEKEEPING_URL || 'http://localhost:4021';
...
class HousekeepingPredictor {
  ...
  predictCheckoutTime(roomHistory: any): { time: Date; confidence: number } {
    ...
    const patterns = roomHistory.patterns || {};
  ...
  predictCleaningDuration(roomType: string, condition: string): number {
    ...
    let duration = baseDurations[roomType] || 25;
  ...
  calculatePriority(
    ...
    if (roomType === 'suite' || roomType === 'executive_suite') {
      return 'high';
    }
  ...
  try {
    ...
    try {
      ...
      if (response.ok) {
        ...
        departures = (data.data || []).filter((b: any) => {
...
app.get('/stats', authMiddleware, async (req: Request, res: Response) => {
  try {
    ...
    const aggregates = stats.length > 0 ? {
      avgCleanings: stats.reduce((sum, s) => sum + (s.totalCleanings || 0), 0) / stats.length,
      avgDuration: stats.reduce((sum, s) => sum + (s.avgDuration || 0), 0) / stats.length,
      avgOnTimeRate: stats.reduce((sum, s) => sum + (s.onTimeRate || 0), 0) / stats.length,
      avgAccuracy: stats.reduce((sum, s) => sum + (s.predictionsAccuracy || 0), 0) / stats.length
    } : null;
...
function optimizeRoomAssignment(predictions: any[]): any[] {
  ...
  predictions.forEach(p => {
    const floor = p.roomNumber?.match(/^\d+/)?.[0] || '1';
