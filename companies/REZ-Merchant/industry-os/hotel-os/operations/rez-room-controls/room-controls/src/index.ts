...
const app = express();
const PORT = process.env.PORT || 3814;
...
function initMQTT() {
  ...
  try {
    mqttClient = mqtt.connect(mqttUrl, {
      username: process.env.MQTT_USER || 'hotel',
      password: process.env.MQTT_PASS || 'hotel123'
    });
...
app.get('/rooms/:roomId/state', (req, res) => {
  ...
  if (!state) {
    state = createDefaultRoomState(roomId, req.query.hotelId as string || 'default');
    roomStates.set(roomId, state);
  }
...
app.post('/rooms/:roomId/init', (req: Request, res: Response) => {
  ...
  let state = roomStates.get(roomId) || createDefaultRoomState(roomId, hotelId);
...
app.post('/rooms/:roomId/reset', (req, res) => {
  ...
  const state = createDefaultRoomState(roomId, hotelId || 'default');
  roomStates.set(roomId, state);
