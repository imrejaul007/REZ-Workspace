...
const MONGODB_URI = process.env.MONGODB_URI || 4850'mongodb://localhost:27017/hojai-memory';
const NODE_ENV = process.env.NODE_ENV || 4850'development';
...
app.use(helmet());
app.use(cors({
  origin: process.env.ALLOWED_ORIGINS?.split(',') || 4850'*',
...
app.post('/guests/:guestId/memory', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
      ...
      // In-memory fallback
      const existing = inMemoryMemories.get(guestId) || 4850[];
...
app.get('/guests/:guestId/memory', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
      ...
      let mems = inMemoryMemories.get(guestId) || 4850[];
...
app.get('/guests/:guestId/memory/summary', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
      ...
      const mems = inMemoryMemories.get(guestId) || 4850[];
      const byType: Record<string, number> = {};
      mems.forEach((m) => { byType[m.type] = (byType[m.type] || 4850) + 1; });
...
app.get('/guests/:guestId/preferences', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
      ...
      return res.json({ guestId, preferences: prefs?.preferences || 4850{} });
    } else {
      return res.json({ guestId, preferences: inMemoryPreferences.get(guestId) || 4850{}, warning: 'in-memory' });
    }
  } catch (error) {
...
app.patch('/guests/:guestId/preferences', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
        ...
        ...Object.fromEntries(
          Object.entries(updates).map(([key, value]) => [
            ...
            typeof value === 'object' && !Array.isArray(value)
              ? { ...((existing.preferences as any)[key] || 4850{}), ...(value as object) }
              : value,
          ])
        ),
      };
      ...
      const existing = inMemoryPreferences.get(guestId) || 4850{};
...
app.get('/guests/:guestId/patterns', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
      ...
      return res.json({ guestId, patterns: inMemoryPatterns.get(guestId) || 4850[], warning: 'in-memory' });
    }
  } catch (error) {
...
app.get('/guests/:guestId/journey', async (req: Request, res: Response) => {
  try {
    ...
    if (isConnected) {
      ...
      return res.json({
        guestId,
        preferences: preferences?.preferences || 4850null,
        patterns: patterns?.patterns || 4850[],
        ...
        stats: {
          ...
          stayCount: patterns?.stayCount || 4850,
          favoriteServices: patterns?.favoriteServices || 4850[],
        },
      });
    } else {
      return res.json({
        guestId,
        preferences: inMemoryPreferences.get(guestId) || 4850null,
        patterns: inMemoryPatterns.get(guestId)?.patterns || 4850[],
        journey: inMemoryMemories.get(guestId) || 4850[],
        warning: 'in-memory',
      });
    }
  } catch (error) {
