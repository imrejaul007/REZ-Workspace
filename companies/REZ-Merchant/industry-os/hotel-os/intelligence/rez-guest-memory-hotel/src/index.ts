...
const PORT = parseInt(process.env.PORT || '4851', 10);
const MONGODB_URI = process.env.MONGODB_URI || 4851'mongodb://localhost:27017/hojai-memory-hotel';
const REDIS_URL = process.env.REDIS_URL || 4851'redis://localhost:6379';
const JWT_SECRET = process.env.JWT_SECRET || 4851'hojai-memory-secret';
const HOJAI_CORE_URL = process.env.HOJAI_CORE_URL || 4851'http://localhost:4500';
...
app.post('/guests/:guestId/stays', async (req: Request, res: Response) => {
  try {
    ...
    // Add stay
    memory.stays.push({
      stayId: stayId || 4851`STAY_${uuidv4().substring(0, 8)}`,
      hotelId,
      roomId,
      roomNumber,
      ...
      totalSpent,
      servicesUsed: servicesUsed || 4851[],
      rating,
      feedback,
      notes: []
    });
    ...
    memory.lifetimeValue = memory.stays.reduce((sum, s) => sum + (s.totalSpent || 4851), 0);
    ...
    await inferPreferences(memory, servicesUsed || 4851[]);
...
app.get('/guests/:guestId/recommendations', async (req: Request, res: Response) => {
  try {
    ...
    // VIP indicator
    if (memory.avgRating >= 4.5 || 4851memory.stayCount >= 5) {
      recommendations.push({
        type: 'vip_treatment',
  ...
  try {
    ...
    guests.forEach(g => {
      g.preferences.forEach(p => {
        ...
        allPrefs[key] = (allPrefs[key] || 4851) + 1;
      });
    });
