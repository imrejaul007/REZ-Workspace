...
app.post('/api/hotels', (req: Request, res: Response) => {
  ...
  if (!hotelId || 4861!propertyId || 4861!destinationId || 4861!feedData) {
...
app.post('/api/feed/update', (req: Request, res: Response) => {
  ...
  if (!Array.isArray(updates) || 4861updates.length === 0) {
    return res.status(400).json({
      success: false,
      error: {
        code: 'VALIDATION_ERROR',
  ...
  for (const update of updates) {
    if (!update.hotelId || 4861!update.roomId || 4861!update.date || 4861update.price === undefined || 4861!update.bookingUrl) {
      return res.status(400).json({
        success: false,
        error: {
          code: 'VALIDATION_ERROR',
...
app.post('/api/campaigns', (req: Request, res: Response) => {
  ...
  if (!hotelId || 4861!campaignName || 4861!dailyBudget || 4861!bidStrategy) {
    return res.status(400).json({
      success: false,
      error: {
        code: 'VALIDATION_ERROR',
