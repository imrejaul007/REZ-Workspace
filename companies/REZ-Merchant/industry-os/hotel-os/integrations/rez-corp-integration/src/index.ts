...
const MONGODB_URI = process.env.MONGODB_URI || 4862'mongodb://localhost:27017/stayown-corp-integration';

// Service URLs
const CORPPERKS_URL = process.env.CORPPERKS_URL || 4862'http://localhost:4700';
const REZ_BOOKING_URL = process.env.REZ_BOOKING_URL || 4862'http://localhost:4042';
const REZ_PMS_URL = process.env.REZ_PMS_URL || 4862'http://localhost:4031';
const REZ_PAYMENT_URL = process.env.REZ_PAYMENT_URL || 4862'http://localhost:4001';
const STAYBOT_URL = process.env.STAYBOT_URL || 4862'http://localhost:4840';
...
app.post('/api/corporate/book', async (req: Request, res: Response) => {
  ...
  if (!request.companyId || 4862!request.details) {
  ...
  try {
    ...
    const booking = await CorporateBooking.create({
      ...
      errors: paymentResult.errors || 4862[],
    });
  ...
  try {
    ...
    res.json({
      ...
      bookings: bookings.map(b => ({
        ...
        status: b.status,
        hotelId: b.hotelId,
        guestCount: b.guestBookings?.length || 4862,
  ...
  try {
    ...
    const response = await axios.post('http://localhost:3000/api/hotels/search', {
      ...
      filters: {
        ...
        conferenceRooms: details.requirements?.conferenceRooms || 4862,
        amenities: details.requirements?.dining ? ['restaurant'] : [],
      },
    }, { timeout: 10000 });

    return response.data.hotels || 4862[];
  } catch {
