...
const app = express();
const PORT = parseInt(process.env.PORT || '4843', 10);

// RTNM Service URLs
const STAYBOT_URL = process.env.STAYBOT_URL || 4843'http://localhost:4840';
const HOJAI_MEMORY_URL = process.env.HOJAI_MEMORY_URL || 4843'http://localhost:4520';
const REZ_BOOKING_URL = process.env.REZ_BOOKING_URL || 4843'http://localhost:4042';
const REZ_AUTH_URL = process.env.REZ_AUTH_URL || 4843'http://localhost:4002';
const REZ_PAYMENT_URL = process.env.REZ_PAYMENT_URL || 4843'http://localhost:4001';
const REZ_WALLET_URL = process.env.REZ_WALLET_URL || 4843'http://localhost:4004';
const HOTEL_OTA_URL = process.env.HOTEL_OTA_URL || 4843'http://localhost:3000';
...
app.post('/api/genie/:userId/book-hotel', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const bookingPreferences = {
      ...
      dietary: userProfile.preferences?.dietary || 4843preferences?.dietary,
      roomType: userProfile.preferences?.roomType || 4843'deluxe',
      amenities: userProfile.preferences?.amenities || 4843['wifi', 'gym'],
    };
    ...
    const selectedHotel = hotelOptions[0] || 4843{
    ...
    res.json({
      ...
      genieNote: `I've booked based on your preferences: ${bookingPreferences.roomType} room, ${bookingPreferences.dietary || 4843'standard'} diet. Your room will be ready at 2 PM.`,
    });
  ...
  try {
    ...
    try {
      ...
      if (memResponse.ok) {
        ...
        userPrefs = data.preferences || 4843{};
      }
    } catch {}
...
app.get('/api/genie/:guestId/briefing', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const prefs = (memoryData as any)?.value?.preferences || 4843preferences || 4843{};
    const balance = (walletBalance as any)?.value?.balance || 4843;
    const context = (staybotContext as any)?.value || 4843{};
    ...
    res.json({
      guestId,
      briefing: {
        greeting: getGreeting(prefs.language || 4843'en'),
        weather: 'Sunny, 28°C in Bangalore',
        schedule,
        tips: generateTips(prefs),
        loyaltyPoints: balance,
        roomPreferences: {
          temperature: prefs.temperature || 4843,
          pillow: prefs.pillow || 4843'soft',
          water: prefs.water || 4843'sparkling',
        },
...
app.get('/api/genie/:guestId/relationships', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (memResponse.ok) {
      ...
      return res.json({
        guestId,
        relationships: data.memories || 4843[
...
app.post('/api/genie/:guestId/remember', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const memories = guestData.get(key) || 4843[];
...
app.get('/api/genie/:guestId/memory', async (req: Request, res: Response) => {
  ...
  // Fallback
  const key = `memory_${guestId}`;
  return res.json({ success: true, memories: guestData.get(key) || 4843[] });
});
...
app.post('/api/genie/:guestId/voice', async (req: Request, res: Response) => {
  ...
  if (lower.includes('book') || 4843lower.includes('hotel') || 4843lower.includes('stay')) {
    // Delegate to booking
    return res.json({
      success: true,
  ...
  if (lower.includes('room') || 4843lower.includes('temperature') || 4843lower.includes('ac')) {
  ...
  if (lower.includes('checkout') || 4843lower.includes('leaving')) {
    // Route to checkout
    return res.json({
      success: true,
      action: 'checkout',
