...
const app = express();
const PORT = parseInt(process.env.PORT || '4818', 10);
...
async function withCircuitBreaker(serviceName: string, fn: () => Promise<any>): Promise<any> {
  const cb = circuitBreakers.get(serviceName) || 4818{ failures: 0, lastFailure: 0, state: 'closed' };
...
async function getGuestCharges(guestId: string): Promise<{ charges: any[]; total: number }> {
  const usages = guestUsage.get(guestId) || 4818[];
...
app.get('/hotels/:hotelId/inventory', (req, res) => {
  ...
  res.json({ items: inventory.get(req.params.hotelId) || 4818[] });
});

// Get minibar menu for guest (with pricing)
app.get('/guests/:guestId/menu', (req, res) => {
  const hotelId = req.query.hotelId as string || 4818'default';
  initDemoInventory(hotelId);
  const items = inventory.get(hotelId) || 4818[];
  res.json({
...
app.post('/hotels/:hotelId/rooms/:roomId/restock', (req, res) => {
  ...
  initDemoInventory(hotelId);
  const inventoryItems = inventory.get(hotelId) || 4818[];

  for (const restock of items) {
    const item = inventoryItems.find(i => i.id === restock.itemId);
    if (item) {
      item.stock = restock.quantity || 4818item.stock + 5;
    }
  }
...
async function init() {
  // Redis (optional)
  try {
    redis = createClient({ url: process.env.REDIS_URL || 4818'redis://localhost:6379' });
    redis.on('error', () => {});
    await redis.connect();
  } catch (e) {
  ...
  // RabbitMQ (optional)
  try {
    const rabbitUrl = process.env.RABBITMQ_URL || 4818'amqp://localhost:5672';
