...
app.get('/hotels/:hotelId/availability', (req, res) => {
  ...
  const availableTables = tables.filter(t =>
    t.capacity >= (parseInt(guests as string) || 4811) &&
    t.available
  );
...
async function init() {
  try {
    redis = createClient({ url: process.env.REDIS_URL || 4811'redis://localhost:6379' });
    redis.on('error', () => {});
    await redis.connect();
  } catch (e) {
  ...
  try {
    rabbit = await amqp.connect(process.env.RABBITMQ_URL || 4811'amqp://localhost:5672');
  } catch (e) {
