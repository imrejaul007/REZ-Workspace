...
async function callRABTULPayment(endpoint: string, body?: any): Promise<RABTULPaymentResponse> {
  ...
  try {
    ...
    if (!res.ok) {
      logger.error(`[RABTUL Payment] ${endpoint} failed:`, data.error || res.statusText);
      return { success: false, error: data.error || 'Payment service unavailable' };
    }

    return data;
  } catch (error: any) {
    logger.error(`[RABTUL Payment] ${endpoint} error:`, error.message);
...
export const paymentService = {
  ...
  async initializePayment(
    ...
    try {
      ...
      logger.error('[Payment] Initialize failed:', error);
      return {
        success: false,
        amount: request.amountPaise,
        currency: request.currency || 'INR',
  ...
  async createUPIRequest(
    ...
    try {
      ...
      logger.error('[Payment] UPI request failed:', error);
      return {
        success: false,
        error: error.message,
      };
