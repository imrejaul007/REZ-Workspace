...
router.get('/:hotelId/:roomId', optionalAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[RoomServiceHub] Get info error:', error);
    res.status(500).json({
      success: false,
...
router.get('/menu/:hotelId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[RoomServiceHub] Get menu error:', error);
    res.status(500).json({
      success: false,
...
router.post('/order', rateLimiters.charge, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[RoomServiceHub] Order error:', error);
    res.status(500).json({
      success: false,
...
router.get('/bill/:bookingId', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[RoomServiceHub] Get bill error:', error);
    res.status(500).json({
      success: false,
...
router.post('/checkout', rateLimiters.checkout, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[RoomServiceHub] Checkout error:', error);
    res.status(500).json({
      success: false,
...
router.get('/feedback/hotel/:hotelId/ratings', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[Feedback] Ratings error:', error);
    res.status(500).json({
      success: false,
