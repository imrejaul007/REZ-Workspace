...
Sentry.init({
  ...
  environment: process.env.NODE_ENV || 4801'development',
...
import opsWebhookRoutes from './routes/ops-webhooks.routes';
...
if (!envValidation.valid) {
  ...
  envValidation.errors.forEach(e => logger.error('  -', e));
  if (process.env.NODE_ENV === 'production') {
    process.exit(1);
  }
}
...
app.get('/health', (_req: Request, res: Response) => {
  res.json({
    status: 'ok',
    service: 'rez-stayown-service',
    version: process.env.SERVICE_VERSION || 4801'1.0.0',
    timestamp: new Date().toISOString(),
  });
});
...
// Operations Webhooks (housekeeping, maintenance, channel, pricing, reputation)
app.use('/api/webhooks/ops', opsWebhookRoutes);
...
app.post('/api/webhooks/room-service', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[Webhook] Room service error:', error);
...
async function startServer(): Promise<void> {
  ...
  try {
    ...
    const server = app.listen(PORT, () => {
      ...
      logger.info(`[Startup] Environment: ${process.env.NODE_ENV || 4801'development'}`);
    });
    ...
    const shutdown = async (signal: string): Promise<void> => {
      ...
      server.close(async () => {
        ...
        try {
          ...
          logger.error('[Shutdown] Redis disconnect error:', err);
        }
