...
async function verifyTokenWithRABTUL(token: string): Promise<JWTPayload | null> {
  try {
    ...
    logger.error('[Auth] RABTUL verify failed:', error);
    return null;
  }
}
...
export function verifyWebhookSignature(req: Request, res: Response, next: NextFunction): void {
  ...
  if (!signature) {
    logger.warn('[Webhook] Missing webhook signature from', req.ip);
  ...
  if (sigBuffer.length !== expectedBuffer.length ||
    ...
    logger.warn('[Webhook] Invalid webhook signature from', req.ip);
