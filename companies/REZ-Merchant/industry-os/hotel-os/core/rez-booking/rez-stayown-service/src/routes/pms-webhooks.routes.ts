...
router.post('/check-in', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[Webhook] Check-in received:', {
      bookingId,
      roomId,
      roomNumber,
    });
    ...
    logger.error('[Webhook] Check-in error:', error);
    res.status(500).json({
      success: false,
...
router.post('/check-out', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[Webhook] Check-out received:', { bookingId, roomId });
    ...
    logger.error('[Webhook] Check-out error:', error);
    res.status(500).json({
      success: false,
      message: error.message || 'Webhook processing failed',
    });
...
router.post('/booking-update', async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[Webhook] Booking update:', { bookingId, updateType });
    ...
    logger.error('[Webhook] Booking update error:', error);
    res.status(500).json({
      success: false,
      message: error.message || 'Webhook processing failed',
    });
