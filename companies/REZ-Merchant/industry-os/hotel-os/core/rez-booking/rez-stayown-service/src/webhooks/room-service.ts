...
export async function processWebhook(
  ...
  try {
    ...
    logger.error('[RoomService Webhook] Processing failed:', error);
...
export async function registerWebhook(): Promise<boolean> {
  try {
    ...
    logger.error('[RoomService Webhook] Registration failed:', response.data.message);
    return false;
  } catch (error: any) {
    logger.error('[RoomService Webhook] Registration error:', error.message);
    return false;
  }
}
...
export async function retryWebhook(
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      logger.error([RoomService Webhook] Retry ${attempt} failed:`, error.message);
...
export async function logWebhookEvent(
  ...
  logger.info('[RoomService Webhook]', JSON.stringify(logEntry));
