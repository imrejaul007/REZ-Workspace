...
router.get('/hotels.xml', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error('[GoogleHotelAds] Feed generation error:', error);
...
router.get('/inline', async (_req: Request, res: Response) => {
  try {
    ...
    logger.error('[GoogleHotelAds] Inline data error:', error);
    res.status(500).json({
      success: false,
...
router.post('/click', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[GoogleHotelAds] Click webhook error:', error);
...
router.post('/conversion', async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[GoogleHotelAds] Conversion webhook error:', error);
    res.status(500).json({ success: false, error: 'Webhook processing failed' });
  }
});
