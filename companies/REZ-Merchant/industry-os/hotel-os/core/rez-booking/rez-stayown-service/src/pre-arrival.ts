...
export async function getPreArrivalPreferences(bookingId: string): Promise<PreArrivalPreferences | null> {
  try {
    ...
    logger.error('[PreArrival] Failed to get preferences:', error);
    return null;
  }
}
...
export async function getPreArrivalByGuest(guestId: string): Promise<PreArrivalPreferences[]> {
  try {
    ...
    logger.error('[PreArrival] Failed to get preferences by guest:', error);
    return [];
  }
}
...
export async function savePreArrivalPreferences(preferences: Partial<PreArrivalPreferences>): Promise<PreArrivalPreferences> {
  try {
    ...
    logger.info('[PreArrival] Preferences saved for booking', bookingId);
    return documentToPreferences(doc);
  } catch (error) {
    logger.error('[PreArrival] Failed to save preferences:', error);
...
export async function deletePreArrivalPreferences(bookingId: string): Promise<boolean> {
  try {
    ...
    logger.error('[PreArrival] Failed to delete preferences:', error);
    return false;
  }
}
...
export async function syncPreferencesToRoomQR(bookingId: string): Promise<boolean> {
  try {
    ...
    if (!preferences) {
      logger.warn('[PreArrival] No preferences to sync for booking', bookingId);
      return false;
    }
    ...
    if (response.status === 200) {
      ...
      logger.info('[PreArrival] Preferences synced to Room QR for booking', bookingId);
      return true;
    }

    return false;
  } catch (error) {
    logger.error('[PreArrival] Failed to sync to Room QR:', error);
    return false;
  }
}
...
export async function onGuestCheckIn(bookingId: string): Promise<void> {
  try {
    ...
    if (!success) {
      logger.warn('[PreArrival] Auto-sync failed for booking', bookingId);
      // Could queue for retry or notify staff
    }
  } catch (error) {
    logger.error('[PreArrival] Check-in sync error:', error);
  }
}
...
export async function sendPreArrivalReminder(
  ...
  try {
    ...
    logger.info('[PreArrival] Reminder sent for booking', bookingId);
    return true;
  } catch (error) {
    logger.error('[PreArrival] Failed to send reminder:', error);
    return false;
  }
}
...
async function sendEmailReminder(email: string, message: string, hotelName: string, checkInDate: Date): Promise<void> {
  ...
  try {
    ...
    logger.error('[PreArrival] Email reminder failed:', error);
  }
}
...
async function sendSMSReminder(phone: string, message: string): Promise<void> {
  ...
  try {
    ...
    logger.error('[PreArrival] SMS reminder failed:', error);
  }
}
...
export async function checkPreArrivalReminders(): Promise<void> {
  try {
    ...
    logger.error('[PreArrival] Reminder check failed:', error);
  }
}
...
export async function getPreArrivalAnalytics(hotelId: string, period: 'week' | 'month' = 'week'): Promise<{
  ...
  try {
    ...
    logger.error('[PreArrival] Analytics failed:', error);
    return {
      totalBookings: 0,
      preferencesSet: 0,
      conversionRate: 0,
