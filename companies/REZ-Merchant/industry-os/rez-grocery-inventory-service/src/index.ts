...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'healthy', service: 'rez-grocery-inventory-service', version: '1.0.0', timestamp: new Date().toISOString() });
});
...
const connectDB = async (): Promise<void> => {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/rez_grocery_inventory';
  try {
...
const startServer = async (): Promise<void> => {
  await connectDB();
  app.listen(PORT, () => {
    logger.info(`REZ Grocery Inventory Service running on port ${PORT}`);
