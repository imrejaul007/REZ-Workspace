...
app.get('/health', (_req: Request, res: Response) => {
  res.json({ status: 'healthy', service: 'rez-healthcare-appointment-service', version: '1.0.0', timestamp: new Date().toISOString() });
});

app.use('/api', appointmentRoutes, scheduleRoutes);
...
const connectDB = async (): Promise<void> => {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/rez_healthcare_appointment';
  try {
...
const startServer = async (): Promise<void> => {
  await connectDB();
  app.listen(PORT, () => {
    logger.info(`REZ Healthcare Appointment Service running on port ${PORT}`);
