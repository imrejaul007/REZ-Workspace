...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/hojai-staybot';
const NODE_ENV = process.env.NODE_ENV || 'development';
const HOJAI_BRAIN_URL = process.env.HOJAI_BRAIN_URL || 'http://localhost:4530';
const HOJAI_BRAIN_API_KEY = process.env.HOJAI_BRAIN_API_KEY || '';
const INTERNAL_TOKEN = process.env.INTERNAL_SERVICE_TOKEN || 'stayown-internal';
...
const HOTEL_SERVICES = {
  ...
  lostFound: process.env.LOST_FOUND_URL || 'http://localhost:3816',
};
...
app.post('/api/chat', async (req: Request, res: Response) => {
  try {
    ...
    const response = generatePatternResponse(intent, message, context?.language || 'en');
    const actions = getActionsForIntent(intent);

    // Store messages
    if (isConnected) {
      const sessionToUse = sessionId || conversation?.sessionId || `session_${Date.now()}`;

      await Conversation.findOneAndUpdate(
        { sessionId: sessionToUse },
        {
          $set: {
            guestId: guestId || conversation?.guestId,
            hotelId: context?.hotelId || conversation?.hotelId,
            lastIntent: intent,
...
app.get('/api/guest/:guestId/context', async (req: Request, res: Response) => {
  try {
    ...
    const mostFrequentIntent = Object.entries(intentCounts).sort((a, b) => b[1] - a[1])[0]?.[0] || 'general';

    return res.json({
      guestId,
      preferences: conversation?.context?.preferences || { language: 'en' },
  ...
  try {
    ...
    if (payment?.success) {
      await fetch(`${RABTUL_SERVICES.wallet}/loyalty/earn`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ guestId, points: Math.floor((folio.total || 0) / 100), reason: 'hotel_stay' }),
      });
    }
    ...
    return res.json({
      ...
      checkout: {
        ...
        pointsEarned: Math.floor((folio.total || 0) / 100),
      },
    });
  } catch (error: any) {
...
function getServiceEndpoint(serviceType: string, action: string): string {
  ...
  return endpoints[serviceType]?.[action] || '/api';
}
...
function getRABTULEndpoint(service: string, action: string): string {
  ...
  return endpoints[service]?.[action] || '/api';
}
...
function generatePatternResponse(intent: string, query: string, language: string): string {
  ...
  const langResponses = responses[intent] || responses.general;
  return langResponses[language as 'en' | 'hi'] || langResponses.en;
}
