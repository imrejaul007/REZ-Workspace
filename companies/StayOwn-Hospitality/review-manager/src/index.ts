...
const app = express();
const PORT = process.env.PORT || 3819;
...
app.post('/reviews', async (req: Request, res: Response) => {
  ...
  const review: Review = {
    ...
    categories: categories || { cleanliness: rating, service: rating, location: rating, value: rating, amenities: rating },
    title,
    text,
  ...
  if (sentimentResult.sentiment === 'negative' || rating <= 2) {
