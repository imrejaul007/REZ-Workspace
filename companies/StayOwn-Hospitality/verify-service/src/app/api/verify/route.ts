  ...
  try {
    ...
    if (fraudCheck.decision === 'BLOCK') {
      ...
      if (data.userId && validation.brandId) {
        ...
        ).catch((err) => logger.error('[Mind] Fraud signal failed:', err))
      }
    ...
    if (data.userId && validation.brandId) {
      sendVerificationToMind({
        ...
        location: data.location,
        timestamp: new Date(),
      }).catch((err) => logger.error('[Mind] Verification signal failed:', err, { correlationId }))
      ...
      ).catch((err) => logger.error('[Mind] Intent capture failed:', err, { correlationId }))
    }
    ...
    logger.error('Verify error:', error, { correlationId })
    return NextResponse.json({
      success: false,
