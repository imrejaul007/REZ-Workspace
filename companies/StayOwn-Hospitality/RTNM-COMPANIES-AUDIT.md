...
### RTNM Ecosystem Integration Endpoints (NEW - June 2026)

StayOwn Hotel OS now integrates with all RTNM ecosystem companies via the hotel-os-integration service (port 3899).

#### Airzy Flight Integration

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/airzy/flight/update` | POST | Receive flight delay/cancellation notifications |

**Flow:** Airzy detects flight delay → POST to StayOwn → Hotel notified → Room prep adjusted

#### KHAIRMOVE Transfer Integration

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/hotels/:hotelId/transfer-requests` | GET | Get scheduled airport transfers |
| `/transfers/book` | POST | Book airport transfer for guest |
| `/transfers/:transferId/status` | POST | Update transfer status |
| `/guests/:guestId` | GET | Get guest info |
| `/rides/link` | POST | Link ride to hotel booking |

**Flow:** Guest requests transfer → KHAIRMOVE books → StayOwn coordinates with arrival

#### Nexha Procurement Integration

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/hotels/:hotelId/procurement-requirements` | GET | Get hotel supply needs |
| `/hotels/:hotelId/supply-orders` | POST | Submit supply order |
| `/hotels/:hotelId/approved-suppliers` | GET | Get approved suppliers |

**Flow:** Hotel needs supplies → Nexha fetches requirements → Order submitted → Delivered

#### RidZa Finance Integration

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/hotels/:hotelId/finance-overview` | GET | Get hotel financial overview |
| `/hotels/:hotelId/revenue-analytics` | GET | Revenue breakdown |
| `/hotels/:hotelId/expense-breakdown` | GET | Expense analysis |
| `/hotels/:hotelId/occupancy-analytics` | GET | Occupancy metrics |

**Flow:** Hotel views finance → RidZa provides analytics → Credit/insurance options

#### AdBazaar Marketing Integration

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/hotels/:hotelId/guests` | GET | Get guest list for targeting |
| `/hotels/:hotelId/marketing/campaigns` | POST | Create hotel campaign |
| `/campaigns/:campaignId/performance` | GET | Campaign metrics |
| `/campaigns/:campaignId/track` | POST | Track conversions |
| `/guests/:guestId/preferences` | GET | Guest preferences |
| `/hotels/:hotelId/loyalty/members` | GET | Loyalty program members |
| `/guests/:guestId/offers` | POST | Send targeted offer |

**Flow:** Hotel runs campaign → AdBazaar targets guests → Track conversions → Optimize

#### CorpPerks Integration

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/api/hotels/search` | GET | Search hotels for corporate |
| `/api/hotels/book` | POST | Book hotel for employee |
| `/api/hotels/bookings/:id` | GET | Get booking status |

**Flow:** CorpPerks employee travels → Book via StayOwn → Corporate billing
