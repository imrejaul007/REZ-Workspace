...
router.post('/send', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[WhatsApp Routes] Send error:', error);
    return res.status(500).json({
      success: false,
...
router.post('/send/booking-confirmation', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[WhatsApp Routes] Booking confirmation error:', error);
    return res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/send/checkin-reminder', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[WhatsApp Routes] Checkin reminder error:', error);
    return res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/send/service-ready', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[WhatsApp Routes] Service ready error:', error);
    return res.status(500).json({
      success: false,
      error: error.message
    });
...
router.post('/send/bulk', validateApiKey, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[WhatsApp Routes] Bulk send error:', error);
    return res.status(500).json({
      success: false,
      error: error.message
    });
...
async function handleIncomingMessage(message: any): Promise<void> {
  ...
  logger.info([WhatsApp] Incoming message from ${message.from}:`, message.text?.body || message);
