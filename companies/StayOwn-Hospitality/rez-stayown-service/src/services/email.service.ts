...
class EmailService {
  ...
  async sendBookingConfirmation(data: BookingConfirmationData): Promise<EmailResult> {
    ...
    try {
      if (this.mockMode) {
        logger.info('[EmailService] Mock mode - skipping send:', {
          to: email.to,
          subject: email.subject
        });
      ...
      logger.error('[EmailService] Failed to send booking confirmation:', errorMessage);
  ...
  async sendCheckinReminder(data: CheckinReminderData): Promise<EmailResult> {
    ...
    try {
      if (this.mockMode) {
        logger.info('[EmailService] Mock mode - skipping checkin reminder:', {
          to: email.to,
          subject: email.subject
        });
        return { success: true, messageId: `mock-${Date.now()}` };
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      logger.error('[EmailService] Failed to send checkin reminder:', errorMessage);
      return { success: false, error: errorMessage };
    }
  }
  ...
  async sendReviewRequest(data: ReviewRequestData): Promise<EmailResult> {
    ...
    try {
      if (this.mockMode) {
        logger.info('[EmailService] Mock mode - skipping review request:', {
          to: email.to,
          subject: email.subject
        });
        return { success: true, messageId: `mock-${Date.now()}` };
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      logger.error('[EmailService] Failed to send review request:', errorMessage);
      return { success: false, error: errorMessage };
    }
  }
  ...
  async sendSpecialOffer(data: SpecialOfferData): Promise<EmailResult> {
    ...
    try {
      if (this.mockMode) {
        logger.info('[EmailService] Mock mode - skipping special offer:', {
          to: email.to,
          subject: email.subject
        });
        return { success: true, messageId: `mock-${Date.now()}` };
      ...
      const errorMessage = error instanceof Error ? error.message : 'Unknown error';
      logger.error('[EmailService] Failed to send special offer:', errorMessage);
      return { success: false, error: errorMessage };
    }
  }
  ...
  async sendBatch(
    ...
    for (const email of emails) {
      try {
        if (this.mockMode) {
          logger.info('[EmailService] Mock mode - batch email:', { to: email.to, subject: email.subject });
