import { rtnmRoutes } from './rtnm-integrations.js';
 ...
 * - HOJAI AI (Staybot, Memory, Agents, BrandPulse)
 *
...
const SERVICES = {
  ...
  aiFrontDesk: process.env.AI_FRONT_DESK_URL || 'http://localhost:3800',
  rezStayown: process.env.REZ_STAYOWN_URL || 'http://localhost:4015',
  hotelHabixo: process.env.HOTEL_HABIXO_URL || 'http://localhost:3007',

  // REZ-Merchant Hotel OS
  pms: process.env.REZ_PMS_URL || 'http://localhost:4031',
  hotelService: process.env.REZ_HOTEL_SERVICE_URL || 'http://localhost:4020',
  housekeeping: process.env.REZ_HOUSEKEEPING_URL || 'http://localhost:4021',
  maintenance: process.env.REZ_MAINTENANCE_URL || 'http://localhost:4019',
  messaging: process.env.REZ_MESSAGING_URL || 'http://localhost:4024',
  analytics: process.env.REZ_ANALYTICS_URL || 'http://localhost:4025',
  mind: process.env.REZ_MIND_URL || 'http://localhost:4017',
  channelManager: process.env.REZ_CHANNEL_URL || 'http://localhost:4022',
  roomService: process.env.REZ_ROOM_SERVICE_URL || 'http://localhost:4043',
  spa: process.env.REZ_SPA_URL || 'http://localhost:4049',
  laundry: process.env.REZ_LAUNDRY_URL || 'http://localhost:4048',
  dynamicPricing: process.env.REZ_PRICING_URL || 'http://localhost:4040',
  bookingEngine: process.env.REZ_BOOKING_ENGINE_URL || 'http://localhost:4042',
  guestMobile: process.env.REZ_GUEST_MOBILE_URL || 'http://localhost:4028',
  ...
  genie: process.env.HOJAI_GENIE_URL || 'http://localhost:4703',
  ...
  airzyBridge: process.env.AIRZY_BRIDGE_URL || 'http://stayown-airzy-bridge:3891',
  corpIntegration: process.env.CORP_INTEGRATION_URL || 'http://stayown-corp-integration:3890',
  staybotRouter: process.env.STAYBOT_ROUTER_URL || 'http://staybot-service-router:4841',

  // RTNM Sister Companies
  airzy: process.env.AIRZY_URL || 'http://airzy:3000', // Flight tracking
  corpPerks: process.env.CORPPERKS_URL || 'http://corpperks:4700', // HR/Staff
  khairMove: process.env.KHAIRMOVE_URL || 'http://khaimove:4000', // Transport
  nexha: process.env.NEXHA_URL || 'http://nexha:4600', // Procurement
  ridza: process.env.RIDZA_URL || 'http://ridza:4500', // Finance
  adBazaar: process.env.ADBazaar_URL || 'http://adbazaar:4500', // Marketing
  brandPulse: process.env.BRANDPULSE_URL || 'http://brandpulse:4770', // Reputation
...
class HotelOSIntegration {
  ...
  private setupRoutes(): void {
    ...
    // ============================================
    // RTNM INTEGRATION ENDPOINTS (BrandPulse, Sentiment, etc.)
    // ============================================

    // Mount RTNM routes (BrandPulse, Alerts, Reputation)
    this.app.use('/rtnm', rtnmRoutes);

    // ============================================
    // BRANDPULSE DIRECT ENDPOINTS
    // ============================================

    // Get brand overview from BrandPulse
    this.app.get('/api/rtnm/brand/:brandId/overview', async (req, res) => {
      ...
      try {
        ...
        logger.error('[BrandPulse] Overview failed:', error.message);
        res.status(500).json({ success: false, error: 'Failed to fetch brand overview' });
      }
    });

    // Get sentiment trend from BrandPulse
    this.app.get('/api/rtnm/brand/:brandId/sentiment', async (req, res) => {
      const { brandId } = req.params;
      ...
      try {
        ...
        logger.error('[BrandPulse] Sentiment failed:', error.message);
        res.status(500).json({ success: false, error: 'Failed to fetch sentiment' });
      }
    });

    // Create review in BrandPulse
    this.app.post('/api/rtnm/reviews', async (req, res) => {
      try {
        ...
        logger.error('[BrandPulse] Review creation failed:', error.message);
        res.status(500).json({ success: false, error: 'Failed to create review' });
      }
    });

    // Get brand reputation (legacy endpoint)
    this.app.get('/api/rtnm/reputation/:hotelId', async (req, res) => {
      const { hotelId } = req.params;
      try {
        ...
        logger.error('[BrandPulse] Reputation failed:', error.message);
  ...
  async start(): Promise<void> {
    this.app.listen(PORT, () => {
      ...
      logger.info('   HOJAI AI: Staybot, Memory, Agents, BrandPulse');
