...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/stayown-airzy-bridge';
...
const PREDICTIVE_HOUSEKEEPING_URL = process.env.PREDICTIVE_HOUSEKEEPING_URL || 'http://localhost:3826';
...
app.post('/api/flight/update', async (req: Request, res: Response) => {
  ...
  if (!guestId || !bookingId || !flightNumber || !newArrival) {
  ...
  console.log(`[Airzy Bridge] Flight ${flightNumber} delayed by ${delayMinutes || 0} minutes`);
  ...
  try {
    ...
    const updates = calculateUpdates(newArrival, delayMinutes || 0, stayContext);
...
app.post('/api/guest/:guestId/register-flight', async (req: Request, res: Response) => {
  ...
  if (!bookingId || !flightNumber || !arrivalDate) {
    return res.status(400).json({
      success: false,
...
async function updateRoomPreparation(guestId: string, roomId: string | undefined, updates: HotelUpdateRequest['updates']): Promise<void> {
  ...
  try {
    await axios.post(`${PREDICTIVE_HOUSEKEEPING_URL}/api/requests`, {
      ...
      guestId,
      roomId,
      delayMinutes: updates.roomPreparation?.delayMinutes || 0,
