...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/hotel-twins';
...
function predictDemand(historical: any[]): PredictionSchema[] {
  ...
  for (let i = 1; i <= 7; i++) {
    ...
    if (dayOfWeek === 0 || dayOfWeek === 6) demandMultiplier = 0.9; // Weekend
...
function generateRecommendations(twin: any): any[] {
  ...
  // Staffing
  const occupancyPerStaff = twin.operations.occupancy / (twin.efficiency?.staff || 50);
...
app.get('/twins/:hotelId/dashboard', async (req, res) => {
  try {
    ...
    const dashboard = {
      ...
      criticalIssues: twin.issues.filter((i: any) => i.severity === 'high' || i.severity === 'critical'),
...
app.post('/twins/:hotelId/agents', async (req, res) => {
  try {
    ...
    if (existingAgent) {
      existingAgent.status = status || 'active';
