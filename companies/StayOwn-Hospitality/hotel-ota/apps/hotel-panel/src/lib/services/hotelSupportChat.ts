...
class HotelSupportChatService {
  ...
  async initSession(guestContext: HotelGuestContext): Promise<ChatResponse> {
    try {
      ...
      logger.error('[HotelSupportChat] Session init failed:', error);
      return {
        success: false,
  ...
  async sendMessage(
    ...
    try {
      ...
      logger.error('[HotelSupportChat] Message send failed:', error);
      return {
        success: false,
  ...
  async requestRoomService(
    ...
    try {
      ...
      logger.error('[HotelSupportChat] Room service request failed:', error);
      return {
        success: false,
  ...
  async requestConcierge(
    ...
    try {
      ...
      logger.error('[HotelSupportChat] Concierge request failed:', error);
      return {
        success: false,
  ...
  async getSuggestedServices(guestContext: HotelGuestContext): Promise<string[]> {
    try {
      ...
      logger.error('[HotelSupportChat] Get suggestions failed:', error);
      return [];
    }
  }
  ...
  async getUpsellSuggestions(guestContext: HotelGuestContext): Promise<Array<{
    ...
    try {
      ...
      logger.error('[HotelSupportChat] Upsell suggestions failed:', error);
      return [];
    }
  }
  ...
  async processCheckout(
    ...
    try {
      ...
      logger.error('[HotelSupportChat] Checkout failed:', error);
      return {
        success: false,
  ...
  async trackInteraction(
    ...
    try {
      ...
      logger.debug('[HotelSupportChat] Analytics tracking failed:', error);
    }
  }
}
