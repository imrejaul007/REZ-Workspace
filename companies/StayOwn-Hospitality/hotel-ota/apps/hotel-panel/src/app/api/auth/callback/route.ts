...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!tokenRes.ok) {
      logger.error('[oauth-callback] Token exchange failed:', await tokenRes.json().catch(() => ({})));
    ...
    if (!access_token) {
      logger.error('[oauth-callback] Missing access_token in response:', tokenData);
    ...
    if (!ssoRes.ok) {
      logger.error('[oauth-callback] REZ SSO failed:', await ssoRes.json().catch(() => ({})));
    ...
    if (!otaToken) {
      logger.error('[oauth-callback] Missing otaToken in SSO response:', ssoData);
