...
export class BookingService {
  ...
  static async hold(params: {
    ...
    if (otaCoinApplied > 0) {
      try {
        ...
        logger.error('[BookingService] OTA coin burn failed — scheduling refund', { bookingId: booking.id, burnErr });
        await CoinService.scheduleRefund({ userId, coinType: 'ota', amountPaise: otaCoinApplied, bookingId: booking.id, reason: 'hold_burn_failed' }).catch((err: unknown) => { logger.warn('[BookingService] scheduleRefund failed', { err, bookingId: booking.id, reason: 'hold_burn_failed' }); });
        throw burnErr;
      }
    }
    if (rezCoinApplied > 0) {
      try {
        ...
        logger.error('[BookingService] REZ coin burn failed — scheduling refunds', { bookingId: booking.id, burnErr });
        // Refund OTA coins already burned above
        if (otaCoinApplied > 0) await CoinService.scheduleRefund({ userId, coinType: 'ota', amountPaise: otaCoinApplied, bookingId: booking.id, reason: 'partial_burn_rollback' }).catch((err: unknown) => { logger.warn('[BookingService] scheduleRefund failed', { err, bookingId: booking.id, reason: 'partial_burn_rollback' }); });
        await CoinService.scheduleRefund({ userId, coinType: 'rez', amountPaise: rezCoinApplied, bookingId: booking.id, reason: 'hold_burn_failed' }).catch((err: unknown) => { logger.warn('[BookingService] scheduleRefund failed', { err, bookingId: booking.id, reason: 'hold_burn_failed' }); });
        throw burnErr;
      }
    }
    if (hotelBrandCoinApplied > 0) {
      try {
        ...
        logger.error('[BookingService] Hotel brand coin burn failed — scheduling refunds', { bookingId: booking.id, burnErr });
        if (otaCoinApplied > 0) await CoinService.scheduleRefund({ userId, coinType: 'ota', amountPaise: otaCoinApplied, bookingId: booking.id, reason: 'partial_burn_rollback' }).catch((err: unknown) => { logger.warn('[BookingService] scheduleRefund failed', { err, bookingId: booking.id, reason: 'partial_burn_rollback' }); });
        if (rezCoinApplied > 0) await CoinService.scheduleRefund({ userId, coinType: 'rez', amountPaise: rezCoinApplied, bookingId: booking.id, reason: 'partial_burn_rollback' }).catch((err: unknown) => { logger.warn('[BookingService] scheduleRefund failed', { err, bookingId: booking.id, reason: 'partial_burn_rollback' }); });
        await CoinService.scheduleRefund({ userId, coinType: 'hotel_brand', amountPaise: hotelBrandCoinApplied, bookingId: booking.id, reason: 'hold_burn_failed' }).catch((err: unknown) => { logger.warn('[BookingService] scheduleRefund failed', { err, bookingId: booking.id, reason: 'hold_burn_failed' }); });
        throw burnErr;
      }
    }
  ...
  static async confirm(params: {
    ...
    if (rezCoinEarned > 0 && booking.user.rezUserId) {
      ...
      }).catch((err) => logger.error('ReZ webhook failed (will retry):', err));
    }
