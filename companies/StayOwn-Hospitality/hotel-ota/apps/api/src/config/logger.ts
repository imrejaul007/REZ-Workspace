...
function makeLogger(service: string) {
  return {
    info: (message: string, meta?: Record<string, unknown>) => {
      logger.info(format('info', service, message, meta));
    },
    warn: (message: string, meta?: Record<string, unknown>) => {
      logger.warn(format('warn', service, message, meta));
    },
    error: (message: string, meta?: Record<string, unknown>) => {
      logger.error(format('error', service, message, meta));
    },
    debug: (message: string, meta?: Record<string, unknown>) => {
      if (env.NODE_ENV !== 'production') {
        logger.debug(format('debug', service, message, meta));
      }
    },
  };
}
