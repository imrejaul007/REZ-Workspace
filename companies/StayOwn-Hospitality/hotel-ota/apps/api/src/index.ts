...
process.on('uncaughtException', (err) => {
  ...
  if (err.message?.includes('Connection is closed') ||
    ...
    logger.warn('[Process] Caught non-fatal error:', err.message);
    return;
  }
...
process.on('unhandledRejection', (reason, promise) => {
  ...
  if (reasonStr.includes('Connection is closed') ||
    ...
    logger.warn('[Process] Unhandled rejection (non-fatal):', reasonStr);
    return;
  }
  ...
  httpServer.listen(env.PORT, () => {
    ...
    setTimeout(() => {
      initializeScheduledJobs()
        ...
        .catch((err) => logger.warn('[Scheduler] Jobs unavailable (Redis required):', err.message));
