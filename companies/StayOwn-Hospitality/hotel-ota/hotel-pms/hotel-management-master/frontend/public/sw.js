  ...
  try {
    ...
    for (const requestData of requests) {
      try {
        ...
        logger.error(Error syncing request ${requestData.url}:`, error);
    ...
    logger.info(Sync completed for ${syncTag}:`, results);
      ...
      (async () => {
        ...
        for (const syncTag of Object.values(SYNC_TAGS)) {
          try {
            ...
            logger.error(Error in periodic sync for ${syncTag}:`, error);
          }
        }
