...
export default function GuestChatPanel({
  ...
  const loadConversations = useCallback(async () => {
    try {
      ...
      logger.error('[GuestChat] Error loading conversations:', error);
    }
  }, [apiBaseUrl, guestId]);
  ...
  const loadMessages = useCallback(async (conversationId: string) => {
    ...
    try {
      ...
      logger.error('[GuestChat] Error loading messages:', error);
    } finally {
      setIsLoading(false);
    }
  }, [apiBaseUrl]);
  ...
  const startConversation = useCallback(async (type: HotelConversationType, department: HotelDepartment) => {
    ...
    try {
      ...
      logger.error('[GuestChat] Error starting conversation:', error);
    } finally {
      setIsLoading(false);
    }
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      logger.error('[GuestChat] Error sending message:', error);
