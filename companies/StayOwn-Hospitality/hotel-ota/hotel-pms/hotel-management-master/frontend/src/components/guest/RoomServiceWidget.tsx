...
export function RoomServiceWidget({
    ...
    try {
      ...
      if (guestId) {
        try {
          ...
          logger.error('Failed to fetch guest inventory charges:', error);
        }
      }
      ...
      if (roomId) {
        try {
          ...
          logger.error('Failed to fetch room inventory data:', error);
        }
      }
      ...
      logger.error('Failed to fetch room services:', error);
      setSummary(null);
    } finally {
      setLoading(false);
    }
  ...
  const requestService = async () => {
    ...
    try {
      ...
      logger.error('Failed to submit room service request:', error);
