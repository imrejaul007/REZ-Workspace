...
class EnhancedAPIDataSeeder {
  ...
  async initialize() {
    try {
      ...
      logger.info(✅ Found ${this.hotels.length} hotels`);
      logger.info(✅ Using admin user: ${this.adminUser._id}`);
  ...
  async seedEnhancedAPIKeys() {
    try {
      ...
      logger.info(✅ Created ${apiKeysToCreate.length} new API keys (total: ${existingCount + apiKeysToCreate.length})`);
  ...
  async seedComprehensiveWebhooks() {
    try {
      ...
      logger.info(✅ Created ${webhooksToCreate.length} new webhook endpoints (total: ${existingCount + webhooksToCreate.length})`);
