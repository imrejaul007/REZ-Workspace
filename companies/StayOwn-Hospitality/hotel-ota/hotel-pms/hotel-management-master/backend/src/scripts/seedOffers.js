...
async function seedOffers() {
  try {
    ...
    logger.info(🏨 Using hotel: ${hotel.name} (${hotel._id})`);
    ...
    logger.info(🏗️ Creating ${offersToCreate.length} sample offers...`);
    const createdOffers = await Offer.insertMany(offersToCreate);
    
    logger.info(✅ Successfully created ${createdOffers.length} offers:`);
    ...
    Object.entries(offersByCategory).forEach(([category, count]) => {
      logger.info(   📋 ${category}: ${count} offers`);
    });
    ...
    Object.entries(offersByType).forEach(([type, count]) => {
      logger.info(   🎯 ${type.replace('_', ' ')}: ${count} offers`);
    });

    console.log('\n🎉 Offer seeding completed successfully!');
    logger.info(💡 You can now view and manage these offers at: /admin/offers`);
