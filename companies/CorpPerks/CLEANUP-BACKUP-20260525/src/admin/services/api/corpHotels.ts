...
export interface HotelBooking {
  ...
  billing: {
    ...
    paymentMethod: 'credit_card' | 'debit_card' | 'net_banking' | 'upi' | 'wallet' | 'corporate_billing';
  };
...
export const corpHotelsApi = {
  ...
  async getBookings(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: HotelBooking[]; pagination: { total: number; page: number; limit: number } }> {
    const queryParams = new URLSearchParams();
  ...
  async cancelBooking(id: string, reason?: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/hotels/bookings/${id}/cancel`, {
      reason,
    }, {
      headers: { 'x-company-id': getCompanyId() },
    });
  ...
  async approveBooking(id: string, notes?: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/hotels/bookings/${id}/approve`, {
      notes,
    }, {
      headers: { 'x-company-id': getCompanyId() },
    });
  ...
  async rejectBooking(id: string, reason: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/hotels/bookings/${id}/reject`, {
      reason,
    }, {
      headers: { 'x-company-id': getCompanyId() },
    });
