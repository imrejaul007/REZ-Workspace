 ...
 *
 * Integration with StayOwn and other hotel booking APIs
...
export const hotelOTAApi = {
  ...
  async createBooking(data: OTABookingRequest): Promise<OTABooking> {
    const response = await apiClient.post<{ data: OTABooking }>(
      '/api/ota/bookings',
      data,
      { headers: { 'x-company-id': getCompanyId() } }
    );
  ...
  async getBooking(bookingId: string): Promise<OTABooking | null> {
    const response = await apiClient.get<{ data: OTABooking }>(
      `/api/ota/bookings/${bookingId}`,
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || null;
  },
  ...
  async cancelBooking(bookingId: string, reason?: string): Promise<void> {
    const response = await apiClient.post(`/api/ota/bookings/${bookingId}/cancel`, { reason }, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async saveToFavorites(propertyId: string): Promise<void> {
    const response = await apiClient.post(`/api/ota/favorites/${propertyId}`, {}, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
   ...
   */
  async getContractedChains(): Promise<Array<{
    chainId: string;
    name: string;
    logo: string;
