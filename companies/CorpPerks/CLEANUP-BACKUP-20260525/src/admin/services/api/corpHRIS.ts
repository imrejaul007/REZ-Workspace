...
export const corpHRISApi = {
  ...
  async cancelSync(): Promise<void> {
    const response = await apiClient.post('/api/corp/hris/sync/cancel', {}, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async getFieldMappings(): Promise<EmployeeMapping[]> {
    const response = await apiClient.get<{ data: EmployeeMapping[] }>(
  ...
  async previewEmployees(): Promise<HRISEmployee[]> {
    const response = await apiClient.get<{ data: HRISEmployee[] }>(
      '/api/corp/hris/preview',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || [];
  },
