...
export const corpGiftingApi = {
  ...
  async getCampaigns(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: GiftCampaign[]; pagination: { total: number; page: number; limit: number } }> {
    const queryParams = new URLSearchParams();
  ...
  async launchCampaign(id: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/gifting/campaigns/${id}/launch`, {}, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async cancelCampaign(id: string, reason?: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/gifting/campaigns/${id}/cancel`, { reason }, {
      headers: { 'x-company-id': getCompanyId() } ,
    });
    if (!response.success) {
  ...
  async getOrder(id: string): Promise<GiftOrder | null> {
    const response = await apiClient.get<{ data: GiftOrder }>(
      `/api/corp/gifting/orders/${id}`,
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || null;
  },
  ...
  async createOrder(data: CreateOrderRequest): Promise<GiftOrder> {
    const response = await apiClient.post<{ data: GiftOrder }>(
      '/api/corp/gifting/orders',
      data,
      { headers: { 'x-company-id': getCompanyId() } }
    );
    if (!response.success) {
   ...
   */
  async bulkCreateOrders(campaignId: string, recipientIds: string[]): Promise<{
    orderCount: number;
    totalAmount: number;
  }> {
  ...
  async getCampaignAnalytics(campaignId: string): Promise<{
    ...
    return response.data?.data ?? {
      overview: { totalRecipients: 0, sent: 0, delivered: 0, opened: 0, redeemed: 0, deliveryRate: 0, redemptionRate: 0 },
      dailyStats: [],
      topGifts: [],
    };
  },
