...
export const corpRewardsApi = {
  ...
  async getRewardSummary(employeeId?: string): Promise<RewardSummary> {
    ...
    return response.data?.data || {
      ...
      balance: 0,
      lifetimeEarned: 0,
      currentTier: { tierId: 'bronze', name: 'Bronze', minCoins: 0, maxCoins: 1000, benefits: ['Basic rewards'], icon: '🥉', color: '#CD7F32' },
  ...
  async getTransactions(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: RewardTransaction[]; pagination: { total: number; page: number; limit: number } }> {
  ...
  async getTiers(): Promise<RewardTier[]> {
    const response = await apiClient.get<{ data: RewardTier[] }>(
      '/api/corp/rewards/tiers',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || [];
  },
  ...
  async getMyTier(): Promise<{ current: RewardTier; next?: RewardTier; progress: number }> {
    const response = await apiClient.get<{ data: { current: RewardTier; next?: RewardTier; progress: number } }>(
      '/api/corp/rewards/tiers/me',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || {
      current: { tierId: 'bronze', name: 'Bronze', minCoins: 0, maxCoins: 1000, benefits: ['Basic rewards'], icon: '🥉', color: '#CD7F32' },
      progress: 0,
    };
  },
  ...
  async getCatalog(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: RewardCatalog[]; pagination: { total: number; page: number; limit: number } }> {
    const queryParams = new URLSearchParams();
