...
export const corpPerksApi = {
  ...
  async getBenefits(params?: {
    ...
    const response = await apiClient.get<{ data: Benefit[] }>(
      `/api/corp/benefits?${queryParams}`,
  ...
  async createBenefit(data: CreateBenefitRequest): Promise<Benefit> {
    const response = await apiClient.post<{ data: Benefit }>(
      '/api/corp/benefits',
      data,
      { headers: { 'x-company-id': getCompanyId() } }
    );
  ...
  async updateBenefit(id: string, data: Partial<CreateBenefitRequest>): Promise<Benefit> {
    const response = await apiClient.put<{ data: Benefit }>(
      `/api/corp/benefits/${id}`,
      data,
      { headers: { 'x-company-id': getCompanyId() } }
    );
  ...
  async getEmployees(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: Employee[]; pagination: { total: number; page: number; limit: number; pages: number } }> {
    const queryParams = new URLSearchParams();
   ...
   */
  async getMyBenefitSummary(): Promise<Record<string, { allocated: number; utilized: number; remaining: number }>> {
    const response = await apiClient.get<{ data: Record<string, { allocated: number; utilized: number; remaining: number }> }>(
      '/api/corp/me/benefits/summary',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data ?? {};
  },
  ...
  async checkITCeligibility(params: {
    ...
    const response = await apiClient.post<{ data: { eligible: boolean; itcAmount: number; reason?: string } }>(
      '/api/gst/itc-check',
      params,
      { headers: { 'x-company-id': getCompanyId() } }
    );
   ...
   */
  async submitEInvoice(invoiceNumber: string): Promise<{ irn: string; ackNo: string; qrCode: string }> {
