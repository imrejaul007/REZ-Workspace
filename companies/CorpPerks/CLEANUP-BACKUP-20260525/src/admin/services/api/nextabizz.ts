...
export const nextaBizzApi = {
  ...
  async searchProducts(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: NextaProduct[]; pagination: { total: number; page: number; limit: number } }> {
   ...
   */
  async getBulkPricing(productIds: string[], quantities: number[]): Promise<Array<{
    productId: string;
    unitPrice: number;
    totalPrice: number;
    discountPercent: number;
  }>> {
    const response = await apiClient.post<{ data: Array<{
      productId: string;
      unitPrice: number;
      totalPrice: number;
      discountPercent: number;
  ...
  async getBulkOrder(orderId: string): Promise<BulkOrder | null> {
    const response = await apiClient.get<{ data: BulkOrder }>(
  ...
  async cancelBulkOrder(orderId: string, reason?: string): Promise<void> {
    const response = await apiClient.post(`/api/nextabizz/orders/${orderId}/cancel`, { reason }, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async getGiftBoxTemplates(): Promise<CustomGiftBox[]> {
    const response = await apiClient.get<{ data: CustomGiftBox[] }>(
      '/api/nextabizz/gift-boxes',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || [];
  },
  ...
  async getOrderInvoice(orderId: string): Promise<{
    ...
    const response = await apiClient.get<{ data: {
      ...
      `/api/nextabizz/orders/${orderId}/invoice`,
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || null;
  },
