...
export const corpKarmaApi = {
  ...
  async getCampaigns(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: KarmaCampaign[]; pagination: { total: number; page: number; limit: number } }> {
  ...
  async launchCampaign(id: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/karma/campaigns/${id}/launch`, {}, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async joinCampaign(campaignId: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/karma/campaigns/${campaignId}/join`, {}, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async getActivities(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: KarmaActivity[]; pagination: { total: number; page: number; limit: number } }> {
    const queryParams = new URLSearchParams();
  ...
  async verifyActivity(activityId: string, approved: boolean, notes?: string): Promise<void> {
    const response = await apiClient.post(`/api/corp/karma/activities/${activityId}/verify`, {
      approved,
      notes,
    }, {
      headers: { 'x-company-id': getCompanyId() },
  ...
  async getMyStats(): Promise<KarmaStats> {
    const response = await apiClient.get<{ data: KarmaStats }>(
      '/api/corp/karma/me/stats',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || {
      totalKarma: 0,
  ...
  async getBadges(): Promise<KarmaBadge[]> {
    const response = await apiClient.get<{ data: KarmaBadge[] }>(
      '/api/corp/karma/badges',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || [];
  },
  ...
  async getImpactStats(params?: {
    ...
    const response = await apiClient.get<{ data: any }>(
      `/api/corp/karma/impact?${queryParams}`,
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || {
      totalKarmaPoints: 0,
