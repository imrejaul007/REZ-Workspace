...
export const rtmnFinanceApi = {
  ...
  async getWallet(): Promise<CorpWallet | null> {
    const response = await apiClient.get<{ data: CorpWallet }>(
      '/api/finance/wallet',
   ...
   * Add funds to wallet
   */
  async addFunds(amount: number, paymentMethod: 'upi' | 'netbanking' | 'neft' | 'rtgs'): Promise<PaymentResult> {
  ...
  async issueExpenseCard(data: {
    ...
    const response = await apiClient.post<{ data: ExpenseCard }>(
      '/api/finance/cards',
      data,
      { headers: { 'x-company-id': getCompanyId() } }
    );
  ...
  async updateCardStatus(cardId: string, status: 'active' | 'blocked'): Promise<void> {
    const response = await apiClient.post(`/api/finance/cards/${cardId}/status`, { status }, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
   ...
   */
  async getCardTransactions(cardId: string): Promise<Array<{
    transactionId: string;
    merchant: string;
    amount: number;
    category: string;
  ...
  async getBNPLPlans(): Promise<BNPLPlan[]> {
    const response = await apiClient.get<{ data: BNPLPlan[] }>(
      '/api/finance/bnpl/plans',
      { headers: { 'x-company-id': getCompanyId() } }
    );
    return response.data?.data || [];
  },
  ...
  async getBNPLTransactions(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: BNPLTransaction[]; pagination: { total: number; page: number; limit: number } }> {
    const queryParams = new URLSearchParams();
    if (params?.status) queryParams.set('status', params.status);
    if (params?.page) queryParams.set('page', String(params.page));
  ...
  async getExpenseClaims(params?: {
    ...
    page?: number;
    limit?: number;
  }): Promise<{ data: ExpenseClaim[]; pagination: { total: number; page: number; limit: number } }> {
    const queryParams = new URLSearchParams();
    if (params?.employeeId) queryParams.set('employeeId', params.employeeId);
    if (params?.status) queryParams.set('status', params.status);
    if (params?.startDate) queryParams.set('startDate', params.startDate);
  ...
  async approveExpenseClaim(claimId: string, notes?: string): Promise<void> {
    const response = await apiClient.post(`/api/finance/expenses/claims/${claimId}/approve`, { notes }, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
  ...
  async rejectExpenseClaim(claimId: string, reason: string): Promise<void> {
    const response = await apiClient.post(`/api/finance/expenses/claims/${claimId}/reject`, { reason }, {
      headers: { 'x-company-id': getCompanyId() },
    });
    if (!response.success) {
   ...
   * Pay expense claim
   */
  async payExpenseClaim(claimId: string, method: 'wallet' | 'bank_transfer' | 'card'): Promise<PaymentResult> {
    const response = await apiClient.post<{ data: PaymentResult }>(
