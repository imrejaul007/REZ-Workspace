...
import { z } from 'zod';
import { requireAuth, requireAdminAuth, requireInternalToken } from './middleware/auth';
...
const OAUTH_CONFIGS: Record<string, OAuthConfig> = {
  stayown: {
    ...
    authUrl: 'https://api.stayown.com/oauth/authorize',
    tokenUrl: 'https://api.stayown.com/oauth/token',
...
/**
 * StayOwn Webhook Handler
 * POST /api/integrations/stayown/webhook
 */
const stayownWebhookSchema = z.object({
...
router.post('/stayown/webhook', async (req: Request, res: Response) => {
  try {
    // Verify webhook signature
    const signature = req.headers['x-stayown-signature'] as string;
    const payload = JSON.stringify(req.body);

    // In production, verify signature using HMAC
    // const isValid = verifyStayOwnSignature(signature, payload, secret);

    const result = stayownWebhookSchema.safeParse(req.body);
    if (!result.success) {
    ...
    switch (event) {
      case 'booking.created':
        logger.info('[StayOwn Webhook] Booking created', { bookingId: data.bookingId });
        // Process new booking - update CorpPerks records
        break;

      case 'booking.updated':
        logger.info('[StayOwn Webhook] Booking updated', { bookingId: data.bookingId, status: data.status });
        // Update booking status
        break;

      case 'booking.cancelled':
        logger.info('[StayOwn Webhook] Booking cancelled', { bookingId: data.bookingId });
        // Handle cancellation
        break;

      case 'payment.received':
        logger.info('[StayOwn Webhook] Payment received', { bookingId: data.bookingId, amount: data.amount });
        // Record payment
        break;
    }
    ...
    logger.error('[StayOwn Webhook] Failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/nextabizz/webhook', async (req: Request, res: Response) => {
  try {
...
router.post('/hris/webhook', async (req: Request, res: Response) => {
  try {
...
router.post('/finance/webhook', async (req: Request, res: Response) => {
  try {
...
/**
 * Proxy to StayOwn API
 * GET /api/integrations/stayown/proxy/:path
 */
router.get('/stayown/proxy/:path(*)', requireAdminAuth, async (req: Request, res: Response) => {
  try {
    const { path } = req.params;
    const companyId = req.headers['x-company-id'] as string;

    // Get stored access token for this company
    // const token = await getIntegrationToken(companyId, 'stayown');

    // Forward request to StayOwn
    const response = await fetch(`https://api.stayown.com/${path}?${new URLSearchParams(req.query as any)}`, {
      headers: {
    ...
    logger.error('[StayOwn Proxy] Failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
