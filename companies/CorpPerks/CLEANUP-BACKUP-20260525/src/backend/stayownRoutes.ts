 ...
 *
 * Integration with StayOwn API for corporate hotel bookings.
...
router.get('/search', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // In production, call StayOwn API:
    ...
    logger.info('[StayOwn] Hotel search', { city, checkIn, checkOut, guests, results: properties.length });

    res.json({ success: true, data: properties });
  } catch (err: any) {
    logger.error('[StayOwn] Search failed', { error: err.message });
...
router.get('/:propertyId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[StayOwn] Get property failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.get('/:propertyId/availability', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[StayOwn] Room availability', { propertyId, checkIn, checkOut, available: availableRooms.length });

    res.json({ success: true, data: availableRooms });
  } catch (err: any) {
    logger.error('[StayOwn] Availability check failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/bookings', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[StayOwn] Booking created', {
      bookingId,
      confirmationNumber,
    ...
    logger.error('[StayOwn] Create booking failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.get('/bookings', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[StayOwn] Get bookings', { companyId, count: paginatedBookings.length });
    ...
    logger.error('[StayOwn] Get bookings failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.get('/bookings/:bookingId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.error('[StayOwn] Get booking failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/bookings/:bookingId/cancel', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[StayOwn] Booking cancelled', { bookingId, reason, companyId });

    res.json({ success: true, data: booking });
  } catch (err: any) {
    logger.error('[StayOwn] Cancel booking failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/pricing/calculate', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    logger.info('[StayOwn] Price calculated', { propertyId, roomId, nights, totalAmount });
    ...
    logger.error('[StayOwn] Calculate price failed', { error: err.message });
    res.status(500).json({ success: false, message: err.message });
  }
});
