...
router.get('/orders/:orderId', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // IDOR prevention: verify company ownership
    if (user.role !== 'admin' && order.companyId !== companyId) {
      logger.warn('[NextaBizz] IDOR attempt detected', {
        userId: user.id,
        orderId,
...
router.post('/orders/:orderId/cancel', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    // IDOR prevention: verify company ownership
    if (user.role !== 'admin' && order.companyId !== companyId) {
      logger.warn('[NextaBizz] IDOR cancel attempt', {
        userId: user.id,
        orderId,
        orderCompanyId: order.companyId,
        requestCompanyId: companyId
