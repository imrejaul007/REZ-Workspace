...
CorpPerks integrates with StayOwn Hotel OS for corporate travel and hotel booking.

#### Integration Endpoints

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/api/hotels/search` | GET | Search hotels for corporate travel |
| `/api/hotels/book` | POST | Book hotel for employee |
| `/api/hotels/bookings/:id` | GET | Get booking status |
| `/api/hotels/bookings/:id/cancel` | POST | Cancel booking |
| `/api/hotels/recommendations` | GET | Get hotel recommendations |
| `/api/hotels/integration-status` | GET | Check StayOwn integration health |

#### Integration Flow

```
Corporate employee needs hotel
        ↓
GET /api/hotels/search (search by city, dates, guests)
        ↓
POST /api/hotels/book (book with corporate billing)
        ↓
StayOwn creates booking with cost center
        ↓
GET /api/hotels/bookings/:id (track status)
```
...
|---------|---------|------------------|
| **StayOwn** | Hotel OS (3899) | Corporate hotel booking |
