...
# Deploys all corporate perks services to Render
# =============================================================================
...
LOG_FILE="${SCRIPT_DIR}/logs/deploy-corpperks_${TIMESTAMP}.log"
...
ALL_SERVICES=("${CORPPERKS_SERVICES[@]}")
...
check_prerequisites() {
    log_header "CORPPERKS PRE-DEPLOYMENT CHECKS"
...
main() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                     CORPPERKS DEPLOYMENT                        ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    log_header "DEPLOYING ${total_services} CORPPERKS SERVICES"
    ...
    # Print summary
    log_header "DEPLOYMENT SUMMARY - CORPPERKS"
    ...
    if [ $FAILED_COUNT -eq 0 ]; then
        log_success "All CorpPerks services deployed successfully!"
    else
