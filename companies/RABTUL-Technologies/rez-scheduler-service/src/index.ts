...
import { closeAllDlqQueues } from './queues/dlq';
...
app.listen(port, () => {
  ...
  await scheduleAllCronJobs().catch((err) => logger.error('[STARTUP] scheduleAllCronJobs failed', { error: err instanceof Error ? err.message : String(err) }));
  ...
  const shutdown = async (signal: string) => {
    ...
    try {
      ...
      // 2b. Close all DLQ queues
      await closeAllDlqQueues();
      logger.info('[SHUTDOWN] All DLQ queues closed');
