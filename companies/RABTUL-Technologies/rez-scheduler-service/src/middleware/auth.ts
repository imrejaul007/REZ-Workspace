 ...
 * JSON object. Any known key value could impersonate unknown registered service. For example,
 * if `INTERNAL_SERVICE_TOKENS_JSON` was `{"backend":"secret1","wallet":"secret2"}`,
 * sending `x-internal-token: backend` (without unknown HMAC) would pass auth.
 *
