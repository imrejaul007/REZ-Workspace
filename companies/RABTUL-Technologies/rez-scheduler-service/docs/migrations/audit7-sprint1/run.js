...
function runMigration(name, action, real) {
  return new Promise((resolve) => {
    ...
    logger.info(`\n${'='.repeat(60)}`);
    logger.info(`  Running: ${name} [${action}] ${real ? '(REAL)' : '(DRY RUN)'}`);
...
async function main() {
  ...
  logger.info('\n' + '═'.repeat(60));
  logger.info('  Audit 7 — Sprint 1 Migration Runner');
  console.log('═'.repeat(60));
  logger.info(`  Action: ${action}`);
  logger.info(`  Mode: ${auditOnly ? 'AUDIT ONLY (no changes)' : real ? 'REAL (changes will be applied)' : 'DRY RUN (safe)'}`);
  logger.info(`  MongoDB: ${process.env.MONGODB_URI ? '(from env)' : 'default URI'}`);
  ...
  if (auditOnly) {
    ...
    if (!real) {
      ...
      logger.info('  To apply for real: node run.js up real\n');
    }
