...
async function up(client) {
  ...
  logger.info(`  Orders missing cashbackAmount (can be calculated): ${missingCashback}`);
  ...
  logger.info('  Cashback percentage distribution:');
  pctDist.forEach(({ _id, count }) => {
    logger.info(`    ${_id}%: ${count} orders`);
  });
  ...
  if (DRY_RUN) {
    ...
    samples.forEach(s => {
      logger.info(`  Order ${s.orderId}: total=${s.totalAmount}, ${s.cashbackPercentage}% → ${s.calculated} paise`);
    });
  ...
  while (await cursor.hasNext()) {
    ...
    logger.info(`  Updated batch: ${result.modifiedCount} (total: ${totalUpdated})`);
  }

  logger.info(`\n=== MIGRATION COMPLETE ===`);
  logger.info(`  Total orders updated: ${totalUpdated}`);
  ...
  logger.info(`  Still missing: ${stillMissing}`);
}
...
async function down(client) {
  ...
  if (DRY_RUN) {
    ...
    logger.info(`[DRY RUN] Orders to mark stale: ${count}`);
  ...
  logger.info(`  Marked ${result.modifiedCount} orders for recalculation.`);
}
...
async function main() {
  ...
  try {
    ...
    else { logger.error('Usage: node 06_FM01_...js [up|down]'); process.exit(1); }
  } finally {
    await client.close();
  }
}
