...
jest.mock('../../models/TransactionAuditLog', () => mockAuditLogModel);
jest.mock('bullmq', () => ({
...
import { PaymentMetadata, InitiateInput } from '../../services/paymentService';
  ...
  describe('initiatePayment', () => {
    it('should reject wallet topup exceeding limit', async () => {
      const input: InitiateInput = {
        userId: 'user-123',
        orderId: 'order-456',
        amount: 150000, // Exceeds 100000 limit
    ...
    it('should reject financial service payment exceeding limit', async () => {
      const input: InitiateInput = {
        userId: 'user-123',
        orderId: 'order-456',
        amount: 600000, // Exceeds 500000 limit
        paymentMethod: 'card',
