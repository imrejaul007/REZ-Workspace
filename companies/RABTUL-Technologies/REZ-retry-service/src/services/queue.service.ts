    ...
    if (callbacks?.onJobComplete) {
      events.on('completed', ({ jobId }) => {
        this.getJob(queueName, jobId)
          ...
          .catch((err) => {
            logger.error(`Failed to get job ${jobId} for completion callback: ${err.message}`);
          });
      });
    }
    ...
    if (callbacks?.onJobFailed) {
      events.on('failed', ({ jobId, failedReason }) => {
        this.getJob(queueName, jobId)
          ...
          .catch((err) => {
            logger.error(`Failed to get job ${jobId} for failure callback: ${err.message}`);
          });
      });
    }
    ...
    if (callbacks?.onJobProgress) {
      events.on('progress', ({ jobId, data }) => {
        this.getJob(queueName, jobId)
          ...
          .catch((err) => {
            logger.error(`Failed to get job ${jobId} for progress callback: ${err.message}`);
          });
      });
    }
    ...
    if (callbacks?.onJobRetrying) {
      events.on('retries-exhausted', ({ jobId }) => {
        this.getJob(queueName, jobId)
          ...
          .catch((err) => {
            logger.error(`Failed to get job ${jobId} for retry callback: ${err.message}`);
          });
      });
    }
