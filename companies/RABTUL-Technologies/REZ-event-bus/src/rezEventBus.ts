...
export interface REZEvent<T = unknown> {
  ...
  causationId?: string;          // Original event that caused this
  ...
  sourceId: string;              // Specific instance/pod
...
class SchemaRegistry {
  ...
  validate(event: REZEvent): { valid: boolean; errors: string[] } {
...
class DeadLetterQueue {
  ...
  add(event: REZEvent, error: Error): void {
  ...
  getBySource(source: string): DeadLetterEvent[] {
    return this.events.filter(e => e.source === source);
  }

  retry(dlqEvent: DeadLetterEvent): REZEvent {
    return {
      ...dlqEvent,
...
export class REZEventBus extends EventEmitter {
  ...
  private eventStore: REZEvent[] = [];
  private schemaRegistry: SchemaRegistry;
  private deadLetterQueue: DeadLetterQueue;
  private consumerOffsets: Map<string, Map<string, number>> = new Map();
  private readonly MAX_EVENT_STORE = 100000;
   ...
   */
  async publish<T = unknown>(event: Omit<REZEvent<T>, 'id' | 'timestamp'>): Promise<string> {
    const fullEvent: REZEvent<T> = {
      ...event as REZEvent<T>,
      id: randomUUID(),
  ...
  onEvent(
    ...
    const wrappedHandler = async (event: REZEvent) => {
      try {
        await handler(event);
   ...
   * Get events by type
   */
  getEventsByType(type: string, limit = 100): REZEvent[] {
   ...
   */
  getEventsByUser(userId: string, limit = 100): REZEvent[] {
    return this.eventStore
   ...
   * Get recent events
   */
  getRecentEvents(limit = 100): REZEvent[] {
  ...
  private async handleProcessingError(event: REZEvent, error: Error): Promise<void> {
   ...
   */
  async retryDeadLetter(dlqEvent: DeadLetterEvent): Promise<string> {
  ...
  createEvent<T>(type: string, data: T): Omit<REZEvent<T>, 'id' | 'timestamp'> {
    return {
      ...
      sourceId: process.env.INSTANCE_ID || 'default',
      data
    } as Omit<REZEvent<T>, 'id' | 'timestamp'>;
  }
...
export const CommerceEvents = {
  orderCreated: (data, options?) =>
    rezEventBus.publishCommerce('order.created', data, options),

  orderCompleted: (data, options?) =>
    rezEventBus.publishCommerce('order.completed', data, options),

  orderCancelled: (data, options?) =>
    rezEventBus.publishCommerce('order.cancelled', data, options),

  paymentCompleted: (data, options?) =>
    rezEventBus.publishCommerce('payment.completed', data, options),

  cartUpdated: (data, options?) =>
    rezEventBus.publishCommerce('cart.updated', data, options),

  checkoutStarted: (data, options?) =>
    rezEventBus.publishCommerce('checkout.started', data, options),
};

export const IdentityEvents = {
  userCreated: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('identity.user.created', data), ...options }),

  userLinked: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('identity.user.linked', data), ...options }),

  profileUpdated: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('identity.profile.updated', data), ...options }),
};

export const LoyaltyEvents = {
  pointsEarned: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('loyalty.points.earned', data), ...options }),

  pointsRedeemed: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('loyalty.points.redeemed', data), ...options }),

  tierChanged: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('loyalty.tier.changed', data), ...options }),
};

export const IntelligenceEvents = {
  intentDetected: (data, options?) =>
    rezEventBus.publishIntelligence('intent.detected', data, options),

  churnRisk: (data, options?) =>
    rezEventBus.publishIntelligence('churn.risk', data, options),

  ltvUpdated: (data, options?) =>
    rezEventBus.publishIntelligence('ltv.updated', data, options),

  segmentChanged: (data, options?) =>
    rezEventBus.publishIntelligence('segment.changed', data, options),
};

export const EngagementEvents = {
  pageViewed: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('engagement.page.viewed', data), ...options }),

  qrScanned: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('engagement.qr.scanned', data), ...options }),

  searchPerformed: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('engagement.search.performed', data), ...options }),
};

export const SupportEvents = {
  ticketCreated: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('support.ticket.created', data), ...options }),

  csatSubmitted: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('support.csat.submitted', data), ...options }),

  ticketResolved: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('support.ticket.resolved', data), ...options }),
};

export const MediaEvents = {
  adImpression: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('media.ad.impression', data), ...options }),

  adConversion: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('media.ad.conversion', data), ...options }),

  screenViewed: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('media.screen.viewed', data), ...options }),
};

export const WhatsAppEvents = {
  messageReceived: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('whatsapp.message.received', data), ...options }),

  messageSent: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('whatsapp.message.sent', data), ...options }),

  sessionStarted: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('whatsapp.session.started', data), ...options }),

  sessionEnded: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('whatsapp.session.ended', data), ...options }),

  conversationCreated: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('whatsapp.conversation.created', data), ...options }),
};

export const CartEvents = {
  cartAbandoned: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('commerce.cart.abandoned', data), ...options }),

  cartRecovered: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('commerce.cart.recovered', data), ...options }),
};

export const LoyaltyTierEvents = {
  tierUpgraded: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('loyalty.tier.upgraded', data), ...options }),

  tierDowngraded: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('loyalty.tier.downgraded', data), ...options }),
};

export const CampaignEvents = {
  campaignStarted: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('engagement.campaign.started', data), ...options }),

  campaignCompleted: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('engagement.campaign.completed', data), ...options }),

  campaignEngaged: (data, options?) =>
    rezEventBus.publish({ ...rezEventBus.createEvent('engagement.campaign.engaged', data), ...options }),
};
