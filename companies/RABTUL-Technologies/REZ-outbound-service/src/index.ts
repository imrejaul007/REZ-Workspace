 ...
 *
 * Multi-channel Outbound Sequences Service
 * Port: 4130
 */
...
app.get('/api/v1', (_req: Request, res: Response) => {
  res.json({
    ...
    service: 'REZ Outbound Service',
    version: '1.0.0',
    description: 'Multi-channel Outbound Sequences Service',
    endpoints: {
...
function scheduleTasks(): void {
  // Process due prospects every 5 minutes
  cron.schedule('*/5 * * * *', async () => {
    logger.info('Running scheduled task: process due prospects');
    // In production, this would call the sequence processor
  });

  // Update sequence stats every hour
  cron.schedule('0 * * * *', async () => {
    logger.info('Running scheduled task: update sequence stats');
    // In production, this would update aggregate stats
  });
}
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(config.PORT, () => {
      logger.info('REZ Outbound Service started', {
        port: config.PORT,
        env: config.NODE_ENV,
      });
    });
