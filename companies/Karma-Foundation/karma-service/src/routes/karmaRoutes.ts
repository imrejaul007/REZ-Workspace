...
router.patch('/booking/:bookingId/approve', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    // SECURITY FIX: Validate approved is explicitly a boolean
    const { approved } = req.body as { approved: boolean };

    if (typeof approved !== 'boolean') {
      res.status(400).json(err('SRV_001', 'approved must be a boolean'));
      return;
    }
    ...
    await EventBookingModel.findByIdAndUpdate(bookingId, {
      $set: {
        ngoApproved: approved,
        ngoApprovedAt: approved ? new Date() : undefined,
      },
    });

    logger.info('[karmaRoutes] Booking approval updated', { bookingId, approved });

    res.json({ success: true, message: approved ? 'Booking approved' : 'Approval revoked' });
  } catch (err) {
