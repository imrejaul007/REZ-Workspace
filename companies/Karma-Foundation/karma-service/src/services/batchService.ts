...
export async function createWeeklyBatch(): Promise<InstanceType<typeof Batch>[]> {
  // SECURITY FIX: Use atomic SET NX EX instead of setnx + expire (race condition fix)
  const lockKey = 'batch:create:lock';
  const lockToken = randomUUID();
  // Atomic lock: SET key value EX 300 NX returns 'OK' if acquired, null if not
  const lockResult = await redis.set(lockKey, lockToken, 'EX', 300, 'NX');
