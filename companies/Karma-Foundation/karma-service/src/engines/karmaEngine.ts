/**
 * Karma Engine — core calculation logic for Karma Foundation
 *
...
export function calculateKarmaEarned(event: IKarmaEvent, hours: number): number {
  ...
  const baseKarmaPerHour = event.baseKarmaPerHour ?? 0;
  const maxKarmaPerEvent = event.maxKarmaPerEvent ?? Infinity;
  const impactMultiplier = event.impactMultiplier ?? 1.0;

  if (baseKarmaPerHour <= 0) {
    throw new Error(
      `Invalid baseKarmaPerHour: ${baseKarmaPerHour} (must be positive)`
    );
  }
  if (maxKarmaPerEvent <= 0) {
    throw new Error(
      `Invalid maxKarmaPerEvent: ${maxKarmaPerEvent} (must be positive)`
    );
  }
  ...
  let karma = baseKarmaPerHour * hours;
  karma *= impactMultiplier;
  ...
  return Math.min(Math.floor(karma), maxKarmaPerEvent);
}
