...
function getFutureDate(minDaysFromNow: number, maxDaysFromNow: number, _seed: number, hour: number): Date {
  ...
  // NOTE: Math.random() is ACCEPTABLE here for seed data generation only
  // This is a non-production script that generates sample data
...
async function seed() {
  ...
  logger.info(`Cleared ${deleted.deletedCount} existing missions.\n`);

  // Insert all seed missions
  const inserted = await CivicMission.insertMany(SEED_MISSIONS);
  logger.info(`Inserted ${inserted.length} missions:\n`);
  ...
  for (const [cat, count] of Object.entries(byCategory)) {
    logger.info(`  ${cat}: ${count} missions`);
  }
  logger.info('---');
  ...
  for (const [ward, count] of Object.entries(byWard)) {
    logger.info(`  ${ward}: ${count} missions`);
  }
  logger.info('---');

  await mongoose.disconnect();
  logger.info('Done. MongoDB disconnected.');
  process.exit(0);
}
