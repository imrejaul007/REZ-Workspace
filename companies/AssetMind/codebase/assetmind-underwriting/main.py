...
Usage:
    python main.py
    uvicorn main:app --host 0.0.0.0 --port 5281
"""
...
def calculate_metrics(inputs: FinancialInputs) -> Dict[str, Any]:
    ...
    ocf = net_income + inputs.depreciation + inputs.amortization
    ...
    current_liabilities = inputs.accounts_payable
...
@app.get("/")
async def root():
    return {"service": "AssetMind Underwriting Agent", "version": "1.0.0", "docs": "/docs"}
...
async def underwrite(request: UnderwritingRequest):
    ...
    base_loan = min(request.loan_amount_requested, metrics["ebitda"] * 5)
    if request.loan_type == LoanType.ABL:
        base_loan = min(base_loan, metrics["current_assets"] * 0.8)
    elif request.loan_type == LoanType.MEZZANINE:
        base_loan = min(base_loan, metrics["ebitda"] * 3)
...
async def coverage_analysis(ebit: float, interest_expense: float):
    ...
    return {
        ...
        "interest_coverage": round(interest_coverage, 2),
        "implied_stress_level": stress_level
    }


@app.get("/loan-types")
async def list_loan_types():
    """List supported loan types."""
    return {
        "loan_types": [
            {"type": "term_loan", "description": "Standard term loan with fixed amortization"},
            {"type": "revolving_credit", "description": "Revolving credit facility with draw/pay flexibility"},
            {"type": "abl", "description": "Asset-based lending secured by receivables/inventory"},
            {"type": "mezzanine", "description": "Subordinated debt with equity-like returns"},
            {"type": "unitranche", "description": "Combined senior and subordinated in single facility"}
        ]
    }
...
async def list_grades():
    """List lender grades with descriptions."""
    return {
        "grades": [
            {"grade": "AAA", "range": "85-100", "description": "Best-in-class credit"},
            {"grade": "AA", "range": "75-84", "description": "Strong credit"},
            {"grade": "A", "range": "65-74", "description": "Good credit"},
            {"grade": "BBB", "range": "50-64", "description": "Acceptable credit"},
            {"grade": "BB", "range": "35-49", "description": "Speculative grade"},
            {"grade": "B", "range": "20-34", "description": "High risk"},
            {"grade": "CCC", "range": "0-19", "description": "Very high risk"}
        ]
    }
