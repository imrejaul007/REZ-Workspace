...
class RevenueAnalysis(BaseModel):
    ...
    revenue_concentration: Dict[str, float]  # customer -> percentage
...
def analyze_revenue_concentration(text: str) -> RevenueAnalysis:
    ...
    if total_revenue == 0:
        total_revenue = 1000000  # Default for demo
...
def analyze_debt_capacity(financials: Dict[str, float]) -> DebtCapacityAnalysis:
    ...
    # Determine acceptable leverage based on industry
...
def calculate_overall_score(
    revenue: Optional[RevenueAnalysis],
    customers: Optional[CustomerAnalysis],
    risks: Optional[RiskAnalysis],
    debt: Optional[DebtCapacityAnalysis]
) -> tuple:
...
async def analyze_diligence(
    ...
    score, rating = calculate_overall_score(
        revenue_analysis, customer_analysis, risk_analysis, debt_capacity
    )
...
async def compare_deals(deals: List[Dict[str, Any]]):
    ...
    return {
        ...
        "best_deal": comparisons[0] if comparisons else None
    }
