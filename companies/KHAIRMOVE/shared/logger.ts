...
function redactObject(obj: unknown): Record<string, unknown> | null {
...
function formatLog(level: LogLevel, message: string, meta?: Record<string, unknown>): string {
  ...
  if (meta) {
    ...
    if (redactedMeta) {
      Object.assign(entry, redactedMeta);
    }
  }
...
const logger = {
  ...
  error(message: string, error?: Error | unknown, meta?: Record<string, unknown>): void {
    if (shouldLog('error')) {
      ...
      const mergedMeta = meta ? { ...meta, error: errorObj } : { error: errorObj };
      const entry = formatLog('error', message, mergedMeta);
