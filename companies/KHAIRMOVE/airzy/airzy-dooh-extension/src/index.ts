...
// Apply auth middleware to all DOOH routes
app.use('/api/v1/dooh', authenticate, routes);
app.use('/api/v1/screens', authenticate, routes);
app.use('/api/v1/campaigns', authenticate, routes);

app.get('/', (req: Request, res: Response) => {
  res.json({ service: 'Airzy DOOH Extension', version: config.version, status: 'running' });
});

// Health checks
app.get('/health/live', (req: Request, res: Response) => {
  res.json({ status: 'alive', service: 'airzy-dooh-extension', timestamp: new Date() });
});
...
async function startServer() {
  try {
    ...
    server = app.listen(config.server.port, config.server.host, () => {
      logger.info(`Airzy DOOH Extension started on port ${config.server.port}`);
    });
  } catch (error) {
