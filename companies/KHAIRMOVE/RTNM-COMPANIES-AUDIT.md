...
KHAIRMOVE integrates with StayOwn Hotel OS for airport transfers and guest transportation.

#### Integration Endpoints

| Endpoint | Method | Purpose |
|----------|--------|---------|
| `/api/hotels/:hotelId/transfer-requests` | GET | Get scheduled airport transfers |
| `/api/transfers/book` | POST | Book airport transfer for guest |
| `/api/transfers/:transferId/status` | POST | Update transfer status |
| `/api/guests/:guestId` | GET | Get guest info |
| `/api/rides/:rideId/link-hotel` | POST | Link ride to hotel booking |
| `/api/integration/stayown/status` | GET | Check StayOwn integration health |

#### Integration Flow

```
Hotel guest requests airport transfer
        ↓
GET /api/hotels/:hotelId/transfer-requests
        ↓
KHAIRMOVE assigns driver
        ↓
Driver status updates via POST /api/transfers/:id/status
        ↓
StayOwn notified of ETA for guest coordination
        ↓
POST /api/rides/:id/link-hotel for seamless checkout
```
...
|---------|---------|------------------|
| **StayOwn** | Hotel OS (3899) | Airport transfers, guest pickup |
