...
import qbrRoutes from './routes/qbr';
import { logger } from 'utils/logger.js';
import { metricsMiddleware, metricsEndpoint } from './utils/metrics';

const PORT = parseInt(process.env.PORT || '5081');
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar_qbr';
...
app.get('/health', async (_req: Request, res: Response) => {
  ...
  res.status(status === 'healthy' ? 200 : 503).json({
    status,
    service: 'qbr-automation-service',
    port: PORT,
...
app.use('/api/qbr', authMiddleware, qbrRoutes);
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`QBR Automation Service started on port ${PORT}`);
      logger.info(`Health check: http://localhost:${PORT}/health`);
      logger.info(`API: http://localhost:${PORT}/api/qbr`);
    });
  } catch (error) {
