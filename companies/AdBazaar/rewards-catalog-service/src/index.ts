/**
 * rewards-catalog-service - Rewards catalog and redemption
 * Port: 5102
 */
...
const app = express();
const PORT = parseInt(process.env.PORT || '5102', 10);
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/rewards-catalog';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({ status: mongoOk ? 'healthy' : 'degraded', service: 'rewards-catalog-service', version: '1.0.0', mongodb: mongoOk ? 'connected' : 'disconnected' });
});
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => { logger.info(`[${new Date().toISOString()}] rewards-catalog-service running on port ${PORT}`); });
  } catch (error) {
