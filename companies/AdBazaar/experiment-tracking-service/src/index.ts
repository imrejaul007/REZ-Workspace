/**
 * experiment-tracking-service - Centralized experiment tracking
 * Port: 5094
 */
...
const app = express();
const PORT = parseInt(process.env.PORT || '5094', 10);
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/experiment-tracking';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({
    ...
    service: 'experiment-tracking-service',
    version: '1.0.0',
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`[${new Date().toISOString()}] experiment-tracking-service running on port ${PORT}`);
    });
  } catch (error) {
