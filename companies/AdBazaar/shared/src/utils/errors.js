...
exports.errors = {
    ...
    validation: (message = 'Validation error') => new AppError(message, 400, 'VALIDATION_ERROR'),
    tooManyRequests: (message = 'Too many requests') => new AppError(message, 429, 'RATE_LIMIT_EXCEEDED'),
    internal: (message = 'Internal server error') => new AppError(message, 500, 'INTERNAL_ERROR'),
    serviceUnavailable: (message = 'Service unavailable') => new AppError(message, 503, 'SERVICE_UNAVAILABLE'),
};
