/**
 * recommendation-engine-service - Product/content recommendations
 * Port: 5095
 */
...
const app = express();
const PORT = parseInt(process.env.PORT || '5095', 10);
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`[${new Date().toISOString()}] recommendation-engine-service running on port ${PORT}`);
    });
  } catch (error) {
