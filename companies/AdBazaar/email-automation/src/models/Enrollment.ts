...
export interface IAction extends Document {
  ...
  actionType: 'sent' | 'delivered' | 'opened' | 'clicked' | 'replied' | 'bounced' | 'unsubscribed' | 'skipped';
  timestamp: Date;
  metadata?: {
    linkUrl?: string;
    userAgent?: string;
...
export interface IEnrollment extends Document {
  ...
  userId: string;
  email: string;
  status: 'active' | 'completed' | 'opted_out' | 'bounced' | 'paused';
  enrolledAt: Date;
  completedAt?: Date;
...
const ActionSchema = new Schema<IAction>(
  {
    ...
    actionType: {
      type: String,
      enum: ['sent', 'delivered', 'opened', 'clicked', 'replied', 'bounced', 'unsubscribed', 'skipped'],
      required: true,
    },
...
const EnrollmentSchema = new Schema<IEnrollment>(
  {
    ...
    status: {
      type: String,
      enum: ['active', 'completed', 'opted_out', 'bounced', 'paused'],
      default: 'active',
      index: true,
    },
