/**
 * personalization-rules-service - Rule-based personalization management
 * Port: 5096
 */
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/personalization-rules';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({ status: mongoOk ? 'healthy' : 'degraded', service: 'personalization-rules-service', version: '1.0.0', mongodb: mongoOk ? 'connected' : 'disconnected' });
});
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => { logger.info(`[${new Date().toISOString()}] personalization-rules-service running on port ${PORT}`); });
  } catch (error) {
