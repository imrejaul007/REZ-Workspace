...
app.use('/api/cohorts', authMiddleware, cohortRoutes);
app.use('/api/analyses', authMiddleware, analysisRoutes);
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar-cohorts';
...
const startServer = async () => {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Cohort Analysis Service running on port ${PORT}`);
    });
  } catch (error) {
