/**
 * multivariate-testing-service - Full multivariate testing capabilities
 * Port: 5093
 */
...
const app = express();
const PORT = parseInt(process.env.PORT || '5093', 10);
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/multivariate-testing';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({
    ...
    service: 'multivariate-testing-service',
    version: '1.0.0',
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`[${new Date().toISOString()}] multivariate-testing-service running on port ${PORT}`);
    });
  } catch (error) {
