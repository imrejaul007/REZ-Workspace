...
const PORT = parseInt(process.env.PORT || '5076');
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar_customer_health';
...
app.get('/health', async (_req: Request, res: Response) => {
  ...
  res.status(status === 'healthy' ? 200 : 503).json({
    status,
    service: 'customer-health-score-service',
    port: PORT,
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Customer Health Score Service started on port ${PORT}`);
