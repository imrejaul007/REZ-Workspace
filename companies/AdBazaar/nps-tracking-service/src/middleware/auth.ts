/**
 * NPS Tracking Service - Authentication & Security Middleware
 */
...
export function requestIdMiddleware(req: Request, res: Response, next: NextFunction): void {
  const requestId = (req.headers['x-request-id'] as string) || `nps-${Date.now()}-${randomUUID().replace(/-/g, '').substring(0, 9)}`;
