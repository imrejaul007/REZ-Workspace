...
const PORT = parseInt(process.env.PORT || '5078');
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar_nps';
...
app.use('/api/nps', authMiddleware, npsRoutes);
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`NPS Tracking Service started on port ${PORT}`);
      logger.info(`Health check: http://localhost:${PORT}/health`);
      logger.info(`API: http://localhost:${PORT}/api/nps`);
    });
  } catch (error) {
