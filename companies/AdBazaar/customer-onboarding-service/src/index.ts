...
const PORT = parseInt(process.env.PORT || '5077');
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar_onboarding';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Customer Onboarding Service started on port ${PORT}`);
      logger.info(`Health check: http://localhost:${PORT}/health`);
      logger.info(`API: http://localhost:${PORT}/api/onboarding`);
    });
  } catch (error) {
