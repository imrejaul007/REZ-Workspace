...
class Logger {
  ...
  constructor(context: string = 'App', level: LogLevel = LogLevel.INFO) {
    ...
    this.logger = winston.createLogger({
      ...
      defaultMeta: { service: 'REZ-email-validator', context: this.context },
      transports: [
  ...
  logValidation(email: string, isValid: boolean, score: number, duration: number): void {
    this.info(`Validated ${email}: valid=${isValid}, score=${score}`, { duration: `${duration}ms` });
  }

  logMXCheck(domain: string, hasMX: boolean, recordCount: number): void {
    this.info(`MX check for ${domain}: hasMX=${hasMX}, records=${recordCount}`);
  }

  logSMTPCheck(domain: string, connected: boolean, responseTime: number): void {
    this.info(`SMTP check for ${domain}: connected=${connected}`, { responseTime: `${responseTime}ms` });
  }

  logDisposableCheck(email: string, isDisposable: boolean, provider: string | null): void {
    this.info(`Disposable check for ${email}: isDisposable=${isDisposable}`, { provider });
  }
