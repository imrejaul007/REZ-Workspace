...
export const metricsMiddleware = (req: Request, res: Response, next: NextFunction) => {
  ...
  res.on('finish', () => trackRequest(req.path, Date.now() - start, res.statusCode));
  next();
};
export const getMetrics = () => ({ endpoints: metrics, totalRequests: Object.values(metrics).reduce((s, m) => s + m.requests, 0), totalErrors: Object.values(metrics).reduce((s, m) => s + m.errors, 0), uptime: process.uptime() });