...
export class AnalyticsService {
  ...
  private generateHistoricalDataPoint(
    ...
    // Simulate realistic data with some variance using crypto
    const baseImpressions = granularity === 'hour' ? 500 : granularity === 'day' ? 12000 : 50000;
    const variance = 0.2; // 20% variance

    const impressions = Math.round(baseImpressions * (1 + (randomInt(-100, 101) / 1000) * variance));
    const ctr = 0.02 + randomInt(20, 51) / 1000; // 2-5% CTR
    const clicks = Math.round(impressions * ctr);
    const conversionRate = 0.03 + randomInt(20, 41) / 1000; // 3-5% conversion
    const conversions = Math.round(clicks * conversionRate);
    const cpc = 0.3 + randomInt(30, 81) / 100; // $0.30-$0.80 CPC
    const spend = clicks * cpc;
    const aov = 35 + randomInt(0, 21); // $35-$55 AOV
  ...
  private getMockRealTimeMetrics(): RealTimeMetrics {
    return {
      ...
      spend: Math.round((10 + randomInt(0, 6)) * 100) / 100,
      timestamp: new Date(),
    };
  }
}
