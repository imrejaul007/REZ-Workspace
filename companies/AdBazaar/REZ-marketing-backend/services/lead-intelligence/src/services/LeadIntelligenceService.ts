...
class ReZMindClient {
  ...
  constructor() {
    this.baseUrl = config.services.mind || 'http://localhost:4008';
...
export class LeadIntelligenceService {
   ...
   */
  private calculateIntentStrength(activityCache): number {
    ...
    const totalShown = searches.reduce((sum: number, s) => sum + (s.resultsCount || 0), 0);
    const totalClicked = searches.reduce(
      (sum: number, s) => sum + (s.clickedResults?.length || 0),
      0
    );
    if (totalShown > 0) {
    ...
    const hasStrongIntent = searches.some(
      (s) => s.intentDetected && ['buy', 'purchase', 'order', 'price'].includes(s.intentDetected.toLowerCase())
    );
    if (hasStrongIntent) {
      strength += 0.2;
    }
   ...
   */
  private heuristicPurchaseProbability(activityCache): number {
