...
async function handleIncomingWhatsApp(message) {
  try {
    ...
    logger.error('[WhatsApp] Handle error:', { error: error instanceof Error ? error.message : String(error) });
  }
}
...
app.listen(PORT, () => {
  logger.info(`[ReZ Unified Messaging] Running on port ${PORT}`);
  logger.info(`[ReZ Unified Messaging] WhatsApp webhook: /webhook/whatsapp`);
  logger.info(`[ReZ Unified Messaging] ReZ Mind: ${process.env.REZMIND_URL || 'localhost:4010'}`);
});
