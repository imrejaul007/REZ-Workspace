...
export class RealTimeTriggerEngine {
   ...
   * Evaluate operator
   */
  private evaluateOperator(value, operator: string, target): boolean {
    switch (operator) {
      case 'eq':
        return value === target;
      case 'ne':
...
Your coins are on the way! 🪙`
   ...
   * Get decision from Supreme Controller
   */
  private async getDecision(event: TriggerContext, rule: TriggerRule, actionConfig): Promise<unknown> {
    ...
    const response = await fetch(`${process.env.RDE_URL || 'http://localhost:4027'}/api/rde/decide`, {
      method: 'POST',
