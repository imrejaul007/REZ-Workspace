...
export class ScreenManager {
  ...
  updateStatus(screenId: string, status: ScreenStatus): void {
    ...
    if (screen) {
      (screen as unknown).last_seen = new Date()
  ...
  processHeartbeat(heartbeat: ScreenHeartbeat): ContentUpdate | null {
    ...
    ;(screen as unknown).last_seen = heartbeat.timestamp
    screen.status = heartbeat.status
    ;(screen as unknown).last_sync = heartbeat.timestamp
  ...
  private needsPlaylistUpdate(screen: Screen, heartbeat: ScreenHeartbeat): boolean {
    const version = (screen as unknown).playlist_version as number | undefined
    if (!version) return true
    if (heartbeat.playlist_version !== version) return true

    const lastSync = (screen as unknown).last_sync as Date | undefined
  ...
  private generateContentUpdate(screen: Screen): ContentUpdate {
    return {
      ...
      version: ((screen as unknown).playlist_version as number || 0) + 1,
      timestamp: new Date()
    }
  }
