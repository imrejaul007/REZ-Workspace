...
/**
 * Raw response from ReZ Mind API
 */
interface ReZMindUserKnowledgeResponse {
  userId?: string;
  id?: string;
  preferences?: {
    cuisines?: string[];
...
/**
 * Raw voucher from marketing service
 */
...
class ReZMindClient {
   ...
   */
  private transformMindData(data: ReZMindUserKnowledgeResponse): UserKnowledge {
    const getDate = (dateValue: string | Date | undefined): Date => {
    ...
    return {
      ...
      history: {
        ...
        lastOrderDate: getDate(data.history?.lastOrderDate ?? data.lastOrderDate),
      },
      signals: {
      ...
      lastUpdated: getDate(data.lastUpdated),
    };
  }
...
class MarketingServiceClient {
  ...
  async getVouchers(options?: {
    ...
    try {
      ...
      return vouchers.map((v: RawVoucher) => this.transformVoucherToOffer(v));
    } catch (error) {
      const axiosError = error as AxiosError;
   ...
   */
  private transformVoucherToOffer(voucher: RawVoucher): AvailableOffer {
    const getOfferType = (type?: string): AvailableOffer['type'] => {
      if (type === 'percentage' || type === 'fixed' || type === 'bogo' || type === 'free_delivery' || type === 'cashback') {
        return type;
      }
      return 'percentage';
    };
    ...
    return {
      ...
      merchantId: String(voucher.merchantId || voucher.storeId || ''),
      productId: voucher.productId,
      category: voucher.category,
      type: getOfferType(voucher.type),
      value: voucher.value ?? 0,
      code: voucher.code,
      description: voucher.description,
      minOrderValue: voucher.minOrderValue ?? 0,
      maxDiscount: voucher.maxDiscount,
      validFrom: getDate(voucher.validFrom),
      validUntil: getDate(voucher.validUntil),
