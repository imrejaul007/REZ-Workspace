...
function log(message: string, meta?) {
  ...
  logger.info(`${timestamp} [FEEDBACK] ${message}`, meta ? JSON.stringify(meta) : '');
}
...
function calculateLearningSignal(data): unknown {
...
async function calculateDeltaStdDev(filter): Promise<number> {
  try {
...
// Build delta distribution histogram
async function buildDeltaHistogram(filter): Promise<unknown[]> {
  try {
    const buckets = [
...
app.get('/insights/weekly', async (req: Request, res: Response) => {
  try {
    ...
    // 3. Merchant patterns
    const merchantPatterns: Record<string, unknown> = {};
    weekFeedback.forEach(fb => {
      if (fb.merchantId) {
    ...
    Object.entries(merchantPatterns).forEach(([merchantId, m]: [string, unknown]) => {
...
function detectClusters(data: unknown[]) {
