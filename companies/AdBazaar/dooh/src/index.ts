...
export class DOOHNetwork {
  private deliveryEngine: unknown
  private playlistGenerator: unknown
  private screenManager: unknown
  ...
  getAdsForScreen(screenId: string, campaigns: unknown[]) {
  ...
  generatePlaylist(screenId: string, campaigns: unknown[]) {
    const screen = this.screenManager.get(screenId)
    if (!screen) throw new Error('Screen not found')
