...
export async function GET(
  ...
  try {
    ...
    logger.error('[consultations/bookingId] error:', e)
    return NextResponse.json(
...
export async function PATCH(
  ...
  try {
    ...
    if (updateError) {
      logger.error('[consultations/bookingId] update error:', updateError)
      return NextResponse.json(
...
export async function DELETE(
  ...
  try {
    ...
    if (cancelError) {
      logger.error('[consultations/bookingId] cancel error:', cancelError)
      return NextResponse.json(
    ...
    logger.error('[consultations/bookingId] delete error:', e)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
