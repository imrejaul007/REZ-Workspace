...
export async function getAvailableConsultations(): Promise<Consultation[]> {
  try {
    ...
    logger.error('Get available consultations error:', error);
    return [];
  }
}
...
export async function bookConsultation(params: BookConsultationParams): Promise<{
  ...
  try {
    ...
    const bookingId = `book_${Date.now()}_${randomUUID().replace(/-/g, '').substring(0, 9)}`;
    ...
    if (bookingError) {
      logger.error('Book consultation error:', bookingError);
...
export async function generateBookingQR(bookingId: string): Promise<{
  ...
  try {
    ...
    logger.error('Generate booking QR error:', error);
