...
export async function getAvailableSamples(): Promise<FreeSample[]> {
  try {
    ...
    logger.error('Get available samples error:', error);
    return [];
  }
}
...
export async function getSamplesByBrand(brandId: string): Promise<FreeSample[]> {
  try {
    ...
    logger.error('Get samples by brand error:', error);
    return [];
  }
}
...
export async function checkSampleAvailability(
  ...
  try {
    ...
    const availability = inventory.map(inv => ({
      ...
      storeName: (inv.stores as unknown)?.name || 'Unknown Store',
      storeAddress: (inv.stores as unknown)?.address || '',
      available: inv.quantity > 0,
      quantity: inv.quantity,
      distance: location ? calculateDistance(
        location.lat,
        location.lng,
        (inv.stores as unknown)?.latitude || 0,
        (inv.stores as unknown)?.longitude || 0
      ) : undefined,
    }));
    ...
    logger.error('Check sample availability error:', error);
    return [];
  }
}
...
export async function requestSample(params: RequestSampleParams): Promise<{
  ...
  try {
    ...
    const requestId = `req_${Date.now()}_${randomUUID().replace(/-/g, '').substring(0, 9)}`;
    ...
    if (error) {
      logger.error('Request sample error:', error);
...
export async function generatePickupQR(requestId: string): Promise<{
  ...
  try {
    ...
    logger.error('Generate pickup QR error:', error);
...
export async function getUserSampleRequests(userId: string): Promise<SampleRequest[]> {
  try {
    ...
    logger.error('Get user sample requests error:', error);
    return [];
  }
}
...
export async function getActiveSampleRequest(requestId: string): Promise<SampleRequest | null> {
  try {
    ...
    logger.error('Get active sample request error:', error);
    return null;
  }
}
...
export async function cancelSampleRequest(requestId: string, userId: string): Promise<{
  ...
  try {
    ...
    logger.error('Cancel sample request error:', error);
