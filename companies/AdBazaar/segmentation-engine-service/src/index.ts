/**
 * segmentation-engine-service - Real-time audience segmentation
 * Port: 5097
 */
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/segmentation-engine';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({ status: mongoOk ? 'healthy' : 'degraded', service: 'segmentation-engine-service', version: '1.0.0', mongodb: mongoOk ? 'connected' : 'disconnected' });
});
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => { logger.info(`[${new Date().toISOString()}] segmentation-engine-service running on port ${PORT}`); });
  } catch (error) {
