...
const CreateDataSourceSchema = z.object({
  name: z.string().min(1),
  type: z.enum(['mongodb', 'postgresql', 'mysql', 'api', 'rest', 'graphql']),
  connection: z.object({
  ...
  queryExamples: z.array(z.string()).optional(),
...
router.post('/:id/query', authMiddleware, async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { query } = QuerySchema.parse(req.body);
    const orgId = req.user?.organizationId || 'default';

    const result = await dataSourceService.executeQuery(id, query, orgId);
    ...
    logger.error(`Error executing query for ${req.params.id}:`, error);
    res.status(400).json({ success: false, error: error.message });
  }
});
