/**
 * unified-checkout-service - Cross-channel checkout experience
 * Port: 5098
 */
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/unified-checkout';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({ status: mongoOk ? 'healthy' : 'degraded', service: 'unified-checkout-service', version: '1.0.0', mongodb: mongoOk ? 'connected' : 'disconnected' });
});
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => { logger.info(`[${new Date().toISOString()}] unified-checkout-service running on port ${PORT}`); });
  } catch (error) {
