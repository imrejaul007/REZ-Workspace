...
export const merchantBidSchema = z.object({
  merchantId: z.string().min(1),
  campaignId: z.string().min(1),
  userId: z.string().min(1),
  baseBid: z.number().positive(),
  cpm: z.number().nonnegative().optional(),
  cpc: z.number().nonnegative().optional(),
  cpa: z.number().nonnegative().optional(),
  qualityScore: z.number().min(0).max(100).optional(),
  intentMatch: z.number().min(0).max(100).optional(),
  historicalCTR: z.number().min(0).max(1).optional(),
  conversionRate: z.number().min(0).max(1).optional(),
  discount: z.number().min(0).max(100).optional(),
  coinReward: z.number().nonnegative().optional(),
  startTime: z.union([z.date(), z.string()]).transform(val => val instanceof Date ? val : new Date(val)),
  endTime: z.union([z.date(), z.string()]).transform(val => val instanceof Date ? val : new Date(val)),
  status: z.enum(['active', 'paused', 'completed', 'out_of_budget']).default('active')
});
...
export type MerchantBidInput = z.infer<typeof merchantBidSchema>;
export type AuctionRequestInput = z.infer<typeof auctionRequestSchema>;
...
export interface ScoredBid extends MerchantBid {
  finalScore: number;
...
export class AuctionEngine {
  ...
  async runAuction(request: AuctionRequest): Promise<AuctionResult> {
    const auctionId = `auction_${Date.now()}_${randomUUID().replace(/-/g, '').substring(0, 9)}`;
    ...
    const reasons = winner ? this.generateReasons(winner, scored[1] || null) : ['No bids'];
   ...
   * Score all bids
   */
  private async scoreBids(bids: MerchantBid[], request: AuctionRequest): Promise<ScoredBid[]> {
    ...
    for (const bid of bids) {
      ...
      // 2. Quality score (25%)
      components.qualityScore = (bid.qualityScore || 50) * 0.25;
      ...
      // 4. Historical CTR (15%)
      components.historicalCTR = (bid.historicalCTR || 0.05) * 0.15;

      // 5. Conversion rate (15%)
      components.conversionRate = (bid.conversionRate || 0.05) * 0.15;

      const finalScore = Object.values(components).reduce((a, b) => a + b, 0);

      // Merge bid properties with score
      scored.push({
        ...bid,
        finalScore,
        components
   ...
   */
  private generateReasons(winner: ScoredBid, second: ScoredBid | null): string[] {
  ...
  private async getRank(bid: MerchantBid): Promise<number> {
    ...
    const index = scored.findIndex(s => s.merchantId === bid.merchantId);
    return index + 1;
  }
  ...
  private parseBid(data: Record<string, string>): MerchantBid {
    return {
      ...
      status: (data.status as 'active' | 'paused' | 'completed' | 'out_of_budget') || 'active'
    };
  }
