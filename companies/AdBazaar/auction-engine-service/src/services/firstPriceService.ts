...
import { Bid, AuctionResult, AdSlot, Deal, AuctionAnalytics, FirstPriceAuctionRequest } from '../types';
import { Auction } from '../models/Auction';
import { AuctionHistory } from '../models/AuctionHistory';
import logger from 'utils/logger.js';
import { recordAuctionMetrics, recordBidMetrics } from '../utils/metrics';

export class FirstPriceService {
  async execute(request: FirstPriceAuctionRequest): Promise<AuctionResult> {
    const startTime = Date.now();
    logger.info(`Executing first-price auction: ${request.auctionId}`, {
    ...
    try {
      ...
      if (dealWinner) {
        ...
        secondPrice = sortedBids[1]?.price || 0;
        ...
        if (highestBid.price >= reservePrice) {
          ...
          reasoning = `Seat ${highestBid.seatId} wins with highest bid ${price}`;
        } else {
          status = 'no-fill';
        ...
        if (sortedBids[0].price >= reservePrice) {
          winner = sortedBids[0];
          price = winner.price;
          reasoning = `Single bidder ${winner.seatId} wins at ${price}`;
        } else {
          status = 'no-fill';
      ...
      recordBidMetrics('first-price', request.bids.length);
      recordAuctionMetrics('first-price', status, duration / 1000, price, winner?.seatId);
      ...
      logger.info(`First-price auction completed: ${request.auctionId}`, { auctionId: request.auctionId, winner: winner?.seatId, price, duration });
      return result;
    } catch (error) {
      logger.error(`First-price auction failed: ${request.auctionId}`, { auctionId: request.auctionId, error: error instanceof Error ? error.message : 'Unknown error' });
      throw error;
    }
  }
  ...
  private calculateAnalytics(bids: Bid[]): AuctionAnalytics {
    ...
    return {
      totalBids,
      bidFloor: prices[0] || 0,
      bidCeiling: prices[prices.length - 1] || 0,
      spread: (prices[prices.length - 1] || 0) - (prices[0] || 0),
      avgBidPrice: prices.reduce((a, b) => a + b, 0) / totalBids || 0,
      medianBidPrice: totalBids > 0 ? (totalBids % 2 !== 0 ? prices[Math.floor(totalBids / 2)] : (prices[Math.floor(totalBids / 2) - 1] + prices[Math.floor(totalBids / 2)]) / 2) : 0,
    };
  }
  ...
  private checkDeals(deals: Deal[] | undefined, sortedBids: Bid[], adSlots: AdSlot[]): { bid: Bid; deal: Deal } | null {
    ...
    for (const deal of sortedDeals) {
      if (deal.price >= reservePrice) {
        ...
        if (matchingBid && matchingBid.price >= deal.price) {
          return { bid: matchingBid, deal };
        }
      }
    }
    return null;
  ...
  private async saveToHistory(auction: any, winner: Bid | null, price: number, secondPrice: number, status: 'completed' | 'no-fill', analytics: AuctionAnalytics) {
    try {
      ...
      const history = new AuctionHistory({
        auctionId: auction.auctionId,
        auctionType: 'first-price',
        results: { winner: winner ? { seatId: winner.seatId, adId: winner.adId, price: winner.price } : null as any, price, secondPrice, status: status === 'completed' ? 'completed' : 'no-fill' },
        analytics,
        topBidders,
        revenue: { total: status === 'completed' ? price : 0, byType: new Map([['first-price', status === 'completed' ? price : 0]]) },
        timestamp: new Date(),
        metadata: { date: new Date().toISOString().split('T')[0], hour: new Date().getHours(), dayOfWeek: new Date().getDay() },
      });
      await history.save();
    } catch (error) {
      logger.error('Failed to save auction history', { auctionId: auction.auctionId, error: error instanceof Error ? error.message : 'Unknown error' });
    }
  }
}

export const firstPriceService = new FirstPriceService();