...
import { preferenceRoutes } from './routes';
...
app.get('/health', (req: Request, res: Response) => {
  res.json({
    status: 'healthy',
    service: 'notification-preference-center',
...
async function startServer(): Promise<void> {
  try {
    const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar_notification_preferences';
    ...
    app.listen(PORT, () => {
      logger.info(`Notification preference center running on port ${PORT}`);
    });
