...
export async function checkBudgetAlerts(
  ...
  if (dailyBudget > 0) {
    ...
    if (dailyPct >= 100) {
      const alerted = await redisSet('daily100:' + adId, '1');
      if (alerted !== undefined) await notifyBudgetAlert(merchantId, adId, adTitle, dailyBudget, dailySpent, 'daily_100');
    } else if (dailyPct >= 90) {
      const alerted = await redisSet('daily90:' + adId, '1');
      if (alerted !== undefined) await notifyBudgetAlert(merchantId, adId, adTitle, dailyBudget, dailySpent, 'daily_90');
    } else if (dailyPct >= 80) {
      const alerted = await redisSet('daily80:' + adId, '1');
      if (alerted !== undefined) await notifyBudgetAlert(merchantId, adId, adTitle, dailyBudget, dailySpent, 'daily_80');
    }
  }
