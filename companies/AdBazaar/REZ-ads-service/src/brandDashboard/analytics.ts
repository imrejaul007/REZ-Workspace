...
export class AnalyticsService {
  ...
  private generateHistoricalDataPoint(
    ...
    const impressions = Math.round(baseImpressions * (1 + ((randomInt(0, 1000) / 1000) - 0.5) * variance));
    const ctr = 0.02 + (randomInt(0, 1000) / 1000) * 0.03; // 2-5% CTR
    const clicks = Math.round(impressions * ctr);
    const conversionRate = 0.03 + (randomInt(0, 1000) / 1000) * 0.02; // 3-5% conversion
    const conversions = Math.round(clicks * conversionRate);
    const cpc = 0.3 + (randomInt(0, 1000) / 1000) * 0.5; // $0.30-$0.80 CPC
    const spend = clicks * cpc;
    const aov = 35 + (randomInt(0, 1000) / 1000) * 20; // $35-$55 AOV
  ...
  private getMockRealTimeMetrics(): RealTimeMetrics {
    return {
      impressions: Math.round(1000 + randomInt(0, 500)),
      clicks: Math.round(20 + randomInt(0, 15)),
      conversions: Math.round(1 + randomInt(0, 3)),
      spend: Math.round((10 + randomInt(0, 5)) * 100) / 100,
      timestamp: new Date(),
    };
  }
}
