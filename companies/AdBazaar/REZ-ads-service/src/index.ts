...
function validateEnv(): void {
  ...
  if (!process.env.ADS_JWT_SECRET && !process.env.JWT_SECRET) {
    throw new Error('ADS_JWT_SECRET or JWT_SECRET must be configured');
  }
  // Fail-closed: Redis is required for click deduplication and session caching
...
async function boot() {
  ...
  try {
    ...
    logger.error('[Boot] Redis connection failed — ensure REDIS_URL is set', { error: err instanceof Error ? err.message : String(err) });
