...
export function requestId(req: Request, res: Response, next: NextFunction): void {
  const requestId = (req.headers['x-request-id'] as string) || `pricing-${Date.now()}-${randomUUID().replace(/-/g, '').substring(0, 7)}`;
...
export function errorHandler(err: Error, req: Request, res: Response, _next: NextFunction): void {
  const errorId = `ERR-${Date.now()}-${randomUUID().replace(/-/g, '').substring(0, 7)}`;
