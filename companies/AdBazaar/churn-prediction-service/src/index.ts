...
const PORT = parseInt(process.env.PORT || '5079');
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/adbazaar_churn';
...
app.use('/api/churn', authMiddleware, churnRoutes);
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Churn Prediction Service started on port ${PORT}`);
      logger.info(`Health check: http://localhost:${PORT}/health`);
      logger.info(`API: http://localhost:${PORT}/api/churn`);
    });
  } catch (error) {
