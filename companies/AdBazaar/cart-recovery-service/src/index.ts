/**
 * cart-recovery-service - Advanced cart abandonment recovery
 * Port: 5099
 */
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/cart-recovery';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({ status: mongoOk ? 'healthy' : 'degraded', service: 'cart-recovery-service', version: '1.0.0', mongodb: mongoOk ? 'connected' : 'disconnected' });
});
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => { logger.info(`[${new Date().toISOString()}] cart-recovery-service running on port ${PORT}`); });
  } catch (error) {
