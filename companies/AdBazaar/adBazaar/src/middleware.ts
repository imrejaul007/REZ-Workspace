...
// Session cache cookie names
const SESSION_CACHE_COOKIE = 'adb_session_cache'
const SESSION_SIGNATURE_COOKIE = 'adb_session_sig'

// Cache TTL in seconds (5 minutes)
const SESSION_CACHE_TTL = 300

// CRITICAL FIX: HMAC signature key must be set - session cookie is now signed
const SESSION_SIGNATURE_KEY = process.env.SESSION_SIGNATURE_SECRET || process.env.JWT_SECRET;
if (!SESSION_SIGNATURE_KEY) {
  logger.error('CRITICAL: SESSION_SIGNATURE_SECRET or JWT_SECRET must be set - sessions will be rejected');
}
...
/**
 * Creates HMAC-SHA256 signature for session data to prevent tampering.
 * Uses timing-safe comparison to prevent timing attacks.
 */
function createSessionSignature(sessionData: string): string {
  if (!SESSION_SIGNATURE_KEY) {
    throw new Error('SESSION_SIGNATURE_SECRET is not configured');
  }
  // Using Web Crypto API for edge runtime compatibility
  const encoder = new TextEncoder();
  const key = encoder.encode(SESSION_SIGNATURE_KEY);
  const data = encoder.encode(sessionData);

  // Simple HMAC simulation for Node.js runtime
  // In edge runtime, this would use crypto.subtle
  const crypto = require('crypto') as typeof import('crypto');
  return crypto.createHmac('sha256', SESSION_SIGNATURE_KEY!).update(sessionData).digest('hex');
}

/**
 * Verifies HMAC signature using timing-safe comparison.
 */
function verifySessionSignature(sessionData: string, signature: string): boolean {
  if (!SESSION_SIGNATURE_KEY) return false;

  try {
    const expectedSignature = createSessionSignature(sessionData);
...
/**
 * Get cached session from cookie with HMAC signature verification.
 * CRITICAL SECURITY FIX: Without signature verification, an attacker could forge
 * a session cookie with arbitrary userId and userRole (including 'admin').
 */
function getCachedSession(req: NextRequest): CachedSession | null {
  const cached = req.cookies.get(SESSION_CACHE_COOKIE)?.value
  const signature = req.cookies.get(SESSION_SIGNATURE_COOKIE)?.value

  if (!cached || !signature) return null

  // CRITICAL: Verify signature BEFORE trusting session data
  if (!verifySessionSignature(cached, signature)) {
    logger.warn('[SESSION] Invalid session signature - possible tampering attempt');
    return null
  }
...
/**
 * Set session cache cookie with HMAC signature.
 * CRITICAL SECURITY FIX: The signature cookie prevents attackers from forging sessions.
 */
function setSessionCache(
  ...
  const signature = createSessionSignature(sessionData)

  // Set session data cookie
  res.cookies.set(SESSION_CACHE_COOKIE, sessionData, {
    httpOnly: true,
  ...
  // Set signature cookie (httpOnly to prevent JS access)
  res.cookies.set(SESSION_SIGNATURE_COOKIE, signature, {
    httpOnly: true,
    secure: process.env.NODE_ENV === 'production',
    sameSite: 'lax',
    maxAge: SESSION_CACHE_TTL,
