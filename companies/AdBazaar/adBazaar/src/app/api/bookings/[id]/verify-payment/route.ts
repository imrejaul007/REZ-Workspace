...
export async function POST(req: NextRequest, { params }: RouteContext) {
  try {
    ...
    try {
      ...
      logger.warn('[verify-payment] initiatePaymentForOrder failed, attempting capture anyway', { error: err instanceof Error ? err.message : String(err) })
    }
    ...
    if (paymentId) {
      try {
        ...
        logger.warn('[verify-payment] capturePayment failed', { error: err instanceof Error ? err.message : String(err), paymentId })
