  ...
  try {
    ...
    try {
      ...
      if (result.error) {
        ...
        return NextResponse.json({ error: result.error instanceof Error ? result.error.message : String(result.error) }, { status: 500 })
      }
    } catch (e) {
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (error) {
      logger.error('[inquiries] Database error', { error: error instanceof Error ? error.message : String(error) })
      return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
    }
