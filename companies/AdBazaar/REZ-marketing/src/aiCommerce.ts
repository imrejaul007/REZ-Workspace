...
export function generateResponse(
  ...
  // NOTE: Math.random() is ACCEPTABLE here for non-security template selection
  // This is just choosing a response template, not generating sensitive data
 ...
 * SECURITY: Added TTL cleanup to prevent memory leaks
 */
const carts = new Map<string, Cart>();
const cartTimestamps = new Map<string, number>(); // Track when cart was created
...
const orderTimestamps = new Map<string, number>(); // Track when order was created
...
const lastCleanup = { carts: 0, orders: 0 };

/**
 * Cleanup expired carts and orders to prevent memory leaks
 */
function cleanupExpiredEntries(): void {
  const now = Date.now();

  // Cleanup carts (run every cleanup interval)
  if (now - lastCleanup.carts > CLEANUP_INTERVAL_MS) {
    let cleaned = 0;
    for (const [userId, timestamp] of cartTimestamps) {
      if (now - timestamp > CART_TTL_MS) {
        carts.delete(userId);
        cartTimestamps.delete(userId);
        cleaned++;
      }
    }
    if (cleaned > 0) {
      logger.info(`[aiCommerce] Cleaned up ${cleaned} expired carts`);
    }
    lastCleanup.carts = now;
  }

  // Cleanup orders (run every cleanup interval)
  if (now - lastCleanup.orders > CLEANUP_INTERVAL_MS) {
    let cleaned = 0;
    for (const [orderId, timestamp] of orderTimestamps) {
      if (now - timestamp > ORDER_TTL_MS) {
        orders.delete(orderId);
        orderTimestamps.delete(orderId);
        orderAttributions.delete(orderId);
        cleaned++;
      }
    }
    if (cleaned > 0) {
      logger.info(`[aiCommerce] Cleaned up ${cleaned} expired orders`);
    }
    lastCleanup.orders = now;
  }
}

/**
 * Get or create cart for user
 * SECURITY: Runs cleanup before accessing cart to prevent memory leaks
 */
export function getCart(userId: string): Cart {
  // Run cleanup before accessing cart
  ...
  cartTimestamps.set(userId, Date.now());
  return newCart;
}
...
/**
 * Clear cart
 * SECURITY: Also removes timestamp to prevent memory leak
 */
...
export function createOrder(
  ...
  orderTimestamps.set(orderId, Date.now());
