...
import { authenticateAny, authenticateInternalService } from '../middleware/auth';
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    if (!VALID_EVENT_TYPES.includes(signal.eventType.toLowerCase())) {
      throw new ValidationError(`Invalid event type: ${signal.eventType}. Valid types: ${VALID_EVENT_TYPES.join(', ')}`);
    }

    // Validate category
    if (!VALID_CATEGORIES.includes(signal.category.toUpperCase())) {
      throw new ValidationError(`Invalid category: ${signal.category}. Valid categories: ${VALID_CATEGORIES.join(', ')}`);
    }
    ...
    metrics.httpRequestDuration.labels('POST', '/api/signals/ingest').observe(duration);
...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    // Record metrics
    const duration = (Date.now() - startTime) / 1000;
    metrics.httpRequestDuration.labels('POST', '/api/signals/batch').observe(duration);
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { IntentSignalModel } = await import('../models/IntentSignal');
...
router.get(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    const { IntentSignalModel } = await import('../models/IntentSignal');
    const signal = await IntentSignalModel.findOne({ signalId }).exec();
