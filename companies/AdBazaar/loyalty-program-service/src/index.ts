/**
 * loyalty-program-service - Core loyalty program management
 * Port: 5101
 */
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/loyalty-program';
...
app.get('/health', async (_req, res) => {
  ...
  res.json({ status: mongoOk ? 'healthy' : 'degraded', service: 'loyalty-program-service', version: '1.0.0', mongodb: mongoOk ? 'connected' : 'disconnected' });
});
...
app.use('/api/programs', loyaltyRoutes);
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => { logger.info(`[${new Date().toISOString()}] loyalty-program-service running on port ${PORT}`); });
  } catch (error) {
