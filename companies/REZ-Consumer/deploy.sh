...
# Deploys all consumer-facing services to Render
# =============================================================================
...
LOG_FILE="${SCRIPT_DIR}/logs/deploy-rez-consumer_${TIMESTAMP}.log"
...
ALL_SERVICES=("${CONSUMER_APPS[@]}" "${RENDEZ_SERVICES[@]}" "${DO_SERVICES[@]}")
...
check_prerequisites() {
    log_header "REZ-CONSUMER PRE-DEPLOYMENT CHECKS"
...
main() {
    echo ""
    echo -e "${CYAN}╔═══════════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${CYAN}║                    REZ-CONSUMER DEPLOYMENT                     ║${NC}"
    echo -e "${CYAN}╚═══════════════════════════════════════════════════════════════════╝${NC}"
    echo ""
    ...
    log_header "DEPLOYING ${total_services} CONSUMER SERVICES"
    ...
    # Print summary
    log_header "DEPLOYMENT SUMMARY - REZ-CONSUMER"
    ...
    if [ $FAILED_COUNT -eq 0 ]; then
        log_success "All REZ-Consumer services deployed successfully!"
    else
