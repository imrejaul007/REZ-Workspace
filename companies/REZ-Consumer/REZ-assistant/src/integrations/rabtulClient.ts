...
export async function verifyToken(token: string) {
  try {
    ...
    console.error('Auth verification failed:', axiosError.message);
    throw error;
  }
}
...
export async function processPayment(paymentData: Record<string, unknown>) {
  try {
    ...
    const axiosError = error as AxiosError;
    console.error('Payment failed:', axiosError.message);
    throw error;
  }
}
...
export async function addCoins(userId: string, amount: number, reason: string) {
  try {
    ...
    const axiosError = error as AxiosError;
    console.error('Wallet credit failed:', axiosError.message);
    throw error;
  }
}
...
export async function sendNotification(userId: string, notification: Record<string, unknown>) {
  try {
    ...
    const axiosError = error as AxiosError;
    console.error('Notification failed:', axiosError.message);
    throw error;
  }
}
...
export async function trackEvent(eventType: string, eventData: Record<string, unknown>) {
  try {
    ...
    const axiosError = error as AxiosError;
    console.error('Event tracking failed:', axiosError.message);
    throw error;
  }
}
 ...
 */
export async function getPredictions(userId: string, features: Record<string, unknown>) {
  try {
    ...
    const axiosError = error as AxiosError;
    console.error('Prediction failed:', axiosError.message);
    throw error;
  }
}
