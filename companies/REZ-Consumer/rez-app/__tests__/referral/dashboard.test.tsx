...
describe('ReferralDashboard', () => {
  ...
  describe('Initial Rendering', () => {
    test('shows loading indicator initially', () => {
      ...
      const indicators = UNSAFE_getAllByType('ActivityIndicator' as unknown);
  ...
  describe('Data Loading', () => {
    ...
    test('hides loading indicator after data loads', async () => {
      ...
      await waitFor(() => {
        const indicators = UNSAFE_queryAllByType('ActivityIndicator' as unknown);
  ...
  describe('Tier Progress Display', () => {
    ...
    test('renders progress bar with correct width', async () => {
      ...
      await waitFor(() => {
        const views = UNSAFE_getAllByType('View' as unknown);
        const progressBars = views.filter(v =>
          v.props.style &&
          Array.isArray(v.props.style) &&
          v.props.style.some((s) => s.width === '60%')
        );
  ...
  describe('Refresh Functionality', () => {
    test('supports pull-to-refresh', async () => {
      ...
      await waitFor(() => {
        const scrollView = UNSAFE_getByType('ScrollView' as unknown);
  ...
  describe('Error Handling', () => {
    ...
    test('logs errors to console', async () => {
      ...
      await waitFor(() => {
        expect(consoleSpy).toHaveBeenCalledWith('Error loading referral data:', expect.unknown(Error));
      });
