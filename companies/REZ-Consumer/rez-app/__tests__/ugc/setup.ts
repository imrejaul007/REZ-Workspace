...
jest.mock('expo-image', () => {
  ...
  return {
    __esModule: true,
    Image: (props) => React.createElement('Image', { ...props, testID: 'mock-expo-image' }),
  };
});
...
jest.mock('expo-linear-gradient', () => {
  ...
  return {
    __esModule: true,
    LinearGradient: (props) => React.createElement('View', { ...props, testID: 'mock-linear-gradient' }, props.children),
  };
});
...
export const createMockFormData = () => {
  const data = new Map();
  return {
    append: (key: string, value) => data.set(key, value),
...
export const resetAllMocks = () => {
  ...
  (mockApiClient as unknown).get?.mockReset?.();
  (mockApiClient as unknown).post?.mockReset?.();
  (mockApiClient as unknown).patch?.mockReset?.();
  (mockApiClient as unknown).delete?.mockReset?.();
  (mockApiClient as unknown).uploadFile?.mockReset?.();
...
export const setupSuccessfulApiResponses = () => {
  (mockVideosApi.getVideos as unknown).mockResolvedValue({
    success: true,
    data: {
      videos: [],
  ...
  (mockVideosApi.uploadVideo as unknown).mockResolvedValue({
    success: true,
    data: { videoId: 'test-video-id', uploadUrl: 'https://cloudinary.com/upload' },
  });

  (mockProductsApi.searchProducts as unknown).mockResolvedValue({
    success: true,
    data: {
      products: [],
      pagination: { current: 1, pages: 1, total: 0, limit: 20 },
