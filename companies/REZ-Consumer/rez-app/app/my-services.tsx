  ...
  const fetchProjects = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data?.submissions) {
        const mappedProjects: ServiceProject[] = response.data.submissions.map((submission) => ({
  ...
  const renderProject = useCallback(
    ({ item }: { item: ServiceProject }) => (
      <Pressable
        ...
        onPress={() => {
          router.push(`/earn/my-submissions?projectId=${(item as unknown)._id || (item as unknown).id || ''}` as unknown);
        }}
      >
