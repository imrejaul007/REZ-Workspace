...
const FinancialServiceDetailPage: React.FC<FinancialServiceDetailPageProps> = () => {
  ...
  const { id } = useLocalSearchParams<unknown>();
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        trackEvent(ANALYTICS_EVENTS.SERVICE_VIEWED, {
          ...
          service_type: (response.data.serviceCategory as unknown)?.slug || 'unknown',
  ...
  const getServiceType = () => {
    ...
    const categorySlug = (service.serviceCategory as unknown)?.slug || '';
    return categorySlug;
  };

  const serviceType = getServiceType();
  const cashbackPercentage = service?.cashback?.percentage || service?.serviceCategory?.cashbackPercentage || 0;
  const maxCashback = service?.cashback?.maxAmount || (service?.serviceCategory as unknown)?.maxCashback || 0;
  ...
  const handleProceed = async () => {
    ...
    try {
      ...
      router.push(`/payment?${paymentParams.toString()}` as unknown);
    } catch (error) {
  ...
  const categoryColor = (service.serviceCategory as unknown)?.metadata?.color || COLORS.blue500;
  const categoryName = (service.serviceCategory as unknown)?.name || 'Financial Service';
