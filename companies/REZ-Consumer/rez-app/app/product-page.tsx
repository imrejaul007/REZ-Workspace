...
function toReviews(storeReviews: StoreReview[]): Review[] {
  return storeReviews.map((r) => ({
    // SECURITY FIX: Use crypto.randomUUID() instead of Math.random() for IDs
...
function StorePage() {
  ...
  useEffect(() => {
    ...
    const existingItem = cartState.items.find(
      (item) => (item.productId || item.product?._id || item.product) === productId,
    );
    ...
    try {
      ...
      if (lockedResponse.success && lockedResponse.data) {
        const lockedItem = lockedResponse.data.lockedItems.find(
          (item) => item.product?._id === productId || (item.product as unknown)?.id === productId,
        );
        if (!isMounted()) return;
