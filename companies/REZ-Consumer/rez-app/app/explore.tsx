...
const AutoPlayVideoReel: React.FC<{ uri: string; poster?: string; style?: unknown }> = React.memo(
  ({ uri, poster, style }) => {
    const videoRef = useRef<Video>(null);
    const webVideoRef = useRef<unknown>(null); // Use any for cross-platform compatibility
...
const ExplorePage = () => {
  ...
  const [ugcReels, setUgcReels] = useState<unknown[]>([]);
  const [categories, setCategories] = useState<unknown[]>([]);
  const [trendingStores, setTrendingStores] = useState<unknown[]>([]);
  ...
  // Get REZ coins from wallet - use context's pre-computed rezBalance
    ...
    try {
      ...
      if (reelsRes.status === 'fulfilled') {
        ...
        if (rRes.success && rRes.data && rRes.data.length > 0) {
          const transformedReels = rRes.data.map((reel, index: number) => {
      ...
      if (categoriesRes.status === 'fulfilled') {
        ...
        if (cRes.success && cRes.data && cRes.data.length > 0) {
          const transformedCategories = cRes.data.slice(0, 6).map((cat) => ({
            id: cat.slug || cat.id,
            name: cat.name,
      ...
      if (storesRes.status === 'fulfilled') {
        ...
        const storesData: unknown[] = (sRes.data as unknown)?.stores || sRes.data || [];
        if (sRes.success && storesData && storesData.length > 0) {
          const transformedStores = storesData.slice(0, 5).map((store) => ({
