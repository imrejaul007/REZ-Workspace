...
import { View, Pressable, StyleSheet, Dimensions, ActivityIndicator, StyleProp, ViewStyle } from 'react-native';
...
import { createInitialButtonState, ButtonStateManager, createButtonHandler } from '@/utils/button-state-manager';
...
function StoreActionButtons({
  ...
  const dynamicBuyText =
    customBuyText || (dynamicData?.price ? `Buy for ${currencySymbol}${dynamicData.price}` : 'Buy Now');

  const dynamicLockText = isLocked
    ? 'Already Locked'
    : customLockText || (dynamicData?.availabilityStatus === 'in_stock' ? 'Reserve Item' : 'Lock Price');

  const dynamicBookingText = customBookingText || (storeType === 'SERVICE' ? 'Book Service' : 'Schedule Pickup');

  // Component state management
  const [buttonState, setButtonState] = useState(() => createInitialButtonState());
  const stateManager = useMemo(() => new ButtonStateManager(buttonState, setButtonState), [buttonState]);
  ...
  const layout = useMemo(() => getButtonLayout(buttonConfigs.length, width), [buttonConfigs.length, width]);
  ...
  const getAnimRef = (buttonId: string) => {
    switch (buttonId) {
      case 'buy':
        return buyScaleAnim;
      case 'lock':
        return lockScaleAnim;
      case 'booking':
        return bookingScaleAnim;
      case 'call':
        return callScaleAnim;
      case 'product':
        return productScaleAnim;
      case 'location':
        return locationScaleAnim;
      case 'custom':
        return customScaleAnim;
      case 'pay':
        return payScaleAnim;
      default:
        return buyScaleAnim;
  ...
  const getAnimStyle = (buttonId: string) => {
    switch (buttonId) {
      case 'buy':
        return buyScaleStyle;
      case 'lock':
        return lockScaleStyle;
      case 'booking':
        return bookingScaleStyle;
      case 'call':
        return callScaleStyle;
      case 'product':
        return productScaleStyle;
      case 'location':
        return locationScaleStyle;
      case 'custom':
        return customScaleStyle;
      case 'pay':
        return payScaleStyle;
      default:
        return buyScaleStyle;
    }
  };
          ...
          >
            <LinearGradient
              colors={
                shouldDisable
                  ...
                  : (config.backgroundColor as readonly [string, string, ...string[]])
              }
              start={{ x: 0, y: 0 }}
              end={{ x: 1, y: 0 }}
            ...
            >
              <View style={styles.buttonContent}>
                ...
                {/* Button text */}
                <ThemedText
                  style={[styles.buttonText, { color: config.textColor }, textStyle]}
                  numberOfLines={1}
                  ellipsizeMode="clip"
                >
