  ...
  const searchResults: SearchResults | null = {
    ...
    stores: stores.map((store, storeIndex) => {
      ...
      return {
        ...
        hasRezPay:
          (store as unknown).paymentSettings?.acceptRezCoins || store.operationalInfo?.acceptsWalletPayment || false,
        cashbackPercent: (store as unknown).rewardRules?.baseCashbackPercent || 0,
            ...
            <Pressable style={styles.subcategoryDropdown} onPress={() => setShowSubcategoryModal(true)}>
              <Text style={styles.subcategoryLabel}>
                {selectedSubcategory === 'all'
                  ? 'All Subcategories'
                  : (() => {
                      const found = subcategories.find((s) => s._id === selectedSubcategory);
                      if (!found) return 'All';
