...
const FashionPage: React.FC = () => {
  ...
  const fetchCategoryData = useCallback(async () => {
    try {
      const res = await apiClient.get<unknown>(`/categories/${FASHION_SLUG}`);
  ...
  const fetchProducts = useCallback(
    async (page: number, append: boolean = false) => {
      try {
        ...
        const res = await apiClient.get<unknown>(`/products/category/${FASHION_SLUG}`, {
          page,
        ...
        if (res.success) {
          ...
          const rawProducts: unknown[] = Array.isArray(res.data) ? res.data : res.data?.products || [];
          const mapped: FashionProduct[] = rawProducts.map((p) => ({
  ...
  const fetchTrending = useCallback(async () => {
    try {
      const res = await apiClient.get<unknown>('/products/trending', { category: FASHION_SLUG, limit: 6 });
      if (!isMounted()) return;
      if (res.success) {
        const raw: unknown[] = Array.isArray(res.data) ? res.data : res.data?.products || [];
        if (!isMounted()) return;
        setTrendingProducts(
          raw.map((p) => ({
            id: p._id?.toString() || p.id,
  ...
  const fetchStores = useCallback(async () => {
    try {
      const res = await apiClient.get<unknown>('/stores', { category: FASHION_SLUG, limit: 8, sortBy: 'rating' });
      if (!isMounted()) return;
      if (res.success) {
        const raw: unknown[] = Array.isArray(res.data) ? res.data : res.data?.stores || [];
        if (!isMounted()) return;
        setStores(
          raw.map((s) => ({
                  ...
                  {subcategories.map((cat) => (
                    ...
                    >
                      <View style={styles.categoryIcon}>
                        {cat.icon ? (
                          <Ionicons
                            name={(cat.icon as unknown) || 'pricetag-outline'}
                            size={24}
