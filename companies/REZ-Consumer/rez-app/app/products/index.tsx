...
const ProductsPage: React.FC = () => {
  ...
  // Uses canonical format: pricing.original (marked retail price) and pricing.selling (actual selling price)
  const transformProduct = (product): DisplayProduct => {
    const original = product.pricing?.original || product.pricing?.mrp || product.pricing?.basePrice || product.price || 0;
    const selling = product.pricing?.selling || product.pricing?.salePrice || original;
    const discount = original > selling ? Math.round((1 - selling / original) * 100) : 0;
