...
const getRefName = (ref: string | { _id: string; name: string; [key: string]: unknown }): string | null => {
...
function CouponsPage() {
  ...
  const handleUseCoupon = (coupon: Coupon) => {
    ...
    if (stores.length === 1 && typeof stores[0] === 'object') {
      router.push(`/MainStorePage?storeId=${(stores[0] as unknown)._id || (stores[0] as unknown).id}` as unknown);
    } else if (categories.length === 1 && typeof categories[0] === 'object') {
      router.push(`/explore/category/${(categories[0] as unknown)._id || (categories[0] as unknown).id}` as unknown);
    } else {
      router.push('/explore');
    }
  };
