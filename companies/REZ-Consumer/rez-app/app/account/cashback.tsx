...
import cashbackService, { CashbackSummary, UserCashback, CashbackCampaign } from '@/services/cashbackApi';
...
function CashbackPage() {
  ...
  // or a status change made on another screen is immediately visible here
  useFocusEffect(
    useCallback(() => {
      loadData();
      // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  const loadCashbackHistory = async () => {
    try {
      const filters: unknown = { page: 1, limit: 20 };
      if (activeTab !== 'all') {
    ...
    try {
      ...
      if (purchasesRes.status === 'fulfilled' && purchasesRes.value) {
        ...
        if (activeTab === 'pending')
          filtered = filtered.filter((p) => p.status === 'pending' || p.status === 'confirmed');
        else if (activeTab === 'credited') filtered = filtered.filter((p) => p.status === 'credited');
        else if (activeTab === 'expired')
          filtered = filtered.filter((p) => p.status === 'rejected' || p.status === 'refunded');
      ...
      <View style={styles.sourceToggleRow}>
        ...
        >
          <Ionicons
            ...
            size={14}
            color={activeSource === 'store' ? colors.text.inverse : '#7C8A97'}
          />
          <Text style={[styles.sourceToggleText, activeSource === 'store' && styles.sourceToggleTextActive]}>
            Store Cashback
          </Text>
        </Pressable>
        <Pressable
        ...
        >
          <Ionicons
            name="globe-outline"
            size={14}
            color={activeSource === 'cashstore' ? colors.text.inverse : '#7C8A97'}
          />
          <Text style={[styles.sourceToggleText, activeSource === 'cashstore' && styles.sourceToggleTextActive]}>
            Cash Store
          </Text>
        </Pressable>
      </View>
      ...
      >
        ...
        {activeSource === 'cashstore' ? (
                ...
                affiliatePurchases.map((purchase) => (
                  <View key={purchase._id} style={styles.historyCard}>
                    <View
                      style={[
                        styles.historyIcon,
                        { backgroundColor: `${getAffiliatePurchaseStatusColor(purchase.status)}12` },
                      ]}
                    >
                      <Ionicons
                        name="globe-outline"
                        size={20}
                        color={getAffiliatePurchaseStatusColor(purchase.status)}
                      />
                    </View>
                        ...
                        <View style={styles.historyExpiryRow}>
                          ...
                          <Text style={styles.historyExpiryText}>
                            {purchase.status === 'pending'
                              ...
                              : 'Confirmed — coins crediting soon'}
                          </Text>
                        </View>
                      )}
                      {purchase.creditedAt && (
                    ...
                    <View style={styles.historyRight}>
                      ...
                      <View
                        style={[
                          styles.statusPill,
                          { backgroundColor: `${getAffiliatePurchaseStatusColor(purchase.status)}15` },
                        ]}
                      >
                        <Text
                          style={[styles.statusPillText, { color: getAffiliatePurchaseStatusColor(purchase.status) }]}
                        >
          ...
          <>
            ...
            {/* ─── Redeem Section ────────────────────────────────── */}
            {hasRedeemable && (
              <Pressable style={styles.redeemCard} onPress={handleRedeemCashback} disabled={isRedeeming}>
                <LinearGradient
            ...
            {hasExpiringSoon && (
              <View style={styles.expiringCard}>
                ...
                <Pressable onPress={() => setActiveTab('pending')} style={styles.expiringAction}>
                ...
                {campaigns.map((campaign) => (
                  <View key={campaign.id} style={styles.campaignCard}>
                    ...
                    >
                      ...
                      {campaign.validTo && (
                        <Text style={styles.campaignValidity}>Valid till {formatDate(campaign.validTo)}</Text>
                      )}
                    </LinearGradient>
                  </View>
                ))}
            ...
            <View style={styles.historySection}>
              <Text style={styles.historyTitle}>
                {activeTab === 'all'
                  ? 'All Cashback'
                  : `${activeTab.charAt(0).toUpperCase() + activeTab.slice(1)} Cashback`}
                {historyTotal > 0 && <Text style={styles.historyCount}> ({historyTotal})</Text>}
              </Text>
              ...
              {cashbacks.length > 0 ? (
                ...
                <View style={styles.emptyContainer}>
                  ...
                  <Text style={styles.emptySubtext}>
                    {activeTab === 'all' ? 'Start shopping to earn cashback!' : `No ${activeTab} cashback yet`}
                  </Text>
                </View>
              )}
            </View>
    ...
    <View style={[styles.summaryCard, { borderLeftColor: color, borderLeftWidth: 3 }]}>
      <View style={styles.summaryTop}>
        <View style={[styles.summaryIconWrap, { backgroundColor: `${color}15` }]}>
          <Ionicons name={icon as unknown} size={16} color={color} />
        </View>
    ...
    return (
      <Animated.View style={[styles.historyCard, fadeAnimStyle]}>
        ...
        <View style={[styles.historyIcon, { backgroundColor: `${statusColor}12` }]}>
          <Ionicons name={getSourceIcon(cashback.source) as unknown} size={20} color={statusColor} />
        </View>
        ...
        {/* Amount + status */}
        <View style={styles.historyRight}>
          <Text style={[styles.historyAmount, { color: statusColor }]}>{formatAmount(cashback.amount)}</Text>
