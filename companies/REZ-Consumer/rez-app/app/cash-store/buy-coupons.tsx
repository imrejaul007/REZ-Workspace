  ...
  const fetchGiftCards = useCallback(
    async (pageNum: number, append: boolean) => {
      try {
        ...
        const params: unknown = { limit: PAGE_SIZE, page: pageNum };
        ...
        if (response.success && response.data) {
          ...
          const mapped: VoucherBrandItem[] = brandsArray.map((b) => ({
            _id: b._id,
            name: b.name || '',
            logo: b.logo || '',
            category: b.category || '',
  ...
  const fetchCoupons = useCallback(async () => {
    try {
      ...
      if (debouncedSearch.length >= 2) {
        ...
        if (response.success && response.data) {
          if (!isMounted()) return;
          setCoupons(((response.data as unknown as Record<string, unknown>)?.coupons || []) as unknown);
        }
      } else {
