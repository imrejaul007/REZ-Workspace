...
function transformToCashStoreBrand(brand): CashStoreBrand {
  return {
    _id: brand._id,
    id: brand._id,
    name: brand.name || '',
...
function CashStoreBrandsPage() {
  ...
  const { filter } = useLocalSearchParams<unknown>();
    ...
    async (pageNum: number, append: boolean) => {
      try {
        ...
        if (debouncedSearch.length >= 2) {
          ...
          const params: unknown = { limit: PAGE_SIZE, page: pageNum, sort: sortBy };
