...
function PhotoUploadPage() {
  ...
  const fetchHistory = useCallback(async () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        setUploads((result.data as unknown).uploads);
      }
    } catch (error) {
      // silently handle
    } finally {
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const response = await apiClient.get<unknown>(`/stores/search?q=${encodeURIComponent(storeSearch)}&limit=5`);
        if (response.success && response.data) {
          const stores = Array.isArray(response.data) ? response.data : response.data.stores || [];
          setStoreResults(stores.map((s) => ({ _id: s._id || s.id, name: s.name, logo: s.logo })));
        }
      } catch (error) {
        // silently handle
      } finally {
