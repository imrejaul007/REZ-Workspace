...
function mapRefundResponse(raw): RefundDetails {
  ...
  const fetchRefund = useCallback(async () => {
    ...
    try {
      ...
      if (refundId) {
        ...
        const response = await apiClient.get<unknown>(`/orders/refunds/${refundId}`);
        ...
        const response = await apiClient.get<unknown>(`/orders/refunds?page=1&limit=1`);
        if (response.success && response.data?.refunds?.length > 0) {
          // Find a refund matching this order, or use first one
          const match =
            response.data.refunds.find((r) => (typeof r.order === 'object' ? r.order._id : r.order) === orderId) ||
