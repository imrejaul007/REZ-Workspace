...
function DetailedOrderTrackingPage() {
  ...
  const renderTimelineUpdate = (update, index: number, totalItems: number) => {
  ...
  const renderOrderItem = (item, index: number) => (
        ...
        <View style={styles.itemsCard}>
          ...
          <View style={styles.itemsList}>
            {order.items && order.items.length > 0 ? (
              order.items.map((item, index: number) => renderOrderItem(item, index))
            ) : (
