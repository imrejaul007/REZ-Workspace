...
function PartnerProfilePage() {
  ...
  const normalizeMilestone = (m, idx: number): OrderMilestone => ({
  ...
  const normalizeTask = (t, idx: number): RewardTask => ({
  ...
  const normalizeJackpot = (j, idx: number): JackpotMilestone => ({
  ...
  const normalizeOffer = (o, idx: number): ClaimableOffer => ({
  ...
  const loadPartnerData = async () => {
    try {
      ...
      if (dashboardResponse.success && dashboardResponse.data) {
        ...
        if (!isMounted()) return;
        setPartnerState({
          profile: dashboardResponse.data.profile as unknown,
          ...
          faqs: (dashboardResponse.data.faqs || []).map((f, idx: number) => ({
  ...
  const handleJackpotMilestonePress = async (milestone: JackpotMilestone) => {
    ...
    if (!milestone.achieved) {
      const remaining = (milestone.spendAmount ?? 0) - ((partnerState.profile as unknown)?.totalSpent || 0);
      platformAlertSimple(
        milestone.title,
  ...
  const handleUpgradeLevel = (targetLevel) => {
    platformAlertSimple(
  ...
  const getLevelCards = () => {
    if (partnerState.levels && partnerState.levels.length > 0) {
      return partnerState.levels.map((level, index: number) => ({
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
            ...
            <View style={styles.statsRow}>
              ...
              <View style={styles.statItem}>
                <Text style={styles.statValue}>
                  {currencySymbol}
                  {(profile as unknown)?.totalSpent?.toLocaleString() || '0'}
                </Text>
        ...
        <JackpotTimeline
          ...
          currentSpent={(profile as unknown)?.totalSpent || 0}
