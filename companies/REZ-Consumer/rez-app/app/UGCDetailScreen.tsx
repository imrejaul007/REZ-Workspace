...
function UGCDetailScreen() {
  ...
  const handleFollow = useCallback(async () => {
    ...
    try {
      if (wasFollowing) {
        ...
        const creatorName = (video?.creator as unknown)?.name || (video?.creator as unknown)?.username || 'this creator';
  ...
  const creatorName = useMemo(() => {
    ...
    return (
      (video?.creator as unknown)?.name || (video?.creator as unknown)?.username || (video?.stores as unknown)?.[0]?.name || 'User'
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  return (
    <View style={styles.container}>
      {/* StatusBar - controlled by video playback mode */}
