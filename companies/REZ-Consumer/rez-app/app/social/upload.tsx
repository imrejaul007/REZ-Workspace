...
function UploadPage() {
  ...
  const handlePickMedia = async () => {
    ...
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: contentType === 'reel' ? ('videos' as unknown) : ('mixed' as unknown),
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const res = await apiClient.get<unknown>(`/products/search?q=${encodeURIComponent(productSearchQuery)}&limit=5`);
  ...
  React.useEffect(() => {
    ...
    const timeout = setTimeout(async () => {
      ...
      try {
        const res = await apiClient.get<unknown>(`/stores/search?q=${encodeURIComponent(storeSearchQuery)}&limit=5`);
        if (res.success && res.data) {
          const stores = Array.isArray(res.data) ? res.data : res.data.stores || [];
          setStoreSearchResults(stores.map((s) => ({ id: s._id || s.id, name: s.name })));
        }
      } catch (_e) {
        /* silently handle */
      } finally {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      for (let i = 0; i < media.length; i++) {
        ...
        formData.append('file', { uri: media[i], type: 'image/jpeg', name: `${contentType}_${i}.jpg` } as unknown);
