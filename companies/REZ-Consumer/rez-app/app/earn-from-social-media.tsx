...
function EarnFromSocialMediaPage() {
    ...
    try {
      ...
      if (postsResponse.posts && postsResponse.posts.length > 0) {
        postsResponse.posts.forEach((post: SocialPost) => {
          ...
          const spId = (post.metadata as unknown as Record<string, unknown>)?.storePaymentId;
  ...
  const handleSubmitUrl = async () => {
    ...
    try {
      ...
      const postId: string = String(
        response?.post?.id ?? (response as unknown as Record<string, unknown>)?.id ?? 'unknown',
      );
  ...
  const renderOrdersList = () => (
    ...
    >
      {/* Earnings Summary Card — only show if user has unknown activity */}
              ...
              <View key={post._id} style={styles.submissionCard}>
                <View style={styles.submissionLeft}>
                  <View style={styles.submissionPlatform}>
                    <Ionicons
                      name={`logo-${post.platform}` as unknown as string}
                      size={18}
                ...
                <View style={styles.submissionRight}>
                  ...
                  >
                    <Ionicons
                      name={(statusIcons[post.status] || 'help-circle-outline') as unknown as string}
                      size={12}
        ...
        {[
          ...
          <View key={index} style={styles.tipRow}>
            <View style={styles.tipIconCircle}>
              <Ionicons name={tip.icon as unknown as keyof typeof Ionicons.glyphMap} size={16} color={colors.nileBlue} />
            </View>
