  ...
  const fetchSavedOffers = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        setOffers(Array.isArray(response.data) ? response.data : (response.data as unknown).offers || []);
      }
    } catch (err) {
    ...
    <SafeAreaView style={styles.container}>
      ...
      {offers.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <ThemedText style={styles.emptySubtitle}>Tap the bookmark icon on unknown offer to save it here</ThemedText>
