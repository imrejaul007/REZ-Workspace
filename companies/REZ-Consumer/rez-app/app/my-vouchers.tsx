    ...
    async (pageNum = 1, append = false) => {
      try {
        ...
        if (vouchersArray.length > 0) {
          const mappedVouchers: UserVoucher[] = vouchersArray.map((voucher) => ({
        ...
        const redemptionsArray: unknown[] = Array.isArray(redemptionsData)
          ? redemptionsData
          : (redemptionsData?.items ?? []);
        logger.debug('[MY VOUCHERS] Redemptions array:', redemptionsArray);

        if (redemptionsArray.length > 0) {
          const mappedRedemptions: UserVoucher[] = redemptionsArray.map((redemption) => {
              ...
              <View style={styles.applicabilityContainer}>
                ...
                <Text style={styles.applicabilityText}>
                  {item.metadata.isProductSpecific
                    ...
                    : `Valid on unknown product from ${item.metadata.storeName}`}
                </Text>
              </View>
            )}
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>
        You don't have unknown vouchers right now. Explore stores and grab great deals to earn vouchers!
      </Text>
      <Pressable
