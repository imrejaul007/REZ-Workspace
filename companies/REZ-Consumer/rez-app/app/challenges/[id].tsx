...
function ChallengeDetailPage() {
  ...
  const loadChallengeDetail = async () => {
    try {
      ...
      let progress = allProgress.find((p) => p._id === id);
      ...
      if (!challenge) {
        ...
        challenge = allChallenges.find((c) => c._id === id);
      ...
      if (!challenge) {
        ...
        logger.error('Available challenge IDs:', allChallenges.map((c) => c._id).slice(0, 5));
        logger.error('Progress IDs:', allProgress.map((p) => p._id).slice(0, 5));
