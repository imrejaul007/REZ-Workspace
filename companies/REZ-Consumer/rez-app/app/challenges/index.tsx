...
function ChallengesPage() {
  ...
  const loadChallengesData = async () => {
    try {
      ...
      const mergedChallenges = availableChallenges.map((challenge) => {
        const progress = userProgress.find((p) => p.challenge._id === challenge._id);
  ...
  const handleClaimReward = async (challengeId: string) => {
    ...
    try {
      ...
      const responseData: unknown = (response as unknown).success ? response : response.data;
