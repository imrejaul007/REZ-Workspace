...
function UGCSection({
  ...
  const viewableHandlerRef = useRef((info: { viewableItems: unknown[] }) => {
    const ids = info.viewableItems.map((v) => v.item.id);
    setVisibleItems(ids);
  });

  // keep the ref current (can include other dependencies if needed)
  useEffect(() => {
    viewableHandlerRef.current = (info: { viewableItems: unknown[] }) => {
      const ids = info.viewableItems.map((v) => v.item.id);
      setVisibleItems(ids);
    };
  }, []);
  ...
  const stableOnViewableItemsChanged = useCallback((info: { viewableItems: unknown[]; changed?: unknown[] }) => {
