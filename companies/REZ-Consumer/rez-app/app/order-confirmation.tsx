...
function OrderConfirmationPage() {
  ...
  const handleTrackOrder = () => {
    if (order) {
      ...
      if (ft === 'pickup') {
        router.push(`/pickup-tracking?orderId=${oid}`);
      } else if (ft === 'drive_thru') {
        router.push(`/drivethru-tracking?orderId=${oid}`);
      } else if (ft === 'dine_in') {
        router.push(`/dinein-tracking?orderId=${oid}`);
      } else {
        router.push(`/tracking?orderId=${oid}`);
      }
    }
  };
  ...
  const storeId = typeof storeData === 'string' ? storeData : (storeData as { _id?: string })?.['_id'] || '';
  const storeName = typeof storeData === 'string' ? storeData : (storeData as { name?: string })?.name || 'Store';
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {order?.totals?.cashback > 0 && (
          <SavingsCelebration
            ...
            delay={800}
            onViewWallet={() => router.push('/wallet-screen')}
          />
        )}
        ...
        <Animated.View style={[styles.card, contentAnimStyle]}>
          ...
          {order.fulfillmentType === 'dine_in' &&
            order.fulfillmentDetails?.tableNumber && (
          ...
          {(order.fulfillmentType === 'pickup' ||
            ...
            <View style={styles.deliveryInfo}>
              ...
              <View style={styles.deliveryDetails}>
                <ThemedText style={styles.deliveryAddress}>
                  {order.fulfillmentDetails?.storeAddress || 'Store Address'}
                </ThemedText>
                {order.fulfillmentDetails?.vehicleInfo && (
                  <ThemedText style={styles.deliveryAddressText}>
                    Vehicle: {order.fulfillmentDetails?.vehicleInfo}
                  </ThemedText>
                )}
              </View>
            </View>
