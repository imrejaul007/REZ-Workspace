...
function HeroesZonePage() {
  ...
  const { profile: profileParam } = useLocalSearchParams<unknown>();
  ...
  const fetchProfileOffers = async (profileSlug: string) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        if (!isMounted()) return;
        setProfileOffers((prev) => ({ ...prev, [profileSlug]: response.data as unknown }));
      }
    } catch (err) {
      // silently handle
    } finally {
  ...
  const isProfileVerified = (profile: SpecialProfile): boolean => {
    ...
    const verifications = (user as unknown)?.verifications;
  ...
  const renderSkeletonProfile = () => (
    <View style={styles.profileCard}>
      <Animated.View style={[styles.skeletonHeader, shimmerOpacityStyle]} />
    </View>
  );
  ...
  const renderProfileCard = (profile: SpecialProfile) => {
    ...
    return (
      <View key={profile._id} style={styles.profileCard}>
        {/* Profile Header */}
        <Pressable onPress={() => handleProfileToggle(profile._id, profile.slug)}>
          <LinearGradient
            colors={config.gradientColors as [string, string, ...string[]]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
            style={[styles.profileHeader, isExpanded ? styles.profileHeaderExpanded : null]}
          >
            ...
            {!isVerified ? (
              <View style={styles.verificationCard}>
                <View style={styles.verificationContent}>
                  ...
                  <Pressable style={styles.verifyButton} onPress={() => handleVerify(profile.slug)}>
            ...
            <View style={styles.dealsList}>
              {loadingOffers === profile.slug ? (
                <View style={styles.loadingDeals}>
                  <Animated.View style={[styles.skeletonDeal, shimmerOpacityStyle]} />
                  <Animated.View style={[styles.skeletonDeal, shimmerOpacityStyle]} />
                </View>
              ) : offers.length > 0 ? (
                offers.map((deal) => (
                  <Pressable
                    key={deal._id}
                    style={[styles.dealItem, !isVerified ? styles.dealItemDisabled : null]}
    ...
    <View style={styles.container}>
      ...
      >
          ...
          >
            <View style={styles.heroContent}>
              ...
              <View style={styles.heroTextContainer}>
                ...
                <ThemedText style={styles.heroSubtitle}>Exclusive deals for special community members</ThemedText>
              </View>
            </View>
              ...
              {profiles.slice(0, 4).map((profile) => {
                ...
                return (
                  <View key={profile._id} style={styles.profileIconCard}>
                    ...
                    <ThemedText style={styles.profileIconLabel}>{profile.name.split(' ')[0]}</ThemedText>
                  </View>
                );
              })}
            </View>
        ...
        <View style={styles.supportCard}>
          <ThemedText style={styles.supportText}>
            Don't see your category? <ThemedText style={styles.supportLink}>Contact us</ThemedText> to request special
      ...
      <View style={styles.fixedCTA}>
        <Pressable
          ...
          onPress={() => {
            const profile = selectedProfile ? profiles.find((p) => p._id === selectedProfile) : profiles[0];
            router.push({
              pathname: '/profile/verification',
