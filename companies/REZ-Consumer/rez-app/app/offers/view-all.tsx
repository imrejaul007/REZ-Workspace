...
import { View, StyleSheet, Pressable, Dimensions, RefreshControl, ActivityIndicator } from 'react-native';
...
function ViewAllOffersScreen() {
  ...
  const { category, discount, title } = useLocalSearchParams<unknown>();
  ...
  const fetchUserPoints = async () => {
    try {
      ...
      if (response.success && response.data) {
        const points =
          response.data.userEngagement?.userPoints || (response.data as unknown).userPoints || user?.wallet?.balance || 0;
        if (!isMounted()) return;
  ...
  const fetchOffers = useCallback(
    async (pageNum: number, append: boolean = false) => {
      try {
        ...
        if (
          filterRef.current.category !== currentFilter.category ||
        ...
        if (response.success && response.data) {
          ...
          if (pageNum === 1) {
            if (!isMounted()) return;
            setError((response as unknown).message || 'Failed to load offers');
          }
          if (!isMounted()) return;
          setHasMore(false);
        }
  ...
  const ProductCard = React.memo(({ offer }: { offer: Offer }) => {
    ...
    return (
      <Pressable style={styles.productCard} onPress={() => handleOfferPress(offer)}>
        ...
        <View style={styles.productInfo}>
          ...
          <ThemedText style={styles.cashBack}>Upto {offer.cashbackPercentage}% cash back</ThemedText>
          {offer.store?.name && (
  ...
  return (
    <SafeAreaView style={styles.container} edges={['top']}>
      ...
      <LinearGradient colors={[colors.brand.purpleLight, colors.brand.purpleMedium]} style={styles.header}>
      ...
      {/* Content - FlashList with virtualization */}
      <FlashList
        ...
        estimatedItemSize={CARD_ROW_HEIGHT}
      />
    </SafeAreaView>
  );
}
