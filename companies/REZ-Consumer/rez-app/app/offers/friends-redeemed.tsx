...
function mapRedemptionToFriendOffer(item, currencySymbol: string): FriendOffer {
    ...
    async (pageNum: number, append = false) => {
      ...
      try {
        ...
        if (response.success && response.data?.redemptions) {
          const mapped = response.data.redemptions.map((r) => mapRedemptionToFriendOffer(r, currencySymbol));
          if (!isMounted()) return;
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <ThemedText style={styles.emptyText}>
                {selectedFriend === 'all'
                  ? "Your friends haven't redeemed unknown offers recently"
                  : "This friend hasn't redeemed unknown offers recently"}
              </ThemedText>
            </View>
          }
        />
