...
function OffersDetailPage() {
  ...
  const { id, slug } = useLocalSearchParams<unknown>();
  ...
  useEffect(() => {
    const fetchOffer = async () => {
      try {
        ...
        const bankRes = await apiClient.get<unknown>(`/offers/bank/${id}`).catch(() => null);
        if (bankRes?.success && bankRes.data) {
          setOffer({ ...bankRes.data, _type: 'bank' });
          return;
        }
        const offerRes = await apiClient.get<unknown>(`/offers/${id}`).catch(() => null);
  ...
  const handleApplyOffer = async () => {
    ...
    try {
      setIsRedeeming(true);
      const res = await apiClient.post<unknown>(`/offers/${id}/redeem`);
      if (res.success) {
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {applicableStores.length > 0 && (
          <View style={styles.storesSection}>
            ...
            {applicableStores.map((store, i: number) => {
