  ...
  const fetchOffers = useCallback(async () => {
    try {
      ...
      const [bankRes, dealsRes, couponsRes] = await Promise.all([
        apiClient.get<unknown>('/offers/bank-offers', { limit: 20 }).catch(() => null),
        apiClient.get<unknown>('/offers/flash-sales', { limit: 20 }).catch(() => null),
        apiClient.get<unknown>('/coupons', { category: slug }).catch(() => null),
      ]);
      ...
      if (couponsRes?.success && couponsRes.data) {
        ...
        if (!isMounted()) return;
        setCoupons(c.filter((cp) => cp.status === 'active' || cp.isActive !== false));
      }
    } catch (err) {
      // silently handle
    } finally {
