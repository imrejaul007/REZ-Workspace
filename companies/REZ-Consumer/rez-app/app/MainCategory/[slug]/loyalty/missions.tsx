...
function ElectronicsMissionsPage() {
  ...
  const { slug } = useLocalSearchParams<unknown>();
  ...
  const handleClaimMission = useCallback(
    async (missionId: string) => {
      try {
        ...
        if (res.success) {
          platformAlertSimple('Mission Complete!', `You earned ${currencySymbol}${res.data?.reward || 0} coins!`);
  ...
  const renderMission = useCallback(
    ({ item }: { item: Mission }) => {
      ...
      return (
        <View style={[styles.missionCard, isCompleted ? styles.missionCardCompleted : null]}>
          ...
          {isCompleted ? (
            <View style={styles.completedRow}>
              ...
              <Text style={styles.completedText}>Completed {new Date(item.completedAt!).toLocaleDateString()}</Text>
            </View>
          ) : canClaim ? (
            <Pressable
            ...
            <View style={styles.inProgressRow}>
              ...
              <Text style={styles.inProgressText}>{item.target - item.progress} more to complete</Text>
            </View>
          )}
        </View>
      );
