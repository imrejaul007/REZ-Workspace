...
function SharedCategoryPage() {
  ...
  const [filteredStores, setFilteredStores] = useState<unknown[]>([]);
  ...
  const applySorting = useCallback((data: unknown[], sort: SortOption) => {
    const sorted = [...data];
    switch (sort) {
      case 'popularity':
  ...
  const fetchStores = useCallback(async () => {
    try {
      ...
      if (response.success && response.data) {
        ...
        const topRated = allStores.filter((s) => (s.ratings?.average || 0) >= 4.0);
        if (!isMounted()) return;
        setStores(topRated);
        if (!isMounted()) return;
