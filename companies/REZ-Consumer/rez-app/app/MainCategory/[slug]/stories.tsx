  ...
  const fetchUGC = useCallback(async () => {
    try {
      ...
      const [videosRes, reviewsRes] = await Promise.all([
        apiClient.get<unknown>('/videos', { category: slug, limit: 20, status: 'approved' }).catch(() => null),
        apiClient.get<unknown>('/reviews/featured', { category: slug, limit: 20 }).catch(() => null),
      ]);
