...
import { View, Text, StyleSheet, TextInput, Pressable, ActivityIndicator, ScrollView, Keyboard } from 'react-native';
...
function buildServiceChips(slug: string): { id: string; label: string; icon: string }[] {
  ...
  const chips: { id: string; label: string; icon: string }[] = [{ id: 'all', label: 'All', icon: '\u26A1' }];
...
function SharedCategoryPage() {
  ...
  const { q, slug } = useLocalSearchParams<unknown>();
  ...
  useEffect(() => {
    const fetchRecent = async () => {
      try {
        const response = await apiClient.get<unknown>('/search/history/recent', { limit: 5 });
        if (response.success && response.data) {
          const searches = Array.isArray(response.data) ? response.data : response.data.searches || [];
          setRecentSearches(searches.map((s) => s.query || s).filter(Boolean));
        }
      } catch (err) {
  ...
  const renderStoreSearchItem = useCallback(
    ({ item }: { item: unknown }) => (
      <Pressable
      ...
      >
        ...
        <View style={styles.resultContent}>
          ...
          <View style={styles.resultMeta}>
            ...
            {item.offers?.cashback > 0 && <Text style={styles.resultCashback}>{item.offers.cashback}% cashback</Text>}
          </View>
        </View>
  ...
  const renderProductSearchItem = useCallback(
    ({ item }: { item: unknown }) => (
      <Pressable
        style={styles.resultCard}
        ...
        <View style={styles.resultContent}>
          ...
          <View style={styles.resultMeta}>
            <Text style={styles.resultPrice}>
              {currencySymbol}
              {item.pricing?.salePrice || item.pricing?.basePrice || item.price}
            </Text>
      ...
      <View style={styles.serviceBar}>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.serviceList}>
          {SERVICE_CHIPS.map((chip) => (
            <Pressable
              key={chip.id}
              style={[styles.serviceChip, activeService === chip.id ? styles.serviceChipActive : null]}
              onPress={() => handleServicePress(chip.id)}
            >
              <Text style={styles.serviceChipIcon}>{chip.icon}</Text>
              <Text style={[styles.serviceChipText, activeService === chip.id ? styles.serviceChipTextActive : null]}>
                {chip.label}
              </Text>
            </Pressable>
          ))}
