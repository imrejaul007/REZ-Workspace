...
const FitnessBookingPage: React.FC = () => {
  ...
  const { storeId, storeName, cashback, type } = useLocalSearchParams<unknown>();
  ...
  const fetchStore = useCallback(async () => {
    ...
    try {
      ...
      const storeData = (storeRes.data as unknown)?.store || storeRes.data;
      if (!isMounted()) return;
      setStore(storeData);
  ...
  const getTrainerPrice = () => {
    ...
    const apiPrices = (store as unknown)?.trainerPricing;
    if (apiPrices) {
      switch (sessionType) {
        case 'single':
  ...
  const getDayPassPrice = () => {
    const apiDayPassPrice = (store as unknown)?.dayPassPrice ?? 499;
