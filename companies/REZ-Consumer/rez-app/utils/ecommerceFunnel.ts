...
export class EcommerceFunnelTracker {
   ...
   */
  async trackProductDiscovery(method: 'browse' | 'search' | 'category', details?): Promise<void> {
   ...
   */
  private async loadFunnel(): Promise<unknown> {
    try {
      const stored = await AsyncStorage.getItem(FUNNEL_KEY);
   ...
   */
  private calculateDropOffRates(data): EcommerceFunnel['dropOffRates'] {
    return {
