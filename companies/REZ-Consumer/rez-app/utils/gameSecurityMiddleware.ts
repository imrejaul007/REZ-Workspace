...
export class GameSecurityMiddleware {
   ...
   */
  private async validateGameData(action: string, data): Promise<void> {
    switch (action) {
      case 'SPIN_WHEEL':
  ...
  async logSuspiciousActivity(
    ...
    try {
      ...
      errorLogger.error('gameSecurityMiddleware:logSuspiciousActivity', error);
    }
  }
  ...
  async checkSuspiciousActivity(userId: string): Promise<boolean> {
    try {
      ...
      errorLogger.error('gameSecurityMiddleware:checkSuspiciousActivity', error);
      return false;
    }
  }
  ...
  private async getSuspiciousActivities(userId: string): Promise<SuspiciousActivity[]> {
    try {
      ...
      errorLogger.logStorageError('gameSecurityMiddleware:getSuspiciousActivities', error, 'async', `suspicious_${userId}`);
      return [];
    }
  }
  ...
  async clearSuspiciousActivities(userId: string): Promise<void> {
    try {
      ...
      errorLogger.logStorageError('gameSecurityMiddleware:clearSuspiciousActivities', error, 'async', `suspicious_${userId}`);
    }
  }
   ...
   */
  sanitizeRequest<T extends Record<string, unknown>>(data: T): T {
...
export function useGameSecurity(userId: string, action: string) {
  ...
  const verifyResult = async (sessionId: string, result) => {
    return await gameSecurityMiddleware.verifyGameResult(sessionId, result);
  };

  const logActivity = async (type: string, details, severity: 'low' | 'medium' | 'high') => {
