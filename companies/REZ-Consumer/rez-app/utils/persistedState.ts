...
class PersistedStateManager {
  ...
  async clearAll(): Promise<void> {
    try {
      ...
      errorLogger.logStorageError('persistedStateManager:clearAll', error, 'async', 'all_persisted');
    }
  }
  ...
  async getSize(): Promise<number> {
    try {
      ...
      errorLogger.logStorageError('persistedStateManager:getSize', error, 'async', 'persisted_states');
      return 0;
    }
  }
      ...
      items.forEach(([key, value]) => {
        if (value) {
          try {
            ...
            errorLogger.logStorageError('persistedStateManager:export:parse', e, 'async', key);
            exported[key] = value;
          }
        }
      });
 ...
 * Persist cart state
 */
export function usePersistedCart<T = unknown>(defaultValue: T[] = []) {
 ...
 */
export function usePersistedWishlist<T = unknown>(defaultValue: T[] = []) {
  return usePersistedState({
 ...
 */
export function usePersistedPreferences<T = Record<string, unknown>>(
  defaultValue: T = {} as T
) {
  return usePersistedState({
 ...
 * Persist form draft
 */
export function usePersistedFormDraft<T = Record<string, unknown>>(
  formKey: string,
  defaultValue: T = {} as T
) {
  return usePersistedState({
