 ...
 */
export function createMockFn<T = unknown>(returnValues: T[]): jest.Mock {
 ...
 */
export function createMockAsyncFn<T = unknown>(
  returnValue: T,
  delay: number = 0
): jest.Mock {
 ...
 */
export function createSequentialMock<T = unknown>(
 ...
 */
export async function getAllAsyncStorageData(): Promise<Record<string, unknown>> {
  const AsyncStorage = require('@react-native-async-storage/async-storage');
  const keys = await AsyncStorage.getAllKeys();
  const data = await AsyncStorage.multiGet(keys);

  return data.reduce((acc: Record<string, unknown>, [key, value]: [string, string]) => {
    try {
 ...
 */
export function setupApiClientMock(responses: Record<string, unknown> = {}): void {
  ...
  apiClient.post = jest.fn((endpoint: string, data) => {
    return Promise.resolve(
      responses[endpoint] || createMockApiResponse(null, false, 'Not mocked')
    );
  });

  apiClient.uploadFile = jest.fn((endpoint: string, formData) => {
    return Promise.resolve(
      responses[endpoint] || createMockApiResponse(null, false, 'Not mocked')
    );
  });
 ...
 */
export function createValidBillData(overrides: Record<string, unknown> = {}) {
  return {
