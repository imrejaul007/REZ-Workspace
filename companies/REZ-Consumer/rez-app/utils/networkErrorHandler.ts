 ...
 */
export function handleNetworkError(error): NetworkErrorInfo {
 ...
 */
export function classifyError(error): NetworkErrorInfo {
 ...
 */
function classifyHttpError(statusCode: number, error): NetworkErrorInfo {
  ...
  if (statusCode === 422) {
    return {
      ...
      suggestions: [
        ...
        'Verify the format of your input',
        'Review unknown error messages shown',
      ],
      isRetryable: false,
      statusCode,
      originalError: error,
 ...
 */
export function isNoInternetError(error): boolean {
  const errorMessage = error?.message?.toLowerCase() || '';
 ...
 */
export function getUserFriendlyMessage(error): string {
  const errorInfo = classifyError(error);
 ...
 */
export function getErrorSuggestions(error): string[] {
  const errorInfo = classifyError(error);
