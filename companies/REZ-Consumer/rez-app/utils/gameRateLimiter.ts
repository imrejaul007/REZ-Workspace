...
export class GameRateLimiter {
  ...
  private async getAttemptRecord(key: string): Promise<AttemptRecord> {
    ...
    try {
      ...
      errorLogger.logStorageError('gameRateLimiter:getAttemptRecord', error, 'async', `rate_limit_${key}`);
    }
  ...
  private async setAttemptRecord(key: string, record: AttemptRecord): Promise<void> {
    ...
    try {
      ...
      errorLogger.logStorageError('gameRateLimiter:setAttemptRecord', error, 'async', `rate_limit_${key}`);
    }
  }
  ...
  async clearAll(): Promise<void> {
    ...
    try {
      ...
      errorLogger.logStorageError('gameRateLimiter:clearAll', error, 'async', 'rate_limits');
    }
  }
}
