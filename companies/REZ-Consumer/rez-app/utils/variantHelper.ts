...
export function hasVariants(product: ProductItem): boolean {
  ...
  const hasVariantsProperty = !!(product as unknown).variants;

  // Check if product has variant requirements
  const hasVariantRequirements = !!(product as unknown).requiresVariantSelection;

  // Check if product has size/color/attributes
  const hasAttributes = !!(
    (product as unknown).sizes ||
    (product as unknown).colors ||
    (product as unknown).attributes
  );
...
export function getVariantDisplayName(product: ProductItem): string {
  ...
  if ((product as unknown).sizes && !(product as unknown).colors) {
    return 'Select Size';
  }

  if ((product as unknown).colors && !(product as unknown).sizes) {
    return 'Select Color';
  }
...
export function extractVariantAttributes(product: ProductItem) {
  return {
    sizes: (product as unknown).sizes || [],
    colors: (product as unknown).colors || [],
    attributes: (product as unknown).attributes || {},
    requiresVariantSelection: (product as unknown).requiresVariantSelection || false,
    variants: (product as unknown).variants || [],
  };
}
