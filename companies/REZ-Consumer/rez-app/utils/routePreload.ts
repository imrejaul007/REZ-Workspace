...
class RoutePreloadManager {
   ...
   */
  addToQueue(component, strategy: PreloadStrategy): void {
    ...
    if (strategy.platforms && !strategy.platforms.includes(Platform.OS as unknown)) {
      return;
    }
  ...
  async processQueue(): Promise<void> {
    ...
    for (const task of this.preloadQueue) {
      ...
      try {
        ...
        errorLogger.warn('routePreload:preloadTask:skipFailed', { component: componentName });
      }
  ...
  private shouldPreloadNow(strategy: PreloadStrategy): boolean {
    ...
    if (strategy.networkConditions) {
      if (!this.networkType || !strategy.networkConditions.includes(this.networkType as unknown)) {
        return false;
      }
    }
   ...
   */
  async preloadNow(component): Promise<void> {
    const componentName = component.displayName || component.name || 'Unknown';
    ...
    try {
      ...
      errorLogger.warn('routePreload:preloadNow:skipFailed', { component: componentName });
    }
  }
   ...
   */
  async preloadMultiple(components: unknown[]): Promise<void> {
 ...
 * ```
 */
export function preloadOnInteraction(component): void {
