...
class MemoryMonitor {
  ...
  private triggerMemoryWarning() {
    this.memoryWarningCallbacks.forEach(callback => {
      try {
        ...
        errorLogger.logError('memoryMonitor:triggerMemoryWarning:callback', error);
      }
    });
  }
  ...
  async clearCaches(): Promise<void> {
    try {
      ...
      errorLogger.logStorageError('memoryMonitor:clearCaches', error, 'async', 'cache');
    }
  }
  ...
  async getStorageStats(): Promise<{ totalKeys: number; estimatedSize: string }> {
    try {
      ...
      errorLogger.logStorageError('memoryMonitor:getStorageStats', error, 'async', 'storage_stats');
      return {
        totalKeys: 0,
