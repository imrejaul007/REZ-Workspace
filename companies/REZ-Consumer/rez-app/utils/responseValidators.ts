 ...
 */
export function validateProduct(rawProduct): ProductItem | null {
  try {
 ...
 */
export function validateStore(rawStore): unknown | null {
  try {
 ...
 */
export function validateProductArray(products: unknown[]): ProductItem[] {
 ...
 */
export function validateStoreArray(stores: unknown[]): unknown[] {
 ...
 */
function normalizePrice(data): ProductItem['price'] | null {
  try {
 ...
 */
function normalizeRating(data): ProductItem['rating'] {
  try {
 ...
 */
function normalizeStoreRating(data): unknown {
  try {
 ...
 */
function normalizeImages(data): Array<{ id: string; url: string; alt: string; isMain: boolean }> {
  try {
    if (Array.isArray(data.images) && data.images.length > 0) {
      return data.images.map((img, index: number) => ({
 ...
 */
function normalizeCashback(data): { percentage: number; maxAmount?: number } | undefined {
  try {
 ...
 */
function normalizeAvailabilityStatus(data): 'in_stock' | 'low_stock' | 'out_of_stock' {
  try {
 ...
 */
function normalizeInventory(data): { stock: number; lowStockThreshold?: number } | undefined {
  try {
 ...
 */
function normalizeLocation(data): unknown {
  try {
 ...
 */
export function validateCriticalFields(data, requiredFields: string[]): boolean {
 ...
 */
export function normalizeId(data): string | null {
  if (!data) return null;
