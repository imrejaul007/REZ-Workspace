...
type CategoryAction = 
  ...
  | { type: 'UPDATE_FILTERS'; payload: Record<string, unknown> }
  ...
  const loadCategory = useCallback(async (slug: string) => {
    try {
      ...
      try {
        ...
        localCategory = categoryData.default.categories.find((cat) => cat.slug === slug) || null;
      ...
      try {
        ...
        if (response.data) {
            ...
            if (productsResponse.success && productsResponse.data) {
              ...
              if (slug === 'fleet') {
                ...
                try {
                  ...
                  if (storesResponse.success && storesResponse.data && Array.isArray(storesResponse.data)) {
                    ...
                    const vehicleTypes: { [key: string]: unknown[] } = {};
                    ...
                    if (carouselItems.length === 0 && featuredStores.length > 0) {
                      carouselItems = featuredStores.map((store) => ({
              ...
              if (carouselItems.length === 0) {
                  ...
                  if (!storesFound) {
                    ...
                    for (const tag of tagQueries) {
                      ...
                      if (storesResponse.success && storesResponse.data && Array.isArray(storesResponse.data) && storesResponse.data.length > 0) {
                        ...
                        carouselItems = allStores.map((store) => ({
                          id: store._id || store.id,
                          title: store.name || 'Store',
                          subtitle: store.description?.substring(0, 50) || store.location?.address || '',
                          image: store.banner || store.logo || 'https://placehold.co/300x200?text=No+Image',
                ...
                if (carouselItems.length === 0 && products.length > 0) {
                  ...
                  carouselItems = productsToShow.map((product) => ({
                    id: product._id || product.id,
          ...
          const category = {
            ...
            carouselItems: carouselItems as unknown[],
...
let __useCategoryStore: () => unknown;
try {
