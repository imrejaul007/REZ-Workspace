...
let __useWishlistStore: () => unknown;
try {
...
export const WishlistProvider: React.FC<WishlistProviderProps> = ({ children }) => {
  ...
  const loadWishlist = async () => {
    try {
      ...
      const wishlistItems: WishlistItem[] = defaultWishlist.items.map((backendItem) => {
    ...
    try {
      ...
      try {
        ...
        if (wishlistsResponse.data && wishlistsResponse.data.wishlists && wishlistsResponse.data.wishlists.length > 0) {
          wishlistId = wishlistsResponse.data.wishlists[0].id || (wishlistsResponse.data.wishlists[0] as unknown)._id;
        } else {
          const newWishlistResponse = await wishlistApi.createWishlist({
            name: 'My Wishlist',
            description: 'Default wishlist',
        ...
        if (response.data) {
          ...
          setWishlistItems(prev => prev.map(i =>
            i.id === optimisticItem.id
              ? { ...i, id: responseData.id || (responseData as unknown)._id }
              : i
          ));
        } else {
