...
export function CartProvider({ children }: CartProviderProps) {
    ...
    try {
      ...
      try {
        ...
        if (response.success && response.data) {
          ...
          }) as unknown[]) as CartItemWithQuantity[];
  ...
  const optimizeCartForStorage = useCallback((items: CartItemWithQuantity[]) => {
    return items.map(item => ({
      ...
      store: (item as unknown).store ? (typeof (item as unknown).store === 'string' ? (item as unknown).store : (item as unknown).store.id || (item as unknown).store.name) : undefined,
      variant: (item.variant as unknown) ? (typeof item.variant === 'string' ? item.variant : (item.variant as unknown)?.id || (item.variant as unknown)?.name) : undefined,
    }));
  }, []);
