...
export const fetchWalletBalance = async (): Promise<WalletBalance> => {
  ...
  if (__DEV__) {
    logger.error('[DEPRECATED] walletData mock used in production: fetchWalletBalance — use walletApi.getBalance() instead');
  }
...
export const fetchTransactions = async (
  ...
  if (__DEV__) {
    logger.error('[DEPRECATED] walletData mock used in production: fetchTransactions — use walletApi.getTransactions() instead');
  }
  // Simulate API delay
...
export const fetchTransactionDetails = async (transactionId: string): Promise<Transaction | null> => {
  if (__DEV__) {
    logger.error('[DEPRECATED] walletData mock used in production: fetchTransactionDetails — use walletApi.getTransactionById() instead');
  }
  await new Promise(resolve => setTimeout(resolve, 300));
  return mockTransactions.find(t => t.id === transactionId) || null;
};

export const createTransaction = async (transactionData): Promise<Transaction> => {
