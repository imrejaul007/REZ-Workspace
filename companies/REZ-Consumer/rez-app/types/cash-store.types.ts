...
export interface CashStoreHomepageApiData {
  ...
  topBrands: unknown[]; // MallBrand from backend
  trendingBrands: unknown[]; // featured MallBrands
  highCashbackBrands: unknown[]; // high cashback MallBrands
}
...
export function getBadgeColor(badge: DealBadge): string {
  // REZ palette badge colors - using theme imports
  const badgeColors: Record<DealBadge, string> = {
...
export function getTravelDealGradient(category: TravelDealCategory): string[] {
  ...
  return travelGradients[category] || ['#ffcd57', '#1a3a52'];
}
