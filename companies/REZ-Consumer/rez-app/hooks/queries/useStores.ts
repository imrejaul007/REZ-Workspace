...
export function useNearbyStores(lat: number, lon: number, radius?: number) {
  return useQuery({
    ...
    queryFn: () => (storesApi as unknown).getNearbyStores({ latitude: lat, longitude: lon, radius }),
    enabled: !!lat && !!lon,
  });
}

export function useSearchStores(query: string, filters?: Record<string, unknown>) {
  return useQuery({
...
export function useStoreProducts(storeId: string, filters?: Record<string, unknown>) {
  return useQuery({
...
export function useStoreReviews(storeId: string, filters?: Record<string, unknown>) {
  return useQuery({
    queryKey: queryKeys.stores.reviews(storeId, filters),
    queryFn: () => storesApi.getStoreReviews(storeId, filters as unknown),
    enabled: !!storeId,
  });
}
...
export function useFollowedStores(params?: { page?: number; limit?: number }) {
  return useQuery({
    ...
    queryFn: () => storesApi.getFollowedStores(params as unknown),
  });
}
