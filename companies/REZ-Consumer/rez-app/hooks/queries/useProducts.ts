...
export function useProductsByStore(storeId: string, params?: Record<string, unknown>) {
  return useQuery({
...
export function useProductRecommendations(params?: Record<string, unknown>) {
  return useQuery({
    queryKey: queryKeys.products.recommendations(),
    queryFn: () => productsApi.getRecommendations(params as unknown),
  });
}

export function useRelatedProducts(productId: string, params?: Record<string, unknown>) {
  return useQuery({
    queryKey: ['products', 'related', productId] as const,
    queryFn: () => productsApi.getRelatedProducts(productId, params as unknown),
    enabled: !!productId,
  });
}
