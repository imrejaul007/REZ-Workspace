...
const isStoreOpenNow = (hours): { isOpen: boolean; openTime?: string; closeTime?: string } => {
...
const detectIsBrand = (store): boolean => {
...
const detectIsService = (store): boolean => {
...
const detectIsLocal = (store, isBrand: boolean, isService: boolean): boolean => {
...
export const transformToPaymentStore = (store, distance?: number): PaymentStoreInfo => {
    ...
    queryFn: async (): Promise<PaymentStoreInfo[]> => {
      try {
        ...
        if (response.success && response.data?.stores?.length) {
          return response.data.stores.map((store) => transformToPaymentStore(store));
        }
      } catch {
      ...
      if (fallbackResponse.success && fallbackResponse.data?.stores) {
        return fallbackResponse.data.stores.map((store) => transformToPaymentStore(store));
      }
      return [];
    },
  });
