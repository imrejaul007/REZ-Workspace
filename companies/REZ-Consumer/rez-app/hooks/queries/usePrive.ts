...
export function usePriveTier() {
  return useQuery({
    ...
    queryFn: () => (priveApi as unknown).getTier(),
  });
}
...
export function usePriveSummary() {
  return useQuery({
    ...
    queryFn: () => (priveApi as unknown).getSummary(),
  });
}

export function usePriveOffers(filters?: Record<string, unknown>) {
  return useQuery({
...
export function usePriveHabits() {
  return useQuery({
    ...
    queryFn: () => (priveApi as unknown).getHabits(),
  });
}
