...
const mapBackendProductToGoingOut = (product): GoingOutProduct => {
...
const mapBackendCategories = (categories: unknown[]): GoingOutCategory[] => {
...
export function useGoingOutPage(): UseGoingOutPageReturn {
  ...
  const currentPageProducts = useMemo(() => {
    ...
    const rawProducts = Array.isArray(productsQuery.data.data)
      ...
      : (productsQuery.data.data as unknown).products || [];
    return rawProducts.map(mapBackendProductToGoingOut);
  }, [productsQuery.data]);

  const hasMore = useMemo(() => {
    if (!productsQuery.data?.data) return false;
    const pagination = (productsQuery.data.data as unknown)?.pagination;
  ...
  const cashbackHubSections = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        products: products.filter((p) => p.rating && p.rating.value >= 4.5).slice(0, 10),
        showViewAll: false,
      },
    ];
  }, [accumulatedProducts]);
  ...
  const debouncedApiSearch = useDebouncedCallback(
    async (query: string, activeCat: string) => {
      ...
      try {
        ...
        const response: unknown = await productsApi.searchProducts(searchQueryParams);
