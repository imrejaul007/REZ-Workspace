...
export function useStoreReviews(
  ...
  const fetchReviews = useCallback(async (page: number = 1, append: boolean = false) => {
    ...
    try {
      ...
      const response: unknown = await storesService.getStoreReviews(storeId, {
        page,
      ...
      if (response.success && response.data) {
        ...
        const transformedReviews: Review[] = fetchedReviews.map((review, index: number) => {
          ...
          const transformed = {
            ...
            images: review.images?.map((img, idx: number) => {
        ...
        if (summary) {
          ...
          const summaryAny = summary as unknown;
        ...
        // Update pagination
        if (pagination) {
          setHasMore((pagination as unknown).hasNext || false);
        }
      } else {
