...
export function useMainStorePageData({ productId, initialProduct }: MainStorePageProps = {}) {
  ...
  const fullStoreDataRef = useRef<unknown>(null);
  ...
  const [ugcContent, setUgcContent] = useState<unknown[]>([]);
    ...
    if ((activeTab === "reviews" || showReviewModal) && reviewStoreId) {
      const fetchReviewTabData = async () => {
        await Promise.allSettled([
          (async () => {
            try {
              const response: unknown = await reviewApi.canUserReviewStore(reviewStoreId);
          ...
          (async () => {
            try {
              setUgcLoading(true);
              const response: unknown = await apiClient.get(`/ugc/store/${reviewStoreId}`, { limit: 20, offset: 0 });
              if (response.success && response.data) {
                const content = (response.data as unknown).content || [];
                setUgcContent(content.map((item) => ({
                  id: item._id || item.id,
  ...
  const handleReviewHelpful = useCallback(async (reviewId: string) => {
    try {
      const response: unknown = await reviewsApi.markHelpful(reviewId);
  ...
  const handleReviewReport = useCallback(async (reviewId: string) => {
    try {
      const response: unknown = await reviewsApi.reportReview(reviewId, 'inappropriate');
      if (response.success) {
    ...
    if (isDynamic && storeData) {
      ...
      if (storeData.location) {
        if (typeof storeData.location === 'object') {
          ...
          cityStr = locObj.city || "";
          stateStr = (locObj as unknown).state || "";
          pincodeStr = (locObj as unknown).pincode || (locObj as unknown).pinCode || "";
      ...
      const distanceStr = (storeData as unknown).distance ? `${(storeData as unknown).distance} Km` : "";
      ...
      return {
        ...
        cashbackPercentage: typeof storeData.cashback === 'object'
          ? (storeData.cashback as unknown).percentage?.toString() || "0"
  ...
  useEffect(() => {
    ...
    const task = InteractionManager.runAfterInteractions(async () => {
      try {
        const response: unknown = await storesApi.getUserStoreVisits(storeId);
        if (response.success && response.data) {
          setUserVisitsData({
  ...
  const handleFavoritePress = useCallback(async () => {
    ...
    try {
      if (wasFollowing) {
        const response: unknown = await wishlistApi.removeFromWishlist('store', storeIdToUse);
        if (response.success) {
        ...
        const response: unknown = await wishlistApi.addToWishlist({
