...
export const useSocialProof = (options: UseSocialProofOptions = {}): UseSocialProofResult => {
  ...
  const fetchSocialProof = useCallback(async () => {
    ...
    if (!coordinates?.latitude || !coordinates?.longitude) {
      ...
      if (city) {
        try {
          const response: unknown = await socialProofApi.getCityWideStats(city);
          if (response.success && response.data) {
            setCityWideStats(response.data);
          }
        } catch (err) {
          errorLogger.logApiError('useSocialProof:getCityWideStats', err, '/api/social-proof/city', 500);
        }
      }
      setIsLoading(false);
      return;
    ...
    try {
      ...
      const response: unknown = await socialProofApi.getNearbyActivity({
