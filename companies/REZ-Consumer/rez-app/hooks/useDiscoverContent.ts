...
export function useDiscoverContent(): UseDiscoverContentReturn {
  ...
  const transformToReel = (video): DiscoverReel => {
  ...
  const transformToPost = (item): DiscoverPost => {
  ...
  const transformToArticle = (article): DiscoverArticle => {
  ...
  const transformToImage = (item): DiscoverImage => {
  ...
  const fetchTabContent = useCallback(async (tab: DiscoverTabType, refresh = false) => {
    ...
    try {
      ...
      switch (tab) {
        case 'reels': {
          const response: unknown = await realVideosApi.getVideos({
            contentType: 'ugc',
            hasProducts: true,
            sortBy: 'trending',
            page,
        ...
        case 'posts': {
          ...
          const merchantVideos = merchantResponse?.data?.videos || (merchantResponse as unknown)?.videos || [];
          const ugcVideos = ugcResponse?.data?.videos || (ugcResponse as unknown)?.videos || [];
        ...
        case 'articles': {
          const response: unknown = await articlesApi.getArticles({
            sortBy: 'popular',
            page,
            limit: ITEMS_PER_PAGE,
            isPublished: true,
