 ...
 * ```
 */
export function useCachedMutation<T = unknown, V = unknown>(
...
export interface UseInfiniteCachedQueryOptions extends UseCachedQueryOptions {
  getNextPageParam?: (lastPage, allPages: unknown[]) => unknown;
}
 ...
 * ```
 */
export function useInfiniteCachedQuery<T = unknown>(
  queryKey: unknown[],
  fetcher: (context: { pageParam: unknown }) => Promise<T>,
  ...
  const currentPage = useRef<unknown>(0);
