...
export const useUserStatistics = (autoFetch: boolean = true): UseUserStatisticsReturn => {
  ...
  const fetchStatistics = useCallback(async (forceRefresh: boolean = false) => {
    ...
    try {
      ...
      const response: unknown = await authService.getUserStatistics();
      ...
      if (response.success && response.data) {
        ...
        const walletBalance = typeof response.data.wallet?.balance === 'object'
          ? (response.data.wallet.balance as unknown).available || (response.data.wallet.balance as unknown).total || 0
        ...
        const statsData: UserStatistics = {
          ...
          reviews: (response.data as unknown).reviews || { total: 0 },
          achievements: (response.data as unknown).achievements || { total: 0, unlocked: 0 },
          activities: (response.data as unknown).activities || { total: 0 },
        };
