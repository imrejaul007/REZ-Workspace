...
const normalizeId = (item) => ({ ...item, id: item._id || item.id });

// Helper function to transform offers to ensure proper store structure
const transformOfferWithStore = (offer) => {
...
const transformOffersArray = (offers: unknown[]): unknown[] => {
...
const transformFlashSaleToLightningDeal = (flashSale) => {
...
const transformFlashSalesArray = (flashSales: unknown[]): unknown[] => {
...
interface OffersPageApiData {
  // Existing sections
  lightningDeals: unknown[];
  nearbyOffers: unknown[];
  trendingOffers: unknown[];
  friendsRedeemed: unknown[];

  // Discount buckets (real-time counts)
  discountBuckets: unknown[];

  // Cashback tab
  hotspots: unknown[];
  doubleCashback: unknown[];
  coinDrops: unknown[];
  uploadBillStores: unknown[];
  bankOffers: unknown[];
  superCashbackStores: unknown[];

  // Exclusive tab
  exclusiveZones: unknown[];
  specialProfiles: unknown[];
  loyaltyMilestones: unknown[];

  // Offer types
  bogoOffers: unknown[];
  saleOffers: unknown[];
  freeDeliveryOffers: unknown[];

  // Additional sections (previously using dummy data)
  todaysOffers: unknown[];
  aiRecommendedOffers: unknown[];
  lastChanceOffers: unknown[];
  newTodayOffers: unknown[];

  // Hero banner
  heroBanner: unknown | null;
}
...
export function useOffersData() {
  ...
  const fetchAggregatedData = useCallback(async () => {
    ...
    try {
      const response: unknown = await realOffersApi.getAggregatedPageData({
        lat: 12.9716, lng: 77.5946, tab: 'all',
      });

      if (response.success && response.data) {
        const { sections } = response.data;
        const getSectionData = (key: string): unknown[] => sections?.[key]?.data || [];

        // Transform friends redeemed data
        const friendsRedeemedRaw = getSectionData('friendsRedeemed');
        const transformedFriendsRedeemed = friendsRedeemedRaw.map((item) => ({
          id: item._id || item.id,
        ...
        setApiData({
          ...
          hotspots: getSectionData('hotspots').map((d) => ({ ...normalizeId(d), areaId: d._id || d.id })),
          ...
          superCashbackStores: getSectionData('superCashbackStores').map((s) => ({ ...normalizeId(s), isSuper: s.isSuperCashback ?? true })),
          exclusiveZones: getSectionData('exclusiveZones').map(normalizeId),
          specialProfiles: getSectionData('specialProfiles').map((p) => ({ ...normalizeId(p), isVerified: p.userEligible ?? false })),
          loyaltyMilestones: getSectionData('loyaltyMilestones').map((m) => ({ ...normalizeId(m), currentValue: m.currentProgress ?? 0 })),
  ...
  const fetchOffersDataLegacy = useCallback(async () => {
    ...
    try {
      ...
      const extractData = (result: PromiseSettledResult<unknown>) => {
      ...
      const transformedFriendsRedeemed = friendsRedeemedData.map((item) => ({
        id: item._id || item.id,
        friendId: item.friendId,
        friendName: item.friendName,
        friendAvatar: item.friendAvatar,
      ...
      setApiData({
        ...
        friendsRedeemed: transformedFriendsRedeemed,
        hotspots: extractData(hotspotsRes).map((d) => ({ ...normalizeId(d), areaId: d._id || d.id })),
        ...
        superCashbackStores: extractData(superCashbackStoresRes).map((s) => ({ ...normalizeId(s), isSuper: s.isSuperCashback ?? true })),
        exclusiveZones: extractData(exclusiveZonesRes).map(normalizeId),
        specialProfiles: extractData(specialProfilesRes).map((p) => ({ ...normalizeId(p), isVerified: p.userEligible ?? false })),
        loyaltyMilestones: extractData(loyaltyRes).map((m) => ({ ...normalizeId(m), currentValue: m.currentProgress ?? 0 })),
