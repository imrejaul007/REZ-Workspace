...
export const usePriveSection = (): UsePriveSectionReturn => {
  ...
  const fetchDashboardData = useCallback(async () => {
    ...
    try {
      const response: unknown = await priveApi.getDashboard();
      ...
      if (response.success && response.data) {
        ...
        if (dashboard.user) {
          setUserData(prev => ({
            ...prev,
            name: (dashboard.user as unknown).name || prev.name,
        ...
        if (dashboard.dailyProgress) {
          ...
          setDailyProgress({
            ...
            loops: (dashboard.dailyProgress.loops || []).map((loop) => ({
              ...
              description: (loop as unknown).description,
              deepLink: (loop as unknown).deepLink,
            })),
            allCompleted: (dashboard.dailyProgress as unknown).allCompleted || false,
          });
        }
  ...
  const checkIn = useCallback(async () => {
    try {
      const response: unknown = await priveApi.checkIn();
