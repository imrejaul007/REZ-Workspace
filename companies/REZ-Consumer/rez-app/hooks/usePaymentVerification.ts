...
export function usePaymentVerification(paymentMethodId?: string) {
  ...
  const getVerificationStatus = useCallback(async (methodId?: string) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.getVerificationStatus(id);
  ...
  const verifyCard = useCallback(async (request: CardVerificationRequest) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.initiateCardVerification(request);
  ...
  const complete3DSAuth = useCallback(async (verificationId: string, authData) => {
    setState(prev => ({ ...prev, isLoading: true, error: null }));

    try {
      const response: unknown = await paymentVerificationService.complete3DSAuthentication(
        verificationId,
        authData
      );
      if (response.success && response.data) {
  ...
  const verifyBankAccount = useCallback(async (request: BankVerificationRequest) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.initiateBankVerification(request);
  ...
  const verifyMicroDeposits = useCallback(
    async (verificationId: string, deposits: MicroDepositVerification) => {
      ...
      try {
        const response: unknown = await paymentVerificationService.verifyMicroDeposits(
          verificationId,
          deposits
        );
        if (response.success && response.data) {
  ...
  const verifyUPI = useCallback(async (request: UPIVerificationRequest) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.initiateUPIVerification(request);
  ...
  const uploadKYC = useCallback(async (request: KYCVerificationRequest) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.uploadKYCDocuments(request);
  ...
  const sendOTP = useCallback(async (request: OTPVerificationRequest) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.sendOTP(request);
  ...
  const validateOTP = useCallback(async (verificationId: string, otp: string) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.validateOTP({
        verificationId,
        otp,
      });
  ...
  const authenticateWithBiometric = useCallback(async (prompt?: string) => {
    ...
    try {
      const response: unknown = await paymentVerificationService.authenticateWithBiometric(prompt);
