...
export function useStoreProductsPage(
      ...
      try {
        ...
        if (response.success && response.data) {
          const categoryList = Array.isArray(response.data)
            ? response.data
            : ((response.data as unknown).categories || []);
  ...
  const fetchProducts = useCallback(async (
    ...
    try {
      ...
      const queryParams: unknown = { page: pageNum, limit: 20 };
      if (searchQuery.trim()) queryParams.search = searchQuery.trim();
      if (selectedCategory) queryParams.category = selectedCategory;
      if (sortBy) queryParams.sortBy = sortBy;

      const response: unknown = await storesApi.getStoreProducts(storeId, queryParams);
      ...
      if (response.success && response.data) {
        ...
        const newProducts: ProductItem[] = (response.data.products || []).map((product) => {
