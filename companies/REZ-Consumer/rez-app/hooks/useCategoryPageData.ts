...
const getDummyData = (slug: string): unknown => {
...
export const useCategoryPageData = (slug: string, _options?: { storesPerPage?: number }): UseCategoryPageDataResult => {
  ...
  const [cuisineCounts, setCuisineCounts] = useState<unknown[]>([]);
  ...
  const subcategories = useMemo<SubcategoryItem[]>(() => {
    ...
    if (!hasChildCategories) {
      ...
      if (dummyData.categories) {
        return dummyData.categories.map((cat) => ({
          id: cat.id,
          name: cat.name,
          slug: cat.id,
          icon: cat.icon,
    ...
    return (category.childCategories || []).map((child) => {
      ...
      for (const [key, value] of Object.entries(cuisineIconMap)) {
        if (nameLower.includes(key) || slugLower.includes(key)) {
          ...
          fallbackColor = value.color as unknown;
          break;
        }
      }
  ...
  const hashtags = useMemo<CategoryHashtag[]>(() => {
    if (category) return (category as unknown).trendingHashtags || [];
    if (categoryQuery.isError || (categoryQuery.isSuccess && !categorySuccess)) {
      const dummyData = getDummyData(slug);
  ...
  const stores = useMemo<CategoryStoreItem[]>(() => {
    ...
    const rawStores: unknown[] = Array.isArray(storesData.data)
      ? storesData.data
      : Array.isArray((storesData.data as unknown)?.stores)
        ? (storesData.data as unknown).stores
        : [];
    ...
    (async () => {
      try {
        const [videosRes, reviewsRes] = await Promise.all([
          apiClient.get<unknown>('/videos', { category: slug, limit: 6, status: 'approved' }).catch(() => null),
          apiClient.get<unknown>('/reviews/featured', { category: slug, limit: 6 }).catch(() => null),
        ]);
