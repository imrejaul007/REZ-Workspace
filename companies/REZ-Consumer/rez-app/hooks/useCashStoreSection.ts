 ...
 */
function transformMallBrandToCashStoreBrand(brand): CashStoreBrand {
  return {
    _id: brand._id,
    id: brand._id,
    name: brand.name,
 ...
 */
function transformMallBrandToTrendingDeal(brand): TrendingDeal {
 ...
 */
function transformMallBrandToHighCashbackDeal(brand): HighCashbackDeal {
  const cashback = brand.cashback?.percentage || 0;
  return {
    _id: brand._id,
    id: brand._id,
...
export function useCashStoreSection(
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response: unknown = await travelApi.getCategories();
  ...
  const cashbackSummary = useMemo(() => {
    if (!(summaryQuery.data as unknown)?.success || !(summaryQuery.data as unknown)?.data) {
      return { total: 0, pending: 0, confirmed: 0, available: 0 };
    }
    const s = (summaryQuery.data as unknown).data;
    return {
      total: s.totalEarned ?? (s as unknown).total ?? 0,
      pending: s.pending ?? 0,
      confirmed: s.credited ?? (s as unknown).confirmed ?? 0,
      available: s.credited ?? (s as unknown).available ?? 0,
    };
  }, [summaryQuery.data]);
      ...
      try {
        ...
        if (!cancelled) {
          ...
          setFilteredHighCashbackDealsOverride(
            brands
              .filter((b) => (b.cashback?.percentage || 0) >= 10)
