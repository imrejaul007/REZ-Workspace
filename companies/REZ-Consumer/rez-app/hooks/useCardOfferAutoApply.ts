...
export function useCardOfferAutoApply({
  ...
  const validateAndApplyBestOffer = useCallback(async (cardNum: string) => {
    ...
    try {
      ...
      const response: unknown = await discountsApi.validateCardForOffers({
        cardNumber: cardNum,
        storeId,
        orderValue,
      });
  ...
  const removeOffer = useCallback(async () => {
    if (cartActions && typeof (cartActions as unknown).removeCardOffer === 'function') {
      await (cartActions as unknown).removeCardOffer();
    }
  ...
  useEffect(() => {
    ...
    if (appliedOffer || (cartState as unknown).appliedCardOffer) {
      return;
    }
