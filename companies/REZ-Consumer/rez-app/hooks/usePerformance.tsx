...
export const usePerformance = (options: UsePerformanceOptions = {}): UsePerformanceReturn => {
  ...
  const trackOperation = useCallback(
    async <T,>(label: string, operation: () => Promise<T>): Promise<T> => {
      ...
      try {
        ...
        // Track in analytics
        if (typeof (window as unknown).analyticsService !== 'undefined') {
          (window as unknown).analyticsService?.trackPerformance(
  ...
  const logMetrics = useCallback(() => {
    ...
    if (trackMemory && (performance as unknown).memory) {
      logger.debug(
        'Memory used:',
        `${((performance as unknown).memory.usedJSHeapSize / 1048576).toFixed(2)}MB`
      );
      logger.debug(
        'Memory limit:',
        `${((performance as unknown).memory.jsHeapSizeLimit / 1048576).toFixed(2)}MB`
      );
    }
