...
export function useAppServices(fontsLoaded: boolean) {
    ...
    const subscription = AppState.addEventListener('change', nextAppState => {
      ...
      if (previousState.match(/inactive|background/) && nextAppState === 'active') {
        import('@/services/cacheWarmingService').then(mod => {
          (mod.default as unknown).onAppForeground?.();
        }).catch(() => {});
      }
  ...
  useEffect(() => {
    if (fontsLoaded && !cacheWarmed) {
      const interaction = InteractionManager.runAfterInteractions(() => {
        ...
        (interaction as unknown)._timer = timer;
      });

      return () => {
        interaction.cancel();
        if ((interaction as unknown)._timer) {
          clearTimeout((interaction as unknown)._timer);
        }
      };
    }
  ...
  const initializeApp = async () => {
    try {
      ...
      apiClient.setMaintenanceCallback(() => {
        try { router.replace('/system/maintenance' as unknown); } catch (_e) { /* navigation may fail if unmounted */ }
      });
      apiClient.setAppUpdateCallback((_minVersion) => {
        try { router.replace('/system/app-update' as unknown); } catch (_e) { /* navigation may fail if unmounted */ }
      });
  ...
  const handleQueueSyncComplete = useCallback((result) => {
