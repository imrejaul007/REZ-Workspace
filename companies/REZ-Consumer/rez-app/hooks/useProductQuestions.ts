...
export const useProductQuestions = ({
  ...
  const loadQuestions = useCallback(
    async (pageNum: number = 1, refresh: boolean = false) => {
      ...
      try {
        ...
        const response: unknown = await questionsApi.getProductQuestions(productId, {
          page: pageNum,
          limit: 10,
          sortBy,
        });
  ...
  const askQuestion = useCallback(
    async (questionText: string) => {
      try {
        ...
        const response: unknown = await questionsApi.askQuestion({
          productId,
  ...
  const answerQuestion = useCallback(
    async (questionId: string, answerText: string) => {
      try {
        ...
        const response: unknown = await questionsApi.answerQuestion({
          questionId,
          answer: answerText,
        });
  ...
  const markAnswerHelpful = useCallback(async (questionId: string, answerId: string) => {
    try {
      ...
      const response: unknown = await questionsApi.markAnswerHelpful(questionId, answerId);
