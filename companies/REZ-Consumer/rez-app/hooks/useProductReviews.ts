...
export function useProductReviews({
  ...
  const loadReviews = useCallback(async (page: number = 1, append: boolean = false) => {
    try {
      ...
      const response: unknown = await reviewsApi.getTargetReviews('product', productId, query);
  ...
  const submitReview = useCallback(async (data: {
    ...
    try {
      ...
      const response: unknown = await reviewsApi.createReview({
        targetType: 'product',
        targetId: productId,
        rating: data.rating,
        title: data.title,
  ...
  const updateReview = useCallback(async (reviewId: string, updates: Partial<Review>) => {
    try {
      ...
      const response: unknown = await reviewsApi.updateReview(reviewId, updates as unknown);
  ...
  const deleteReview = useCallback(async (reviewId: string) => {
    try {
      ...
      const response: unknown = await reviewsApi.deleteReview(reviewId);
  ...
  const canUserReview = useCallback(async (): Promise<boolean> => {
    try {
      const response: unknown = await reviewsApi.canReview('product', productId);
