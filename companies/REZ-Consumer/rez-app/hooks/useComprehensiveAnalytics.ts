...
export const useComprehensiveAnalytics = () => {
  ...
  const trackProductSearched = useCallback((query: string, resultsCount: number, filters?) => {
  ...
  const trackProductFiltered = useCallback((filters: Record<string, unknown>, resultsCount: number) => {
  ...
  const trackCheckoutStarted = useCallback((itemCount: number, totalValue: number, items: unknown[]) => {
