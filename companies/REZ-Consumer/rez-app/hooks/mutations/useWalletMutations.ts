...
export function useWithdrawFunds() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (data: { amount: number; bankDetails: unknown }) =>
      walletApi.withdraw({ amount: data.amount, method: 'bank', accountDetails: data.bankDetails } as unknown),
    retry: 0,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.wallet.all });
    },
...
export function useInitiateTransfer() {
  ...
  return useMutation({
    mutationFn: (data: { recipientId: string; amount: number; message?: string }) =>
      walletApi.initiateTransfer(data as unknown),
    retry: 0,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.wallet.all });
    },
...
export function useSendGift() {
  ...
  return useMutation({
    mutationFn: (data: { recipientPhone: string; amount: number; message?: string }) =>
      walletApi.sendGift(data as unknown),
    retry: 0,
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.wallet.all });
    },
