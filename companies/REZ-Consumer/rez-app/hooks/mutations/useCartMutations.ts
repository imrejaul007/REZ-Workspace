...
export function useAddToCart() {
  ...
  return useMutation({
    mutationFn: (data: { productId: string; storeId: string; quantity?: number }) =>
      cartApi.addToCart({ ...data, quantity: data.quantity ?? 1 } as unknown),
...
export function useUpdateCartItem() {
  ...
  return useMutation({
    mutationFn: ({ itemId, quantity }: { itemId: string; quantity: number }) =>
      cartApi.updateCartItem(itemId, { quantity } as unknown),
...
export function useRemoveFromCart() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (itemId: string) => (cartApi as unknown).removeCartItem(itemId),
...
export function useApplyCoupon() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (code: string) => cartApi.applyCoupon({ couponCode: code } as unknown),
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.cart.all });
    },
  });
