...
export function useStoreMessaging(options: UseStoreMessagingOptions = {}): UseStoreMessagingReturn {
  ...
  const messagesEndRef = useRef<unknown>(null); // Use any for cross-platform compatibility
  ...
  const getConversation = useCallback(async () => {
    ...
    try {
      ...
      if (conversationId) {
        const response: unknown = await storeMessagingService.getConversation(conversationId);
        if (response.success && response.data) {
          conv = response.data;
        }
      } else if (storeId) {
        const response: unknown = await storeMessagingService.getOrCreateConversation(storeId, orderId);
        if (response.success && response.data) {
          conv = response.data;
  ...
  const loadMessages = useCallback(async (page: number = 1) => {
    ...
    try {
      const response: unknown = await storeMessagingService.getMessages(conversationId, page, 50);
  ...
  const sendMessage = useCallback(async (content: string, type: Message['type'] = 'text') => {
    ...
    try {
      ...
      const response: unknown = await storeMessagingService.sendMessage(request);
  ...
  const sendMessageWithAttachments = useCallback(async (content: string, attachments: File[]) => {
    ...
    try {
      const response: unknown = await storeMessagingService.sendMessageWithAttachments(
        conversationId,
        content,
        attachments
      );
  ...
  const archiveConversation = useCallback(async () => {
    ...
    try {
      const response: unknown = await storeMessagingService.archiveConversation(conversationId);
      if (response.success && response.data) {
