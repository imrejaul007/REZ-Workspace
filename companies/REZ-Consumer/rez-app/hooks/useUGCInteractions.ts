...
export function useUGCInteractions() {
   ...
   */
  const initializeState = useCallback((content: unknown[]) => {
  ...
  const toggleLike = useCallback(
    async (contentId: string) => {
      ...
      try {
        // Call backend API
        const response: unknown = await ugcApi.toggleLike(contentId);
  ...
  const toggleBookmark = useCallback(
    async (contentId: string) => {
      ...
      try {
        // Call backend API
        const response: unknown = await ugcApi.toggleBookmark(contentId);
