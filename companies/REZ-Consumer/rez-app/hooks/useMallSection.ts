 ...
 */
function transformStoreToMallBrand(store): MallBrand {
    ...
    try {
      ...
      if (useStores) {
        ...
        if (isMountedRef.current) {
          ...
          setCategories((batchData.categories || []).map((cat) => ({
            _id: cat._id,
            id: cat._id,
            name: cat.name,
            slug: cat.slug,
          ...
          setCollections((batchData.collections || []).map((col) => ({
            _id: col._id,
            id: col._id,
            name: col.name,
            slug: col.slug,
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (debounceRef.current) {
        ...
        debounceRef.current = undefined as unknown;
      }
    };
  }, []);
