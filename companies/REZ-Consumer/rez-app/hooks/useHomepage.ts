...
export function useHomepageNavigation() {
  ...
  const handleItemPress = useCallback((sectionId: string, item) => {
    try {
      ...
      if (sectionId === 'events' || item.type === 'event') {
        try {
          ...
          router.push({ pathname: '/EventPage', params: { id: item.id } } as unknown);
        }
        return;
      }
      ...
      switch (item.type) {
        case 'event':
          ...
          break;
        case 'store':
          router.push('/StorePage' as unknown);
          break;
        case 'product':
          router.push({
            pathname: '/product-page',
  ...
  const handleAddToCart = useCallback(async (item) => {
    try {
      ...
      if (item.price) {
        ...
        originalPrice = item.pricing.original || item.pricing.mrp || item.pricing.selling || 0;
      }
