...
export const useSearchPage = () => {
  ...
  const [matchingStores, setMatchingStores] = useState<unknown[]>([]);
  ...
  const mapToSearchCategory = useCallback((cat): SearchCategory => {
  ...
  const mapProductToSearchResult = useCallback((product): SearchResult => ({
    id: product._id,
    title: product.name,
  ...
  const mapStoreToSearchResult = useCallback((store): SearchResult => ({
    id: store._id,
    title: store.name,
  ...
  const loadCategories = useCallback(async () => {
    try {
      ...
      const response: unknown = await apiClient.get('/categories', {
        featured: true
      });
  ...
  const loadSuggestions = useCallback(async (query: string) => {
    ...
    try {
      ...
      const response: unknown = await searchService.getSearchSuggestions(query);

      if (response.success && response.data) {
        // Map API suggestions to UI suggestions with ids
        const suggestions: SearchSuggestion[] = response.data.map((suggestion, index: number) => ({
  ...
  const performGroupedSearch = useCallback(async (query: string, userLocation?: { latitude: number; longitude: number }, filters?: FilterState) => {
    ...
    try {
      ...
      const response: unknown = await searchService.searchProductsGrouped(params);
  ...
  const loadDiscoveryData = useCallback(async () => {
    try {
      ...
      if (popularProductsRes.success && popularProductsRes.data) {
        const products = Array.isArray(popularProductsRes.data) ? popularProductsRes.data : (popularProductsRes.data as unknown).products || [];
