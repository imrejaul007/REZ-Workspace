...
export const usePriveEligibility = (): UsePriveEligibilityReturn => {
  ...
  const fetchEligibilityFromBackend = async (): Promise<PriveEligibility | null> => {
    try {
      const response: unknown = await priveApi.getEligibility();

      if (response.success && response.data) {
        const backendData = response.data;

        // Transform backend pillars to frontend format (map backend IDs to frontend keys)
        const pillars: PillarScore[] = backendData.pillars.map((p) => {
  ...
  return {
    ...
    isPrive: (eligibility as unknown).hasAccess === true || eligibility.tier !== 'none',
    tier: (eligibility as unknown).effectiveTier || eligibility.tier,
    refresh,
    markGlowSeen,
    // Invite-based access fields
    hasAccess: (eligibility as unknown).hasAccess === true,
    accessSource: (eligibility as unknown).accessSource || 'none',
    isWhitelisted: (eligibility as unknown).isWhitelisted === true,
  };
};
