  ...
  const initializeVoucherData = useCallback(async () => {
    ...
    try {
      ...
      if (!brandsRes.success || !brandsRes.data) {
        ...
        const brands: Brand[] = brandsRes.data.map((brand) => ({
          id: brand._id,
          name: brand.name,
          logo: brand.logo,
  ...
  const searchBrands = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (searchRes.success && searchRes.data) {
        // Transform backend data to match frontend types
        const brands: Brand[] = searchRes.data.map((brand) => ({
          id: brand._id,
          name: brand.name,
          logo: brand.logo,
          backgroundColor: brand.backgroundColor || '#F3F4F6',
      ...
      if (Platform.OS === 'web') {
        ...
        } else if ((error as unknown)?.response?.status >= 500) {
        ...
        try {
          ...
          if ((error as unknown)?.response?.status >= 500) {
            errorMsg = 'Server error. Please try again later.';
          }
        }
      }
  ...
  const updateFilters = useCallback(async (newFilters: Partial<FilterOptions>) => {
    ...
    try {
      ...
      if (!brandsRes.success || !brandsRes.data) {
        ...
        let brands: Brand[] = brandsRes.data.map((brand) => ({
          id: brand._id,
          name: brand.name,
          logo: brand.logo,
          backgroundColor: brand.backgroundColor || '#F3F4F6',
