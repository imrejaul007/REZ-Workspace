...
export function useNavigation(): UseNavigationReturn {
  ...
  const navigate = useCallback((path: string, params?: NavigationParams) => {
    try {
      if (params) {
        router.navigate({ pathname: path as unknown, params });
      } else {
        router.navigate(path as unknown);
      }
    } catch (_error) {
      // silently handle
    }
  ...
  const push = useCallback((path: string, params?: NavigationParams) => {
    try {
      if (params) {
        router.push({ pathname: path as unknown, params });
      } else {
  ...
  const replace = useCallback((path: string, params?: NavigationParams) => {
    try {
      if (params) {
        router.replace({ pathname: path as unknown, params });
      } else {
        router.replace(path as unknown);
      }
    } catch (_error) {
      // silently handle
    }
  ...
  const refreshCurrentRoute = useCallback(() => {
    ...
    router.replace(pathname as unknown);
  }, [router, pathname]);
