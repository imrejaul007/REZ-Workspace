...
export const useRecentlyViewed = (): UseRecentlyViewedResult => {
  ...
  const addStore = useCallback(async (store: RecentlyViewedStore) => {
    try {
      ...
      errorLogger.logStorageError('useRecentlyViewed:addStore', err, 'async', 'recently_viewed');
    }
  }, []);
  ...
  const addProduct = useCallback(async (product: RecentlyViewedProduct) => {
    try {
      ...
      errorLogger.logStorageError('useRecentlyViewed:addProduct', err, 'async', 'recently_viewed');
    }
  }, []);
  ...
  const clearAll = useCallback(async () => {
    try {
      ...
      errorLogger.logStorageError('useRecentlyViewed:clearAll', err, 'async', 'recently_viewed');
    }
  }, []);
