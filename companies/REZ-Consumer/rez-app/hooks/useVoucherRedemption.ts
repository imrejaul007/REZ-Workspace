...
export function useVoucherRedemption() {
  ...
  const validateVoucher = useCallback(
    async (
      ...
      try {
        ...
        const response: unknown = await apiClient.post('/vouchers/validate', {
          voucherId,
          orderAmount,
        });
        ...
        return (response.data as unknown).validation;
      } catch (err) {
        const errorMessage =
  ...
  const redeemVoucher = useCallback(
    async (
      ...
      try {
        ...
        const response: unknown = await apiClient.post('/vouchers/redeem', request as unknown);

        if (!response.success || !response.data) {
          throw new Error('Redemption failed');
        }

        return (response.data as unknown).redemption;
      } catch (err) {
        const errorMessage =
  ...
  const getRedemptionHistory = useCallback(
    async (filters?: {
      ...
      try {
        ...
        const response: unknown = await apiClient.get('/vouchers/redemptions', filters);
  ...
  const getSavingsStats = useCallback(async () => {
    try {
      ...
      const response: unknown = await apiClient.get('/vouchers/savings-stats');

        if (!response.success || !response.data) {
          throw new Error('Failed to load stats');
        }

        return (response.data as unknown).stats;
    } catch (err) {
      const errorMessage =
