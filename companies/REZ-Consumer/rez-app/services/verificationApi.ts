...
class VerificationService {
  ...
  async getStatus(): Promise<ApiResponse<AllVerifications>> {
    return apiClient.get<unknown>('/user/verifications');
  }
  ...
  async getZoneStatus(zone: string): Promise<ApiResponse<VerificationStatus>> {
    return apiClient.get<unknown>(`/user/verifications/${zone}`);
  }
  ...
  async getMethods(zone: string): Promise<ApiResponse<VerificationMethods>> {
    return apiClient.get<unknown>(`/user/verifications/methods/${zone}`);
  }
  ...
  async submitVerification(
    ...
    if (data.documentImage) {
      ...
      return apiClient.post<unknown>(`/user/verifications/${zone}`, formData as unknown);
    }

    // No document, send as JSON
    return apiClient.post<unknown>(`/user/verifications/${zone}`, data as unknown);
  }
  ...
  async uploadDocument(
    ...
    return apiClient.post<unknown>(`/user/verifications/${zone}/upload`, formData as unknown);
  }
}
