...
export type ConnectionEventCallback = (data?) => void;
...
const getSocketUrl = (): string => {
  ...
  // In development on the Android emulator, replace localhost with the standard
  // emulator host alias (10.0.2.2) so the socket can reach the dev server.
  // Gated with __DEV__ so production and physical-device builds are unaffected.
...
class RealTimeService {
  ...
  private ioFactory: ((url: string, opts) => SocketIOClient) | null = null;
  ...
  subscribeToOfferUpdates(callback: (offer) => void): string {
    return this.subscribe(
      ...
      ].includes(message.type as unknown)
    );
  }

  // Subscribe to order status updates
  subscribeToOrderUpdates(callback: (order) => void): string {
    return this.subscribe(
      'orders',
      callback,
      (message) => [
  ...
  subscribeToCartSync(callback: (cart) => void): string {
    return this.subscribe(
      'cart',
      callback,
      (message) => [
  ...
  subscribeToSupportChat(ticketId: string, callback: (event) => void): string {
    return this.subscribe(
      ...
      (message) => {
        const supportTypes = Object.keys(MESSAGE_TYPES)
          ...
          .map(key => (MESSAGE_TYPES as unknown)[key]);

        return supportTypes.includes(message.type as unknown);
      }
    );
  }

  // Subscribe to agent typing indicators
  subscribeToAgentTyping(ticketId: string, callback: (typing) => void): string {
    return this.subscribe(
      `support_ticket_${ticketId}`,
      callback,
      (message) => [
  ...
  subscribeToQueueUpdates(callback: (queue) => void): string {
    return this.subscribe(
      'support_queue',
      callback,
      (message) => [
  ...
  subscribeToAgentStatus(callback: (status) => void): string {
    return this.subscribe(
      'support_agents',
      callback,
      (message) => [
  ...
  sendSupportChatEvent(ticketId: string, eventType: string, data): boolean {
    return this.send({
      type: eventType,
      data: {
        ticketId,
  ...
  private setupSocketListeners(): void {
    ...
    this.socket.onAny((eventType: string, data) => {
  ...
  private emit(event: ConnectionEventType, data?): void {
    const listeners = this.eventListeners.get(event);
    if (listeners) {
...
function getRealTimeService(): RealTimeService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[REALTIME_SERVICE_KEY]) {
      (globalThis as unknown)[REALTIME_SERVICE_KEY] = new RealTimeService();
    }
    return (globalThis as unknown)[REALTIME_SERVICE_KEY];
  }
...
export const RealTimeUtils = {
  ...
  getMessagePriority: (type: string): 'high' | 'medium' | 'low' => {
    ...
    if (highPriority.includes(type as unknown)) return 'high';
    if (mediumPriority.includes(type as unknown)) return 'medium';
    return 'low';
  },
};
