...
// REZ Intelligence: Taste Profile for personalization
import { tasteProfileService, TasteProfile, TasteRecommendation } from './tasteProfileService';
...
class HomepageDataService {
  ...
  ): Promise<{ store; products: ProductItem[] } | null> {
    try {
      const isBackendAvailable = await this.checkBackendAvailability();
  ...
  private transformBatchResponseToSections(response: HomepageBatchResponse): {
    ...
    return {
      justForYou: {
        ...
        items: (sections.justForYou || []).map((item) => ({
          ...item,
  ...
  // ==========================================================================
  // REZ INTELLIGENCE: Taste Profile Personalization
  // ==========================================================================

  /**
   * Get personalized recommendations using REZ-taste-profile
   * Enriches the "Just for You" section with AI-powered personalization
   */
  async getPersonalizedRecommendations(
    ...
    try {
      // Get taste context for personalization
      const contextResponse = await tasteProfileService.getTasteContext({
        userId,
        currentContext: {
          location,
          time: new Date(),
      ...
      if (contextResponse.success && contextResponse.data) {
        return {
          tasteProfile: contextResponse.data.userProfile,
          recommendations: contextResponse.data.recommendations,
        };
      }

      return { tasteProfile: null, recommendations: [] };
    } catch (error) {
      logger.debug('[HomepageDataService] Taste profile fetch failed:', error);
      return { tasteProfile: null, recommendations: [] };
    }
  }

  /**
   * Update taste profile after user action
   * Call this after purchases, views, searches to improve recommendations
   */
  async updateTasteProfile(
    userId: string,
    action: 'purchase' | 'view' | 'search' | 'wishlist' | 'share',
    data: {
    ...
    try {
      ...
      if (action === 'purchase' && data.value) {
        await tasteProfileService.updateTasteWithOrder(userId, {
        ...
        await tasteProfileService.captureTasteSignal(userId, {
          type: action === 'purchase' ? 'view' : action,
          entityType: data.entityType,
          entityId: data.entityId,
        });
      }
    } catch (error) {
      // Silently fail - taste profile is non-critical
      logger.debug('[HomepageDataService] Taste profile update failed:', error);
    }
  }

  /**
   * Get behavioral insights for the user
   * Use for targeting deals and promotions
   */
  async getUserBehavioralInsights(userId: string): Promise<{
    ...
    try {
      const profileResponse = await tasteProfileService.getTasteProfile(userId);
      if (profileResponse.success && profileResponse.data) {
        return {
          isDiscountSeeker: tasteProfileService.isDiscountSeeker(profileResponse.data),
          isQualitySeeker: tasteProfileService.isQualitySeeker(profileResponse.data),
          engagementTier: tasteProfileService.getEngagementTier(profileResponse.data),
          topCategories: profileResponse.data.topCategories
            .slice(0, 5)
            .map(c => c.category),
        };
      }
    } catch (error) {
      logger.debug('[HomepageDataService] Behavioral insights failed:', error);
    }

    return {
      isDiscountSeeker: false,
      isQualitySeeker: false,
      engagementTier: 'bronze',
      topCategories: [],
    };
  }
}
