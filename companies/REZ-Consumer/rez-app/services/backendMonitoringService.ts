...
class BackendMonitoringService {
  ...
  setEnabled(enabled: boolean): void {
    ...
    if (enabled) {
      this.init().then(() => this.startHealthChecks()).catch((err) => {
        if (__DEV__) logger.warn('[BackendMonitoring] Initialization failed:', err?.message);
      });
    } else {
  ...
  /**
   * Calculate uptime percentage from API metrics
   */
  private calculateUptime(): number {
    // Calculate uptime from API metrics (last 24 hours)
    const oneDayAgo = Date.now() - 24 * 60 * 60 * 1000;
    const recentChecks = this.apiMetrics.filter(m => m.timestamp.getTime() >= oneDayAgo);

    if (recentChecks.length === 0) {
      return 100; // No checks recorded = assume healthy
    }

    const successfulChecks = recentChecks.filter(m => m.success).length;
    const uptimePercent = (successfulChecks / recentChecks.length) * 100;
  ...
  private async loadMetrics(): Promise<void> {
    try {
      ...
      if (stored) {
        ...
        this.apiMetrics = (data.apiMetrics || []).map((m) => ({
          ...m,
          timestamp: new Date(m.timestamp),
        }));

        this.dbMetrics = (data.dbMetrics || []).map((m) => ({
          ...m,
          timestamp: new Date(m.timestamp),
        }));

        this.cacheMetrics = (data.cacheMetrics || []).map((m) => ({
          ...m,
          timestamp: new Date(m.timestamp),
        }));
