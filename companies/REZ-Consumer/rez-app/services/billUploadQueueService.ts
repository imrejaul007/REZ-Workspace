  ...
  emit(event: string, ...args: unknown[]): boolean {
    ...
    listeners.forEach(listener => {
      try {
        ...
        logger.error('EventEmitter error', { event, error });
      }
    });
...
function getBillUploadQueueService(): BillUploadQueueService {
  ...
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[BILL_UPLOAD_QUEUE_SERVICE_KEY]) {
      const instance = new BillUploadQueueService();
      (globalThis as unknown)[BILL_UPLOAD_QUEUE_SERVICE_KEY] = instance;
    }
    return (globalThis as unknown)[BILL_UPLOAD_QUEUE_SERVICE_KEY];
  }
