...
class SpecialProgramApi {
   ...
   */
  async activate(slug: SpecialProgramSlug): Promise<{ success: boolean; data?; message?: string }> {
    try {
      const response = await apiClient.post<unknown>(`/special-programs/${slug}/activate`);
      return response as unknown;
    } catch (error) {
   ...
   */
  async getMyMemberships(): Promise<{ success: boolean; data?: unknown[]; message?: string }> {
    try {
      const response = await apiClient.get<unknown>('/special-programs/my-memberships');
      return response as unknown;
    } catch (error) {
