...
export interface CourierPreferences {
  preferredCourier: 'unknown' | 'delhivery' | 'bluedart' | 'ekart' | 'dtdc' | 'fedex';
...
class UserSettingsApiService {
  ...
  async updateGeneralSettings(data: Partial<GeneralSettings>): Promise<ApiResponse<UserSettings>> {
    ...
    return apiClient.put<unknown>(`${this.baseUrl}/general`, data as unknown);
  }

  // Update notification preferences
  async updateNotificationPreferences(data: Partial<NotificationPreferences>): Promise<ApiResponse<UserSettings>> {
    this.invalidateCache();
    return apiClient.put<unknown>(`${this.baseUrl}/notifications`, data as unknown);
  }

  // Update privacy settings
  async updatePrivacySettings(data: Partial<PrivacySettings>): Promise<ApiResponse<UserSettings>> {
    this.invalidateCache();
    return apiClient.put<unknown>(`${this.baseUrl}/privacy`, data as unknown);
  }

  // Update security settings
  async updateSecuritySettings(data: Partial<SecuritySettings>): Promise<ApiResponse<UserSettings>> {
    this.invalidateCache();
    return apiClient.put<unknown>(`${this.baseUrl}/security`, data as unknown);
  }

  // Security-specific methods
  async getSecurityStatus(): Promise<ApiResponse<unknown>> {
    return apiClient.get<unknown>(`${this.baseUrl}/security/status`);
  }

  async enableTwoFactorAuth(method: '2FA_SMS' | '2FA_EMAIL' | '2FA_APP'): Promise<ApiResponse<unknown>> {
    return apiClient.post<unknown>(`${this.baseUrl}/security/2fa/enable`, { method });
  }

  async disableTwoFactorAuth(): Promise<ApiResponse<unknown>> {
    return apiClient.post<unknown>(`${this.baseUrl}/security/2fa/disable`);
  }

  async verifyTwoFactorCode(code: string, method: string): Promise<ApiResponse<unknown>> {
    return apiClient.post<unknown>(`${this.baseUrl}/security/2fa/verify`, { code, method });
  }

  async generateBackupCodes(): Promise<ApiResponse<{ backupCodes: string[] }>> {
    return apiClient.post<unknown>(`${this.baseUrl}/security/2fa/backup-codes`);
  }
