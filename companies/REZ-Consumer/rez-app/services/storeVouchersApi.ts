...
class StoreVouchersApi {
  ...
  async getStoreVouchers(
    ...
    try {
      const response = await apiClient.get<unknown>(`/store-vouchers/store/${storeId}`, params);
  ...
  async getMyVouchers(params?: {
    ...
    try {
      const response = await apiClient.get<unknown>('/store-vouchers/my-vouchers', params);
      return response as unknown;
    } catch (error) {
      throw error;
    }
