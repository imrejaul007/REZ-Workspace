...
class ErrorTrackingService {
  ...
  private async loadErrors(): Promise<void> {
    try {
      ...
      if (stored) {
        this.errors = JSON.parse(stored).map((e) => ({
          ...e,
...
function getErrorTrackingService(): ErrorTrackingService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[ERROR_TRACKING_SERVICE_KEY]) {
      (globalThis as unknown)[ERROR_TRACKING_SERVICE_KEY] = new ErrorTrackingService();
    }
    return (globalThis as unknown)[ERROR_TRACKING_SERVICE_KEY];
  }
