...
class PerformanceMetricsService {
   ...
   */
  trackMetric(name: string, value: number, unit: MetricUnit, context?: Record<string, unknown>): void {
    if (!this.isEnabled) return;
  ...
  private async loadMetrics(): Promise<void> {
    try {
      ...
      if (stored) {
        ...
        this.renderMetrics = (data.renderMetrics || []).map((m) => ({
          ...m,
          timestamp: new Date(m.timestamp),
        }));

        this.pageLoadMetrics = (data.pageLoadMetrics || []).map((m) => ({
          ...m,
          timestamp: new Date(m.timestamp),
        }));
        ...
        this.memoryMetrics = (data.memoryMetrics || []).map((m) => ({
          ...m,
          timestamp: new Date(m.timestamp),
        }));
