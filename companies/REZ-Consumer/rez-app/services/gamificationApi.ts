...
class GamificationApiService {
  ...
  async getCheckinConfig(): Promise<ApiResponse<CheckInConfig>> {
    ...
    try {
      const response = await apiClient.get<unknown>('/gamification/checkin-config');
  ...
  async performCheckIn(): Promise<ApiResponse<CheckInResult>> {
    try {
      const response = await apiClient.post<unknown>('/gamification/streak/checkin');
  ...
  async getSpinWheelData(): Promise<ApiResponse<SpinWheelData>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/spin-wheel/data');
  ...
  async getSpinEligibility(): Promise<ApiResponse<SpinEligibility>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/spin-wheel/eligibility');
  ...
  async executeSpin(): Promise<ApiResponse<SpinResult>> {
    try {
      const response = await apiClient.post<unknown>('/gamification/spin-wheel/spin');
  ...
  async getSpinHistory(params?: { limit?: number; page?: number }): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.get<unknown>('/gamification/spin-wheel/history', params);

      if (response.success && response.data) {
        const raw = response.data;
        const history = (raw.history || []).map((spin) => ({
          id: spin.id || spin._id,
  ...
  async getLeaderboard(
    ...
    try {
      const response = await apiClient.get<unknown>('/gamification/leaderboard', {
        type: 'spending',
        period,
        limit,
      });
  ...
  async getGamificationStats(): Promise<ApiResponse<GamificationStats>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/stats');
  ...
  async getCoinBalance(): Promise<ApiResponse<{ balance: number; lifetimeEarned: number }>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/coins/balance');
  ...
  async getAffiliateStats(): Promise<ApiResponse<AffiliateStats>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/affiliate/stats');
  ...
  async getPromotionalPosters(): Promise<ApiResponse<PromotionalPoster[]>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/promotional-posters');

      if (response.success && response.data) {
        // Backend returns { posters: [...] }
        const postersArray = response.data.posters || response.data || [];
        const posters = postersArray.map((poster) => ({
  ...
  async getShareSubmissions(): Promise<ApiResponse<ShareSubmission[]>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/affiliate/submissions');
  ...
  async submitSharePost(data: {
    ...
    try {
      const response = await apiClient.post<unknown>('/gamification/affiliate/submit', data as unknown);
  ...
  async getStreakBonuses(): Promise<ApiResponse<StreakBonus[]>> {
    try {
      const response = await apiClient.get<unknown>('/gamification/streak/bonuses');

      if (response.success && response.data) {
        // Backend returns { bonuses: [...] }
        const bonusesArray = response.data.bonuses || response.data || [];
        const bonuses = bonusesArray.map((b) => ({
          days: b.days || b.day,
  ...
  async getReviewableItems(): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.get<unknown>('/gamification/reviewable-items');
  ...
  async getBonusOpportunities(): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.get<unknown>('/gamification/bonus-opportunities');
  ...
  async claimSurpriseDrop(dropId: string): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.post<unknown>('/gamification/surprise-drop/claim', { dropId });
  ...
  async startQuiz(
    ...
    try {
      const response = await apiClient.post<unknown>('/gamification/quiz/start', {
        difficulty,
        category,
      });
  ...
  async submitQuizAnswer(
    ...
    try {
      const response = await apiClient.post<unknown>(`/gamification/quiz/${quizId}/answer`, {
        questionId,
