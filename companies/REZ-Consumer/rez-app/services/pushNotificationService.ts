...
type DataRefreshCallback = (notificationData) => void;
...
try {
  Notifications.setNotificationHandler({
    handleNotification: async () => ({
      ...
      shouldShowBanner: true as unknown,
      shouldShowList: true as unknown,
    }),
  });
} catch (_e) {
  // silently handle
...
class PushNotificationService {
  ...
  private notificationListener: unknown = null;
  private responseListener: unknown = null;
  private navigationHandler: ((data) => void) | null = null;

  /**
   * Set navigation handler for deep linking
   */
  setNavigationHandler(handler: (data) => void): void {
  ...
  private async registerTokenWithBackend(token: string, _userId: string): Promise<void> {
    try {
      ...
      const response = await apiClient.post<unknown>('/notifications/register-token', {
        token,
  ...
  async unregisterToken(): Promise<void> {
    try {
      if (this.expoPushToken) {
        await apiClient.post<unknown>('/notifications/unregister-token', {
   ...
   */
  private handleNotificationTap(data): void {
