...
class WalletValidationService {
  ...
  validateTransactionFilters(filters): ValidationResult {
    const errors: string[] = [];
  ...
  // Sanitize topup data
  sanitizeTopupData(data): TopupValidation {
    return {
  ...
  sanitizeWithdrawalData(data): WithdrawalValidation {
    return {
  ...
  sanitizePaymentData(data): PaymentValidation {
    return {
  ...
  private sanitizeNumber(value, defaultValue: number, min?: number, max?: number): number {
  ...
  private sanitizeString(value, defaultValue: string, maxLength?: number): string {
  ...
  private sanitizeEnum<T extends string>(value, validValues: readonly T[], defaultValue: T): T {
    return (validValues as readonly string[]).includes(value) ? (value as T) : defaultValue;
  }

  // Validate API response
  validateApiResponse(response): ValidationResult {
    const errors: string[] = [];
