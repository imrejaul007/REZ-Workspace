...
const externalWalletApi = {
  ...
  async getLinkedWallets(): Promise<ExternalWallet[]> {
    try {
      const response = await apiClient.get<unknown>(`${EXTERNAL_WALLET_BASE}/status`);
  ...
  async linkWallet(
    ...
    try {
      const response = await apiClient.post<unknown>(`${EXTERNAL_WALLET_BASE}/link`, {
        provider,
        ...options,
      });
  ...
  async unlinkWallet(provider: ExternalWalletProvider): Promise<{ success: boolean; message: string }> {
    try {
      const response = await apiClient.delete<unknown>(`${EXTERNAL_WALLET_BASE}/unlink/${provider}`);
  ...
  async initiatePaytmPayment(
    ...
    try {
      const response = await apiClient.post<unknown>(`${EXTERNAL_WALLET_BASE}/paytm/initiate`, {
        amount,
        orderId,
        storeId,
      });
  ...
  async initiateAmazonPayment(
    ...
    try {
      const response = await apiClient.post<unknown>(`${EXTERNAL_WALLET_BASE}/amazonpay/initiate`, {
        amount,
        orderId,
        storeId,
      });
  ...
  async initiateMobikwikPayment(
    ...
    try {
      const response = await apiClient.post<unknown>(`${EXTERNAL_WALLET_BASE}/mobikwik/initiate`, {
        amount,
        orderId,
        storeId,
      });
  ...
  async checkPaymentStatus(
    ...
    try {
      const response = await apiClient.get<unknown>(`${EXTERNAL_WALLET_BASE}/status/${provider}/${orderId}`);
