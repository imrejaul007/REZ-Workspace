...
class PhotoUploadApiService {
   ...
   */
  async upload(data: PhotoUploadRequest): Promise<{ success: boolean; data?; error?: string }> {
    try {
      const response = await apiClient.post<unknown>('/photos/upload', data as unknown);
      if (response.success) {
  ...
  async getMyUploads(
    ...
    limit: number = 20,
    status?: string
  ): Promise<{ success: boolean; data?: { uploads: PhotoUploadItem[]; pagination: unknown }; error?: string }> {
    try {
      const params: Record<string, string> = { page: String(page), limit: String(limit) };
      if (status) params.status = status;

      const query = new URLSearchParams(params).toString();
      const response = await apiClient.get<{ uploads: PhotoUploadItem[]; pagination: unknown }>(
  ...
  async getStorePhotos(
    ...
    page: number = 1,
    limit: number = 20
  ): Promise<{ success: boolean; data?: { photos: PhotoUploadItem[]; pagination: unknown }; error?: string }> {
    try {
      const response = await apiClient.get<{ photos: PhotoUploadItem[]; pagination: unknown }>(
