...
export const lockDealApi = {
  ...
  async getDeals(filters: LockDealFilters = {}): Promise<ApiResponse<PaginatedResponse<LockPriceDeal>>> {
    ...
    return apiClient.get<unknown>(`${BASE_URL}${query ? `?${query}` : ''}`);
  },
  ...
  async getDealById(id: string): Promise<ApiResponse<{ deal: LockPriceDeal; userLock: UserLockDeal | null }>> {
    return apiClient.get<unknown>(`${BASE_URL}/${id}`);
  },
  ...
  async initiateLock(dealId: string): Promise<ApiResponse<InitiateLockResponse>> {
    return apiClient.post<unknown>(`${BASE_URL}/${dealId}/lock`);
  },
  ...
  async confirmLock(
    ...
    return apiClient.post<unknown>(`${BASE_URL}/${dealId}/confirm-lock`, {
      razorpayOrderId,
      razorpayPaymentId,
      razorpaySignature,
    });
  ...
  async initiateBalancePayment(lockId: string): Promise<ApiResponse<BalancePaymentResponse>> {
    return apiClient.post<unknown>(`${BASE_URL}/${lockId}/pay-balance`);
  },
  ...
  async confirmBalancePayment(
    ...
    return apiClient.post<unknown>(`${BASE_URL}/${lockId}/confirm-balance`, {
      razorpayOrderId,
      razorpayPaymentId,
      razorpaySignature,
    });
  ...
  async getMyLocks(status?: UserLockDealStatus, page = 1, limit = 20): Promise<ApiResponse<PaginatedResponse<UserLockDeal>>> {
    ...
    return apiClient.get<unknown>(`${BASE_URL}/my-locks?${params.toString()}`);
  },
  ...
  async getMyLockDetail(lockId: string): Promise<ApiResponse<{ lock: UserLockDeal }>> {
    return apiClient.get<unknown>(`${BASE_URL}/my-locks/${lockId}`);
  },
  ...
  async cancelLock(lockId: string, reason?: string): Promise<ApiResponse<CancelLockResponse>> {
    return apiClient.post<unknown>(`${BASE_URL}/${lockId}/cancel`, { reason });
  },
};
