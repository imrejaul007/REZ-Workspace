...
class GroupBuyingApi {
  ...
  async getAvailableGroups(filters?: GroupBuyingFilters): Promise<ApiResponse<GroupBuyingGroup[]>> {
    try {
      ...
      const params: unknown = { status: 'active,filling' };
      if (filters) {
        if (filters.category) params.category = filters.category;
  ...
  async createGroup(data: CreateGroupRequest): Promise<ApiResponse<GroupBuyingGroup>> {
    try {
      ...
      const response = await apiClient.post<GroupBuyingGroup>(`${BASE_PATH}/groups`, data as unknown);
  ...
  async joinGroup(data: JoinGroupRequest): Promise<ApiResponse<GroupBuyingGroup>> {
    try {
      ...
      const response = await apiClient.post<GroupBuyingGroup>(`${BASE_PATH}/groups/join`, data as unknown);
