...
class CashStoreApiService {
  ...
  async getCategories(): Promise<unknown[]> {
    try {
      const response = await apiClient.get<unknown>(CASH_STORE_ENDPOINTS.CATEGORIES);
  ...
  async getBrands(params: {
    ...
    limit?: number;
    page?: number;
  } = {}): Promise<{ brands: unknown[]; total: number }> {
    try {
  ...
  async getHomepageData(): Promise<{
    categories: unknown[];
    topBrands: unknown[];
    trendingBrands: unknown[];
    highCashbackBrands: unknown[];
  } | null> {
    try {
      const response = await apiClient.get<unknown>(CASH_STORE_ENDPOINTS.HOMEPAGE);
   ...
   */
  async searchBrands(query: string, limit: number = 20): Promise<{ brands: unknown[]; total: number }> {
    try {
  ...
  async getTrending(): Promise<{
    ...
    try {
      const response = await apiClient.get<unknown>(CASH_STORE_ENDPOINTS.TRENDING);
      return {
   ...
   */
  async getDoubleCampaigns(): Promise<unknown[]> {
    try {
      const response = await apiClient.get<unknown>(CASH_STORE_ENDPOINTS.DOUBLE_CAMPAIGNS);
      return (response.data as unknown[]) || [];
    } catch (error) {
      return [];
    }
   ...
   */
  async getCoinDrops(): Promise<unknown[]> {
    try {
      const response = await apiClient.get<unknown>(CASH_STORE_ENDPOINTS.COIN_DROPS);
      return (response.data as unknown[]) || [];
    } catch (error) {
      return [];
    }
