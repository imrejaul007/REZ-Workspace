...
class TravelServicesService {
  ...
  async getPaymentGateways(currency: string = 'INR'): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.get<unknown>(`/travel-payment/gateways?currency=${currency}`);
      return response as unknown;
    } catch (error) {
      return {
        success: false,
  ...
  async createPaymentOrder(
    ...
    try {
      const response = await apiClient.post<unknown>('/travel-payment/create-order', {
        bookingId,
        amount,
        currency,
      });
  ...
  ): Promise<ApiResponse<{ verified: boolean; booking: unknown }>> {
    try {
      const response = await apiClient.post<unknown>('/travel-payment/verify', {
        bookingId,
        ...razorpayData,
      });
      return response as unknown;
