...
class CacheService {
   ...
   */
  private async compress(data): Promise<string> {
    try {
  ...
  private async cleanupExpired(): Promise<number> {
    ...
    this.cacheIndex.forEach((entry, key) => {
      if (this.isExpired(entry as unknown)) {
  ...
  async get<T>(key: string): Promise<T | null> {
    ...
    try {
      ...
      // Check if expired
      if (this.isExpired(indexEntry as unknown)) {
        this.misses++;
        await this.remove(key);
        return null;
      }
   ...
   */
  async setMany(entries: Array<{ key: string; data; options?: CacheOptions }>): Promise<void> {
    await this.ensureInitialized();
   ...
   */
  async migrate(migrationFn: (oldData) => unknown): Promise<void> {
    await this.ensureInitialized();
   ...
   */
  async warmCacheWithKeys(keysAndFetchers: Array<{ key: string; fetchFn: () => Promise<unknown>; options?: CacheOptions }>): Promise<void> {
    await this.ensureInitialized();
...
function getCacheService(): CacheService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[CACHE_SERVICE_KEY]) {
      (globalThis as unknown)[CACHE_SERVICE_KEY] = new CacheService();
    }
    return (globalThis as unknown)[CACHE_SERVICE_KEY];
  }
