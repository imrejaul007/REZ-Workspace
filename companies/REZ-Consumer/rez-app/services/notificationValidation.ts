...
class NotificationValidationService {
  ...
  sanitizeNotificationData(data): NotificationData {
    return {
  ...
  sanitizeNotificationSettings(settings): NotificationSettings {
  ...
  private sanitizeEnum<T extends string>(value, validValues: T[], defaultValue: T): T {
    return (validValues as readonly string[]).includes(value) ? (value as T) : defaultValue;
  }

  private sanitizeArray<T extends string>(value, validValues: readonly T[], defaultValue: T[]): T[] {
  ...
  private sanitizeObject(value, defaultValue): unknown {
