...
class StockNotificationsApi {
  ...
  async subscribe(data: StockNotificationRequest) {
    try {
      ...
      const response = await apiClient.post<unknown>(this.baseUrl, data as unknown);
  ...
  async getMyNotifications(params?: {
    ...
    try {
      ...
      const response = await apiClient.get<unknown>(`${this.baseUrl}/my-notifications`, params as unknown);
  ...
  async checkNotification(productId: string, variantId?: string) {
    try {
      const response = await apiClient.get<unknown>(`${this.baseUrl}/check/${productId}`, variantId ? { variantId } : undefined);
  ...
  async cancelNotification(notificationId: string) {
    try {
      ...
      const response = await apiClient.delete<unknown>(`${this.baseUrl}/${notificationId}`);
  ...
  async getMySubscriptions(status?: 'pending' | 'sent' | 'cancelled' | 'notified' | 'expired') {
    try {
      ...
      const response = await this.getMyNotifications({ status: mappedStatus as unknown });
    ...
    try {
      ...
      if (notificationsResponse.success && notificationsResponse.data) {
        const notification = notificationsResponse.data.notifications?.find(
          (n) => n.productId === data.productId && n.status === 'pending'
        );
