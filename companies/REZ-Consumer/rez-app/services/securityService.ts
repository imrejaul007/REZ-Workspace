...
export const performSecurityCheck = async (): Promise<SecurityCheckResult> => {
  ...
  try {
    ...
    try {
      const blacklistResponse = await apiClient.post<unknown>(
        SECURITY_CONFIG.CHECK_BLACKLIST_ENDPOINT,
        { deviceId: deviceFingerprint.id }
      );

      isBlacklisted = (blacklistResponse.data as unknown)?.isBlacklisted || false;
    ...
    try {
      const ipInfoResponse = await apiClient.get<unknown>(SECURITY_CONFIG.GET_IP_INFO_ENDPOINT);
    ...
    try {
      const multiAccountResponse = await apiClient.post<unknown>(
...
export const reportSuspiciousActivity = async (
  ...
  try {
    ...
    await apiClient.post<unknown>(SECURITY_CONFIG.REPORT_SUSPICIOUS_ENDPOINT, {
      deviceId: deviceFingerprint.id,
      activityType,
      details,
      timestamp: Date.now(),
...
export const verifyCaptcha = async (token: string): Promise<boolean> => {
  try {
    ...
    const response = await apiClient.post<unknown>(SECURITY_CONFIG.VERIFY_CAPTCHA_ENDPOINT, {
      token,
      deviceId: (await getDeviceFingerprint()).id,
    });

    if (response.success && (response.data as unknown)?.verified) {
...
export const getIPInfo = async (): Promise<IPInfo> => {
  try {
    const response = await apiClient.get<unknown>(SECURITY_CONFIG.GET_IP_INFO_ENDPOINT);
