...
class BillUploadAnalytics {
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      if (typeof window === 'undefined') {
        ...
        logger.debug('[BillUploadAnalytics] Skipping initialization in Node.js environment');
        return;
      }
   ...
   * Track generic event
   */
  public trackEvent(type: AnalyticsEventType, metadata?: Record<string, unknown>): void {
   ...
   */
  public trackUploadComplete(billId: string, metadata?: Record<string, unknown>): void {
  ...
  private async updateFunnelStep(step: keyof ConversionFunnel): Promise<void> {
    try {
      ...
      if (typeof funnel[step] === 'number') {
        (funnel as unknown)[step] = (funnel[step] as number) + 1;
      }
