...
class ReferralService {
  ...
  async getReferralData(): Promise<ApiResponse<ReferralData>> {
    ...
    return apiClient.get<unknown>('/referral/data');
  }
  ...
  async getReferralHistory(page = 1, limit = 20): Promise<ApiResponse<{
    ...
    return apiClient.get<unknown>('/referral/history', { page, limit });
  }
  ...
  async getReferralStatistics(): Promise<ApiResponse<ReferralStatistics>> {
    ...
    return apiClient.get<unknown>('/referral/statistics');
  }
  ...
  async generateReferralLink(): Promise<ApiResponse<{ referralLink: string; referralCode: string }>> {
    ...
    return apiClient.post<unknown>('/referral/generate-link');
  }
  ...
  async shareReferralLink(platform: 'whatsapp' | 'telegram' | 'email' | 'sms'): Promise<ApiResponse<{ success: boolean }>> {
    ...
    return apiClient.post<unknown>('/referral/share', { platform });
  }
  ...
  async claimReferralRewards(): Promise<ApiResponse<{ 
    ...
    return apiClient.post<unknown>('/referral/claim-rewards');
  }
  ...
  async getReferralLeaderboard(period: 'week' | 'month' | 'year' = 'month'): Promise<ApiResponse<{
    ...
    return apiClient.get<unknown>('/referral/leaderboard', { period });
  }

  /**
   * Get referral tiers from backend
   */
  async getTiers(): Promise<ApiResponse<Record<string, {
    name: string;
    ...
    return apiClient.get<unknown>('/user/referral/tiers');
  }
  /**
   ...
   * Queues the action if offline, sends immediately if online.
   */
  async claimReferralRewardsOffline(): Promise<unknown> {
    const NetInfo = (await import('@react-native-community/netinfo')).default;
    const netState = await NetInfo.fetch();
...
export const getReferralCode = async () => {
  try {
    ...
    const code = (response.data as unknown)?.referralCode || (response.data as unknown)?.data?.referralCode || '';
    const link = (response.data as unknown)?.referralLink || (response.data as unknown)?.data?.referralLink || '';
    return {
      referralCode: code,
      referralLink: link,
