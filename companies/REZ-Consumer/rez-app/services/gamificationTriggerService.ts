...
class GamificationTriggerService {
   ...
   * Returns unknown rewards earned (coins, achievements, challenges)
   */
  async triggerEvent(
    ...
    try {
      ...
      try {
        ...
        logger.warn('Achievement recalculation failed but coin reward proceeded', achievementError);
      }
   ...
   */
  private calculateCoinReward(eventType: GamificationEventType, data?): number {
    switch (eventType) {
      case 'ORDER_PLACED':
   ...
   */
  async onOrderPlaced(orderId: string, orderValue: number, items: unknown[]): Promise<GamificationReward> {
   ...
   */
  private mapEventTypeToSource(eventType: GamificationEventType): unknown {
   ...
   */
  private getRewardDescription(eventType: GamificationEventType, data?): string {
    switch (eventType) {
      case 'ORDER_PLACED':
  ...
  showRewardNotification(reward: GamificationReward): void {
    ...
    if (reward.achievements.length > 0) {
      ...
      logger.debug('[GamificationTrigger] Achievements unlocked:', {
        count: reward.achievements.length,
        names: reward.achievements.map(a => a.name)
      });
    }

    if (reward.challenges.length > 0) {
      // Trigger challenge completion notification
      const completedChallenges = reward.challenges.map(c => c.name).join(', ');
      logger.debug('[GamificationTrigger] Challenges completed:', {
        count: reward.challenges.length,
        names: completedChallenges
      });

      // Emit event for GamificationContext to handle
      if (typeof window !== 'undefined') {
        window.dispatchEvent(new CustomEvent('gamification:challenge_completed', {
          detail: { challenges: reward.challenges }
        }));
      }
    }
  }
...
function getGamificationTriggerService(): GamificationTriggerService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[GAMIFICATION_TRIGGER_SERVICE_KEY]) {
      (globalThis as unknown)[GAMIFICATION_TRIGGER_SERVICE_KEY] = new GamificationTriggerService();
    }
    return (globalThis as unknown)[GAMIFICATION_TRIGGER_SERVICE_KEY];
  }
