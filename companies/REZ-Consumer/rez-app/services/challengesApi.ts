...
class ChallengesApiService {
  ...
  async getActiveChallenge(): Promise<ApiResponse<Challenge>> {
    try {
      const response = await apiClient.get<unknown>(`${this.baseUrl}/active`);
  ...
  async getMyProgress(includeCompleted: boolean = true): Promise<ApiResponse<ChallengeProgress[]>> {
    try {
      const response = await apiClient.get<unknown>(`${this.baseUrl}/my-progress`, {
      ...
      if (response.success && response.data) {
        ...
        const mappedProgress = progressList.map((item) => ({
          ...item,
  ...
  async getUnifiedChallenges(options?: {
    ...
    try {
      ...
      const response = await apiClient.get<unknown>(`${this.baseUrl}/unified`, params);
  ...
  async joinChallenge(challengeId: string): Promise<ApiResponse<ChallengeProgress>> {
    try {
      const response = await apiClient.post<unknown>(`${this.baseUrl}/${challengeId}/join`);
  ...
  async claimReward(progressId: string): Promise<ApiResponse<ChallengeClaimResult>> {
    try {
      const response = await apiClient.post<unknown>(`${this.baseUrl}/${progressId}/claim`);
   ...
   */
  async getChallengeLeaderboard(challengeId: string, limit: number = 10): Promise<ApiResponse<unknown[]>> {
    try {
      const response = await apiClient.get<unknown>(`${this.baseUrl}/${challengeId}/leaderboard`, { limit });
