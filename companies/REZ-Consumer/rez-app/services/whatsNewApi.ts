...
class WhatsNewApiService {
  ...
  async getUnseenCount(): Promise<ApiResponse<IUnseenCountResponse>> {
    try {
      ...
      return {
        success: response.success,
        data: (response.data || { count: 0, hasUnseen: false }) as unknown,
        message: response.message,
      } as unknown;
    } catch (error) {
      throw error;
  ...
  async trackView(storyId: string): Promise<void> {
    try {
      await apiClient.post<unknown>(`${this.baseUrl}/${storyId}/view`, {});
    } catch (error) {
  ...
  async trackClick(storyId: string): Promise<void> {
    try {
      await apiClient.post<unknown>(`${this.baseUrl}/${storyId}/click`, {});
    } catch (error) {
      // Don't throw - tracking failures shouldn't break the UX
    }
  }
  ...
  async trackCompletion(storyId: string): Promise<void> {
    try {
      await apiClient.post<unknown>(`${this.baseUrl}/${storyId}/complete`, {});
    } catch (error) {
      // Don't throw - tracking failures shouldn't break the UX
    }
  }
