...
class NavigationService implements INavigationService {
  ...
  private listeners: Map<NavigationEvent, Set<(data) => void>> = new Map();
   ...
   */
  private emit(event: NavigationEvent, data): void {
  ...
  async goBack(fallbackRoute?: Href): Promise<NavigationResult> {
    ...
    try {
      ...
      const fallback = (fallbackRoute || getDefaultFallbackRoute()) as unknown;
  ...
  getCurrentRoute(): string {
    ...
    try {
      ...
      const segments = (this.router as unknown)?.state?.routes;
