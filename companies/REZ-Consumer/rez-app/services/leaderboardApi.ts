...
class LeaderboardApi {
  ...
  async getLeaderboard(params: {
    ...
    try {
      // CA-GAM-004 FIX: Validate pagination params before API call
      const page = Math.max(1, Math.floor(params.page || 1));
      const limit = Math.max(1, Math.min(Math.floor(params.limit || 50), 100));
      ...
      if (response.success && response.data) {
        ...
        const mappedEntries = entries.map((entry, index: number) => ({
  ...
  async getMyRank(period: 'daily' | 'weekly' | 'monthly' | 'all-time' = 'weekly'): Promise<ApiResponse<UserRank>> {
    try {
      ...
      const statsResponse = await apiClient.get<unknown>('/gamification/stats');
