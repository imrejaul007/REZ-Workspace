...
function mapCreator(creator): Creator {
  return {
...
function mapPick(pick): CreatorPick {
  return {
    id: pick.id || pick._id,
...
class CreatorsApiService {
  ...
  async getFeaturedCreators(limit: number = 6): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.get<unknown>('/creators/featured', { limit });
  ...
  async getTrendingPicks(limit: number = 10, category?: string): Promise<ApiResponse<{
    ...
    try {
      ...
      const response = await apiClient.get<unknown>('/creators/trending-picks', params);
  ...
  async getApprovedCreators(params: {
    ...
    try {
      const response = await apiClient.get<unknown>('/creators/all', params);
  ...
  async getCreatorById(id: string): Promise<ApiResponse<CreatorProfile>> {
    try {
      const response = await apiClient.get<unknown>(`/creators/${id}`);
  ...
  async getCreatorPicks(creatorId: string, limit: number = 10): Promise<ApiResponse<{
    ...
    try {
      const response = await apiClient.get<unknown>(`/creators/${creatorId}/picks`, { limit });
  ...
  async getCreatorStats(creatorId: string): Promise<ApiResponse<CreatorStats>> {
    try {
      const response = await apiClient.get<unknown>(`/creators/${creatorId}/stats`);
  ...
  async getPickById(pickId: string): Promise<ApiResponse<CreatorPick>> {
    try {
      const response = await apiClient.get<unknown>(`/creators/picks/${pickId}`);
  ...
  async trackPickView(pickId: string): Promise<void> {
    try {
      await apiClient.post<unknown>(`/creators/picks/${pickId}/view`, {});
    } catch (error) {
  ...
  async trackPickClick(pickId: string): Promise<void> {
    try {
      await apiClient.post<unknown>(`/creators/picks/${pickId}/click`, {});
    } catch (error) {
      // Silent fail for tracking
    }
  }
  ...
  async togglePickLike(pickId: string): Promise<ApiResponse<{ isLiked: boolean }>> {
    try {
      const response = await apiClient.post<unknown>(`/creators/picks/${pickId}/like`, {});
      if (response.success && response.data) {
  ...
  async togglePickBookmark(pickId: string): Promise<ApiResponse<{ isBookmarked: boolean }>> {
    try {
      const response = await apiClient.post<unknown>(`/creators/picks/${pickId}/bookmark`, {});
      if (response.success && response.data) {
  ...
  async checkEligibility(): Promise<ApiResponse<EligibilityResult>> {
    try {
      const response = await apiClient.get<unknown>('/creators/eligibility');
      if (response.success && response.data) {
  ...
  async applyAsCreator(data: {
    ...
    try {
      const response = await apiClient.post<unknown>('/creators/apply', data as unknown);
      if (response.success) {
        return { success: true, data: response.data };
      }
  ...
  async getMyCreatorProfile(): Promise<ApiResponse<unknown>> {
    try {
      const response = await apiClient.get<unknown>('/creators/my-profile');
      if (response.success) {
        return { success: true, data: response.data };
      }
  ...
  async updateMyProfile(data: {
    ...
    try {
      const response = await apiClient.put<unknown>('/creators/my-profile', data as unknown);
      if (response.success) {
        return { success: true, data: response.data };
      }
  ...
  async submitPick(data: {
    ...
    try {
      const response = await apiClient.post<unknown>('/creators/my-picks', data as unknown);
      if (response.success) {
        return { success: true, data: response.data };
      }
  ...
  async getMyPicks(params: {
    ...
    try {
      const response = await apiClient.get<unknown>('/creators/my-picks', params);
      if (response.success && response.data) {
        return { success: true, data: response.data };
      }
      return { success: true, data: { picks: [], total: 0 } };
  ...
  async deleteMyPick(pickId: string): Promise<ApiResponse<{ deleted: boolean; archived: boolean }>> {
    try {
      const response = await apiClient.delete<unknown>(`/creators/my-picks/${pickId}`);
      if (response.success && response.data) {
        return { success: true, data: response.data };
      }
  ...
  async updateMyPick(pickId: string, data: {
    ...
    try {
      const response = await apiClient.patch<unknown>(`/creators/my-picks/${pickId}`, data as unknown);
      if (response.success) {
        return { success: true, data: response.data };
      }
  ...
  async getMyEarnings(): Promise<ApiResponse<CreatorEarnings>> {
    try {
      const response = await apiClient.get<unknown>('/creators/my-earnings');
      if (response.success && response.data) {
        return { success: true, data: response.data };
      }
