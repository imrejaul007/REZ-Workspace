...
class ExploreApiService {
  ...
  }): Promise<ApiResponse<{ stores: ExploreStore[]; pagination: unknown }>> {
    try {
      const response = await apiClient.get<unknown>('/stores', params);
  ...
  async searchStores(query: string, params?: {
    ...
    try {
      const response = await apiClient.get<unknown>('/stores/search', { q: query, ...params });
  ...
  async getHotDeals(params?: {
    limit?: number;
    page?: number;
  }): Promise<ApiResponse<{ products: HotProduct[]; pagination: unknown }>> {
    try {
      const response = await apiClient.get<unknown>('/products/hot-deals', params);
  ...
  async getTrendingProducts(params?: {
    ...
    try {
      const response = await apiClient.get<unknown>('/products/trending', params);
  ...
  async getCategories(params?: {
    ...
    try {
      const response = await apiClient.get<unknown>('/categories', params);

      if (response.success && response.data) {
        const categories = (response.data.categories || response.data || []).map((cat) => ({
          id: cat._id || cat.id,
          name: cat.name,
          slug: cat.slug,
          icon: cat.icon,
  ...
  async getCategoryBySlug(slug: string): Promise<ApiResponse<Category & { stores?: ExploreStore[] }>> {
    try {
      const response = await apiClient.get<unknown>(`/categories/${slug}`);
  ...
  async getStoresByCategory(slug: string, params?: {
    ...
    lat?: number;
    lng?: number;
  }): Promise<ApiResponse<{ stores: ExploreStore[]; pagination; category?: unknown }>> {
    try {
      const response = await apiClient.get<unknown>(`/stores/by-category-slug/${slug}`, params);
  ...
  async getStoresByTag(tag: string, params?: {
    ...
    limit?: number;
    sortBy?: string;
  }): Promise<ApiResponse<{ stores: ExploreStore[]; tag: string; pagination: unknown }>> {
    try {
      const response = await apiClient.get<unknown>(`/stores/by-tag/${tag}`, params);
  ...
  async getNearbyStores(params: {
    ...
    try {
      ...
      const response = await apiClient.get<unknown>('/stores/nearby', queryParams);
  ...
  async getTrendingStores(params?: {
    ...
    try {
      ...
      const response = await apiClient.get<unknown>('/stores/trending', allowedParams);
  ...
  async getSmartPicks(params?: {
    ...
    try {
      const response = await apiClient.get<unknown>('/recommendations/personalized', params);
  ...
  async getLiveStats(): Promise<ApiResponse<ExploreStats>> {
    try {
      const response = await apiClient.get<unknown>('/explore/live-stats');
  ...
  async getVerifiedReviews(params?: { limit?: number; page?: number }): Promise<ApiResponse<{ reviews: VerifiedReview[]; total?: number; hasMore?: boolean }>> {
    try {
      const response = await apiClient.get<unknown>('/explore/verified-reviews', params);
  ...
  async getFeaturedComparison(): Promise<ApiResponse<{ comparison: FeaturedComparison | null }>> {
    try {
      const response = await apiClient.get<unknown>('/explore/featured-comparison');
  ...
  async getCommunityActivity(params?: { limit?: number }): Promise<ApiResponse<{ activities: CommunityActivity[] }>> {
    try {
      const response = await apiClient.get<unknown>('/explore/friends-activity', params);
  ...
  async getStatsSummary(): Promise<ApiResponse<ExploreStatsSummary>> {
    try {
      const response = await apiClient.get<unknown>('/explore/stats-summary');
