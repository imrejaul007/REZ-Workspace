...
export const verifyInstagramPost = async (url: string): Promise<PostVerificationResult> => {
  ...
  try {
    ...
    const response: unknown = await apiClient.post<unknown>(INSTAGRAM_CONFIG.VERIFY_POST_ENDPOINT, {
      url,
...
export const verifyInstagramAccount = async (
  ...
  try {
    const response: unknown = await apiClient.post<unknown>(INSTAGRAM_CONFIG.VERIFY_ACCOUNT_ENDPOINT, {
...
export const extractInstagramPostData = async (url: string): Promise<{
  ...
  try {
    ...
    const response: unknown = await apiClient.post<unknown>(INSTAGRAM_CONFIG.EXTRACT_POST_DATA_ENDPOINT, {
      url,
      postId: parsed.postId,
    });
    ...
    return {
      ...
      postId: parsed.postId,
      username: parsed.username,
      thumbnailUrl: (response.data as unknown)?.thumbnailUrl,
    };
  } catch (error) {
    return {
      success: false,
