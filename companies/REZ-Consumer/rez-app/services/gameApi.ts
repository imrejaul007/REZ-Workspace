...
class GameApi {
  ...
  // ======== QUIZ ========
  async createQuiz(questions: unknown[]) {
    return apiClient.post<GameSession>('/games/quiz/create', { questions });
  }

  async submitQuizFull(sessionId: string, answers: unknown[], correctAnswers: unknown[]) {
  ...
  async getDailyTrivia() {
    return apiClient.get<unknown>('/games/daily-trivia');
  }
  ...
  async getDailyLimits() {
    const response = await apiClient.get<unknown>('/games/daily-limits');
    ...
    if (response.data) {
      ...
      for (const [key, value] of Object.entries(response.data)) {
        ...
        if (remaining < 0 || remaining > limit || (used + remaining) > limit) {
          logger.warn('Daily limit mismatch', { key, limit, used, remaining });
        }
  ...
  async getAvailableGames() {
    try {
      const response = await apiClient.get<unknown>('/games/available');
