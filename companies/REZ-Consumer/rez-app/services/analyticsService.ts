...
class AnalyticsService {
  ...
  trackConversionFunnel(step: string, metadata: Record<string, unknown>) {
  ...
  async flush() {
    ...
    try {
      ...
      await apiClient.post<unknown>('/analytics/batch', { events });
    } catch (err) {
      // R2-H3/H4 FIX: Log analytics flush failure for monitoring.
      if (__DEV__) logger.warn('[AnalyticsService] flush failed, re-queuing events:', err);
...
function getAnalyticsService(): AnalyticsService {
  ...
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[ANALYTICS_SERVICE_KEY]) {
      (globalThis as unknown)[ANALYTICS_SERVICE_KEY] = new AnalyticsService();
    }
    return (globalThis as unknown)[ANALYTICS_SERVICE_KEY];
  }
