...
class SupportChatApi {
   ...
   */
  private mapTicketToFrontend(raw): SupportTicket {
    const ticket: unknown = {
      ...raw,
      id: raw._id || raw.id,
      status: raw.status || 'open',
  ...
  async reopenTicket(ticketId: string): Promise<boolean> {
    try {
      ...
      const response = await apiClient.post<unknown>(ENDPOINTS.REOPEN_TICKET(ticketId));
  ...
  async markAsRead(ticketId: string, messageIds: string[]): Promise<boolean> {
    try {
      ...
      const response = await apiClient.post<unknown>(ENDPOINTS.MARK_READ(ticketId), {
        messageIds,
      });
  ...
  async uploadAttachment(
    ...
    try {
      ...
      formData.append('file', file as unknown);
  ...
  async markFAQHelpful(faqId: string, helpful: boolean): Promise<boolean> {
    try {
      ...
      const response = await apiClient.post<unknown>(ENDPOINTS.FAQ_HELPFUL(faqId), {
        helpful,
      });
  ...
  async acceptCall(callId: string): Promise<boolean> {
    try {
      ...
      const response = await apiClient.post<unknown>(ENDPOINTS.ACCEPT_CALL(callId));
  ...
  async rejectCall(callId: string): Promise<boolean> {
    try {
      ...
      const response = await apiClient.post<unknown>(ENDPOINTS.REJECT_CALL(callId));
