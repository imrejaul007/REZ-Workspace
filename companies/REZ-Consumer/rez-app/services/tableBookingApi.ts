...
const tableBookingApi = {
  ...
  createTableBooking: async (data: CreateTableBookingRequest): Promise<ApiResponse<TableBooking>> => {
    try {
      const response = await apiClient.post<unknown>('/table-bookings', data as unknown);
  ...
  getUserTableBookings: async (params?: {
    ...
    try {
      ...
      const response = await apiClient.get<unknown>(`/table-bookings/user${queryString}`);
      return response as unknown;
    } catch (error) {
      throw error;
    }
  ...
  getTableBooking: async (bookingId: string): Promise<ApiResponse<TableBooking>> => {
    try {
      const response = await apiClient.get<unknown>(`/table-bookings/${bookingId}`);
      return response as unknown;
    } catch (error) {
      throw error;
    }
  ...
  cancelTableBooking: async (bookingId: string): Promise<ApiResponse<TableBooking>> => {
    try {
      const response = await apiClient.put<unknown>(`/table-bookings/${bookingId}/cancel`, {});
      return response as unknown;
    } catch (error) {
      throw error;
    }
   ...
   */
  checkAvailability: async (storeId: string, date: string): Promise<ApiResponse<unknown>> => {
    try {
      const response = await apiClient.get<unknown>(`/table-bookings/availability/${storeId}?date=${date}`);
      return response as unknown;
    } catch (error) {
      throw error;
    }
