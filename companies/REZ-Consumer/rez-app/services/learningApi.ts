...
const learningApi = {
  ...
  getContent: async (): Promise<ApiResponse<{ content: LearningContent[] }>> => {
    return apiClient.get<unknown>('/learning');
  },
  ...
  getContentBySlug: async (slug: string): Promise<ApiResponse<{ content: LearningContent }>> => {
    return apiClient.get<unknown>(`/learning/${slug}`);
  },
  ...
  completeContent: async (
    ...
    return apiClient.post<unknown>(`/learning/${contentId}/complete`, { timeSpentSeconds });
  },
};
