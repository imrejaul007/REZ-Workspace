...
class CacheWarmingService {
  ...
  private monitorNetworkConditions(): void {
    ...
    this.netInfoUnsubscribe = NetInfo.addEventListener(state => {
      ...
      const effectiveType = (state as unknown).details?.effectiveConnectionType;
  ...
  async startWarming(options: { force?: boolean } = {}): Promise<void> {
    ...
    try {
      ...
      const effectiveType = (netState as unknown).details?.effectiveConnectionType;
...
function getCacheWarmingService(): CacheWarmingService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[CACHE_WARMING_SERVICE_KEY]) {
      (globalThis as unknown)[CACHE_WARMING_SERVICE_KEY] = new CacheWarmingService();
    }
    return (globalThis as unknown)[CACHE_WARMING_SERVICE_KEY];
  }
