...
class ProductsService {
      ...
      if (response.success && response.data) {
        try {
          ...
          if (validation.valid) {
            return {
              ...response,
              data: unifiedProduct as unknown as Product,
            };
          } else {
            return {
              success: false,
      ...
      if (response.success && response.data && Array.isArray(response.data)) {
        try {
          ...
          if (allValid) {
            return {
              ...response,
              data: unifiedProducts as unknown as Product[],
            };
          }
  ...
  async getStoreProducts(
    ...
    try {
      ...
      const response = await apiClient.get<{ store; products: ProductItem[] }>(`/products/store/${storeId}`, cleanParams);
  ...
  async getProductDetails(productId: string): Promise<(ProductItem & { similarProducts?: ProductItem[] }) | null> {
    try {
      ...
      if ((error as unknown).message?.includes('Invalid MongoDB ObjectId')) {
      } else {
      }
      return null;
    }
