...
class FinancialServicesApi {
  ...
  ): Promise<ApiResponse<{ services: FinancialService[]; pagination; query: string }>> {
    try {
      const queryParams: unknown = { q: params.q };
      ...
      const response = await apiClient.get<{ services: FinancialService[]; pagination; query: string }>(
