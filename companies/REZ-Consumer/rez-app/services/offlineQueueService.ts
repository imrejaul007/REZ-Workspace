...
class OfflineQueueService {
  ...
  private async saveQueue(): Promise<void> {
    try {
      ...
      await asyncStorageService.saveOfflineQueue(encryptedQueue as unknown);
    } catch (_error) {
      // silently handle
    }
  }
    ...
    switch (strategy) {
      ...
      case 'merge':
        ...
        if (Array.isArray(localData) && Array.isArray(serverData)) {
          ...
          const localOnlyItems = localData.filter((item) => !serverIds.has(item.id));
...
function getOfflineQueueService(): OfflineQueueService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[OFFLINE_QUEUE_SERVICE_KEY]) {
      (globalThis as unknown)[OFFLINE_QUEUE_SERVICE_KEY] = new OfflineQueueService();
    }
    return (globalThis as unknown)[OFFLINE_QUEUE_SERVICE_KEY];
  }
