 ...
 */
function mapAchievement(a): Achievement {
...
class AchievementApiService {
   ...
   */
  async getAchievementDefinitions(): Promise<ApiResponse<unknown[]>> {
    try {
  ...
  async getAchievementProgress(): Promise<ApiResponse<AchievementProgress>> {
    try {
      const response = await apiClient.get<unknown>(`${this.baseUrl}/progress`);
  ...
  async initializeUserAchievements(): Promise<ApiResponse<Achievement[]>> {
    try {
      const response = await apiClient.post<unknown>(`${this.baseUrl}/initialize`, {});
      if (response.success && response.data) {
  ...
  async recalculateAchievements(): Promise<ApiResponse<Achievement[]>> {
    ...
    const doCall = async (): Promise<ApiResponse<Achievement[]>> => {
      try {
        const response = await apiClient.post<unknown>('/achievements/recalculate', {});
