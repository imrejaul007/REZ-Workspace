...
class RingSizeApiService {
  ...
  async saveRingSize(
    ...
    try {
      ...
      try {
        const response = await apiClient.post<unknown>(`${this.baseUrl}/ring-size`, {
          ringSize,
          method
        });
...
function getRingSizeApiService(): RingSizeApiService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[RING_SIZE_API_SERVICE_KEY]) {
      (globalThis as unknown)[RING_SIZE_API_SERVICE_KEY] = new RingSizeApiService();
    }
    return (globalThis as unknown)[RING_SIZE_API_SERVICE_KEY];
  }
