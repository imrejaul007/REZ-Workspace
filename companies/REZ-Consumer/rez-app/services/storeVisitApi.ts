...
class StoreVisitService {
  ...
  async scheduleStoreVisit(
    ...
    return apiClient.post<unknown>('/store-visits/schedule', request as unknown);
  }
  ...
  async getQueueNumber(
    ...
    return apiClient.post<unknown>('/store-visits/queue', request as unknown);
  }
  ...
  async getQueueStatus(storeId: string): Promise<ApiResponse<{
    ...
    return apiClient.get<unknown>(`/store-visits/queue-status/${storeId}`);
  }
  ...
  async getUserVisits(): Promise<ApiResponse<Array<{
    ...
    return apiClient.get<unknown>('/store-visits/user');
  }
  ...
  async cancelVisit(visitId: string): Promise<ApiResponse<{ message: string }>> {
    return apiClient.put<unknown>(`/store-visits/${visitId}/cancel`, {});
  }
  ...
  async checkStoreAvailability(storeId: string): Promise<ApiResponse<{
    ...
    return apiClient.get<unknown>(`/store-visits/availability/${storeId}`);
  }
  ...
  async getAvailableSlots(storeId: string, date: string, duration?: number): Promise<ApiResponse<{
    ...
    return apiClient.get<unknown>(`/store-visits/available-slots/${storeId}?${params}`);
  }
   ...
   */
  async rescheduleVisit(visitId: string, visitDate: string, visitTime: string): Promise<ApiResponse<unknown>> {
    return apiClient.put<unknown>(`/store-visits/${visitId}/reschedule`, { visitDate, visitTime });
  }
  ...
  async scheduleStoreVisitOffline(
    ...
    if (!netState.isConnected) {
      ...
      const actionId = await offlineSyncService.enqueue('visit_submission', request as unknown);
