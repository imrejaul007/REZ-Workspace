...
class AsyncStorageService {
   ...
   */
  async multiSave(items: Array<[string, unknown]>): Promise<void> {
    try {
   ...
   * Save cart data
   */
  async saveCart(cart): Promise<void> {
   ...
   * Get cart data
   */
  async getCart(): Promise<unknown | null> {
   ...
   * Save offline queue
   */
  async saveOfflineQueue(queue: unknown[]): Promise<void> {
  ...
  async getOfflineQueue(): Promise<unknown[]> {
    const queue = await this.get<unknown[]>(STORAGE_KEYS.CART_OFFLINE_QUEUE);
    return queue || [];
  }
   ...
   */
  async saveUserPreferences(preferences): Promise<void> {
   ...
   */
  async getUserPreferences(): Promise<unknown | null> {
   ...
   */
  async saveRecentlyViewed(products: unknown[]): Promise<void> {
   ...
   */
  async getRecentlyViewed(): Promise<unknown[]> {
    const items = await this.get<unknown[]>(STORAGE_KEYS.RECENTLY_VIEWED);
    return items || [];
  }
   ...
   */
  async addRecentlyViewed(product): Promise<void> {
   ...
   * Save wishlist
   */
  async saveWishlist(wishlist: unknown[]): Promise<void> {
  ...
  async getWishlist(): Promise<unknown[]> {
    const wishlist = await this.get<unknown[]>(STORAGE_KEYS.WISHLIST);
    return wishlist || [];
  }
...
function getAsyncStorageService(): AsyncStorageService {
  if (typeof globalThis !== 'undefined') {
    if (!(globalThis as unknown)[ASYNC_STORAGE_SERVICE_KEY]) {
      (globalThis as unknown)[ASYNC_STORAGE_SERVICE_KEY] = new AsyncStorageService();
    }
    return (globalThis as unknown)[ASYNC_STORAGE_SERVICE_KEY];
  }
