...
class MallApiService {
  ...
  async getMallHomepage(): Promise<MallHomepageData> {
    try {
      const response = await apiClient.get<MallHomepageData>(
  ...
  async getBrands(filters?: MallBrandFilters): Promise<{
    ...
    try {
      ...
      const response = await apiClient.get<{ brands: MallBrand[]; pagination?: { total?: number; pages?: number } }>(
  ...
  async getBrandById(brandId: string): Promise<MallBrand | null> {
    try {
      const response = await apiClient.get<MallBrand>(
  ...
  async getCategories(): Promise<MallCategory[]> {
    try {
      const response = await apiClient.get<MallCategory[]>(
  ...
  async getCollections(limit: number = 10): Promise<MallCollection[]> {
    try {
      const response = await apiClient.get<MallCollection[]>(
  ...
  async getHeroBanners(limit: number = 5): Promise<MallBanner[]> {
    try {
      const response = await apiClient.get<MallBanner[]>(
  ...
  async trackBrandClick(brandId: string): Promise<void> {
    try {
      await apiClient.post<unknown>(`${MALL_ENDPOINTS.BRANDS}/${brandId}/click`);
    } catch (error) {
  ...
  async trackBrandPurchase(brandId: string, cashbackAmount: number = 0): Promise<void> {
    try {
      await apiClient.post<unknown>(`${MALL_ENDPOINTS.BRANDS}/${brandId}/purchase`, {
        cashbackAmount
      });
    } catch (error) {
  ...
  private async _fetchMallHomepageBatch(): Promise<MallHomepageBatchResponse> {
    try {
      const response = await apiClient.get<unknown>(MALL_ENDPOINTS.HOMEPAGE_BATCH);
      return response.data || {
        featuredStores: [],
        newStores: [],
        topRatedStores: [],
  ...
  async getMallStoresHomepage(): Promise<Pick<MallHomepageBatchResponse, 'featuredStores' | 'newStores' | 'topRatedStores' | 'premiumStores' | 'categories'>> {
    try {
      const response = await apiClient.get<unknown>(MALL_ENDPOINTS.STORES_HOMEPAGE);
      return response.data || {
        featuredStores: [],
        newStores: [],
        topRatedStores: [],
  ...
  async getFeaturedMallStores(limit: number = 10): Promise<MallStoreData[]> {
    try {
      const response = await apiClient.get<MallStoreData[]>(
  ...
  async getMallStoreCategories(): Promise<MallHomepageBatchResponse['categories']> {
    try {
      const response = await apiClient.get<MallHomepageBatchResponse['categories']>(
  ...
  async getMallStoresByCategorySlug(
    ...
    try {
      const response = await apiClient.get<{
        category?: { _id: string; name: string; slug: string; icon?: string; color?: string; description?: string };
        stores?: MallStoreData[];
   ...
   */
  async getTrendingStores(limit: number = 10): Promise<MallStoreData[]> {
    try {
      const response = await apiClient.get<MallStoreData[]>(
   ...
   */
  async getRewardBoosterStores(limit: number = 10): Promise<MallStoreData[]> {
    try {
      const response = await apiClient.get<MallStoreData[]>(
   ...
   */
  async getDealsOfDay(limit: number = 10): Promise<MallOffer[]> {
    try {
