...
function assertSuccess<T>(response, fallbackMsg: string): T {
  if (!response.success) {
...
export async function setStatedIdentity(
  ...
  const response = await apiClient.patch<unknown>('/user/auth/profile', { statedIdentity });
...
export async function fetchIdentityFromProfile(): Promise<IdentityState | null> {
  try {
    const response = await apiClient.get<unknown>('/user/auth/me');
    ...
    logger.error('[IdentityAPI] Failed to fetch identity from profile', error, 'IdentityAPI');
    return null;
  }
}
...
export async function searchInstitutions(
  ...
  try {
    ...
    logger.error('[IdentityAPI] Failed to search institutions', error, 'IdentityAPI');
    return [];
  }
}
...
export async function getStudentOffers(
  ...
  const response = await apiClient.get<{ offers: unknown[] }>(
    `/offers/exclusive-zones/student/offers?limit=${limit}`
  );
  return {
    offers: response.data?.offers || [],
    total: (response as unknown).meta?.pagination?.total || 0,
  };
}
