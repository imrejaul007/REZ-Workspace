...
config.resolver = {
  ...
  blockList: [
    ...
    // Block Next.js restaurant folder (leftover code incompatible with Expo)
    /app\/restaurant\/.*/,
  ],
...
/**
 * Optimized resolver for lazy-loaded routes
 *
 * This configuration helps Metro's bundler create better code splitting
 * by providing hints about which modules can be split into separate chunks.
 *
 * Routes identified for lazy loading (see config/lazyRoutes.ts):
 * - /go/* - REZ Go (barcode scanner, offline storage)
 * - /rendez/* - Rendez dating app
 * - /wallet-screen - Heavy wallet with charts
 * - /booking/* - Booking flows
 * - /EventPage - Event details
 * - /product-page - Product details
 * - And 60+ more routes
 *
 * Estimated lazy-loadable size: ~15MB across 100+ routes
 */

/**
 * Custom module resolver for route-based code splitting
 * This helps Metro understand which modules belong to which chunks
 */
config.resolver.extraNodeModules = {
  ...config.resolver.extraNodeModules,
  // Add aliases for heavy modules if needed
  // '@heavy/charts': require.resolve('@shopify/react-native-skia'),
};

// Enable lazy-requires for better tree shaking
config.transformer.lazyImports = true;

// Custom split points hint for Metro
// These patterns tell Metro to create separate chunks for these imports
...
// Log heavy modules for awareness
if (__DEV__) {
  console.log('[Metro] Lazy loading enabled for patterns:', heavyModulePatterns);
}
...
const uuidFilePath = path.resolve(
  __dirname, 'node_modules/uuid/dist/index.js'
);

const sharedTypesPath = path.resolve(
  __dirname, 'types/enums'
);
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  ...
  // Fix: uuid package has dist/index.js instead of index.js
  if (moduleName === 'uuid') {
    return { filePath: uuidFilePath, type: 'sourceFile' };
  }

  // Fix: @rez/shared-types is a monorepo package - resolve to local types
  if (moduleName === '@rez/shared-types') {
    return { filePath: path.resolve(sharedTypesPath, 'index.ts'), type: 'sourceFile' };
  }
  if (moduleName.startsWith('@rez/shared-types/')) {
    const subPath = moduleName.replace('@rez/shared-types/', '');
    const resolvedPath = path.resolve(sharedTypesPath, subPath + '.ts');
