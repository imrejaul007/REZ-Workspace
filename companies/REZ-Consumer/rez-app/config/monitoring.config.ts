...
export const initializeMonitoring = () => {
  ...
  if (GoogleAnalyticsConfig.enabled) {
    if (__DEV__ && (!process.env.EXPO_PUBLIC_GA_TRACKING_ID || process.env.EXPO_PUBLIC_GA_TRACKING_ID === 'UA-XXXXX-Y')) {
      logger.warn('[Monitoring] GA tracking ID is still a placeholder. Set EXPO_PUBLIC_GA_TRACKING_ID to your real ID before going to production.');
    }
  }
  ...
  if (MixpanelConfig.enabled) {
    if (__DEV__ && (!process.env.EXPO_PUBLIC_MIXPANEL_TOKEN || process.env.EXPO_PUBLIC_MIXPANEL_TOKEN === 'YOUR_MIXPANEL_TOKEN')) {
      logger.warn('[Monitoring] Mixpanel token is still a placeholder. Set EXPO_PUBLIC_MIXPANEL_TOKEN to your real project token before going to production.');
    }
  }
...
export const MonitoringHelpers = {
   ...
   * Track page view
   */
  trackPageView: (screenName: string, params?: Record<string, unknown>) => {
  ...
  startTransaction: (name: string, op: string) => {
    ...
    return (Sentry as unknown).startInactiveSpan?.({ name, op }) ?? null;
  },
