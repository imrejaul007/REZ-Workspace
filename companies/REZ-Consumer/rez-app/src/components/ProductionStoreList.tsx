...
const StoreCard = ({ store, index, onPress }: StoreCardProps) => {
  ...
  return (
    <Pressable
      ...
      accessibilityLabel={`${store.name} store with ${cashback}% cashback`}
  ...
  if (stores.length === 0) {
    return (
      <View style={styles.container}>
        ...
        <View style={styles.emptyContainer} accessibilityLabel="No fashion stores available">
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Pressable
          ...
          accessibilityHint="Shows all available fashion stores"
        >
...
const styles = StyleSheet.create({
  ...
  viewAllButton: {
    ...
    paddingVertical: 10, // Fixed: Was 6, now 10 for min 48px touch target
    minWidth: 48,
    minHeight: 40,
  ...
  emptyText: {
    fontSize: 18,
    fontWeight: '700',
    color: '#6B7280', // Fixed: Was #9CA3AF (WCAG AA contrast on white)
    marginBottom: 8,
    textAlign: 'center',
  },
  emptySubtext: {
    fontSize: 14,
    color: '#6B7280', // Fixed: Was #D1D5DB (WCAG AA contrast on white)
    textAlign: 'center',
  },
});
