...
async function compressGif() {
  try {
    ...
    logger.info(`Input file size: ${(inputStats.size / 1024 / 1024).toFixed(2)} MB`);

    // Get metadata of the input GIF
    const metadata = await sharp(inputPath, { animated: true }).metadata();
    logger.info(`Original dimensions: ${metadata.width}x${metadata.height}`);
    logger.info(`Pages/Frames: ${metadata.pages || 1}`);

    // Target size for icon - displayed at 32x32, use 100x100 for quality
    const targetSize = 100;
    logger.info(`Target size: ${targetSize}x${targetSize} per frame`);
    ...
    logger.info(`Output file size: ${(outputStats.size / 1024).toFixed(2)} KB`);
    logger.info(`Compression ratio: ${((1 - outputStats.size / inputStats.size) * 100).toFixed(2)}%`);
