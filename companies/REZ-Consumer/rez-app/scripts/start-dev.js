...
function runGC() {
  if (global.gc) {
    ...
    if (freed > 0) {
      logger.info(`\x1b[36m[Memory] GC freed ${formatBytes(freed)}\x1b[0m`);
    }
  }
}
...
function startMemoryMonitor() {
  setInterval(() => {
    ...
    logger.info(`\x1b[36m[Memory] Heap: ${formatBytes(usage.heapUsed)} / ${formatBytes(usage.heapTotal)} | RSS: ${formatBytes(usage.rss)} | Uptime: ${uptime}\x1b[0m`);
    ...
    if (usage.heapUsed > MEMORY_WARNING_THRESHOLD) {
      logger.info(`\x1b[33m[Memory] ⚠️  High memory usage! Consider restarting the bundler.\x1b[0m`);
    }
...
logger.info('\x1b[32m[Dev Server] Starting with memory management...\x1b[0m');
logger.info(`\x1b[32m[Dev Server] GC available: ${typeof global.gc === 'function' ? 'Yes ✓' : 'No (run with --expose-gc)'}\x1b[0m`);
...
// Handle exit
expo.on('close', (code) => {
  logger.info(`\x1b[33m[Dev Server] Exited with code ${code}\x1b[0m`);
  process.exit(code);
});
