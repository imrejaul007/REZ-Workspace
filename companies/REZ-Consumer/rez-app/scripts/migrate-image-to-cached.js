...
// Main
logger.info(`🔍 Scanning for .tsx files in ${ROOT}...`);
const allFiles = findTsxFiles(ROOT);
logger.info(`Found ${allFiles.length} .tsx files total`);
...
for (const file of allFiles) {
  ...
  try {
    ...
    if (result.skipped) {
      ...
      if (DRY_RUN) {
        logger.info(`[DRY RUN] Would migrate: ${rel}`);
        logger.info(`  Changes: ${result.changes.join(', ')}`);
      } else {
        fs.writeFileSync(file, result.content, 'utf8');
        logger.info(`✅ Migrated: ${rel}`);
      }
      migrated++;
    ...
    const rel = path.relative(ROOT, file).replace(/\\/g, '/');
    logger.error(`❌ Error processing ${rel}: ${err.message}`);
    errors++;
  }
}

logger.info(`\n${'='.repeat(60)}`);
logger.info(`Migration ${DRY_RUN ? '(DRY RUN) ' : ''}Summary:`);
...
if (migratedFiles.length > 0 && !DRY_RUN) {
  logger.info(`\nMigrated files:`);
  migratedFiles.forEach(f => logger.info(`  ${f}`));
}
