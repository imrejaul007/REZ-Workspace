...
class DatabaseAuditor {
  ...
  async getAllCollections() {
    ...
    logger.info(`✅ Found ${this.collections.length} collections`);
  ...
  async analyzeCollection(collectionName) {
    ...
    try {
      ...
      logger.info(`  📝 Sample size: ${samples.length}`);
  ...
  async checkRelationship(sourceCollection, sourceField, targetCollection, targetField, relationships) {
    ...
    try {
      ...
      logger.info(`  ${sourceCollection}.${sourceField} → ${targetCollection}.${targetField}: ${relationship.validPercent} valid`);
      if (invalid > 0) {
        logger.info(`    ⚠️  ${invalid} orphaned references found`);
      }
  ...
  async checkArrayRelationship(sourceCollection, arrayField, itemField, targetCollection, targetField, relationships) {
    ...
    try {
      ...
      logger.info(`  ${sourceCollection}.${arrayField} → ${targetCollection}.${targetField}: ${relationship.validPercent} valid`);
      if (invalid > 0) {
        logger.info(`    ⚠️  ${invalid} orphaned references found`);
      }

    } catch (error) {
      logger.error(  ❌ Error checking array relationship:`, error.message);
    }
  }
  ...
  generateDatabaseAnalysisReport() {
    ...
    logger.info('  ✅ DATABASE_ANALYSIS_REPORT.md');
  }
  ...
  generateRelationshipAnalysisReport() {
    ...
    logger.info('  ✅ RELATIONSHIP_ANALYSIS_REPORT.md');
  }
  ...
  async run() {
    try {
      ...
      logger.info('  ✅ raw-analysis-data.json');

      logger.info('\n✨ Database audit complete!\n');
      console.log('📁 Reports location:', REPORTS_DIR);
      logger.info('\n📊 Quick Stats:');
      logger.info(`   Collections: ${this.collections.length}`);
      logger.info(`   Relationships: ${this.analysisResults.relationships.length}`);
      logger.info(`   Migrations needed: ${this.analysisResults.migrations.reduce((sum, m) => sum + m.migrations.length, 0)}`);
