...
async function compressImages() {
  ...
  for (const image of SOURCE_IMAGES) {
    try {
      ...
      logger.info(`✓ ${image.output}`);
      logger.info(`  Original: ${(inputStats.size / 1024).toFixed(1)} KB`);
      logger.info(`  Compressed: ${(outputStats.size / 1024).toFixed(1)} KB`);
      logger.info(`  Reduction: ${((1 - outputStats.size / inputStats.size) * 100).toFixed(1)}%\n`);
    } catch (error) {
