...
async function testBackendHealth(): Promise<unknown> {
...
async function testLeaderboardAPI_Limit10(): Promise<unknown> {
...
async function testLeaderboardAPI_Limit20(): Promise<unknown> {
...
async function testLeaderboardAPI_Limit50(): Promise<unknown> {
...
async function testWebSocketConnection(): Promise<unknown> {
...
async function testWebSocketEventListeners(): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      socket.disconnect();
...
async function testConcurrentAPIRequests(): Promise<unknown> {
  const numRequests = 5;
...
async function testConcurrentWebSocketConnections(): Promise<unknown> {
...
async function testInvalidEndpoint(): Promise<unknown> {
...
async function testInvalidLimitParameter(): Promise<unknown> {
...
async function testResponseStructure(): Promise<unknown> {
  const response = await fetch(`${API_BASE_URL}/referral/leaderboard?limit=10`, {
    headers: { 'Content-Type': 'application/json' },
  });
...
async function testWebSocketReconnection(): Promise<unknown> {
  return new Promise((resolve, reject) => {
    const timeout = setTimeout(() => {
      socket.disconnect();
...
function generateReport() {
  ...
  testResults.forEach((result, index) => {
    ...
    logger.info(`${index + 1}. ${result.name}`);
    logger.info(`   Status: ${status}`);
    logger.info(`   Duration: ${result.duration}ms`);

    if (result.error) {
      logger.info(`   ${COLORS.red}Error: ${result.error}${COLORS.reset}`);
    }
