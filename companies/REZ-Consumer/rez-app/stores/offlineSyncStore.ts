...
export interface OfflineSyncStoreState {
  ...
  enqueue: (type: OfflineActionType, payload: Record<string, unknown>) => Promise<string>;
...
export const useOfflineSyncStore = create<OfflineSyncStoreState>((set: StoreSet, get: StoreGet) => ({
  ...
  enqueue: async (type: OfflineActionType, payload: Record<string, unknown>): Promise<string> => {
    try {
      set({ error: null });
