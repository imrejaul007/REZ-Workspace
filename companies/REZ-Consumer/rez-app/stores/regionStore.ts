...
let _setHomepageCurrencyGetter: unknown = null;
...
async function fetchRegionConfigFromServer(regionId: RegionId) {
  ...
  try {
    const response = await apiClient.get<unknown>(`/location/region/${regionId}`);
...
async function fetchAvailableRegions() {
  ...
  try {
    const response = await apiClient.get<unknown>('/location/regions');
...
export const useRegionStore = create<RegionStoreState>((set: StoreSet, get: StoreGet) => ({
  ...
  detectRegion: async (): Promise<RegionId> => {
    try {
      ...
      const response = await apiClient.get<unknown>('/location/region/detect');
      if (response.success && response.data?.region) {
