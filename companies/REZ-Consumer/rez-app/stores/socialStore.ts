...
export const useSocialStore = create<SocialStoreState>((set: StoreSet, get: StoreGet) => ({
  ...
  loadFeed: async (refresh: boolean = false) => {
    ...
    try {
      ...
      errorLogger.logApiError('socialStore:loadFeed', error, '/api/feed', 500);
    } finally {
  ...
  loadMoreActivities: async () => {
    ...
    try {
      ...
      errorLogger.logApiError('socialStore:loadMoreActivities', error, '/api/feed', 500);
    } finally {
      set({ isLoadingFeed: false });
    }
  },
  ...
  loadUserActivities: async (userId: string): Promise<Activity[]> => {
    try {
      ...
      errorLogger.logApiError('socialStore:loadUserActivities', error, '/api/activities', 500);
      return [];
    }
  },
  ...
  loadFollowers: async (userId: string): Promise<UserProfile[]> => {
    try {
      ...
      errorLogger.logApiError('socialStore:loadFollowers', error, '/api/followers', 500);
      return [];
    }
  },
  ...
  loadFollowing: async (userId: string): Promise<UserProfile[]> => {
    try {
      ...
      errorLogger.logApiError('socialStore:loadFollowing', error, '/api/following', 500);
      return [];
    }
  },
  ...
  loadSuggestedUsers: async () => {
    try {
      ...
      errorLogger.logApiError('socialStore:loadSuggestedUsers', error, '/api/suggested-users', 500);
      // silently handle
    }
  },
}));
