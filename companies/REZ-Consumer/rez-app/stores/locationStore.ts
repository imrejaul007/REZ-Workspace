...
export const useLocationStore = create<LocationStoreState>((set: StoreSet, get: StoreGet) => ({
  ...
  updateLocation: async (
    ...
    try {
      ...
      errorLogger.logStorageError('locationStore:updateLocation', error, 'async', 'location_cache');
  ...
  setManualLocation: async (location: UserLocation): Promise<void> => {
    try {
      ...
      errorLogger.logStorageError('locationStore:setManualLocation', error, 'async', 'location_cache');
  ...
  getCurrentLocation: async (): Promise<UserLocation | null> => {
    try {
      ...
      errorLogger.logNetworkError('locationStore:getCurrentLocation', error, '/location', 'GET');
  ...
  getLocationHistory: async (): Promise<LocationHistoryEntry[]> => {
    try {
      ...
      errorLogger.logNetworkError('locationStore:getLocationHistory', error, '/location/history', 'GET');
  ...
  requestLocationPermission: async (): Promise<boolean> => {
    try {
      ...
      if (permission.status === 'granted') {
        try {
          ...
          try {
            ...
            errorLogger.logNetworkError('locationStore:requestPermission:updateUserLocation', serverError, '/location', 'POST');
            await locSvc.cacheLocation(userLocation);
          }

          set(s => ({ state: { ...s.state, currentLocation: userLocation, isLoading: false } }));
        } catch (locationError) {
          errorLogger.logNetworkError('locationStore:requestPermission:getLocation', locationError);
      ...
      errorLogger.logNetworkError('locationStore:requestLocationPermission', error);
  ...
  searchAddresses: async (query: string): Promise<AddressSearchResult[]> => {
    try {
      ...
      errorLogger.logNetworkError('locationStore:searchAddresses', error, '/location/search', 'GET');
  ...
  reverseGeocode: async (coordinates: LocationCoordinates): Promise<LocationAddress> => {
    try {
      ...
      errorLogger.logNetworkError('locationStore:reverseGeocode', error, '/location/reverse', 'GET');
  ...
  validateAddress: async (address: string): Promise<boolean> => {
    try {
      ...
      errorLogger.logValidationError('locationStore:validateAddress', error, 'address');
