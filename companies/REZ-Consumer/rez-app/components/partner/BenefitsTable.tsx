...
function BenefitsTable({
  ...
  const getBenefitValue = (level: PartnerLevel, benefitKey: string): string => {
    const raw = (level as unknown).benefits;
    if (!raw) return '-';
          ...
          <View style={styles.headerRow}>
            ...
            {safeLevels.map((level) => (
              <View key={level.id} style={styles.levelColumn}>
                <LinearGradient
                  colors={getLevelColor(level.level) as unknown}
                  style={[
                    styles.levelHeader,
                ...
                >
                  <View style={styles.levelIconContainer}>
                    <Ionicons 
                      name={getLevelIcon(level.level) as unknown} 
                      size={16} 
                      color="white" 
                    />
                  </View>
            ...
            <View key={benefitType.key} style={[
              ...
              <View style={styles.benefitNameCell}>
                <View style={styles.benefitIconContainer}>
                  <Ionicons name={benefitType.icon as unknown} size={16} color={colors.lightMustard} />
                </View>
        ...
        <View style={styles.requirementsGrid}>
          {safeLevels.map((level) => (
            <View key={level.id} style={[
              ...
              <LinearGradient
                colors={(level.level === currentLevel ? getLevelColor(level.level) : [colors.neutral[50], colors.neutral[100]]) as unknown}
