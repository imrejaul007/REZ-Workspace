...
const SkeletonPulse: React.FC<{ width: number | string; height: number; borderRadius?: number; style?: unknown }> = ({ width, height, borderRadius = 8, style }) => {
...
const MallSectionContainer: React.FC<MallSectionContainerProps> = ({
  ...
  const handleCategoryPress = useCallback(
    (category: MallCategory) => {
      ...
      router.push(`/mall/category/${category.slug}` as unknown);
    },
    [router]
  );
  ...
  const handleCollectionPress = useCallback(
    (collection: MallCollection) => {
      ...
      router.push(`/mall/collection/${collection.slug}` as unknown);
    },
    [router]
  );
  ...
  const handleOfferPress = useCallback(
    (offer: MallOffer) => {
      if (offer.store) {
        router.push(`/MainStorePage?storeId=${offer.store._id}` as unknown);
      } else if (offer.brand) {
        const brandId = offer.brand.id || offer.brand._id;
        router.push(`/mall/brand/${brandId}` as unknown);
      } else {
        // Fallback: navigate to offers list if no direct target
        router.push('/mall/offers' as unknown);
      }
    },
    [router]
  );
  ...
  const handleViewAllFeatured = useCallback(() => {
    router.push('/mall/brands?filter=featured' as unknown);
  }, [router]);

  const handleViewAllCollections = useCallback(() => {
    router.push('/mall/collections' as unknown);
  }, [router]);

  const handleViewAllCategories = useCallback(() => {
    router.push('/mall/categories' as unknown);
  }, [router]);
  ...
  const handleViewAllNewArrivals = useCallback(() => {
    router.push('/mall/brands?filter=new' as unknown);
  }, [router]);

  const handleViewAllTopRated = useCallback(() => {
    router.push('/mall/brands?filter=top-rated' as unknown);
  }, [router]);

  const handleViewAllLuxury = useCallback(() => {
    router.push('/mall/brands?filter=luxury' as unknown);
  }, [router]);

  const handleViewAllTrending = useCallback(() => {
    router.push('/mall/brands?filter=trending' as unknown);
  }, [router]);

  const handleViewAllRewardBoosters = useCallback(() => {
    router.push('/mall/brands?filter=reward-boosters' as unknown);
  }, [router]);
  ...
  const handleAllianceStores = useCallback(() => {
    router.push('/mall/alliance-store' as unknown);
  }, [router]);

  const handleLowestPrice = useCallback(() => {
    router.push('/mall/lowest-price' as unknown);
  }, [router]);
    ...
    <View style={styles.outerContainer}>
      ...
      <View style={styles.searchBarSection}>
        <Pressable
          ...
          onPress={() => router.push('/mall/brands?filter=all' as unknown)}
        >
        ...
        <Pressable
          ...
          onPress={() => router.push('/mall/lowest-price' as unknown)}
        >
