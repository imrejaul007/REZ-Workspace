    ...
    switch (banner.ctaAction) {
      case 'store':
        if (banner.ctaStoreId) {
          router.push(`/MainStorePage?storeId=${banner.ctaStoreId}` as unknown);
        }
        break;
      case 'brand':
        if (banner.ctaBrand) {
          const brandId = banner.ctaBrand._id || banner.ctaBrand.id;
          router.push(`/MainStorePage?storeId=${brandId}` as unknown);
        }
        break;
      case 'category':
        if (banner.ctaCategory) {
          router.push(`/mall/category/${banner.ctaCategory.slug}` as unknown);
        }
        break;
      case 'collection':
        if (banner.ctaCollection) {
          router.push(`/mall/collection/${banner.ctaCollection.slug}` as unknown);
        }
        break;
      case 'navigate':
        if (banner.ctaUrl) {
          router.push(banner.ctaUrl as unknown);
        }
        break;
      case 'external':
