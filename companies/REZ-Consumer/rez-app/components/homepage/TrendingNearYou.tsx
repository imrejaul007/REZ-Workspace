...
const WebVideoPlayer: React.FC<{ uri: string; poster?: string }> = ({ uri, poster }) => {
  const videoRef = useRef<unknown>(null); // Use any for cross-platform compatibility
...
const transformStoreData = (backendStore): TrendingStore => {
  // Get store ID
...
const TrendingNearYou: React.FC<TrendingNearYouProps> = ({
    ...
    try {
      ...
      if (response.success && response.data?.stores) {
        const transformedStores = (response.data.stores ?? [])
          .filter((store) => store && store._id) // Filter out undefined/null stores
          .map((store) => {
            try {
  ...
  const handleViewAll = () => {
    if (onViewAllPress) {
      ...
      router.push('/explore/trending' as unknown);
    }
  };
          ...
          <View style={styles.imageContainer}>
            {store.video ? (
              ...
              <CachedImage
                ...
                contentFit="cover"
                {...({ defaultSource: { uri: 'https://picsum.photos/200/150?grayscale' } } as React.ComponentProps<typeof CachedImage>)}
              />
            )}
            {/* Trending Badge */}
