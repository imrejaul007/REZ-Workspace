...
function IdentitySectionContainer() {
  ...
  const { loading: authLoading = false } = useAuth() as unknown;
  ...
  const handleSeeCampusLeaderboard = useCallback(() => router.push('/leaderboard/campus' as unknown), [router]);
  const handleSeeStudentDeals = useCallback(() => router.push('/offers/student' as unknown), [router]);
  const handleSeeCompanyLeaderboard = useCallback(() => router.push('/leaderboard/company' as unknown), [router]);
  const handleSeeCorporateDeals = useCallback(() => router.push('/offers/corporate' as unknown), [router]);
  const handleVerify = useCallback(() => router.push('/onboarding/identity-select' as unknown), [router]);
  const handleRefer = useCallback(() => router.push('/refer-institute' as unknown), [router]);
      ...
      <>
        ...
        {leaderboardData && leaderboardData.leaderboard.length > 0 && (
          <CampusSavingsBoard
            ...
            currentUserId={(user as unknown)?._id || (user as unknown)?.id}
    ...
    if (cfg) {
      return (
        <View style={{
          ...
          <Pressable
            onPress={() => router.push(cfg.dealsRoute as unknown)}
            style={{
          ...
          {cfg.secondaryLabel && cfg.secondaryRoute && (
            <Pressable
              onPress={() => router.push(cfg.secondaryRoute as unknown)}
              style={{
                marginTop: 8,
                paddingVertical: 10,
                borderRadius: 10,
