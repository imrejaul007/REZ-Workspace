...
export default function IdentityPromptModal() {
  ...
  useEffect(() => {
    ...
    // Check for nested auth.isOnboarded property
    const userWithAuth = user as User & { auth?: { isOnboarded?: boolean } };
    if (!userWithAuth.auth?.isOnboarded) return; // Not yet onboarded
  ...
  const handleSelect = async (id: 'student' | 'corporate' | 'general') => {
    ...
    if (!isMounted()) return;
    identityApi.setStatedIdentity(id).catch((e) => logger.warn('[Identity] setStatedIdentity failed:', e));
    if (!isMounted()) return;
    await AsyncStorage.setItem(STORAGE_KEY, 'true');
    setVisible(false);

    if (id === 'student') {
      router.push('/onboarding/student-verify' as unknown);
    } else if (id === 'corporate') {
      router.push('/onboarding/corporate-verify' as unknown);
    }
  };
  ...
  const handleDismiss = async () => {
    ...
    if (!isMounted()) return;
    identityApi.setStatedIdentity('general').catch((e) => logger.warn('[Identity] setStatedIdentity failed:', e));
    if (!isMounted()) return;
    await AsyncStorage.setItem(STORAGE_KEY, 'true');
    setVisible(false);
  };
