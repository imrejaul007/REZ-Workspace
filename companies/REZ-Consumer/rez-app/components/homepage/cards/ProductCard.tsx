...
function ProductCard({
  ...
  const productId = useMemo(() => getProductId(product), [(product as unknown)._id, product.id]);
  ...
  const price = useMemo(() => {
    ...
    const p = (product as unknown).pricing;
  ...
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [product.price, (product as unknown).pricing]);
...
const MemoizedProductCard = memo(ProductCard, (prevProps, nextProps) => {
  ...
  const prevPrice = prevProps.product.price || (prevProps.product as unknown).pricing;
  const nextPrice = nextProps.product.price || (nextProps.product as unknown).pricing;
  if (
    prevPrice?.current !== nextPrice?.current ||
    prevPrice?.original !== nextPrice?.original ||
    prevPrice?.discount !== nextPrice?.discount ||
    (prevPrice as unknown)?.selling !== (nextPrice as unknown)?.selling ||
