...
const arePropsEqual = (prevProps: RecommendationCardProps, nextProps: RecommendationCardProps) => {
  ...
  return (
    ((prevRec as unknown)._id || prevRec.id) === ((nextRec as unknown)._id || nextRec.id) &&
    prevProps.width === nextProps.width &&
    prevProps.showReason === nextProps.showReason &&
    prevRec.name === nextRec.name &&
    (prevRec as unknown).price?.current === (nextRec as unknown).price?.current &&
    (prevRec as unknown).price?.original === (nextRec as unknown).price?.original &&
...
function RecommendationCard({
  ...
  const { productId, cartItem, quantityInCart, isInCart } = useMemo(() => {
    const id = (recommendation as unknown)._id || recommendation.id;
  ...
  }, [(recommendation as unknown)._id, recommendation.id, cartState.items, cartState.items.length]);
  // Get currency from recommendation data
  const productCurrency = useMemo(() => {
    return recommendation.price?.currency || (recommendation as unknown).pricing?.currency || 'INR';
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [recommendation.price?.currency, (recommendation as unknown).pricing?.currency]);
    ...
    try {
      ...
      if (inWishlist) {
        ...
        await addToWishlist({
          ...
          price: (recommendation.price as unknown)?.current,
          originalPrice: (recommendation.price as unknown)?.original,
  ...
  const handleNotifyMe = useCallback((e) => {
    e.stopPropagation();
  ...
  const handleIncreaseQuantity = useCallback(async (e) => {
    e.stopPropagation();
