...
function EventCard({
  ...
  const priceBadgeColor = useMemo(() => {
    return event.price.isFree ? colors.brand.goldWarm : colors.lightMustard; // Sun Gold for free, REZ Gold for paid
  }, [event.price.isFree]);

  // Memoize price badge text color
  const priceBadgeTextColor = useMemo(() => {
    return event.price.isFree ? colors.nileBlue : colors.background.primary; // REZ Navy for free, White for paid
  }, [event.price.isFree]);
