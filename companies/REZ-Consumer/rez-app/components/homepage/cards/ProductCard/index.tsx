...
function ProductCard({
  ...
  const productId = useMemo(() => (product as unknown)._id || product.id, [(product as unknown)._id, product.id]);
  ...
  const productCurrency = useMemo(() => {
    return product.price?.currency || (product as unknown).pricing?.currency || 'INR';
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [product.price?.currency, (product as unknown).pricing?.currency]);
...
const MemoizedProductCard = memo(ProductCard, (prevProps, nextProps) => {
  if (
    ((prevProps.product as unknown)._id || prevProps.product.id) !==
    ((nextProps.product as unknown)._id || nextProps.product.id)
  ) {
    return false;
  }
