...
const HorizontalScrollSection = React.memo(function HorizontalScrollSection({
  ...
  const renderSectionCard = React.useCallback(({ item, index }: { item; index: number }) => (
    <View
      style={[
  ...
  return (
    <ThemedView style={styles.container}>
      ...
      <TypedFlashList
        ...
        keyExtractor={(item, index: number) => item?.id || item?._id || `item-${index}`}
        horizontal
        nestedScrollEnabled
  ...
  const prevIds = prevProps.section.items.map((item, i) => (item as unknown)?.id || (item as unknown)?._id || i).join(',');
  const nextIds = nextProps.section.items.map((item, i) => (item as unknown)?.id || (item as unknown)?._id || i).join(',');
