...
const DealsThatSaveMoney: React.FC<DealsThatSaveMoneyProps> = ({ style }) => {
  ...
  const [offersTabItems, setOffersTabItems] = useState<unknown[]>([]);
  const [cashbackTabItems, setCashbackTabItems] = useState<unknown[]>([]);
  const [exclusiveTabItems, setExclusiveTabItems] = useState<unknown[]>([]);
  ...
  const trackImpressions = useCallback(async (items: unknown[], tabType: TabType) => {
  ...
  const checkUserVerification = useCallback((eligibilityType: string): boolean => {
    ...
    switch (eligibilityType) {
      case 'student':
        return (user as unknown)?.verifications?.student === true;
      case 'corporate_email':
        return (user as unknown)?.verifications?.corporate === true;
      case 'gender':
        return (user as unknown)?.profile?.gender === 'female';
      case 'birthday_month':
        if (!(user as unknown)?.profile?.dateOfBirth) return false;
        const birthMonth = new Date((user as unknown).profile.dateOfBirth).getMonth();
        const currentMonth = new Date().getMonth();
        return birthMonth === currentMonth;
      case 'age':
        if (!(user as unknown)?.profile?.dateOfBirth) return false;
        const age = Math.floor((Date.now() - new Date((user as unknown).profile.dateOfBirth).getTime()) / (365.25 * 24 * 60 * 60 * 1000));
        return age >= 60;
      case 'verification':
        return (user as unknown)?.isFirstOrder !== false;
      default:
        return false;
    }
  }, [user]);
  ...
  const fetchCashbackData = useCallback(async () => {
    ...
    try {
      ...
      if (doubleCBResponse.success && doubleCBResponse.data) {
        doubleCBResponse.data.forEach((campaign) => {
          cards.push({
      ...
      if (coinDropsResponse.success && coinDropsResponse.data) {
        coinDropsResponse.data.forEach((drop) => {
          cards.push({
  ...
  const fetchOffers = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const nearbyCount = (response.data as unknown).filter((o: Offer) => o.distance && o.distance < 5).length;
        if (nearbyCount > 0) {
          categories.push({
            id: 'nearby',
            title: 'Nearby Offers',
        ...
        const todayCount = (response.data as unknown).filter((o: Offer) => {
        ...
        // BOGO deals
        const bogoCount = (response.data as unknown).filter((o: Offer) => o.type === 'combo' || o.title?.toLowerCase().includes('bogo')).length;
        if (bogoCount > 0) {
          categories.push({
            id: 'bogo',
            title: 'BOGO',
        ...
        // Flash Sale
        const flashCount = (response.data as unknown).filter((o: Offer) => o.metadata?.flashSale?.isActive).length;
        if (flashCount > 0) {
          categories.push({
            id: 'flash',
            title: 'Flash Sale',
        ...
        // Cashback offers
        const cashbackCount = (response.data as unknown).filter((o: Offer) => o.type === 'cashback' || o.cashbackPercentage > 0).length;
        if (cashbackCount > 0) {
          categories.push({
            id: 'cashback',
        ...
        // Freebies
        const freebieCount = (response.data as unknown).filter((o: Offer) => 
          ...
          categories.push(
            {
              id: 'all',
              title: 'All Offers',
              subtitle: `${(response.data as unknown).length} offers`,
              icon: 'grid-outline',
  ...
  const handleViewAll = () => {
    if (activeTab === 'offers') {
      ...
      router.push('/offers?tab=cashback');
    } else {
      router.push('/offers');
    }
  };
  ...
  const handleCategoryPress = (categoryId: string) => {
    if (activeTab === 'offers') {
      router.push(`/offers?category=${categoryId}`);
    } else if (activeTab === 'cashback') {
      router.push(`/offers?tab=cashback&category=${categoryId}`);
    } else if (activeTab === 'exclusive') {
      // Use slug directly from API data
      router.push(`/offers/zones/${categoryId}`);
    }
  };
  ...
  const handleExclusivePress = (zone: ExclusiveZoneCard) => {
    ...
    if (heroesProfiles.includes(zone.slug)) {
      router.push(`/offers/zones/heroes?profile=${zone.slug}`);
    } else {
      // Navigate to zone offers page - verification will be handled there
      router.push(`/offers/zones/${zone.slug}`);
    }
  };
  ...
  const handleCashbackPress = (card: CashbackCard) => {
    if (card.type === 'double_cashback') {
      router.push('/offers?tab=cashback&filter=double');
    } else if (card.type === 'coin_drop') {
      router.push('/offers?tab=cashback&filter=coindrops');
    } else {
      router.push('/offers?tab=cashback');
    }
  };
  ...
  const handleAdminItemPress = (item) => {
    // Track click
    if (item._id) {
  ...
  const renderAdminItems = (items: unknown[]) => {
    if (items.length === 0) {
      return (
      ...
      <View style={styles.header}>
        <View style={styles.headerLeft}>
          <View style={styles.headerIconContainer}>
            <Ionicons name={(sectionConfig?.icon || 'flash') as unknown} size={20} color={colors.lightPeach} />
          </View>
