...
const PickedForYou: React.FC<PickedForYouProps> = ({
  ...
  const products = useMemo(() => {
    ...
    const recommendationItems = section.items.slice(0, limit).map((item) => {
      ...
      const productItem = item as unknown;
      return {
        ...productItem,
  ...
  const subtitle = useMemo(() => {
    ...
    const isPersonalized = products.some((item) =>
      item.personalizedFor && item.personalizedFor !== 'general' && item.personalizedFor !== null
    );

    const hasLocationData = products.some((item) =>
  ...
  const handleViewAll = () => {
    if (onViewAllPress) {
      ...
      router.push('/recommendations' as unknown);
    }
  };
