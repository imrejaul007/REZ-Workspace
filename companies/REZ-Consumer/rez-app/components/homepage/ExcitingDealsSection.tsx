...
const ExcitingDealsSection: React.FC = () => {
    ...
    try {
      ...
      if (response.success && response.data && response.data.dealCategories.length > 0) {
        ...
        const processedCategories = uniqueCategories.map((cat) => ({
          ...
          badgeColor: cat.badgeColor || (COLORS as unknown).navy,
  ...
  const handleViewAll = () => {
    router.push('/campaigns' as unknown);
  };
  ...
  const handleDealPress = (deal: CampaignDeal, categoryId: string, dealIndex: number) => {
    ...
    router.push(`/deals/${categoryId}/${dealIndex}` as unknown);
  };

  const handleCategoryPress = (categoryId: string) => {
    router.push(`/deals/${categoryId}` as unknown);
  };
            ...
            >
              ...
              >
                ...
                <View style={styles.categoryArrow}>
                  <Ionicons name="chevron-forward" size={18} color={(COLORS as unknown).navy} />
                </View>
              </LinearGradient>
            </Pressable>
