...
const AutoPlayVideo: React.FC<{ uri: string; poster?: string; style?: unknown }> = ({ uri, poster, style }) => {
  const videoRef = useRef<Video>(null);
  const webContainerRef = useRef<View>(null);
  const videoElementRef = useRef<unknown>(null); // Use any for cross-platform compatibility
  ...
  useEffect(() => {
    ...
    if (Platform.OS === 'web' && webContainerRef.current && typeof document !== 'undefined') {
      ...
      const container = webContainerRef.current as unknown;
...
const NewOnRezSection: React.FC = () => {
  ...
  const transformStoreData = (store): NewStore => {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      if (domElement) {
        ...
        (window as unknown).__trendingHoverElement = domElement;
        (window as unknown).__trendingMouseEnter = handleMouseEnter;
        (window as unknown).__trendingMouseLeave = handleMouseLeave;
      }
    }, 500);

    return () => {
      clearTimeout(timer);
      const domElement = (window as unknown).__trendingHoverElement;
      const mouseEnter = (window as unknown).__trendingMouseEnter;
      const mouseLeave = (window as unknown).__trendingMouseLeave;
  ...
  const handleStorePress = (id: string) => {
    router.push(`/MainStorePage?storeId=${id}` as unknown);
  };
