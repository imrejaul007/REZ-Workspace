...
const RelatedFlightsSection: React.FC<RelatedFlightsSectionProps> = ({
  ...
  const [relatedFlights, setRelatedFlights] = useState<unknown[]>([]);
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        const filtered = (response.data.services || [])
          .filter((flight) => flight._id !== currentFlightId && flight.id !== currentFlightId)
          .slice(0, 3);
        if (!isMounted()) return;
  ...
  const handleFlightPress = (flightId: string) => {
    router.push(`/flight/${flightId}` as unknown);
  };
