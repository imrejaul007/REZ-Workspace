    ...
    try {
      ...
      if (response.success && response.data) {
        const bookingResponse: BookingData = {
          ...bookingData,
          bookingId: (response.data as unknown)._id || (response.data as unknown).id,
          bookingNumber: response.data.bookingNumber,
        };
        (bookingResponse as unknown).requiresPayment = (response as unknown).requiresPayment || (response.data as unknown)?.requiresPaymentUpfront;
        (bookingResponse as unknown).totalAmount = calculateTotalPrice();
          ...
          {mealPlans.map((plan) => {
            ...
            return (
              <Pressable
                ...
                onPress={() => setMealPlan(plan.key as unknown)}
              >
