...
export const OfferRedemptionModal: React.FC<OfferRedemptionModalProps> = ({
  ...
  const storeName = (offer as unknown).store?.name || (offer as unknown).storeName || 'Store';
  const total = (offer as unknown).totalQuantity || (offer as unknown).total || 100;
  const claimed = (offer as unknown).claimedQuantity || (offer as unknown).claimed || 0;
  const subtitle = (offer as unknown).subtitle || (offer as unknown).description || '';
