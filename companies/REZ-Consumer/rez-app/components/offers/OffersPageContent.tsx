...
export const OffersPageContent: React.FC<OffersPageContentProps> = ({
  ...
  const handleNavigateTo = useCallback((path: string, params?: Record<string, string>) => {
    try {
      if (params) {
        router.push({ pathname: path, params } as unknown);
      } else {
  ...
  const isCashbackTabEmpty = useMemo(() => {
    const cashbackOffers = offersData.nearbyOffers.filter((o) => o.cashbackPercentage > 0);
  ...
  const renderOffersTab = () => {
    ...
    return (
      <>
        ...
        {bannerConfig && (
          <Pressable
            onPress={() => router.push(bannerConfig.route as unknown)}
            style={{
              margin: 16,
              padding: 14,
          ...
          >
            <Ionicons name={bannerConfig.icon as unknown} size={20} color={bannerConfig.color} />
  ...
  const renderExclusiveTab = () => {
    ...
    return (
      <>
        ...
        {offersData.exclusiveCategories.some((z) =>
          z.eligibilityType === 'birthday_month' && z.userEligible
        ) && (
          <BirthdayBanner
            isActive={true}
            daysRemaining={
              offersData.exclusiveCategories.find((z) =>
