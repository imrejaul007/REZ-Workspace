...
export const SpecialProfilesSection: React.FC<SpecialProfilesSectionProps> = ({
  ...
  const handleProfilePress = (profile: SpecialProfile) => {
    if (profile.isVerified) {
      router.push(`/offers/zones/${profile.slug}` as unknown);
    } else {
      router.push(`/offers/zones/${profile.slug}/verify` as unknown);
    }
  };
  ...
  const styles = useMemo(() => StyleSheet.create({
    ...
    card: {
      width: 140,
      backgroundColor: isDark ? theme.colors.background.primary : colors.background.primary,
  ...
  return (
    <View style={styles.container}>
      ...
      <Pressable
        ...
        onPress={() => {
          const firstUnverified = profiles.find((p) => !p.isVerified);
          if (firstUnverified) {
            router.push(`/offers/zones/${firstUnverified.slug}/verify` as unknown);
          }
        }}
