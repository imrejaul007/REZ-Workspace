...
function WalkInDealsModal({ visible, onClose, deals = [], storeId }: DealModalProps) {
  ...
  const [apiDeals, setApiDeals] = useState<unknown[]>([]);
      ...
      if (response.success && response.data) {
        ...
        const transformedDeals = fetchedDeals.map((deal) => {
          ...
          if (deal.metadata?.featured || deal.type === 'mega') {
            ...
            badgeTextColor = (COLORS as unknown).goldDark;
          ...
          return {
            ...
            category: mappedCategory as unknown,
  ...
  const handleModalPress = (event) => event.stopPropagation();
