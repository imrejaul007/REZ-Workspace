...
function VideoCard({
  ...
  const handleVolumeToggle = async (e) => {
    e.stopPropagation();
  ...
  return (
    ...
    >
      ...
      <View style={styles.videoContainer}>
        ...
        <LinearGradient
          colors={PLAY_PAGE_COLORS.gradient.cardOverlay as unknown}
        ...
        {!isLoading && !hasError && (
          <View style={styles.socialActionsContainer}>
            ...
            <Pressable
              ...
              accessibilityLabel={`View comments${(item as unknown).engagement?.comments ? `. ${(item as unknown).engagement.comments} comment${(item as unknown).engagement.comments !== 1 ? 's' : ''}` : ''}`}
              accessibilityRole="button"
            ...
            >
              ...
              {(item as unknown).engagement?.comments && (item as unknown).engagement.comments > 0 && (
                <ThemedText style={styles.socialActionText}>
                  {(item as unknown).engagement.comments >= 1000
                    ? `${((item as unknown).engagement.comments / 1000).toFixed(1)}k`
                    : (item as unknown).engagement.comments}
                </ThemedText>
              )}
            </Pressable>
            ...
            <Pressable
              ...
              accessibilityLabel={`Share video${(item as unknown).engagement?.shares ? `. ${(item as unknown).engagement.shares} share${(item as unknown).engagement.shares !== 1 ? 's' : ''}` : ''}`}
              accessibilityRole="button"
            ...
            >
              ...
              {(item as unknown).engagement?.shares && (item as unknown).engagement.shares > 0 && (
                <ThemedText style={styles.socialActionText}>
                  {(item as unknown).engagement.shares >= 1000
                    ? `${((item as unknown).engagement.shares / 1000).toFixed(1)}k`
                    : (item as unknown).engagement.shares}
                </ThemedText>
              )}
            </Pressable>
          </View>
