...
function FoodDiningCategoryPage() {
  ...
  const [newStores, setNewStores] = useState<unknown[]>([]);
  const [popularDishes, setPopularDishes] = useState<unknown[]>([]);
  ...
  const [myBookings, setMyBookings] = useState<unknown[]>([]);
    ...
    const fetchLoyaltyData = async () => {
      try {
        ...
        if (ordersRes.success && ordersRes.data?.orders) {
          ...
          const recentOnly = ordersRes.data.orders.filter((order) => {
    ...
    const fetchMyOrders = async () => {
      try {
        ...
        if (res.success && res.data?.orders) {
          const foodOrders = res.data.orders.filter((order) => {
  ...
  const handleCategoryPress = (category) => {
    ...
    router.push(`/MainCategory/food-dining/${subcategorySlug}` as unknown);
  };

  const handleAISearch = (query: string) => {
    const sanitized = query.trim().slice(0, 100);
    if (!sanitized) return;
    router.push(`/MainCategory/food-dining/search?q=${encodeURIComponent(sanitized)}` as unknown);
  };
  ...
  const handleQuickAction = useCallback((action) => {
    ...
    if (action.id === 'cuisines') {
      ...
      router.push('/MainCategory/food-dining/search?tab=cuisines' as unknown);
      return;
    }
    ...
    <ErrorBoundary onError={() => { /* silently handle */ }}>
      ...
      >
        ...
        <Pressable
          ...
          onPress={() => router.push('/MainCategory/food-dining/loyalty' as unknown)}
        ...
        <View style={styles.tabsContainer}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>
            {dynamicTabs.map((tab) => (
              ...
              >
                <Ionicons name={tab.icon as unknown} size={18} color={activeTab === tab.id ? COLORS.white : COLORS.textSecondary} />
        ...
        <QuickActionBar categorySlug={slug} actions={foodQuickActions as unknown} onActionPress={handleQuickAction} />
        ...
        {(activeTab === 'delivery' || activeTab === 'dineIn' || activeTab === 'takeaway') && (
          <BrowseCategoryGrid
            categories={subcategories as unknown}
          ...
          <View style={styles.tabContent}>
              ...
              <Pressable style={styles.modalOverlay} onPress={() => setShowSortModal(false)}>
                <View style={styles.modalContent}>
                  ...
                  {dynamicSortOptions.map(opt => (
                    ...
                    >
                      <Ionicons name={opt.icon as unknown} size={20} color={sortOption === opt.id ? COLORS.primaryGold : COLORS.textSecondary} />
              ...
              <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.restaurantsList}>
                {dynamicCuratedCollections.map(col => (
                  <Pressable
                    ...
                    onPress={() => router.push(`/MainCategory/food-dining/search?tags=${col.tags}` as unknown)}
            ...
            {filteredStores.length >= 2 && (
              <View onLayout={(e) => { compareSectionY.current = e.nativeEvent.layout.y; }}>
                <RestaurantCompareSection
                  restaurants={filteredStores as unknown}
            ...
            {isLoading ? (
              ...
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  ...
                  <Pressable onPress={() => router.push('/MainCategory/food-dining/fast-delivery' as unknown)} accessibilityLabel="View all fast delivery restaurants" accessibilityRole="link">
                    <Text style={styles.sectionSeeAll}>View All</Text>
                  </Pressable>
                </View>
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.restaurantsList}>
                  {fastDeliveryStores.slice(0, 5).map((store) => (
                    <RestaurantCard key={store._id || store.id} restaurant={store as unknown} variant="compact" userVisitCount={visitCounts[store._id || store.id] || 0} />
                  ))}
                </ScrollView>
              </View>
            ) : null}
            ...
            {isLoading ? (
              ...
              <View style={styles.section}>
                <View style={styles.sectionHeader}>
                  ...
                  <Pressable onPress={() => router.push('/MainCategory/food-dining/top-rated' as unknown)} accessibilityLabel="View all top rated restaurants" accessibilityRole="link">
                    <Text style={styles.sectionSeeAll}>View All</Text>
                  </Pressable>
                </View>
                <View style={styles.restaurantsGrid}>
                  {topRatedStores.slice(0, 4).map((store) => (
                    <RestaurantCard key={store._id || store.id} restaurant={store as unknown} userVisitCount={visitCounts[store._id || store.id] || 0} />
                  ))}
                </View>
              </View>
            ) : null}
              ...
              <View style={styles.section}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.restaurantsList}>
                  {filteredNewStores.map((store: FoodStore) => (
                    <RestaurantCard key={store._id || store.id} restaurant={store as unknown} variant="compact" showNewBadge />
                  ))}
                </ScrollView>
              </View>
            ) : null}
        ...
        {activeTab === 'dineIn' && (
          <View style={styles.tabContent}>
            <Pressable
              ...
              onPress={() => router.push('/MainCategory/food-dining/book-table' as unknown)}
            ...
            <View style={styles.section}>
              ...
              <View style={styles.restaurantsGrid}>
                {dineInStores.map((store) => (
                  <RestaurantCard key={store._id || store.id} restaurant={store as unknown} showReserveButton={!isDineInFallback} userVisitCount={visitCounts[store._id || store.id] || 0} />
                ))}
              </View>
            </View>
          </View>
          ...
          <LinearGradient colors={[colors.nileBlue, '#0f2638']} style={styles.valuePropGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
            ...
            <View style={styles.savingsRow}>
              ...
              <Pressable
                ...
                onPress={() => router.push('/MainCategory/food-dining/loyalty/coins' as unknown)}
        ...
        <EnhancedUGCSocialProofSection
          ...
          onPostPress={(post) => router.push(`/ugc/${post.id}` as unknown)}
          onSharePress={() => router.push('/share' as unknown)}
          onViewAllPress={() => router.push('/MainCategory/food-dining/food-stories' as unknown)}
        />
