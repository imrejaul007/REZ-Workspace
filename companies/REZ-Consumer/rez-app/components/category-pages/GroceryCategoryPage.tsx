...
const StoreCard = ({ store, variant = 'default' }: { store; variant?: 'default' | 'compact' }) => {
  const router = useRouter();
  const isCompact = variant === 'compact';

  const handleStorePress = () => {
    router.push(`/MainStorePage?storeId=${(store as unknown)._id || store.id}` as unknown);
  };
...
const ProductCard = ({ product, currencySymbol }: { product; currencySymbol: string }) => {
  const router = useRouter();
  ...
  const handleAddToCart = async (e) => {
    ...
    try {
      ...
      await apiClient.post('/cart/add', {
        productId: (product as unknown)._id || product.id,
        quantity: 1,
      });
  ...
  return (
    <Pressable
      ...
      onPress={() => {
        ...
        const productId = (product as unknown)._id || product.id;
        router.push(`/product-page?id=${productId}&storeId=${storeId}` as unknown);
      }}
...
function GroceryCategoryPage() {
  ...
  const fetchSuggestions = useCallback(async () => {
    try {
      ...
      if (suggestions.length > 0) {
        if (!true) return;
        setSmartSuggestions(suggestions.map((s) => s.text || s.title || s));
      } else {
        setSmartSuggestions([]);
      }
    } catch {
  ...
  const fetchNewStores = useCallback(async () => {
    try {
      ...
      if (res?.success && (res.data as unknown)?.length > 0) {
        const storesData = Array.isArray(res.data) ? res.data : (res.data?.stores || []);
        const groceryStores = storesData.filter((s) => {
  ...
  const fastDeliveryStores = useMemo(() =>
    filteredStores.filter((s) => s.is60Min || s.deliveryCategories?.fastDelivery),
    [filteredStores]
  );

  const topRatedStores = useMemo(() =>
    filteredStores.filter((s) => (s.ratings?.average || 0) >= 4.0)
  ...
  const handleCategoryPress = (category) => {
    router.push(`/MainCategory/grocery-essentials/${category.slug || category.id}` as unknown);
  };

  const handleAISearch = (query: string) => {
    router.push(`/MainCategory/grocery-essentials/search?q=${encodeURIComponent(query)}` as unknown);
  };
  ...
  return (
    ...
    >
      ...
      <QuickActionBar categorySlug={slug} actions={groceryQuickActions as unknown} />
      ...
      {isLoading ? (
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/grocery-essentials/fast-delivery' as unknown)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storesList}>
            {fastDeliveryStores.slice(0, 5).map((store) => (
              <StoreCard key={(store as unknown)._id || store.id} store={store} variant="compact" />
            ))}
          </ScrollView>
        </View>
      ) : null}
      ...
      {isLoading ? (
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/grocery-essentials/top-rated' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <View style={styles.storesGrid}>
            {topRatedStores.slice(0, 4).map((store) => (
              <StoreCard key={(store as unknown)._id || store.id} store={store} />
            ))}
          </View>
        </View>
      ) : null}
      ...
      <View style={styles.compareBanner}>
        <Pressable
          ...
          onPress={() => router.push('/MainCategory/grocery-essentials/compare' as unknown)}
        >
          <LinearGradient
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <Pressable onPress={() => router.push(`/MainCategory/grocery-essentials/search?tab=${activeTab}` as unknown)}>
            <Text style={styles.sectionSeeAll}>View All</Text>
          </Pressable>
        </View>
        {isLoading ? (
      ...
      {isLoading ? (
        ...
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push(`/MainCategory/grocery-essentials/search?q=${activeTab === 'organic' ? 'organic' : activeTab === 'wholesale' ? 'wholesale' : 'popular'}` as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.productsList}>
            {filteredProducts.slice(0, 8).map((product) => (
              <ProductCard key={(product as unknown)._id || product.id} product={product} currencySymbol={currencySymbol} />
            ))}
          </ScrollView>
        </View>
        ...
        >
          <View style={styles.billUploadContent}>
            ...
            <View style={styles.billUploadText}>
              ...
              <Text style={styles.billUploadSubtitle}>Get cashback on unknown grocery purchase</Text>
            </View>
            <Pressable style={styles.billUploadButton} onPress={() => router.push('/bill-upload' as unknown)}>
      ...
      <EnhancedUGCSocialProofSection
        ...
        onViewAllPress={() => router.push('/MainCategory/grocery-essentials/grocery-stories' as unknown)}
      />
