...
function storeMatchesFilter(store, filterId: string): boolean {
...
function EducationCategoryPage() {
  ...
  const storeCountsByFilter = useMemo(() => {
    ...
    [...educationServiceFilters, ...educationModeFilters].forEach(f => {
      counts[f.id] = stores.filter((s) => storeMatchesFilter(s, f.id)).length;
    });
    return counts;
  }, [stores]);
  ...
  const handleCategoryPress = useCallback((category) => {
    router.push(`/MainCategory/education-learning/${category.slug || category.id}` as unknown);
  }, [router]);

  const handleAISearch = useCallback((query: string) => {
    router.push(`/MainCategory/education-learning/search?q=${encodeURIComponent(query)}` as unknown);
  }, [router]);
  ...
  return (
    ...
    >
      ...
      {/* Rewards Strip */}
      <Pressable style={styles.rewardsStrip} onPress={() => router.push('/MainCategory/education-learning/loyalty' as unknown)}>
      ...
      <QuickActionBar categorySlug={slug} actions={educationQuickActions as unknown} />
      ...
      <BrowseCategoryGrid categories={subcategories as unknown} title="Explore Courses" onCategoryPress={handleCategoryPress} />
      ...
      {filteredCourses.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/education-learning/search?q=popular' as unknown)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.trendingList}>
            {filteredCourses.map(w => (
              <Pressable key={w.id} style={styles.trendingCard} onPress={() => router.push(`/MainCategory/education-learning/enroll-class?service=${w.id}` as unknown)}>
      ...
      {filteredServices.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/education-learning/enroll-class' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.servicesList}>
            {filteredServices.map(s => (
              <Pressable key={s.id} style={styles.serviceCard} onPress={() => router.push(`/MainCategory/education-learning/enroll-class?service=${s.id}` as unknown)}>
      ...
      {filteredProducts.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/education-learning/search?q=materials' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.productsList}>
            {filteredProducts.slice(0, 6).map((p) => (
              <Pressable key={p.id} style={styles.productCardCompact} onPress={() => router.push(`/product-page?productId=${p.id}` as unknown)}>
                {p.image ? (
      ...
      {filteredStores.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/education-learning/top-rated' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.storesList}>
            {filteredStores.slice(0, 5).map((store) => {
              ...
              return (
                <Pressable key={store.id} style={styles.storeCard} onPress={() => router.push(`/MainStorePage?storeId=${store.id}` as unknown)}>
      ...
      {hasActiveFilters && filteredStores.length === 0 && filteredProducts.length === 0 && (
        <View style={styles.filterEmptyState}>
          <Ionicons name="search-outline" size={48} color={(COLORS as unknown).primaryLight} />
      ...
      <OffersSection categorySlug={slug} title="Today's Top Education Deals" onSeeAll={() => router.push('/MainCategory/education-learning/offers' as unknown)} filterTags={activeFilterTags} />

      {/* Learning Challenge */}
      <View style={styles.section}>
        <Pressable style={styles.challengeBanner} onPress={() => router.push('/MainCategory/education-learning/search?q=challenge' as unknown)}>
      ...
      <EnhancedUGCSocialProofSection
        ...
        onPostPress={() => router.push('/MainCategory/education-learning/learning-stories' as unknown)}
        onSharePress={() => router.push('/MainCategory/education-learning/learning-stories' as unknown)}
      />
...
const styles = StyleSheet.create({
  ...
  serviceIcon: { width: 56, height: 56, borderRadius: 28, backgroundColor: (COLORS as unknown).primaryLight, justifyContent: 'center', alignItems: 'center', marginBottom: 8 },
  ...
  productPlaceholder: { backgroundColor: (COLORS as unknown).primaryLight, justifyContent: 'center', alignItems: 'center' },
  ...
  storePlaceholder: { backgroundColor: (COLORS as unknown).primaryLight, justifyContent: 'center', alignItems: 'center' },
