...
function ElectronicsCategoryPage() {
  ...
  const handleCategoryPress = useCallback((category) => {
    router.push(`/MainCategory/electronics/${category.slug || category.id}` as unknown);
  }, [router]);

  const handleAISearch = useCallback((query: string) => {
    router.push(`/MainCategory/electronics/search?q=${encodeURIComponent(query)}` as unknown);
  }, [router]);
  ...
  return (
    ...
    >
      ...
      {/* Rewards Strip */}
      <Pressable style={styles.rewardsStrip} onPress={() => router.push('/MainCategory/electronics/loyalty' as unknown)}>
      ...
      <QuickActionBar categorySlug={slug} actions={electronicsQuickActions as unknown} />
      ...
      {filteredTrending.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/electronics/search?q=trending' as unknown)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.trendingList}>
            {filteredTrending.map(w => (
              <Pressable key={w.id} style={styles.trendingCard} onPress={() => router.push(`/MainCategory/electronics/search?q=${w.id}` as unknown)}>
      ...
      {filteredServices.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/electronics/compare-devices' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.servicesList}>
            {filteredServices.map(s => (
              <Pressable key={s.id} style={styles.serviceCard} onPress={() => router.push(`/MainCategory/electronics/compare-devices` as unknown)}>
      ...
      {filteredStores.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/electronics/top-rated' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
      ...
      <OffersSection categorySlug={slug} title="Today's Top Tech Deals" onSeeAll={() => router.push('/MainCategory/electronics/offers' as unknown)} filterTags={activeFilterTags} />

      {/* Compare Devices Banner */}
      <View style={styles.section}>
        <Pressable style={styles.ctaBanner} onPress={() => router.push('/MainCategory/electronics/compare-devices' as unknown)}>
      ...
      <EnhancedUGCSocialProofSection
        ...
        onPostPress={() => router.push('/MainCategory/electronics/tech-stories' as unknown)}
        onSharePress={() => router.push('/MainCategory/electronics/tech-stories' as unknown)}
      />
