...
function storeMatchesFilter(store, filterId: string): boolean {
  const tags = ((store.tags || []) as unknown[]).map((t: string) => t.toLowerCase());
  const serviceTypes = ((store.serviceTypes || []) as unknown[]).map((t: string) => t.toLowerCase());
...
function FitnessCategoryPage() {
  ...
  const handleCategoryPress = useCallback((category) => {
    router.push(`/MainCategory/fitness-sports/${category.slug || category.id}` as unknown);
  }, [router]);

  const handleAISearch = useCallback((query: string) => {
    router.push(`/MainCategory/fitness-sports/search?q=${encodeURIComponent(query)}` as unknown);
  }, [router]);
  ...
  return (
    ...
    >
      ...
      {/* Rewards Strip */}
      <Pressable style={styles.rewardsStrip} onPress={() => router.push('/MainCategory/fitness-sports/loyalty' as unknown)}>
      ...
      <QuickActionBar categorySlug={slug} actions={fitnessQuickActions as unknown} />
      ...
      <BrowseCategoryGrid categories={subcategories as unknown} title="Explore Fitness" onCategoryPress={handleCategoryPress} />
      ...
      {filteredTrending.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/fitness-sports/search?q=trending' as unknown)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.trendingList}>
            {filteredTrending.map(w => (
              <Pressable key={w.id} style={styles.trendingCard} onPress={() => router.push(`/MainCategory/fitness-sports/book-class?service=${w.id}` as unknown)}>
      ...
      {filteredServices.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/fitness-sports/book-class' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.servicesList}>
            {filteredServices.map(s => (
              <Pressable key={s.id} style={styles.serviceCard} onPress={() => router.push(`/MainCategory/fitness-sports/book-class?service=${s.id}` as unknown)}>
      ...
      {filteredProducts.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/fitness-sports/search?q=equipment' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
      ...
      {filteredStores.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push('/MainCategory/fitness-sports/top-rated' as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
      ...
      <OffersSection categorySlug={slug} title="Today's Top Fitness Deals" onSeeAll={() => router.push('/MainCategory/fitness-sports/offers' as unknown)} filterTags={activeFilterTags} />

      {/* 30-Day Challenge */}
      <View style={styles.section}>
        <Pressable style={styles.challengeBanner} onPress={() => router.push('/MainCategory/fitness-sports/challenges' as unknown)}>
      ...
      <EnhancedUGCSocialProofSection
        ...
        onPostPress={() => router.push('/MainCategory/fitness-sports/fitness-stories' as unknown)}
        onSharePress={() => router.push('/MainCategory/fitness-sports/fitness-stories' as unknown)}
      />
