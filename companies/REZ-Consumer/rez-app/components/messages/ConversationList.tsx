...
function ConversationList({
  ...
  const handleConversationPress = (conversation: Conversation) => {
    router.push({
      pathname: '/store/[id]/chat' as unknown,
      params: {
            ...
            <View style={styles.lastMessageContainer}>
              {lastMessage.senderType === 'customer' && (
                <Ionicons
                  name="checkmark-done"
                  size={14}
                  color={lastMessage.status === 'read' ? PROFILE_COLORS.primary : '#666'} // ACCESSIBILITY FIX: Changed from #999 to #666 for better icon visibility
