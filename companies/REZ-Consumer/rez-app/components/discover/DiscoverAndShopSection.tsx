...
function DiscoverAndShopSection({
  ...
  const handleCategoryPress = useCallback((category: CategoryCard) => {
    if (onCategoryPress) {
      ...
      router.push(`/category/${categorySlug}` as unknown);
    }
  ...
  // Handle like toggle
  const handleLike = useCallback(async (reelId: string, e) => {
    e.stopPropagation();
  ...
  const handleBookmark = useCallback(async (reelId: string, e) => {
    e.stopPropagation();
  ...
  const handleShoppingBag = useCallback((reel: DiscoverReel, e) => {
    e.stopPropagation();
    ...
    if (reel.products && reel.products.length > 0) {
      ...
      router.push(`/MainStorePage?storeId=${reel.store._id}` as unknown);
    }
  }, [router]);
      ...
      <View style={styles.savingsHeader}>
        ...
        <Pressable
          ...
          onPress={() => {
            ...
            router.push('/social/reels' as unknown);
          }}
