  ...
  const performNavigation = useCallback(async (): Promise<void> => {
    try {
      if (canGoBack) {
        ...
        // @ts-ignore — complex union type
        await navigate(fallbackRoute as unknown);
      } else {
        await goBack();
      }
    } catch {
      try {
        await navigate('/(tabs)' as unknown);
      } catch {
        // silently handle
      }
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canGoBack, (goBack as unknown), (navigate as unknown), fallbackRoute]);

  return (
    // @ts-ignore - complex union type from Href<string> intersection
    <Pressable
      onPress={handlePress}
      style={[styles.button, style] as unknown}
