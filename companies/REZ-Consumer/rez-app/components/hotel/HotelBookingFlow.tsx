...
const HotelBookingFlow: React.FC<HotelBookingFlowProps> = ({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      (bookingResponse as unknown).requiresPayment = true;
      (bookingResponse as unknown).totalAmount = holdResponse.totalPaise;
      (bookingResponse as unknown).razorpayOrderId = holdResponse.razorpayOrderId;
      (bookingResponse as unknown).pgAmount = holdResponse.pgAmountPaise;
      (bookingResponse as unknown).coinsApplied = {
    ...
    <View style={styles.stepContainer}>
      ...
      {(['standard', 'deluxe', 'suite'] as const).map((type) => {
        ...
        return (
          <Pressable
            key={type}
            style={[styles.roomCard, isSelected ? styles.roomCardSelected : null]}
