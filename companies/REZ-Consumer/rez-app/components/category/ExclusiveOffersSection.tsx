...
const ExclusiveOffersSection: React.FC<ExclusiveOffersSectionProps> = ({
    ...
    const fetchOffers = async () => {
      try {
        ...
        if (response.success && response.data?.offers?.length && response.data.offers.length > 0) {
          ...
          setApiOffers(exclusiveOffersData as unknown);
        }
      } catch (err) {
  ...
  const handlePress = useCallback((offer: ExclusiveOffer) => {
    if (onOfferPress) {
      ...
      const offerId = offer._id || (offer as unknown).id;
      router.push({
        pathname: '/offer/[id]',
        params: { id: offerId },
      } as unknown);
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <Pressable
          ...
          onPress={() => router.push('/offers/exclusive' as unknown)}
      ...
      >
        {displayOffers.map((offer) => (
          <OfferCard
            key={offer._id || (offer as unknown).id}
            offer={offer}
