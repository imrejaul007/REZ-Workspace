...
const UGCSocialProofSection: React.FC<UGCSocialProofSectionProps> = ({
      ...
      try {
        if (categorySlug) {
          ...
          if (response.success && response.data?.videos && response.data.videos.length > 0) {
            if (!isMounted()) return;
            setVideos(response.data.videos.slice(0, maxItems) as unknown);
          } else {
            ...
            if (trendingResponse.success && trendingResponse.data && trendingResponse.data.length > 0) {
              if (!isMounted()) return;
              setVideos(trendingResponse.data.slice(0, maxItems) as unknown);
            }
          }
        } else {
          ...
          if (response.success && response.data && response.data.length > 0) {
            if (!isMounted()) return;
            setVideos(response.data.slice(0, maxItems) as unknown);
          }
        }
      } catch (err) {
  ...
  const handleLike = useCallback(async (videoId: string) => {
    try {
      await (videosApi as unknown).likeVideo(videoId);
      if (!isMounted()) return;
      setVideos((prev) =>
        prev.map((item) =>
          item.id === videoId
  ...
  return (
    <View style={styles.container}>
      ...
      <FlashList
        ...
        numColumns={2}
        scrollEnabled={false}
        contentContainerStyle={styles.gridContainer as unknown}
