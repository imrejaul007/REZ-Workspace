...
function OffersSection({ categorySlug, categoryId, title, onSeeAll, filterTags }: OffersSectionProps) {
  ...
  const [bankOffers, setBankOffers] = useState<unknown[]>([]);
  const [todaysDeals, setTodaysDeals] = useState<unknown[]>([]);
  ...
  const fetchOffers = useCallback(async () => {
    try {
      ...
      const [bankRes, dealsRes, couponsRes] = await Promise.all([
        apiClient.get<unknown>('/offers/bank-offers', { category: categorySlug, limit: 10 }).catch(() => null),
        apiClient.get<unknown>('/offers/flash-sales', { category: categorySlug, limit: 6 }).catch(() => null),
        apiClient.get<unknown>('/coupons/my-coupons', { category: categorySlug, limit: 6 }).catch(() => null),
      ]);
      ...
      if (couponsRes?.success && couponsRes.data) {
        ...
        setCoupons(couponList.filter((c) => c.status === 'active' || c.isActive !== false).slice(0, 4));
      }
    } catch (error) {
      // silently handle
    } finally {
  ...
  const handleOfferPress = (offer) => {
    ...
    if (id) {
      router.push(`/MainCategory/${categorySlug}/offers/${id}` as unknown);
    }
  };

  const getDealDisplay = (deal) => {
      ...
      {bankOffers.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push(`/MainCategory/${categorySlug}/offers?tab=bank` as unknown)}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.bankOffersList}>
            {bankOffers.map((offer, index) => (
              ...
              >
                <LinearGradient
                  colors={getBankGradient(offer.bankName || offer.bank || '') as unknown}
      ...
      {todaysDeals.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push(`/MainCategory/${categorySlug}/offers?tab=deals` as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
      ...
      {coupons.length > 0 && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable onPress={() => router.push(`/MainCategory/${categorySlug}/offers?tab=promos` as unknown)}>
              <Text style={styles.sectionSeeAll}>View All</Text>
            </Pressable>
          </View>
...
const styles = StyleSheet.create({
  ...
  dealCard: {
    width: '47%' as unknown, padding: 16, borderRadius: 16,
