...
interface ExperiencesSectionProps {
  categorySlug: string;
  pageConfig?; // receives pageConfig for experience benefits overrides
}
...
function ExperiencesSection({ categorySlug, pageConfig }: ExperiencesSectionProps) {
  ...
  const handleExperiencePress = (experience: StoreExperience) => {
    router.push(`/MainCategory/${categorySlug}/experiences/${experience._id || experience.slug}` as unknown);
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.section}>
        <View style={styles.sectionHeader}>
          ...
          <Pressable onPress={() => router.push(`/MainCategory/${categorySlug}/experiences` as unknown)}>
        ...
        <View style={styles.experiencesList}>
          {experiences.filter(e => e.isFeatured).slice(0, 4).map((experience) => (
            ...
            >
              <LinearGradient
                colors={(experience.backgroundColor ? [experience.backgroundColor, experience.backgroundColor + '80'] : ['#F59E0B20', colors.brand.orange] ) as unknown}
...
const styles = StyleSheet.create({
  ...
  whyItem: { width: '47%' as unknown, alignItems: 'center', padding: 12 },
