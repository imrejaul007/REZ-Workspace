        ...
        Object.entries(grouped).map(([type, items]) => (
          <View key={type}>
            <View style={styles.suggestionGroupHeader}>
              <Ionicons
                name={(TYPE_LABELS[type]?.icon || 'search-outline') as unknown}
                size={14}
            ...
            {items.map((suggestion) => (
              <SuggestionItem
                ...
                icon={(TYPE_LABELS[type]?.icon || 'search-outline') as unknown}
  ...
  return (
    ...
    >
      <View style={styles.suggestionIconWrapper}>
        <Ionicons name={icon as unknown} size={16} color={REZ_THEME.nileBlue} />
      </View>
