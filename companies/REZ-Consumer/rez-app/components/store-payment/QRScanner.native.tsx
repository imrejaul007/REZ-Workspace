        ...
        <View style={styles.permissionContainer}>
          ...
          <Pressable style={styles.grantButton} onPress={() => {
            requestPermission?.().catch((err) => {
              logger.error('[QRScanner] Camera permission error:', err);
              onClose();
            });
          }}>
            <Text style={styles.grantButtonText}>Allow Camera Access</Text>
          </Pressable>
          {/* CONSUMER-HIGH-01 FIX: Add manual QR entry option when camera permission denied */}
          {onManualEntry && (
            <Pressable style={styles.manualEntryButton} onPress={onManualEntry}>
              <Ionicons name="keypad-outline" size={20} color={REZ_COLORS.primary} />
    ...
    <View style={styles.container}>
      ...
      <View style={styles.bottomSection}>
        ...
        <View style={styles.infoBanner}>
          <Text style={styles.infoText}>Scan unknown store QR code to pay</Text>
...
const styles = StyleSheet.create({
  ...
  /* CONSUMER-HIGH-01 FIX: Manual entry button styles */
  manualEntryButton: {
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
