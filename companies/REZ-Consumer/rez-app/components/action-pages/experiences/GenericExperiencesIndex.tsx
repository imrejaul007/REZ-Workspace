  ...
  const renderExperience = ({ item }: { item: StoreExperience }) => (
    <Pressable
      ...
      onPress={() => {
        ...
        if (eid) router.push(`/MainCategory/${slug}/experiences/${eid}` as unknown);
      }}
    >
      <LinearGradient
        colors={(item.backgroundColor && /^#[0-9A-Fa-f]{6}$/.test(item.backgroundColor)
          ? [item.backgroundColor, item.backgroundColor + '80']
          : [theme.primaryColorLight + '30', theme.primaryColorLight + '15']) as unknown}
        ...
        <FlashList
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              <Ionicons name={(theme.defaultMissionIcon || 'sparkles-outline') as unknown} size={48} color={colors.neutral[500]} />
