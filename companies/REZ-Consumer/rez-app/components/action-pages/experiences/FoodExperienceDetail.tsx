...
function StoreCard({ store, currencySymbol }: { store; currencySymbol: string }) {
...
function ExperienceDetailPage() {
  ...
  const renderStoreCard = useCallback(({ item }: { item: unknown }) => (
      ...
      {experience && (
        <LinearGradient
          colors={(experience.backgroundColor
            ...
            : [COLORS.primaryGold + '20', COLORS.primaryGold + '10']) as unknown}
        ...
        <View style={styles.bookingCTA}>
          <Pressable
            ...
            onPress={() => {
              ...
              router.push(`/MainCategory/food-dining/book-table?storeId=${firstStore._id || firstStore.id}&storeName=${encodeURIComponent(firstStore.name || '')}` as unknown);
            }}
