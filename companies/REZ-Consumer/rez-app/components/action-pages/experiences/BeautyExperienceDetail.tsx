...
function ExperienceDetailPage() {
  ...
  const fetchExperience = useCallback(async () => {
    ...
    try {
      setError(null);
      setIsLoading(true);
      const response = await apiClient.get<unknown>(`/experiences/${id}`);
  ...
  const handleBookNow = () => {
    ...
    router.push({
      pathname: '/MainCategory/beauty-wellness/book-appointment' as unknown,
      params,
    });
  };
        ...
        <View style={styles.heroContainer}>
          {heroImage && !imageError ? (
            ...
            <View style={[styles.heroImage, styles.heroPlaceholder]}>
              <Ionicons name="sparkles" size={48} color={(COLORS as unknown).pink} />
            </View>
          )}
          <LinearGradient
                ...
                {experience.whatsIncluded.map((item, index) => (
                  <View key={index} style={styles.includedItem}>
                    <View style={styles.includedIcon}>
                      <Ionicons name="checkmark" size={14} color={(COLORS as unknown).pink} />
                    </View>
              ...
              <View style={styles.highlightsList}>
                {experience.highlights.map((item, index) => (
                  <View key={index} style={styles.highlightChip}>
                    <Ionicons name="sparkles" size={12} color={(COLORS as unknown).pink} />
          ...
          {experience.salonName && (
            <View style={styles.section}>
              ...
              <Pressable
                ...
                onPress={() => {
                  if (experience.salonId) {
                    router.push(`/MainStorePage?storeId=${experience.salonId}` as unknown);
                  }
                }}
              >
                <View style={styles.salonImageContainer}>
                  {experience.salonImage ? (
                    ...
                    <View style={[styles.salonImage, styles.salonImagePlaceholder]}>
                      <Ionicons name="flower" size={20} color={(COLORS as unknown).pink} />
                    </View>
                  )}
                </View>
...
const styles = StyleSheet.create({
  ...
  retryButton: {
    ...
    paddingVertical: 10,
    borderRadius: 12,
    backgroundColor: (COLORS as unknown).pink,
  },
  ...
  price: { fontSize: 24, fontWeight: '700', color: (COLORS as unknown).pink },
  originalPrice: {
    fontSize: 16,
    color: COLORS.textSecondary,
  ...
  ctaLeft: {},
  ctaPrice: { fontSize: 20, fontWeight: '700', color: (COLORS as unknown).pink },
