...
function ExperiencesPage() {
  ...
  const renderExperience = ({ item }: { item: StoreExperience }) => (
    <Pressable
      ...
      onPress={() => { const eid = item._id || item.slug; if (eid) router.push(`/MainCategory/food-dining/experiences/${eid}` as unknown); }}
     
    >
      <LinearGradient
        colors={(item.backgroundColor && /^#[0-9A-Fa-f]{6}$/.test(item.backgroundColor)
          ? [item.backgroundColor, item.backgroundColor + '80']
          : [colors.neutral[50], colors.neutral[100]]) as unknown}
