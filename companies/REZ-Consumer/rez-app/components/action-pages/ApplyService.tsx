...
function ApplyServicePage() {
  ...
  const [selectedProvider, setSelectedProvider] = useState<unknown>(
  ...
  const handleSubmit = async () => {
    try {
      ...
      const res = await apiClient.post<unknown>('/financial-services/leads', {
  ...
  const getServiceTags = (store): string => {
  ...
  if (step === 'provider') {
    const renderProviderCard = ({ item: store }: { item: unknown }) => {
        ...
        <Pressable style={styles.storeCard} onPress={() => handleSelectProvider(store)}>
          ...
          <View style={styles.storeInfo}>
            ...
            <View style={styles.storeMetaRow}>
              <View style={styles.storeRating}><Ionicons name="star" size={12} color={(COLORS as unknown).goldDark} /><Text style={styles.storeRatingText}>{rating}</Text><Text style={styles.storeReviewCount}>({reviewCount})</Text></View>
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        {isLoading ? (
          <View style={styles.loadingContainer}><ActivityIndicator size="large" color={(COLORS as unknown).teal} /><Text style={styles.loadingText}>Finding providers...</Text></View>
        ...
        <ScrollView contentContainerStyle={styles.confirmContent}>
          <View style={styles.confirmIconWrap}>
            <LinearGradient colors={[COLORS.tealLight, '#CCFBF1']} style={styles.confirmIconGradient}>
              <Ionicons name="checkmark-circle" size={64} color={(COLORS as unknown).teal} />
            </LinearGradient>
          </View>
          ...
          <View style={styles.confirmCard}>
            <View style={styles.confirmRow}>
              <View style={[styles.confirmRowIcon, { backgroundColor: 'rgba(20,184,166,0.1)' }]}><Ionicons name="business" size={16} color={(COLORS as unknown).teal} /></View>
          ...
          <View style={styles.confirmNote}><Ionicons name="information-circle" size={16} color={(COLORS as unknown).teal} /><Text style={styles.confirmNoteText}>A representative will contact you within 24 hours. Earn bonus coins when your application is approved!</Text></View>
          <Pressable style={styles.doneBtn} onPress={() => router.back()}><Text style={styles.doneBtnText}>Back to Financial</Text></Pressable>
          <Pressable style={styles.viewBookingsBtn} onPress={() => router.push('/orders' as unknown)}><Text style={styles.viewBookingsBtnText}>View My Applications</Text></Pressable>
        </ScrollView>
      </SafeAreaView>
    );
  }
          ...
          {DOC_TYPES.map(doc => {
            ...
            return (
              <Pressable key={doc.id} style={[styles.docCard, isUploaded ? styles.docCardUploaded : null]} onPress={async () => {
                ...
                <Ionicons name={doc.icon as unknown} size={24} color={isUploaded ? (COLORS as unknown).teal : COLORS.textSecondary} />
      ...
      <ScrollView contentContainerStyle={styles.formContent} showsVerticalScrollIndicator={false}>
        {selectedProvider?.name && (
          <View style={styles.storePreview}>
            <LinearGradient colors={[(COLORS as unknown).teal, COLORS.tealDark]} style={styles.storePreviewGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
        ...
        <View style={styles.bonusNote}>
          <View style={styles.bonusIconWrap}><Ionicons name="wallet-outline" size={14} color={(COLORS as unknown).teal} /></View>
...
const styles = StyleSheet.create({
  ...
  typeCard: { width: '47%' as unknown, padding: 20, borderRadius: 16, backgroundColor: COLORS.white, alignItems: 'center', elevation: 1, shadowColor: '#000', shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.05, shadowRadius: 3 },
  ...
  storeCashbackBadge: { position: 'absolute', bottom: -4, right: -4, backgroundColor: (COLORS as unknown).teal, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 2 },
  ...
  verifiedBadgeText: { fontSize: 9, fontWeight: '600', color: (COLORS as unknown).teal },
  ...
  docCardUploaded: { borderColor: (COLORS as unknown).teal, backgroundColor: COLORS.tealLight },
  docLabel: { fontSize: 14, fontWeight: '500', color: COLORS.textPrimary },
  docStatus: { fontSize: 12, color: COLORS.textSecondary, marginTop: 2 },
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: (COLORS as unknown).teal, borderRadius: 16, paddingVertical: 16, marginTop: 20 },
  submitBtnText: { fontSize: 16, fontWeight: '600', color: COLORS.white },
  skipBtn: { alignItems: 'center', paddingVertical: 12, marginTop: 8 },
  skipBtnText: { fontSize: 14, color: (COLORS as unknown).teal, fontWeight: '500' },
  ...
  doneBtn: { width: '100%', paddingVertical: 16, backgroundColor: (COLORS as unknown).teal, borderRadius: 16, alignItems: 'center' },
  ...
  viewBookingsBtnText: { fontSize: 14, fontWeight: '600', color: (COLORS as unknown).teal },
});
