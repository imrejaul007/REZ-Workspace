...
function BookTicketsPage() {
  ...
  const [selectedStore, setSelectedStore] = useState<unknown>(
  ...
  const fetchVenues = useCallback(async () => {
    try {
      ...
      if (res.success && res.data) {
        ...
        const bookable = allStores.filter((s) =>
  ...
  const fetchAvailability = useCallback(async (storeId: string, date: Date) => {
    try {
      ...
      if (res.success && res.data?.slots) {
        const slots: TimeSlot[] = res.data.slots.map((s) => ({
  ...
  const handleSelectVenue = (store) => { setSelectedStore(store); setStep('details'); };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.success) {
        if (!isMounted()) return;
        setBookingId(res.data?.id || (res.data as unknown)?._id || null);
        setBookingNumber((res.data as unknown)?.appointmentNumber || (res.data as unknown)?.bookingNumber || null);
        setStep('confirm');
      } else {
  ...
  if (step === 'venue') {
    const renderStoreCard = ({ item: store }: { item: unknown }) => {
          ...
          <View style={styles.storeInfo}>
            ...
            <View style={styles.storeMetaRow}>
              <View style={styles.storeRating}>
                <Ionicons name="star" size={12} color={(COLORS as unknown).goldDark} />
      ...
      <SafeAreaView style={styles.container}>
        ...
        {isLoading ? (
          <View style={styles.loadingContainer}>
            <ActivityIndicator size="large" color={(COLORS as unknown).violet} />
        ...
        <ScrollView contentContainerStyle={styles.confirmContent}>
          <View style={styles.confirmIconWrap}>
            <LinearGradient colors={[COLORS.violetLight, colors.tint.purple]} style={styles.confirmIconGradient}>
              <Ionicons name="checkmark-circle" size={64} color={(COLORS as unknown).violet} />
            </LinearGradient>
          </View>
          ...
          <View style={styles.confirmCard}>
            <View style={styles.confirmRow}>
              <View style={[styles.confirmRowIcon, { backgroundColor: 'rgba(139,92,246,0.1)' }]}>
                <Ionicons name="location" size={16} color={(COLORS as unknown).violet} />
              </View>
          ...
          <View style={styles.confirmNote}>
            <Ionicons name="information-circle" size={16} color={(COLORS as unknown).violet} />
          ...
          <Pressable style={styles.viewBookingsBtn} onPress={() => router.push('/orders' as unknown)}>
      ...
      <ScrollView contentContainerStyle={styles.formContent} showsVerticalScrollIndicator={false}>
        {selectedStore?.name && (
          <View style={styles.storePreview}>
            <LinearGradient colors={[(COLORS as unknown).violet, COLORS.violetDark]} style={styles.storePreviewGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
        ...
        <View style={styles.formLabelRow}>
          ...
          {isLoadingAvailability ? <ActivityIndicator size="small" color={(COLORS as unknown).violet} /> : <Text style={styles.availableLabel}>{availableCount} slots available</Text>}
        </View>
        {isLoadingAvailability ? (
          <View style={styles.timeLoadingWrap}>
            <ActivityIndicator size="small" color={(COLORS as unknown).violet} />
        ...
        <View style={styles.bonusNote}>
          <View style={styles.bonusIconWrap}><Ionicons name="wallet-outline" size={14} color={(COLORS as unknown).violet} /></View>
...
const styles = StyleSheet.create({
  ...
  storeCashbackBadge: { position: 'absolute', bottom: -4, right: -4, backgroundColor: (COLORS as unknown).violet, borderRadius: 8, paddingHorizontal: 5, paddingVertical: 2 },
  ...
  verifiedBadgeText: { fontSize: 9, fontWeight: '600', color: (COLORS as unknown).violet },
  ...
  serviceChipActive: { backgroundColor: COLORS.violetLight, borderColor: (COLORS as unknown).violet },
  ...
  serviceDurationActive: { color: (COLORS as unknown).violet },
  ...
  dateChipActive: { backgroundColor: (COLORS as unknown).violet, borderColor: (COLORS as unknown).violet },
  ...
  timeChipActive: { backgroundColor: (COLORS as unknown).violet, borderColor: (COLORS as unknown).violet },
  ...
  seatChipActive: { backgroundColor: COLORS.violetLight, borderColor: (COLORS as unknown).violet },
  ...
  seatPriceActive: { color: (COLORS as unknown).violet },
  ...
  submitBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: (COLORS as unknown).violet, borderRadius: 16, paddingVertical: 16, marginTop: 20 },
  ...
  doneBtn: { width: '100%', paddingVertical: 16, backgroundColor: (COLORS as unknown).violet, borderRadius: 16, alignItems: 'center' },
  ...
  viewBookingsBtnText: { fontSize: 14, fontWeight: '600', color: (COLORS as unknown).violet },
});
