...
function CompareDevicesPage() {
  ...
  const [selectedDevices, setSelectedDevices] = useState<unknown[]>([]);
  ...
  const getDeviceSpec = (device, specKey: string): string => {
    switch (specKey) {
      case 'price':
                ...
                return (
                  <View key={device._id || device.id || i} style={styles.deviceCol}>
                    ...
                    <Pressable
                      ...
                      onPress={() => router.push(`/product-page?id=${device._id || device.id}` as unknown)}
                    >
              ...
              <View style={styles.specRow}>
                <View style={styles.specLabelCol}>
                  <View style={styles.specLabelInner}>
                    <Ionicons name={spec.icon as unknown} size={16} color={COLORS.blue} />
  ...
  const renderProductCard = ({ item: product }: { item: unknown }) => {
