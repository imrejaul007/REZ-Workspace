...
const transformToSelectedProduct = (p): SelectedProduct => ({
  _id: p._id || p.id,
  name: p.name || 'Product',
  brand: p.brand,
  images: p.images || [],
...
function ComparePage() {
  ...
  const performSearch = useCallback(async (query: string) => {
    ...
    setIsSearching(true);
    try {
      const res = await apiClient.get<unknown>('/products/search', { q: query, limit: 20 });
      if (res.success && res.data) {
        const products = Array.isArray(res.data) ? res.data : res.data?.products || [];
        const selectedIds = new Set(selectedProducts.map(p => p._id));
        if (!isMounted()) return;
        setSearchResults(products.filter((p) => !selectedIds.has(p._id || p.id)));
      }
    } catch (err) {
      errorReporter.captureError(
  ...
  // Search result item
  const renderSearchResult = ({ item }: { item: unknown }) => {
    ...
    // Action row
    {
      ...
      renderCell: (p) => (
        <Pressable
          ...
          onPress={() => router.push(`/product-page?productId=${p._id}` as unknown)}
        >
