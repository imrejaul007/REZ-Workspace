...
function EventBookingModal({
  ...
  const [bookingResultData, setBookingResultData] = useState<unknown>(null);
  ...
  const validateAge = (age: number | undefined): string | undefined => {
    ...
    const minAge = (event as unknown)?.minAge;
  ...
  const handleBookingSubmit = async () => {
    ...
    let bookingResult: unknown = null;
    try {
      if (!isMounted()) return;
      ...
      onClose();
      router.push(
        `/payment-razorpay?bookingId=${bookingId}&bookingType=event&amount=${event.price.amount}&currency=${event.price.currency || 'INR'}` as unknown
      );
    } catch (error) {
      
      // Track payment failure
      const bookingId = (bookingResult as unknown)?.booking?.id || (bookingResult as unknown)?.booking?._id || '';
      if (event && bookingId) {
              ...
              <View style={styles.inputGroup}>
                ...
                <TextInput
                  ...
                  onChangeText={(value) => {
                    ...
                    handleInputChange('age', numValue as unknown);
                  }}
            ...
            <View style={styles.successActionsRow}>
              <Pressable
                ...
                onPress={() => {
                  ...
                  router.push('/my-events' as unknown);
                }}
              >
