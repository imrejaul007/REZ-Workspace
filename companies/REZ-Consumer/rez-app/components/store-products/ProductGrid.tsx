...
function ProductGrid({
  ...
  const handleQuickView = useCallback((product: ProductItem) => {
    ...
    analyticsService.track('product_quick_view_opened', { productId: (product as unknown)._id || product.id, productName: product.name, storeId });
  }, [storeId]);

  // ─── Render item ──────────────────────────────────────────────────────────

  const renderProduct = useCallback(({ item, index }: { item: ProductItem; index: number }) => {
    const productId = (item as unknown)._id || item.id;
      ...
      <View style={styles.emptyContainer}>
        ...
        <ThemedText style={styles.emptyText}>
          {hasFilters
            ...
            : "This store doesn't have unknown products yet."}
        </ThemedText>
        {hasFilters && (
          <Pressable
    ...
    <View style={styles.content}>
      {loading && products.length === 0 ? (
        ...
        <FlashList
          ...
          keyExtractor={(item, index) => (item as unknown)._id || item.id || `product-${index}`}
