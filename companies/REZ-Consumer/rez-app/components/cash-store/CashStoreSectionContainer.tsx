...
const CashStoreSectionContainer: React.FC<CashStoreSectionContainerProps> = ({
  ...
  const handleGiftCardPress = useCallback(
    (brand: GiftCardBrand) => {
      router.push(`/vouchers/brand/${brand.id}` as unknown);
    },
    [router]
  );
  ...
  const handleHighCashbackPress = useCallback(
    async (deal: HighCashbackDeal) => {
      ...
      if (deal.externalUrl) {
        ...
        router.push(`/cash-store/brands` as unknown);
      }
    },
    [router]
  );
  ...
  const handleTravelDealPress = useCallback(
    (deal: TravelDeal) => {
      ...
      router.push(`/cash-store/brands?category=${deal.category}` as unknown);
    },
    [router]
  );
  ...
  const handleActivityPress = useCallback(
    (activity: CashbackActivity) => {
      router.push(`/account/cashback` as unknown);
    },
    [router]
  );

  const handleLearnMore = useCallback(() => {
    router.push(BRAND.HOW_CASH_STORE_WORKS_ROUTE as unknown);
  }, [router]);
  ...
  const handleQuickActionPress = useCallback(
    (actionId: string) => {
      switch (actionId) {
        case 'buy-coupons':
          router.push('/cash-store/buy-coupons' as unknown);
          break;
        case 'extra-coins':
          router.push('/cash-store/extra-coins' as unknown);
          break;
        case 'trending':
          router.push('/cash-store/trending' as unknown);
          break;
        case 'track-cashback':
          router.push('/account/cashback' as unknown);
          break;
  ...
  const handleViewAllCoupons = useCallback(() => {
    router.push('/cash-store/coupons' as unknown);
  }, [router]);

  const handleViewAllHighCashback = useCallback(() => {
    router.push('/cash-store/brands?filter=high-cashback' as unknown);
  }, [router]);

  const handleViewAllTravel = useCallback(() => {
    router.push('/cash-store/brands?category=travel' as unknown);
  }, [router]);
