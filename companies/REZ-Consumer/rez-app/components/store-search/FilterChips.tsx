...
const FilterChips: React.FC<FilterChipsProps & { parentCategorySlug?: string }> = ({
  ...
  const handleGenderSelect = (genderId: string) => {
    const newGenders = filters.gender.includes(genderId as unknown)
      ? filters.gender.filter(g => g !== genderId)
      : [...filters.gender, genderId as unknown];
  ...
  const renderGenderOption = useCallback(({ item }: { item: { id: string; label: string; icon: string; color: string } }) => (
    <Pressable
      style={[
        styles.genderOption,
        filters.gender.includes(item.id as unknown) && styles.genderOptionSelected
      ]}
      onPress={() => handleGenderSelect(item.id)}
      accessibilityRole="checkbox"
      accessibilityLabel={item.label}
      accessibilityHint={filters.gender.includes(item.id as unknown) ? 'Double tap to deselect' : 'Double tap to select'}
      accessibilityState={{ checked: filters.gender.includes(item.id as unknown) }}
    >
      <Ionicons
        name={item.icon as unknown}
        size={22}
        color={filters.gender.includes(item.id as unknown) ? COLORS.WHITE : item.color}
      />
      <ThemedText style={[
        styles.genderOptionText,
        filters.gender.includes(item.id as unknown) && styles.genderOptionTextSelected
      ]}>
        {item.label}
      </ThemedText>
      {filters.gender.includes(item.id as unknown) && (
        <Ionicons
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {showFashion && (
          ...
          >
            <Ionicons
              name={FILTER_CATEGORIES.FASHION.icon as unknown}
              size={16}
        ...
        {showGender && (
          ...
          >
            <Ionicons
              name={FILTER_CATEGORIES.GENDER.icon as unknown}
              size={16}
        ...
        {/* Clear Filters Button (shows when unknown visible filter is active) */}
