...
interface QuizGameProps {
  ...
  category?: string;
  onGameComplete?: (score: number, coinsEarned: number, tournamentUpdate?) => void;
}
...
function QuizGame({ difficulty, category, onGameComplete }: QuizGameProps) {
  ...
  const [lastTournamentUpdate, setLastTournamentUpdate] = useState<unknown>(null);
  ...
  const submitAnswer = async (answerIndex: number) => {
    ...
    try {
      ...
      if (response.success && response.data) {
        ...
        if ((response.data as unknown).tournamentUpdate) {
          if (!isMounted()) return;
          setLastTournamentUpdate((response.data as unknown).tournamentUpdate);
        }
