...
function ScratchCardGame({
  ...
  const checkEligibility = async () => {
    try {
      ...
      const response = await (gamificationAPI as unknown).canCreateScratchCard();
  ...
  const createCard = async () => {
    ...
    try {
      setIsLoading(true);
      const response = await (gamificationAPI as unknown).createScratchCard();
  ...
  const handleScratch = async () => {
    ...
    try {
      ...
      const response = await (gamificationAPI as unknown).scratchCard(cardId);
        ...
        {prize && (
          ...
          >
            <View style={[styles.prizeIcon, { backgroundColor: prize.color }]}>
              <Ionicons name={prize.icon as unknown} size={48} color={colors.background.primary} />
            </View>
