...
let sharedWebLocation: unknown = null;
let sharedWebLocationPromise: Promise<unknown> | null = null;
let isInitializing = false;
const locationListeners = new Set<(location) => void>();

// Helper to notify all instances when location changes
const notifyLocationChange = (location) => {
...
function LocationDisplay({
  ...
  const [webLocation, setWebLocation] = useState<unknown>(sharedWebLocation);
  ...
  const initializeWebLocation = async () => {
    ...
    sharedWebLocationPromise = (async () => {
      try {
        ...
        if (!isMounted()) return;
        setWebError(`Location error: ${(error as unknown)?.message || 'Unknown error'}`);
        return null;
      } finally {
        if (!isMounted()) return;
        setWebLoading(false);
  ...
  const getWebLocationText = (webLocation) => {
