  ...
  const fetchLocalData = useCallback(async () => {
    try {
      ...
      const recentStores = recent
        .filter((item) => item.type === 'store' || item.storeId || item._id)
        .slice(0, 8);
      if (!isMounted()) return;
      setRecentlyViewed(recentStores);

      // Favorites already sorted by asyncStorageService
      setFavoriteStores(favorites.filter((f) => f.isFavorited).slice(0, 8));
    } catch {
      // Silent fail
    }
  ...
  const fetchNearby = useCallback(async () => {
    ...
    try {
      ...
      if (response.success && response.data) {
        const stores = Array.isArray(response.data) ? response.data : (response.data as unknown)?.stores || [];
        if (!isMounted()) return;
      ...
      {hasFavorites && (
        <View style={styles.section}>
          <View style={styles.sectionHeader}>
            ...
            <Pressable
              onPress={() => router.push('/wishlist' as unknown)}
