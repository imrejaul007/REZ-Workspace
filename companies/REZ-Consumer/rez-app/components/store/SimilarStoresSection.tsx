...
interface SimilarStoresSectionProps {
  ...
  onStorePress?: (storeId: string, storeData) => void;
  limit?: number;
}
  ...
  const fetchSimilarStores = useCallback(async () => {
    try {
      ...
      if (response?.data) {
        ...
        let filteredStores = (response.data as unknown[]).filter(
          (store: Store) =>
        ...
        if (!isMounted()) return;
        setStores(filteredStores as unknown);
      } else {
        setStores([] as unknown);
      }
    } catch (err) {
      if (!isMounted()) return;
