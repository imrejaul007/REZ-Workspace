 ...
 * Sliding Tabs
 *
 * Tab navigation with REZ design palette
 */
...
function SlidingTabs({
  ...
  const underlineStyle = useAnimatedStyle(() => {
    const inputRange = tabs.map((_, index: number) => index * tabWidth);
    const outputRange = tabs.map((_, index: number) => (index * tabWidth) + (tabWidth * 0.2));
    return {
