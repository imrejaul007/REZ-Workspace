...
function CartItem({
  ...
  const handlePress = () => {
    scaleAnim.value = withSequence(withTiming(0.98, { duration: 100 }), withTiming(1, { duration: 100 }));
      ...
      const productId = (item as unknown).productId || item.id;
      if (productId) {
        ...
        const cardData = {
          ...
          store: (item as unknown).store,
          discount: (item as unknown).discount,
        };
        ...
        <View style={styles.infoContainer}>
          <View style={styles.topRow}>
            <View style={styles.nameAndPriceContainer}>
              ...
              {(item as unknown).isEvent && (item as unknown).metadata && (
                <View style={styles.eventDetails}>
                  {(item as unknown).metadata.slotTime && (
                  ...
                  {(item as unknown).metadata.location && (
                    <View style={styles.eventDetailRow}>
              ...
              {(() => {
                const lockedQty = (item as unknown).lockedQuantity || 0;
