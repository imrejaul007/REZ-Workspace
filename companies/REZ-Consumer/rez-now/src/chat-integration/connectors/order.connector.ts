...
export class OrderConnector {
  ...
  async addToCart(params: {
    ...
    try {
      ...
      return {
        ...
        items: (data.items || []).map((item: { productId?: string; name?: string; quantity?: number; price?: number; total?: number; notes?: string }) => ({
      ...
      logger.error('[OrderConnector] Add to cart error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async getCart(userId: string): Promise<AddToCartResult | null> {
    try {
      ...
      logger.error('[OrderConnector] Get cart error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async placeOrder(params: PlaceOrderParams): Promise<OrderResult | null> {
    try {
      ...
      logger.error('[OrderConnector] Place order error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async getOrderStatus(orderId: string): Promise<OrderDetails | null> {
    try {
      ...
      return {
        ...
        timeline: (data.timeline || data.statusHistory || []).map((t: { status?: string; timestamp?: string; createdAt?: string; message?: string }) => ({
          status: t.status,
      ...
      logger.error('[OrderConnector] Get order status error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async cancelOrder(orderId: string, reason?: string): Promise<{ success: boolean; message: string; refundAmount?: number }> {
    try {
      ...
      logger.error('[OrderConnector] Cancel order error:', errorMessage);
      return {
        success: false,
        message: error instanceof Error && 'response' in error ? String((error as unknown as { response?: { data?: { message?: string } } }).response?.data?.message || 'Unable to cancel order') : 'Unable to cancel order',
      };
    }
  }
  ...
  async getOrderHistory(userId: string, limit?: number): Promise<OrderHistoryItem[]> {
    try {
      ...
      return orders.map((o: { id?: string; orderRef?: string; orderNumber?: string; createdAt?: string; date?: string; merchant?: { name?: string }; merchantName?: string; status?: string; total?: number; items?: unknown[] }) => ({
        orderId: o.id,
      ...
      logger.error('[OrderConnector] Get order history error:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }
  ...
  async reorder(userId: string, orderId: string, deliveryAddress?: string): Promise<OrderResult | null> {
    try {
      ...
      logger.error('[OrderConnector] Reorder error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
