...
export class LoyaltyConnector {
  ...
  async getProfile(userId: string): Promise<LoyaltyProfile | null> {
    try {
      ...
      return {
        ...
        benefits: data.benefits || this.getTierBenefitsList(data.tier || 'bronze'),
      ...
      logger.error('[LoyaltyConnector] Get profile error:', error instanceof Error ? error.message : String(error));
      return {
        userId,
        tier: 'bronze',
        points: 0,
        ...
        benefits: this.getTierBenefitsList('bronze'),
        nextTier: {
          name: 'Silver',
          pointsRequired: 1000,
  ...
  async getExpiringRewards(userId: string, daysAhead?: number): Promise<ExpiringReward[]> {
    try {
      ...
      return rewards.map((r: { id?: string; _id?: string; type?: string; title?: string; name?: string; description?: string; value?: number; discount?: number; expiresAt?: string; expiryDate?: string; daysUntilExpiry?: number; minOrderValue?: unknown; merchantId?: string; merchantName?: string }) => ({
        id: r.id || r._id,
        ...
        daysUntilExpiry: r.daysUntilExpiry ?? this.calculateDaysUntil(r.expiresAt || r.expiryDate || ''),
        minOrderValue: r.minOrderValue,
        merchantId: r.merchantId,
        merchantName: r.merchantName,
      }));
    } catch (error) {
      logger.error('[LoyaltyConnector] Get rewards error:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }
