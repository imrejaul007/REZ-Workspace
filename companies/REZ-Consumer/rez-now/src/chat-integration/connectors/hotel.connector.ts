...
interface RawHotelSearchItem {
  ...
  rating?: number;
  reviewCount?: number;
  roomTypes?: Array<{ baseRate?: number }>;
  minRate?: number;
  baseRate?: number;
  images?: string[];
  amenities?: string[];
...
interface RawBooking {
  ...
  hotel?: { id?: string; name?: string; address?: string; phone?: string };
  roomType?: { id?: string; name?: string; images?: string[] };
  checkinDate?: string;
  checkoutDate?: string;
  numNights?: number;
  numRooms?: number;
...
export class HotelOTAConnector {
  ...
  async search(params: HotelSearchParams): Promise<HotelSearchResult> {
    try {
      ...
      return {
        hotels: (data.hotels || data.results || []).map((h: RawHotelSearchItem) => ({
          id: h.id,
          name: h.name,
          city: h.city,
          category: h.category,
      ...
      logger.error('[HotelOTAConnector] Search error:', error instanceof Error ? error.message : String(error));
      return {
        hotels: [],
        total: 0,
        page: 1,
  ...
  async getHotel(hotelId: string): Promise<HotelDetails | null> {
    try {
      ...
      return {
        ...
        roomTypes: (h.roomTypes || []).map((r: RawRoomType) => ({
          id: r.id,
          name: r.name,
          description: r.description || '',
          maxOccupancy: r.maxOccupancy || 2,
          bedType: r.bedType || 'Standard',
          baseRate: r.baseRate ?? (r.baseRatePaise ? r.baseRatePaise / 100 : undefined),
          images: r.images || [],
          amenities: r.amenities || [],
        })),
      };
    } catch (error) {
      logger.error('[HotelOTAConnector] Get hotel error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async checkAvailability(params: {
    ...
    try {
      ...
      logger.error('[HotelOTAConnector] Check availability error:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }
  ...
  async holdBooking(params: {
    ...
    try {
      ...
      logger.error('[HotelOTAConnector] Hold booking error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async confirmBooking(params: {
    ...
    try {
      ...
      logger.error('[HotelOTAConnector] Confirm booking error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async getBooking(bookingId: string): Promise<BookingDetails | null> {
    try {
      ...
      return {
        ...
        timeline: (data.timeline || []).map((t: RawTimeline) => ({
          status: t.status,
      ...
      logger.error('[HotelOTAConnector] Get booking error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async cancelBooking(params: {
    ...
    try {
      ...
      logger.error('[HotelOTAConnector] Cancel booking error:', error instanceof Error ? error.message : String(error));
      return {
        success: false,
        message: error instanceof Error && 'response' in error ? String((error as unknown as { response?: { data?: { message?: string } } }).response?.data?.message || 'Unable to cancel booking') : 'Unable to cancel booking',
      };
    }
  }
  ...
  async getGuestOrders(userId: string, limit?: number): Promise<BookingDetails[]> {
    try {
      ...
      return bookings.map((b: RawBooking) => ({
        bookingId: b.id,
      ...
      logger.error('[HotelOTAConnector] Get guest orders error:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }
}
