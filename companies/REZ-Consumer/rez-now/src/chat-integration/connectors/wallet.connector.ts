...
export class WalletConnector {
  ...
  constructor(config?: { baseUrl?: string; apiKey?: string; logger?: Logger }) {
    const baseURL = config?.baseUrl || process.env.REZ_WALLET_SERVICE_URL || 'https://rez-wallet-service.onrender.com';
  ...
  async getBalance(userId: string): Promise<WalletBalance | null> {
    try {
      ...
      this.log.error('[WalletConnector] Get balance error:', error instanceof Error ? error.message : String(error));
      return {
        userId,
        cashBalance: 0,
        coinBalance: 0,
  ...
  async calculateCheckout(params: {
    ...
    try {
      ...
      const coinsUsable = Math.min(maxCoinDiscount, maxCoinsValueable);
      ...
      this.log.error('[WalletConnector] Calculate checkout error:', error instanceof Error ? error.message : String(error));
      return null;
    }
  }
  ...
  async getTransactions(userId: string, limit?: number): Promise<CoinTransaction[]> {
    try {
      ...
      return transactions.map((t: { id?: string; _id?: string; type?: string; amount?: number; balance?: number; reason?: string; description?: string; timestamp?: string; createdAt?: string; orderId?: string; relatedOrderId?: string }) => ({
        id: t.id || t._id,
        type: t.type,
        amount: t.amount,
        balance: t.balance,
      ...
      this.log.error('[WalletConnector] Get transactions error:', error instanceof Error ? error.message : String(error));
      return [];
    }
  }
}
