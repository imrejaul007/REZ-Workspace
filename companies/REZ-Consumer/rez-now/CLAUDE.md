# REZ NOW - Claude Code Configuration

## What is REZ NOW?

**REZ NOW** is a **QR-powered merchant operating system and customer engagement platform**.
...
| **Payments** | UPI, Razorpay, NFC tap-to-pay, manual amount, QR pay |
| **Loyalty** | REZ Coins, Bronze/Silver/Gold/Platinum tiers, reorder |
| **Bill Split** | Split by total, split by item, GST division |
| **CRM** | Customer segments (VIP, at-risk, new, repeat), lifetime value tracking |
| **Automation** | Dormant customer, birthday, happy hour, weather, low footfall triggers |
| **Hotels** | Room hub, service requests, housekeeping, minibar, express checkout |
| **AI** | Dish recommendations, weather suggestions, taste profiles, AI chat |
| **Group** | Group ordering, kitchen display, waiter calls |
...
| Component | Location |
|-----------|----------|
| Store Pages | `app/[storeSlug]/` |
| QR Scanner | `components/web-qr-scanner/` |
| Menu Components | `components/menu/` |
| Checkout | `components/checkout/` |
| Loyalty | `components/loyalty/` |
| Merchant CRM | `components/merchant/crm/` |
| Room Hub | `components/room/` |
| AI Services | `lib/services/` |
...
- `animate-slide-in-right` - Slide in
...
|---------|-------|----------|
| **ID Generation** | `crypto.randomUUID()` | All ID generation |
| **String IDs** | `substring()` not `substr()` | String manipulation |
| **Browser APIs** | SSR guard via `typeof window !== 'undefined'` | Client-side code |
| **Error Handling** | Always use `logger.error/warn` | All catch blocks |
| **Fetch Calls** | Always add timeout via `AbortSignal.timeout()` | External APIs |
| **LocalStorage** | Always wrap in try-catch | Browser storage |
| **Type Safety** | Validate at boundaries | Input/output |
...
| Server-side | Without prefix |
| API URLs | Provide fallback defaults |
...
// Cache with server/client separation
function getCacheStore() {
  if (typeof window !== 'undefined') {
    return globalCache; // client
  }
  return global.__kbCache ??= {...}; // server
}
```
...
See [CHANGELOG.md](CHANGELOG.md) for code-level fixes and improvements.
