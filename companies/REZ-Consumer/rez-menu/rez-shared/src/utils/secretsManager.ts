...
export class SecretsManager {
  ...
  async logAccess(secretName: string, userId?: string): Promise<void> {
    ...
    if (this.redis) {
      try {
        ...
        logger.warn('[SecretsManager] Failed to log secret access:', error instanceof Error ? error.message : String(error));
      }
    }

    // Also log to console/logger
    logger.info('[SecretsManager] Secret accessed:', logEntry);
  }
  ...
  private async getFromAws(secretName: string): Promise<string | undefined> {
    ...
    try {
      ...
      logger.error(`[SecretsManager] Failed to get AWS secret "${secretName}":`, error instanceof Error ? error.message : String(error));
      return undefined;
    }
  }
  ...
  private async getFromVault(secretName: string): Promise<string | undefined> {
    ...
    try {
      ...
      logger.error(`[SecretsManager] Failed to get Vault secret "${secretName}":`, error instanceof Error ? error.message : String(error));
      return undefined;
    }
  }
...
export async function auditSecrets(
  ...
  logger.info('[SecretsManager] Auditing required secrets...');
  ...
  if (missing.length > 0) {
    logger.error('[SecretsManager] Missing secrets:', missing.join(', '));
    throw new Error(`Missing required secrets: ${missing.join(', ')}`);
  }
  ...
  logger.info(`[SecretsManager] All ${configured.length} required secrets configured`);
}
...
export function scanForHardcodedSecrets(): void {
  ...
  if (Object.keys(found).length > 0) {
    logger.warn('[SecretsManager] Potential hardcoded secrets detected:', JSON.stringify(found));
