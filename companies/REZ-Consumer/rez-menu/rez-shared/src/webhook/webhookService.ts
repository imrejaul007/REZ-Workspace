...
export class WebhookService {
   ...
   */
  async trigger(event: WebhookEventType, data, merchantId?: string): Promise<void> {
    // Find all webhooks subscribed to this event
    let query: unknown = { events: event, isActive: true };
    if (merchantId) {
   ...
   */
  async deliver(webhookId: string, event: string, payload, attempt: number = 1): Promise<boolean> {
    try {
...
export function setupWebhookProcessor(jobQueue: JobQueue, webhookService: WebhookService) {
  jobQueue.process(async (job: Job<unknown, unknown, string>) => {
