...
| `rezapp/rez-master/app.config.js` | Registers `menu.rez.money` as associated domain (iOS) and intent filter (Android) |
| `rezapp/rez-master/app/_layout.tsx` | Handles incoming `menu.rez.money/<slug>` URL → looks up store → navigates to Pay In Store |
