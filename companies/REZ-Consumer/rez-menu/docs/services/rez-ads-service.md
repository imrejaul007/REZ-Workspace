...
**Architecture position**: Standalone HTTP API service sitting alongside rez-backend. Merchants interact via the rez-merchant dashboard. The consumer REZ app (rez-master) calls the serve endpoints to display ads at specific placements. REZ admin panel calls the admin endpoints for moderation. The service does not integrate with BullMQ — all operations are synchronous HTTP.
