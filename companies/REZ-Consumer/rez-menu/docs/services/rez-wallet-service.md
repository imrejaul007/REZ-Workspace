...
- Welcome-coin issuance (50 REZ coins on first registration)
...
```json
{
  ...
  "data": {
    ...
    "pendingCashback": 50,
    "coins": [
      { "type": "rez", "amount": 1200, "isActive": true }
    ],
    "brandedCoins": [],
    "savingsInsights": {
      "totalSaved": 800,
...
**Query params:** `page=1`, `limit=20` (max 100), `coinType=rez`
...
Credits 50 REZ coins with idempotency key `welcome_<userId>` — safe to call multiple times; only credits once.
...
```
`coinType` accepts `rez`, `prive`, `branded`, `promo`.
...
| `coins` | array | Per-type coin buckets (`rez`/`rez`, `prive`, `branded`, `promo`) |
...
| `coinType` | string | `rez`, `prive`, `branded`, `promo` |
| `amount` | number | |
...
| `amount` | number | |
| `coinType` | string | (`prive` is mapped to `rez` in the ledger) |
