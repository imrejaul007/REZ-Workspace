...
**Fix:**
1. Run migration: add `balanceBefore: 0, balanceAfter: balance, coinType: 'rez'` to all backend-written docs where these fields are absent
...
**File:** `rezapp/rez-master/services/authApi.ts:~480`
...
**Fix:** Update all paths in `rezapp/rez-master/services/loyaltyApi.ts` to match backend routes.
**File:** `rezapp/rez-master/services/loyaltyApi.ts` (all endpoints)
...
**Fix:** Update `rezapp/rez-master/services/notificationsApi.ts` Notification interface:
```typescript
...
**Fix:** Update `rezapp/rez-master/services/earningsNotificationService.ts:87` to use `/notifications/register-token`
...
**Files:**
- `rezapp/rez-master/contexts/AuthContext.tsx:~467`
