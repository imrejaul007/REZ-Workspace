...
async function up(client) {
  ...
  logger.info(`  Orders with string status: ${stringStatus}`);
  logger.info(`  Orders with subdoc status: ${subdocStatus}`);
  logger.info(`  Orders with payment but no status: ${missingStatus}`);
  logger.info(`  Total orders with payment: ${await orders.countDocuments({ payment: { $exists: true } })}`);
  ...
  if (stringSamples.length > 0) {
    ...
    stringSamples.forEach(s => logger.info(`    ${s.orderId}: "${s.status}"`));
  }
  ...
  logger.info(`\n=== MIGRATION COMPLETE ===`);
  logger.info(`  Total normalized: ${totalUpdated}`);

  // Verify
  const stillString = await orders.countDocuments({ 'payment.status': { $type: 'string' } });
  logger.info(`  Remaining string statuses: ${stillString}`);
  if (stillString > 0) {
    logger.info(`  WARNING: ${stillString} orders still have string status.`);
  }
}
...
async function down(client) {
  ...
  if (DRY_RUN) {
    ...
    logger.info(`[DRY RUN] Orders to rollback: ${count}`);
...
async function main() {
  ...
  try {
    ...
    else { logger.error('Usage: node 04_SD05_...js [up|down]'); process.exit(1); }
  } finally {
    await client.close();
  }
}
