...
async function up(client) {
  ...
  if (subdocStatuses.length === 0) {
    logger.info('    (no subdoc status values found)');
  } else {
  ...
  for (const [legacy, canonical] of Object.entries(LEGACY_TO_CANONICAL)) {
    ...
    if (result.modifiedCount > 0) {
      logger.info(`  "${legacy}" → "${canonical}": ${result.modifiedCount}`);
  ...
  for (const [legacy, canonical] of Object.entries(LEGACY_TO_CANONICAL)) {
    ...
    if (result.modifiedCount > 0) {
      logger.info(`  subdoc "${legacy}" → "${canonical}": ${result.modifiedCount}`);
      totalSubdocUpdated += result.modifiedCount;
    }
  }

  logger.info(`\n=== MIGRATION COMPLETE ===`);
  logger.info(`  String statuses updated: ${totalStringUpdated}`);
  logger.info(`  Subdoc statuses updated: ${totalSubdocUpdated}`);
  ...
  logger.info(`  Remaining legacy statuses: ${remainingLegacy}`);
  if (remainingLegacy > 0) {
    logger.info('  WARNING: Some legacy statuses could not be updated.');
  }
}
...
async function down(client) {
  ...
  if (DRY_RUN) {
    ...
    logger.info(`[DRY RUN] Orders migrated by this script: ${count}`);
    logger.info('  Note: Rollback maps canonical → legacy. "paid" maps to "success".');
    logger.info('  Multiple legacy values that mapped to the same canonical cannot all be restored.');
  ...
  logger.info(`  Rolled back: ${totalRolledBack} documents.`);
  logger.info('  Note: Some fidelity loss — "paid" was mapped from multiple legacy values.');
}
...
async function main() {
  ...
  try {
    ...
    else { logger.error('Usage: node 09_DM01_...js [up|down]'); process.exit(1); }
  } finally {
    await client.close();
  }
}
