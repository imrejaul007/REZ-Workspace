...
| **Coin normalization** | `rez-wallet-service/src/routes/internalRoutes.ts:27` — `normalizeCoinType()` handles canonical coin type |
...
5. Wallet service credits `rez` coins (canonical type), stores in MongoDB `CoinTransaction`.
...
- **Coin type normalization** — The wallet internal routes handle canonical coin type. The consumer app (walletApi.ts) sends the correct coin type in transfers. Middleware normalizes legacy values (REZ-007, now fixed).
...
- **Coin types:** `rez` (general), `prive` (Prive members only), `branded` (merchant-specific), `promo` (promotional).
...
| Consumer app entry / navigation | `rezapp/rez-master/app/_layout.tsx` |
...
# Start consumer app
cd rezapp/rez-master && npx expo start
