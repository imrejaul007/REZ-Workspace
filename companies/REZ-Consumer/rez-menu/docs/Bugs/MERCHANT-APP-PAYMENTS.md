...
**NOTE:** This audit found 28 payment and wallet bugs in the merchant app, with 4 being CRITICAL (items 001, 003, 004, 006). The most urgent fixes are:
1. Replace Math.random() idempotency with crypto.getRandomValues() (blocks secure payments)
2. Add promise-based double-payment prevention lock (prevent duplicate orders)
3. Implement payment verification retry logic (prevent false payment failures)
4. Add timeout to order creation polling (prevent hanging payments)
