...
> **Migration applied:** Run `db.cointransactions.updateMany({ coinType: 'nuqta' }, { $set: { coinType: 'rez' } })` to migrate legacy data.
...
**Impact:**
- Merchant-service can write `coinType: 'nuqta'` (legacy), `source: 'whatever'`, `status: 'random_value'` → all persisted.
