...
export async function addCoins(userId: string, amount: number, reason: string) {
  try {
    ...
    console.error('Wallet credit failed:', (error as Error).message);
    throw error;
  }
}
...
export async function trackEvent(eventType: string, eventData: EventData) {
  try {
    ...
    console.error('Event tracking failed:', (error as Error).message);
    throw error;
  }
}
 ...
 */
export async function getPredictions(userId: string, features: string[]) {
  try {
    ...
    console.error('Prediction failed:', (error as Error).message);
    throw error;
  }
}
