...
import { handoffRoutes } from './routes';
...
app.get('/health', (req, res) => {
  res.json({ status: 'ok', service: 'hojai-human-handoff', version: '1.0.0' });
});
...
async function start() {
  try {
    const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/hojai_handoff';
