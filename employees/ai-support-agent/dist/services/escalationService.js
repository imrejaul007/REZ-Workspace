...
const escalationRules = new Map(DEFAULT_ESCALATION_RULES.map(rule => [rule.id, rule]));
/**
 * Determine appropriate escalation level based on ticket analysis
 */
export async function determineEscalationLevel(ticket) {
    ...
    for (const rule of Array.from(escalationRules.values())
        ...
        const conditionsMet = rule.conditions.every(condition => {
            const ticketValue = getNestedValue(ticket, condition.field);
            switch (condition.operator) {
                case 'equals':
                    return ticketValue === condition.value;
                case 'contains':
                    return typeof ticketValue === 'string' && ticketValue.includes(condition.value);
                case 'greater_than':
                    return typeof ticketValue === 'number' && ticketValue > condition.value;
                case 'less_than':
                    return typeof ticketValue === 'number' && ticketValue < condition.value;
                case 'in':
                    return Array.isArray(condition.value) && condition.value.includes(ticketValue);
                default:
                    return false;
            }
        });
 ...
 */
function determineReasonFromRule(rule) {
    switch (rule.id) {
 ...
 */
function getTeamForLevel(level) {
    return TEAM_CONFIG[level];
}
/**
 * Get least loaded team at level
 */
function getAvailableAgent(level) {
 ...
 */
export async function createEscalation(input, escalatedBy) {
 ...
 */
export async function getEscalationsForTicket(ticketId) {
    ...
    return ticketEscalations.sort((a, b) => new Date(b.escalatedAt).getTime() - new Date(a.escalatedAt).getTime());
}
/**
 * Accept escalation
 */
export async function acceptEscalation(escalationId, agentId) {
 ...
 * Reject escalation
 */
export async function rejectEscalation(escalationId, reason) {
    const escalation = escalations.get(escalationId);
    if (!escalation) {
        logger.warn('escalation_not_found', { escalationId });
        return null;
 ...
 */
export async function updateEscalationRule(ruleId, updates) {
 ...
 */
export async function toggleEscalationRule(ruleId, enabled) {
    const rule = escalationRules.get(ruleId);
    if (!rule) {
        logger.warn('escalation_rule_not_found', { ruleId });
        return null;
