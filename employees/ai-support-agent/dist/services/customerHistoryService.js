 ...
 */
async function getTicketSummary(customerId) {
 ...
 */
async function getOrderSummary(_customerId) {
...
async function getCustomerInteractions(customerId, limit = 50) {
    ...
    const tickets = await ticketService.getTicketsByCustomerId(customerId);
    const ticketInteractions = tickets.map(ticket => ({
    ...
    const refundInteractions = refunds.map(refund => ({
 ...
 */
export async function recordInteraction(customerId, interaction) {
 ...
 */
export async function getCustomerHistory(customerId, includeRelatedTickets = false) {
 ...
 */
export async function getCustomerSentiment(customerId) {
    const tickets = await ticketService.getTicketsByCustomerId(customerId);
    if (tickets.length === 0) {
        return {
            score: 100,
    ...
    const recentTickets = tickets.filter(t => new Date(t.createdAt).getTime() > Date.now() - 30 * 24 * 60 * 60 * 1000);
    const olderTickets = tickets.filter(t => new Date(t.createdAt).getTime() <= Date.now() - 30 * 24 * 60 * 60 * 1000);
    let trend = 'stable';
 ...
 */
export async function getAtRiskCustomers(threshold = 50) {
    const atRisk = [];
...
export async function addCustomerNote(customerId, note) {
    ...
    const notes = profile.metadata.notes || [];
    notes.push(note);
...
export async function getCustomerNotes(customerId) {
    ...
    return profile.metadata?.notes || [];
}
//# sourceMappingURL=customerHistoryService.js.map