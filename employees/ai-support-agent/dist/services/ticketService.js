 ...
 */
function generateTicketNumber(tenantId) {
 ...
 */
function determineCustomerTier(metadata) {
    if (metadata?.tier) {
 ...
 */
function getAutoTags(category, subject, description) {
    const tags = [];
 ...
 */
export async function createTicket(input, tenantId) {
 ...
 */
export async function listTickets(filters, tenantId) {
    ...
    if (filters.assignedTo) {
        filteredTickets = filteredTickets.filter(t => t.assignedTo?.agentId === filters.assignedTo);
    }
 ...
 */
export async function updateTicketStatus(ticketId, status, updatedBy) {
 ...
 */
export async function addMessage(ticketId, authorId, authorType, content, attachments) {
    const ticket = tickets.get(ticketId);
    if (!ticket) {
        logger.warn('ticket_not_found', { ticketId });
        return null;
 ...
 */
export async function assignTicket(ticketId, agentId, agentName, team) {
    const ticket = tickets.get(ticketId);
    if (!ticket) {
        logger.warn('ticket_not_found', { ticketId });
        return null;
 ...
 */
export async function updateTicketPriority(ticketId, priority, updatedBy) {
    const ticket = tickets.get(ticketId);
    if (!ticket) {
        logger.warn('ticket_not_found', { ticketId });
        return null;
...
export async function getTicketsByCustomerId(customerId) {
    ...
    return customerTickets.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
}
/**
 * Get ticket statistics
 */
export async function getTicketStats(_tenantId) {
    let total = 0;
    const open = 0;
    const inProgress = 0;
    const resolved = 0;
