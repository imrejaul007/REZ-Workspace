 ...
 */
function getSummaryTemplate(style) {
    const templates = {
        brief: {
            minLength: 50,
            maxLength: 150,
...
function extractKeyPoints(content, count) {
    ...
    const scoredSentences = sentences.map((sentence, index) => {
        ...
        if (importantWords.some(w => sentence.toLowerCase().includes(w)))
            score += 2;
 ...
 */
function generateSummaryText(content, maxLength, style) {
 ...
 */
export async function summarizeBatch(contents, style = 'standard') {
    logger.info('summarize_batch', { count: contents.length, style });
    const results = await Promise.all(contents.map(content => summarize({
        content,
        style,
...
export async function extractEntities(content) {
    ...
    const locations = [...new Set((content.match(locationPatterns) || [])
...
export async function compareSummaries(summaries) {
    ...
    const commonKeywords = [...allKeywords[0]].filter(keyword => allKeywords.slice(1).every(set => set.has(keyword)));
