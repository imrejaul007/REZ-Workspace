 ...
 */
export async function getTrends(category, limit = 20) {
    ...
    const allTrends = Object.entries(categoryGroups).map(([cat, trends]) => ({
        category: cat,
        trends,
...
export async function getRelatedTrends(trendId) {
    ...
    const relatedTrends = MOCK_TRENDS.filter(t => {
        if (t.id === trendId)
            return false;
        if (t.category !== trend.category)
            return false;
        // Check for shared related terms
        const sharedTerms = t.relatedTerms?.filter(term => trend.relatedTerms?.includes(term));
...
export async function searchTrends(keyword) {
    ...
    const matchingTrends = MOCK_TRENDS.filter(trend => {
        ...
        const relatedMatch = trend.relatedTerms?.some(term => term.toLowerCase().includes(keywordLower));
        const sourceMatch = trend.sources.some(source => source.toLowerCase().includes(keywordLower));
 ...
 */
function groupByCategory(trends) {
    const groups = {};
...
//# sourceMappingURL=trendsModule.js.map