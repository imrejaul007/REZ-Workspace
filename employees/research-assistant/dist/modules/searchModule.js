 ...
 */
function filterAndRankResults(query, results, filters) {
    ...
    if (filters?.sources && filters.sources.length > 0) {
        scoredResults = scoredResults.filter(({ result }) => filters.sources.some(source => result.source.toLowerCase().includes(source.toLowerCase())));
    }
    // Apply language filters if provided
    if (filters?.language) {
        scoredResults = scoredResults.filter(({ result }) => result.metadata?.language === filters.language);
    }
    // Apply region filters if provided
    if (filters?.region) {
        scoredResults = scoredResults.filter(({ result }) => result.metadata?.region === filters.region);
    }
 ...
 */
export async function saveSearchResult(userId, result) {
