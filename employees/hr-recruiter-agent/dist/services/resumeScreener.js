...
exports.resumeScreener = exports.ResumeScreener = void 0;
...
class ResumeScreener {
     ...
     */
    parseResume(resumeText, candidateId, fileName) {
     ...
     */
    screenResume(resume, criteria, screenedBy = 'system') {
        ...
        const overallScore = Math.round(skillsScore * 0.35 +
        ...
        const matchedSkills = criteria?.requiredSkills
            ? parsedData.skills.filter(s => criteria.requiredSkills.some(rs => rs.toLowerCase() === s.toLowerCase()))
            : parsedData.skills;
        const missingSkills = criteria?.requiredSkills
            ? criteria.requiredSkills.filter(rs => !parsedData.skills.some(s => s.toLowerCase() === rs.toLowerCase()))
            : [];
        // Highlight strengths
     ...
     */
    screenResumes(resumes, criteria) {
        return resumes
     ...
     */
    calculateSkillsScore(skills, criteria) {
        if (!criteria?.requiredSkills?.length) {
            return skills.length > 5 ? 80 : skills.length > 2 ? 60 : 40;
        }
        const matched = skills.filter(s => criteria.requiredSkills.some(rs => rs.toLowerCase() === s.toLowerCase()));
        ...
        if (criteria.preferredSkills?.length) {
            preferredMatch = skills.filter(s => criteria.preferredSkills.some(ps => ps.toLowerCase() === s.toLowerCase())).length;
     ...
     */
    calculateExperienceScore(experience, criteria) {
     ...
     */
    calculateEducationScore(education, criteria) {
        if (!education.length)
            return 50;
        ...
        if (criteria?.educationRequirements?.length) {
            const meetsRequirements = criteria.educationRequirements.some(req => {
                return education.some(edu => edu.degree.toLowerCase().includes(req.toLowerCase()) ||
     ...
     */
    calculateCultureFitScore(parsedData) {
        let score = 60;
        ...
        if (parsedData.experience.length > 3)
            score += 10;
        else if (parsedData.experience.length > 1)
            score += 5;
        // Check for continuous learning (certifications)
        if (parsedData.certifications.length >= 3)
            score += 10;
        else if (parsedData.certifications.length >= 1)
            score += 5;
        // Check for languages (international exposure)
        if (parsedData.languages && parsedData.languages.length > 1)
            score += 5;
     ...
     */
    calculateAtsScore(resumeText, criteria) {
        let score = 50;
        const text = resumeText.toLowerCase();
        ...
        if (criteria?.requiredSkills?.length) {
            const keywordMatch = criteria.requiredSkills.filter(skill => text.includes(skill.toLowerCase())).length;
     ...
     */
    calculateKeywordMatches(resumeText, criteria) {
        const text = resumeText.toLowerCase();
        const allKeywords = [
        ...
        const missing = allKeywords.filter(kw => !text.includes(kw.toLowerCase()));
     ...
     */
    identifyConcerns(parsedData, missingSkills, criteria) {
        const concerns = [];
     ...
     */
    hasCareerGaps(experience) {
        if (experience.length <= 1)
            return false;
        const sorted = [...experience].sort((a, b) => parseInt(a.startDate) - parseInt(b.startDate));
     ...
     */
    makeRecommendation(overallScore, atsScore, missingSkills) {
     ...
     */
    generateRecommendationReason(recommendation, overallScore, matchedSkills, missingSkills) {
     ...
     */
    matchAgainstJobRequirements(resume, requirements) {
        const resumeSkills = resume.parsedData.skills.map(s => s.toLowerCase());
        const matchedRequirements = [];
        const missingRequirements = [];
        for (const req of requirements) {
            const hasSkill = resumeSkills.some(s => s.includes(req.skill.toLowerCase()) ||
...
exports.ResumeScreener = ResumeScreener;
exports.resumeScreener = new ResumeScreener();
//# sourceMappingURL=resumeScreener.js.map