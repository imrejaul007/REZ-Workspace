...
exports.skillsMatcher = exports.SkillsMatcher = void 0;
class SkillsMatcher {
    config;
    // Skill taxonomy for related skills mapping
    skillTaxonomy = new Map([
    ...
    trainingRecommendations = new Map([
        ['JavaScript', [
     ...
     */
    matchSkills(candidateSkills, requiredSkills, requirements) {
        const matches = [];
        for (const requiredSkill of requiredSkills) {
            ...
            const matchingCandidateSkill = candidateSkills.find(cs => this.areSkillsRelated(cs.name, requiredSkill) ||
     ...
     */
    calculateMatchPercent(candidateSkill, requiredSkill, minYears) {
    ...
    findMissingSkills(candidateSkills, requiredSkills) {
        return requiredSkills.filter(required => !candidateSkills.some(cs => this.areSkillsRelated(cs.name, required)));
    }
    /**
     * Calculate skill gaps
     */
    calculateSkillGaps(candidateSkills, requirements) {
        const gaps = [];
        for (const requirement of requirements) {
            const candidateSkill = candidateSkills.find(cs => this.areSkillsRelated(cs.name, requirement.skill));
     ...
     */
    calculateSkillCoverage(candidateSkills, requiredSkills) {
        if (requiredSkills.length === 0)
            return 100;
        const matched = requiredSkills.filter(required => candidateSkills.some(cs => this.areSkillsRelated(cs.name, required))).length;
        return Math.round((matched / requiredSkills.length) * 100);
    }
    /**
     * Calculate skill relevance score
     */
    calculateSkillRelevanceScore(candidateSkills, job) {
        if (!job) {
            // Score based on skill diversity and level
            const avgLevel = candidateSkills.reduce((sum, s) => sum + this.skillLevelScores[s.level], 0) / (candidateSkills.length || 1);
        ...
        const preferredMatch = candidateSkills.filter(cs => job.requirements.some(r => r.priority === 'should_have' && this.areSkillsRelated(cs.name, r.skill))).length;
     ...
     */
    getRecommendedTrainings(candidateSkills, missingSkills) {
    ...
    matchCandidateToJob(candidate, job) {
        ...
        const overallMatchScore = Math.round(skillsMatchScore * 0.4 +
     ...
     */
    calculateCultureMatch(candidate, job) {
        let score = 60;
        ...
        const noticeShort = ['immediate', '15 days', '30 days'].some(np => candidate.noticePeriod?.toLowerCase().includes(np));
        if (noticeShort)
            score += 15;
        // Availability
    ...
    calculateSalaryMatch(candidate, job) {
        ...
        if (overage <= 0.1)
            return 50; // 10% over - negotiate
        if (overage <= 0.2)
            return 30; // 20% over - stretch
        return 10; // Significantly over budget
    }
    /**
     * Make match recommendation
     */
    makeMatchRecommendation(overallScore, skillsScore, missingRequirements) {
     ...
     */
    generateMatchReason(recommendation, score, analysis) {
     ...
     */
    findBestMatchingJobs(candidate, jobs) {
        const matches = jobs
     ...
     */
    compareCandidates(candidate1, candidate2, job) {
...
exports.SkillsMatcher = SkillsMatcher;
exports.skillsMatcher = new SkillsMatcher();
//# sourceMappingURL=skillsMatcher.js.map