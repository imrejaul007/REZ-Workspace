...
function searchNotes(notesList, search) {
    const searchLower = search.toLowerCase();
    return notesList.filter(note => note.title.toLowerCase().includes(searchLower) ||
...
router.post('/', async (req, res) => {
    try {
        ...
        const collaborators = (input.collaborators || []).map(col => ({
            userId: col.userId,
...
router.get('/', async (req, res) => {
    try {
        const userId = getUserId(req);
        const tenantId = getTenantId(req);
        const { folder, tag, search, isArchived, pinned, page = '1', limit = '20', sort = 'updatedAt', order = 'desc' } = req.query;
        let filtered = Array.from(notes.values()).filter(note => note.userId === userId && note.tenantId === tenantId);
        // Filter by folder
        if (folder) {
...
router.get('/folders/list', async (req, res) => {
    try {
        const userId = getUserId(req);
        const tenantId = getTenantId(req);
        const userNotes = Array.from(notes.values()).filter(n => n.userId === userId && n.tenantId === tenantId && n.folder);
        ...
        const folderStats = folders.map(folder => {
            ...
            return {
                ...
                lastUpdated: folderNotes.sort((a, b) => new Date(b.updatedAt).getTime() - new Date(a.updatedAt).getTime())[0]?.updatedAt
            };
        });
...
router.get('/tags/list', async (req, res) => {
    try {
        const userId = getUserId(req);
        const tenantId = getTenantId(req);
        const userNotes = Array.from(notes.values()).filter(n => n.userId === userId && n.tenantId === tenantId && n.tags && n.tags.length > 0);
