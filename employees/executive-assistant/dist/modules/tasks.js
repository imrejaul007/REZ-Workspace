...
router.post('/', async (req, res) => {
    try {
        ...
        const checklist = (input.checklist || []).map(cl => ({
...
router.get('/', async (req, res) => {
    try {
        const userId = getUserId(req);
        const tenantId = getTenantId(req);
        const { status, priority, project, folder, tag, assignee, search, dueToday, overdue, upcoming, page = '1', limit = '20', sort = 'createdAt', order = 'desc' } = req.query;
        let filtered = Array.from(tasks.values()).filter(task => task.userId === userId && task.tenantId === tenantId);
        // Filter by status
        if (status) {
        ...
        // Filter by assignee
        if (assignee) {
            filtered = filtered.filter(t => t.assignees?.some(a => a.userId === assignee));
        }
        // Filter by due today
        if (dueToday === 'true') {
        ...
        const stats = {
            ...
            overdue: filtered.filter(t => t.dueDate &&
...
router.patch('/:id', async (req, res) => {
    try {
        ...
        if (input.dueDate !== undefined)
            task.dueDate = input.dueDate ? parseDate(input.dueDate) : undefined;
        if (input.startDate !== undefined)
            task.startDate = input.startDate ? parseDate(input.startDate) : undefined;
 ...
 */
router.delete('/:id/assign/:assigneeId', async (req, res) => {
    try {
