...
router.get('/', async (req, res) => {
    try {
        ...
        if (labels) {
            ...
            allEmails = allEmails.filter(e => e.labels?.some(label => labelList.includes(label)));
        }
        // Search
        if (search) {
            const searchLower = search.toLowerCase();
            allEmails = allEmails.filter(e => e.subject.toLowerCase().includes(searchLower) ||
...
router.post('/:id/forward', async (req, res) => {
    try {
        ...
        const forward = {
            ...
            userId,
            tenantId: originalEmail.tenantId,
            to: to.map((t) => typeof t === 'string' ? { email: t } : t),
            from: userEmail,
...
router.get('/templates/list', async (req, res) => {
    try {
        const userId = getUserId(req);
        const tenantId = getTenantId(req);
        const templates = Array.from(emails.values()).filter(e => e.userId === userId &&
            e.tenantId === tenantId &&
