...
router.post('/events', async (req, res) => {
    try {
        ...
        const attendees = (input.attendees || []).map(att => ({
            ...att,
...
router.get('/events', async (req, res) => {
    try {
        ...
        let filtered = Array.from(events.values()).filter(event => event.userId === userId && event.tenantId === tenantId && event.status !== 'cancelled');
...
router.patch('/events/:id', async (req, res) => {
    try {
        ...
        if (input.startTime !== undefined)
            event.startTime = parseDate(input.startTime);
        if (input.endTime !== undefined)
            event.endTime = parseDate(input.endTime);
        ...
        if (input.reminders !== undefined)
            event.reminders = input.reminders;
...
router.get('/availability', async (req, res) => {
    try {
        ...
        while (currentTime < end) {
            ...
            const hasConflict = userEvents.some(e => new Date(e.startTime) < slotEnd && new Date(e.endTime) > currentTime);
...
router.get('/freebusy', async (req, res) => {
    try {
        ...
        const freeBusyData = emailList.map(email => {
            const busyEvents = Array.from(events.values()).filter(e => e.attendees?.some(a => a.email === email) &&
                e.status !== 'cancelled' &&
                new Date(e.startTime) < end &&
                new Date(e.endTime) > start);
            return {
