...
exports.packageAdvisorService = exports.PackageAdvisorService = void 0;
/**
...
class PackageAdvisorService {
     ...
     */
    analyzePackages(services, clients, currentPackages) {
        ...
        const totalRevenue = currentPackages.reduce((sum, pkg) => sum + (pkg.projectedSales || 0) * pkg.packagePrice, 0);
        const avgMargin = currentPackages.reduce((sum, pkg) => sum + pkg.margin, 0) / currentPackages.length;
        // Category breakdown
        ...
        return {
            ...
            avgPackageValue: currentPackages.reduce((sum, pkg) => sum + pkg.packagePrice, 0) /
     ...
     */
    analyzeServiceCombinations(services) {
        ...
        if (hairServices.length >= 2) {
            ...
            currentPackages.push({
                ...
                packagePrice: Math.round(sortedHair.slice(0, 3).reduce((sum, s) => sum + s.price, 0) * 0.85),
                discountPercent: 15,
                margin: 55,
                validity: 30,
        ...
        if (makeupServices.length > 0 || skinServices.length > 0) {
            currentPackages.push({
                ...
                totalOriginalPrice: makeupServices.slice(0, 1).reduce((sum, s) => sum + s.price, 0) +
                    skinServices.slice(0, 2).reduce((sum, s) => sum + s.price, 0),
                packagePrice: Math.round((makeupServices.slice(0, 1).reduce((sum, s) => sum + s.price, 0) +
                    skinServices.slice(0, 2).reduce((sum, s) => sum + s.price, 0)) *
                    0.8),
                discountPercent: 20,
                margin: 45,
     ...
     */
    analyzeSeasonalPatterns(packages) {
        const patterns = [
     ...
     * Generate package recommendations
     */
    generateRecommendations(analysis, services, clients) {
        ...
        // New client packages
        const hasNewClientPackages = analysis.currentPackages.some(p => p.targetSegment === 'new_client');
     ...
     * Create new package suggestions
     */
    createNewPackages(services, clients, analysis) {
        const newPackages = [];
        const avgServiceValue = services.reduce((sum, s) => sum + s.price, 0) / services.length || 500;
        ...
        newPackages.push({
            ...
            packagePrice: Math.round(services.slice(0, 3).reduce((sum, s) => sum + s.price, 0) * 0.8),
            discountPercent: 20,
            margin: 55,
            validity: 30,
            targetSegment: 'regular',
        ...
        if (categories.includes('makeup') || categories.includes('hair')) {
            newPackages.push({
                ...
                packagePrice: Math.round(services
                    .filter(s => ['makeup', 'hair', 'skin'].includes(s.category))
                    .slice(0, 2)
     ...
     * Generate seasonal bundles
     */
    generateSeasonalBundles(services) {
        const bundles = [];
        ...
        if (skinServices.length > 0) {
            bundles.push({
                ...
                package: {
                    ...
                    packagePrice: Math.round(skinServices.slice(0, 2).reduce((sum, s) => sum + s.price, 0) * 0.8),
                    discountPercent: 20,
                    margin: 55,
                    validity: 60,
                    targetSegment: 'regular',
        ...
        if (hairServices.length > 0) {
            bundles.push({
                ...
                package: {
                    ...
                    packagePrice: Math.round(hairServices.slice(0, 2).reduce((sum, s) => sum + s.price, 0) * 0.75),
                    discountPercent: 25,
                    margin: 52,
                    validity: 45,
                    targetSegment: 'premium',
        ...
        if (spaServices.length > 0) {
            bundles.push({
                ...
                package: {
                    ...
                    packagePrice: Math.round(spaServices.slice(0, 4).reduce((sum, s) => sum + s.price, 0) * 0.7),
                    discountPercent: 30,
                    margin: 48,
                    validity: 60,
                    targetSegment: 'bridal',
    ...
    createUpsellPaths(analysis) {
        ...
        for (const path of paths) {
            if (packageNames.some(n => n.includes(path.fromPackage)) ||
...
exports.PackageAdvisorService = PackageAdvisorService;
exports.packageAdvisorService = new PackageAdvisorService();
//# sourceMappingURL=packageAdvisor.js.map