...
exports.bookingOptimizerService = exports.BookingOptimizerService = void 0;
/**
 * Booking Optimizer Service
 * Analyzes booking patterns and provides optimization recommendations
 */
class BookingOptimizerService {
    constructor() {
        this.PRIME_HOURS = [10, 11, 12, 14, 15, 16, 17];
        this.OFF_PEAK_HOURS = [9, 13, 18, 19];
    }
    /**
     ...
     */
    performAnalysis(services, appointments, clients) {
        // Total bookings
     ...
     * Analyze service mix
     */
    analyzeServiceMix(services, appointments) {
        ...
        for (const apt of appointments) {
            ...
            if (apt.clientId)
                current.clients.add(apt.clientId);
     ...
     */
    identifyPeakSlots(appointments) {
     ...
     * Generate recommendations
     */
    generateRecommendations(analysis, services) {
     ...
     * Find upsell opportunities
     */
    findUpsellOpportunities(services, appointments) {
        const opportunities = [];
        ...
        for (const pairing of pairings) {
            const baseService = services.find(s => s.name.toLowerCase().includes(pairing.service.toLowerCase()));
            const targetService = services.find(s => s.name.toLowerCase().includes(pairing.toService.toLowerCase()));
            if (baseService && targetService) {
                const pricePoint = Math.round(baseService.price * (1 + pairing.discount / 100) + targetService.price * 0.85);
                opportunities.push({
     ...
     * Generate retention strategies
     */
    generateRetentionStrategies(clients, services) {
        const strategies = [];
...
exports.BookingOptimizerService = BookingOptimizerService;
exports.bookingOptimizerService = new BookingOptimizerService();
//# sourceMappingURL=bookingOptimizer.js.map