...
exports.staffAnalyzerService = exports.StaffAnalyzerService = void 0;
/**
 * Staff Analyzer Service
 * Analyzes staff utilization, performance, and provides optimization recommendations
 */
class StaffAnalyzerService {
    constructor() {
        this.TARGET_UTILIZATION = 0.85; // 85% target
        this.PRIME_HOURS = [10, 11, 12, 14, 15, 16, 17]; // Peak business hours
    }
    /**
     * Analyze staff utilization and provide recommendations
     */
    async analyzeStaff(request) {
     ...
     */
    calculateUtilization(staff, services, appointments) {
        return staff.map(member => {
            ...
            const memberAppointments = appointments.filter(apt => apt.staffId === member.id && apt.status === 'completed');
            const bookedMinutes = memberAppointments.reduce((sum, apt) => sum + apt.duration, 0);
     ...
     * Calculate peak hours utilization
     */
    calculatePeakHours(appointments) {
     ...
     */
    calculateTrend(monthlyClients, rating) {
        if (monthlyClients > 50 && rating >= 4.5)
            return 'up';
        if (monthlyClients < 20 && rating < 4)
            return 'down';
        return 'stable';
    }
    /**
     ...
     */
    performAnalysis(utilizationData, staff, services) {
        ...
        const sorted = [...utilizationData].sort((a, b) => b.utilizationPercent - a.utilizationPercent);
        ...
        const recommendations = this.generateRecommendations(utilizationData, topPerformers, underperformers, overallUtilization);
        return {
     ...
     * Identify capacity gaps
     */
    identifyCapacityGaps(utilizationData) {
        const gaps = [];
     ...
     */
    generateRecommendations(utilizationData, topPerformers, underperformers, overallUtilization) {
     ...
     * Generate rebooking campaigns
     */
    generateRebookingCampaigns(staff, appointments) {
        const campaigns = [];
        for (const member of staff) {
            // Find last visit dates for clients
            const memberClients = new Set(appointments
     ...
     * Identify training needs
     */
    identifyTrainingNeeds(staff, utilizationData) {
        const trainingNeeds = [];
        ...
        // Styling skills
        const stylists = staff.filter(s => s.role.includes('stylist') || s.role.includes('colorist'));
...
exports.StaffAnalyzerService = StaffAnalyzerService;
exports.staffAnalyzerService = new StaffAnalyzerService();
//# sourceMappingURL=staffAnalyzer.js.map