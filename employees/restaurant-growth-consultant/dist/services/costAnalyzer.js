...
exports.costAnalyzerService = exports.CostAnalyzerService = void 0;
/**
...
class CostAnalyzerService {
    ...
    async analyze(request) {
        ...
        const monthlyPotentialSavings = this.calculateSavings(overallFoodCostPercent, request.targetFoodCostPercent, request.monthlyFoodSpend);
        const analysis = {
        ...
        const recommendations = this.generateRecommendations(overallFoodCostPercent, request.targetFoodCostPercent, request.monthlyFoodSpend, vendorAnalysis, wasteAnalysis, recipes);
        // Calculate cost reduction targets
        const costReduction = this.calculateCostReduction(overallFoodCostPercent, request.targetFoodCostPercent, request.monthlyFoodSpend, monthlyPotentialSavings);
        return {
            analysis,
            recommendations,
            costReduction,
     ...
     */
    analyzeVendors(vendors, ingredients) {
     ...
     */
    assessVendorQuality(spend, itemCount) {
        const efficiency = itemCount / (spend / 10000);
        if (efficiency > 0.8 && itemCount > 10)
            return 'excellent';
        if (efficiency > 0.5 && itemCount > 5)
            return 'good';
        if (efficiency > 0.3)
            return 'average';
        return 'poor';
     ...
     */
    calculateVendorSavings(spend, quality) {
        ...
        return spend * (savingsPercentages[quality] || 0);
    }
    /**
     * Analyze waste patterns
     */
    analyzeWaste(recipes) {
     ...
     */
    analyzeRecipes(recipes, ingredients) {
     ...
     */
    calculateSavings(currentCost, targetCost, monthlySpend) {
     ...
     */
    generateRecommendations(currentCost, targetCost, monthlySpend, vendorAnalysis, wasteAnalysis, recipes) {
     ...
     */
    calculateCostReduction(currentCost, targetCost, monthlySpend, totalSavings) {
     ...
     */
    async calculateOptimalOrder(ingredients, dailyUsage, leadTimeDays) {
     ...
     */
    async generatePurchasingReport(request, analysis) {
        const report = `
...
exports.CostAnalyzerService = CostAnalyzerService;
exports.costAnalyzerService = new CostAnalyzerService();
//# sourceMappingURL=costAnalyzer.js.map