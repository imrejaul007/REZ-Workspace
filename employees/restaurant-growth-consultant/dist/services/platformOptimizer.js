...
exports.platformOptimizerService = exports.PlatformOptimizerService = void 0;
/**
...
class PlatformOptimizerService {
    ...
    async optimize(request) {
        ...
        const recommendations = this.generateRecommendations(profileOptimization, menuOptimization, operationalOptimization, reviewStrategy, commissionOptimization);
        return {
            profileOptimization,
            menuOptimization,
     ...
     */
    optimizeProfile(listings, metrics) {
     ...
     */
    suggestBadges(listings, avgRating) {
        const currentBadges = [];
     ...
     */
    optimizeMenu(menuItems, listings, metrics) {
        // Find spotlight items (high popularity + good margin)
        const sortedByPopularity = [...menuItems].sort((a, b) => (b.popularity || 50) - (a.popularity || 50));
     ...
     */
    developPricingStrategy(menuItems, avgPrice, metrics) {
     ...
     */
    optimizeOperations(listings, metrics) {
        ...
        const unavailableItems = listings.flatMap(l => l.menuItems.filter(m => !m.available).map(m => m.name));
        const availableItems = listings.flatMap(l => l.menuItems.filter(m => m.available).map(m => m.name));
        return {
            deliveryTime: {
     ...
     * Generate prioritized recommendations
     */
    generateRecommendations(profileOptimization, menuOptimization, operationalOptimization, reviewStrategy, commissionOptimization) {
     ...
     */
    async comparePlatforms(zomatoMetrics, swiggyMetrics) {
...
exports.PlatformOptimizerService = PlatformOptimizerService;
exports.platformOptimizerService = new PlatformOptimizerService();
//# sourceMappingURL=platformOptimizer.js.map