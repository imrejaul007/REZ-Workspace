...
exports.reviewManagerService = exports.ReviewManagerService = void 0;
...
class ReviewManagerService {
    ...
    async analyze(request) {
        ...
        const responseSuggestions = this.generateResponseSuggestions(processedReviews, request.responseTemplates);
        // Generate strategy
     ...
     */
    generateAnalysis(reviews, competitorRatings) {
     ...
     */
    calculateCategoryScores(reviews) {
     ...
     */
    generateResponseSuggestions(reviews, templates) {
     ...
     */
    generateCampaigns(analysis, platform) {
        const campaigns = [];
     ...
     */
    calculateReviewsNeeded(currentRating, targetRating, currentReviewCount) {
        if (currentRating >= targetRating)
            return 0;
        // Estimate based on weighted average formula
        const requiredPositiveReviews = Math.ceil((targetRating * (currentReviewCount + 100) - currentRating * currentReviewCount) / 5);
        return Math.max(0, requiredPositiveReviews - currentReviewCount);
    }
    /**
     * Generate automation rules
     */
    generateAutomation(reviews) {
        return [
            {
...
exports.ReviewManagerService = ReviewManagerService;
exports.reviewManagerService = new ReviewManagerService();
