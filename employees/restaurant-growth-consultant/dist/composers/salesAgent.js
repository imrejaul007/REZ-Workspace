...
exports.salesAgent = exports.SalesAgent = void 0;
...
class SalesAgent {
     ...
     */
    async generateUpsellSuggestions(currentOrder, menuItems) {
        const suggestions = [];
        // Find complementary items based on current order
        const orderedCategories = currentOrder.items.map(item => menuItems.find(m => m.name.toLowerCase().includes(item.toLowerCase()))?.category || 'unknown');
     ...
     */
    findUpgrades(currentItems, menuItems) {
        const upgrades = [];
        for (const currentItem of currentItems) {
            // Find same-category items with higher price
            const currentMenuItem = menuItems.find(m => m.name.toLowerCase().includes(currentItem.toLowerCase()));
            if (currentMenuItem) {
     ...
     */
    async generateBundleOpportunities(menuItems, targetMargin = 0.65) {
        const bundles = [];
     ...
     */
    async identifyOpportunities(customers, menuItems) {
        const opportunities = [];
     ...
     */
    findRelatedCategory(preference, menuItems) {
        const preferenceItem = menuItems.find(m => m.name.toLowerCase().includes(preference.toLowerCase()) ||
     ...
     */
    async createCampaigns(opportunities, targetRevenue) {
        const campaigns = [];
        // Group by type
        ...
        const highValueOpps = opportunities.filter(o => o.customerSegment === 'high_value_lapsed' ||
     ...
     */
    calculateCampaignROI(campaign, costPerCustomer = 50) {
...
exports.salesAgent = new SalesAgent();
