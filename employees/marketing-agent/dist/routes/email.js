...
// Validation schemas
const CreateEmailCampaignSchema = zod_1.z.object({
...
const UpdateEmailCampaignSchema = zod_1.z.object({
    subject: zod_1.z.string().min(1).max(200).optional(),
    previewText: zod_1.z.string().max(100).optional(),
    htmlContent: zod_1.z.string().optional(),
    plainContent: zod_1.z.string().optional()
});
const SendEmailCampaignSchema = zod_1.z.object({
    sendNow: zod_1.z.boolean().default(true),
    scheduledFor: zod_1.z.string().datetime().optional()
});
const ListEmailCampaignsSchema = validation_1.PaginationSchema.extend({
    status: zod_1.z.enum(['draft', 'scheduled', 'sending', 'sent', 'failed']).optional(),
 ...
 */
router.post('/campaign', (0, validation_1.validateBody)(CreateEmailCampaignSchema), async (req, res) => {
    try {
        ...
        if (req.body.schedule?.sendNow) {
            await models_1.EmailCampaign.findByIdAndUpdate(emailCampaign._id, {
                status: types_1.EmailCampaignStatus.SENDING,
                scheduledFor: new Date()
            });
            // Simulate email sending
            setTimeout(async () => {
                await models_1.EmailCampaign.findByIdAndUpdate(emailCampaign._id, {
                    status: types_1.EmailCampaignStatus.SENT,
                    sentAt: new Date(),
            ...
            await models_1.EmailCampaign.findByIdAndUpdate(emailCampaign._id, {
                status: types_1.EmailCampaignStatus.SCHEDULED,
                scheduledFor: new Date(req.body.schedule.scheduledFor)
            });
        }
        logger_1.logger.info('Email campaign created', { tenantId, emailCampaignId: emailCampaign._id });
        res.json({
            success: true,
            data: {
                emailCampaign: {
        ...
        logger_1.logger.error('Create email campaign failed', { error });
        res.status(500).json({
            success: false,
            error: {
                code: 'CREATE_FAILED',
 ...
 */
router.get('/campaigns', (0, validation_1.validateBody)(ListEmailCampaignsSchema), async (req, res) => {
    try {
        const tenantId = req.tenantId || 'default';
        ...
        if (req.body.campaignId)
            query.campaignId = req.body.campaignId;
        const [docs, total] = await Promise.all([
            models_1.EmailCampaign.find(query)
                .sort({ createdAt: -1 })
                .skip(pagination.skip)
                .limit(pagination.limit)
                .lean(),
            models_1.EmailCampaign.countDocuments(query)
        ]);
        ...
        res.json((0, validation_1.formatPaginatedResponse)(items, total, (req.body.page || 1), (req.body.limit || 20)));
    }
    catch (error) {
        logger_1.logger.error('List email campaigns failed', { error });
        res.status(500).json({
            success: false,
            error: {
                code: 'LIST_FAILED',
...
router.get('/campaigns/:id', async (req, res) => {
    try {
        const tenantId = req.tenantId || 'default';
        const doc = await models_1.EmailCampaign.findOne({
            _id: req.params.id,
            tenantId
        }).lean();
        if (!doc) {
        ...
        logger_1.logger.error('Get email campaign failed', { error });
        res.status(500).json({
            success: false,
            error: {
                code: 'GET_FAILED',
 ...
 */
router.patch('/campaigns/:id', (0, validation_1.validateBody)(UpdateEmailCampaignSchema), async (req, res) => {
    try {
        const tenantId = req.tenantId || 'default';
        const doc = await models_1.EmailCampaign.findOneAndUpdate({
            _id: req.params.id,
            tenantId,
            status: types_1.EmailCampaignStatus.DRAFT
        }, {
        ...
        logger_1.logger.error('Update email campaign failed', { error });
        res.status(500).json({
            success: false,
            error: {
                code: 'UPDATE_FAILED',
 ...
 * Send email campaign
 */
router.post('/campaigns/:id/send', (0, validation_1.validateBody)(SendEmailCampaignSchema), async (req, res) => {
    try {
        const tenantId = req.tenantId || 'default';
        const doc = await models_1.EmailCampaign.findOne({
            _id: req.params.id,
        ...
        if (doc.status !== types_1.EmailCampaignStatus.DRAFT && doc.status !== types_1.EmailCampaignStatus.SCHEDULED) {
            res.status(400).json({
                success: false,
                error: {
                    code: 'INVALID_STATUS',
        ...
        doc.status = types_1.EmailCampaignStatus.SENDING;
        ...
        setTimeout(async () => {
            await models_1.EmailCampaign.findByIdAndUpdate(req.params.id, {
                status: types_1.EmailCampaignStatus.SENT,
                sentAt: new Date(),
                sentCount,
                deliveredCount,
        ...
        logger_1.logger.info('Email campaign sending', { tenantId, emailCampaignId: req.params.id });
        res.json({
            success: true,
            data: {
                id: doc._id.toString(),
        ...
        logger_1.logger.error('Send email campaign failed', { error });
        res.status(500).json({
            success: false,
            error: {
                code: 'SEND_FAILED',
...
router.post('/campaigns/:id/cancel', async (req, res) => {
    try {
        ...
        const doc = await models_1.EmailCampaign.findOneAndUpdate({
            _id: req.params.id,
            tenantId,
            status: { $in: [types_1.EmailCampaignStatus.DRAFT, types_1.EmailCampaignStatus.SCHEDULED] }
        }, { status: types_1.EmailCampaignStatus.FAILED }, { new: true });
        if (!doc) {
            res.status(400).json({
                success: false,
                error: {
        ...
        logger_1.logger.info('Email campaign cancelled', { tenantId, emailCampaignId: req.params.id });
        res.json({
            success: true,
            data: {
                id: doc._id.toString(),
        ...
        logger_1.logger.error('Cancel email campaign failed', { error });
        res.status(500).json({
            success: false,
            error: {
                code: 'CANCEL_FAILED',
