...
const decisionService_js_1 = require("../services/decisionService.js");
...
router.post('/cashback', async (req, res, next) => {
    try {
        ...
        const decision = await decisionService_js_1.decisionService.decideCashback({
            tenantId,
            userId,
            amount,
...
router.post('/offer', async (req, res, next) => {
    try {
        ...
        const decision = await decisionService_js_1.decisionService.decideOffer({ tenantId, userId, offerId });
        res.json({ success: true, data: decision });
    }
    catch (error) {
        next(error);
...
router.post('/fraud', async (req, res, next) => {
    try {
        ...
        const decision = await decisionService_js_1.decisionService.decideFraud({
            tenantId,
            userId,
...
router.get('/:userId', async (req, res, next) => {
    try {
        ...
        const result = await decisionService_js_1.decisionService.getUserDecisions({
            tenantId,
...
router.get('/reviews/pending', async (req, res, next) => {
    try {
        ...
        const decisions = await decisionService_js_1.decisionService.getPendingReviews(tenantId, limit);
...
router.post('/:id/review', async (req, res, next) => {
    try {
        ...
        const decision = await decisionService_js_1.decisionService.reviewDecision({
            tenantId,
