...
class HojaiFlowOrchestrator {
    ...
    async execute(request) {
        ...
        try {
            ...
            return {
                ...
                audioUrl,
                context: {
                    memories: Object.values(byTier).flat().slice(0, 50),
                    byTier: byTier,
                    contextString
                },
                intent,
    ...
    async speechToText(audioData) {
        try {
            const response = await axios.post(`${VOICE_SERVICE_URL}/api/stt`, { audio: audioData }, { timeout: 10000 });
     ...
     * L1 → L2 → L3 → L4 → L5
     */
    async loadContextFromMemory(request) {
        const tiers = request.options?.includeTiers || Object.values(MemoryTier);
        let byTier = {};
        try {
            // Try local memory service first
            const response = await axios.get(`${MEMORY_SERVICE_URL}/api/memories/context`, {
                params: {
     ...
     * Step 3: Intent Prediction
     */
    async predictIntent(userId, input) {
        try {
            const response = await axios.post(`${INTENT_SERVICE_URL}/api/intent/predict`, { userId, text: input }, { timeout: 5000 });
            if (response.data.success) {
                return {
        ...
        if (lower.includes('buy') || lower.includes('order'))
            return { predicted: 'purchase', confidence: 0.7 };
        if (lower.includes('help'))
            return { predicted: 'support', confidence: 0.7 };
        if (lower.includes('where') || lower.includes('location'))
            return { predicted: 'location_query', confidence: 0.6 };
        return { predicted: 'general', confidence: 0.3 };
    }
    /**
     * Step 4: Reasoning
     */
    async reason(userId, input, context) {
        try {
            const response = await axios.post(`${REASONING_SERVICE_URL}/api/reason`, {
                problem: `${input}\n\nContext:\n${context}`,
    ...
    async textToSpeech(text) {
        try {
            const response = await axios.post(`${VOICE_SERVICE_URL}/api/tts`, { text }, { timeout: 15000 });
 ...
 */
app.post('/api/flow/execute', async (req, res, next) => {
    try {
 ...
 */
app.get('/api/flow/memory', async (req, res, next) => {
    try {
 ...
 * Predict intent only
 */
app.post('/api/flow/intent', async (req, res, next) => {
    try {
 ...
 * Reasoning only
 */
app.post('/api/flow/reason', async (req, res, next) => {
    try {
 ...
 */
app.post('/api/flow/voice', async (req, res, next) => {
    try {
