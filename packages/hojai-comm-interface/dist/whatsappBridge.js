...
import { Source, MessageType } from './types/index.js';
...
class WhatsAppBridge {
     ...
     */
    async routeMessage(tenantId, message, contact) {
    ...
    async sendMessage(tenantId, to, text) {
        ...
        try {
            if (WHATSAPP_ACCESS_TOKEN) {
                ...
                const response = await axios.post(`${WHATSAPP_API_URL}/${WHATSAPP_PHONE_NUMBER_ID}/messages`, payload, {
                    headers: {
                        Authorization: `Bearer ${WHATSAPP_ACCESS_TOKEN}`,
     ...
     */
    async sendInteractiveMessage(tenantId, to, header, body, buttons, footer) {
        const payload = {
            messaging_product: 'whatsapp',
            recipient_type: 'individual',
            to,
     ...
     */
    async sendMediaMessage(tenantId, to, mediaUrl, caption, type = 'image') {
        const payload = {
            messaging_product: 'whatsapp',
            recipient_type: 'individual',
            to,
     ...
     */
    startWebSocketServer(port = 4585) {
     ...
     */
    getConnectedTenants() {
        return Array.from(this.clients.keys()).filter(tenantId => this.clients.get(tenantId)?.session.connected);
    }
    /**
     * Shutdown bridge
     */
