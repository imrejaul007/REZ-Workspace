...
exports.requireTenantType = exports.requireActiveTenant = exports.requireRole = exports.requirePermission = exports.authenticate = exports.authenticateInternal = exports.authenticateApiKey = exports.authenticateJWT = void 0;
...
const authenticateJWT = async (req, res, next) => {
    try {
        ...
        try {
            const payload = authService_js_1.authService.verifyAccessToken(token);
            ...
            const tenant = await tenantManager_js_1.TenantModel.findById(payload.tenantId);
            req.isPrivileged = tenant?.type === index_js_1.TenantType.REZ_ECOSYSTEM;
            next();
        }
        catch (error) {
            res.status(401).json({
...
const authenticateApiKey = async (req, res, next) => {
    try {
        ...
        const verifiedKey = await authService_js_1.authService.verifyApiKey(tenantId, apiKey);
        if (!verifiedKey) {
            res.status(401).json({
                success: false,
...
const authenticate = async (req, res, next) => {
    ...
    const internalToken = req.headers['x-internal-token'];
    if (internalToken) {
        return (0, exports.authenticateInternal)(req, res, next);
    }
    if (apiKey) {
    ...
    return async (req, res, next) => {
        try {
            ...
            if (req.userId && req.userRole) {
                const hasPermission = await rbacService_js_1.rbacService.hasAnyPermission(req.tenantId, req.userRole, requiredPermissions);
                if (!hasPermission) {
                    // Log policy violation
                    await auditLogger_js_1.auditLogger.log({
                        tenantId: req.tenantId,
                        userId: req.userId,
                        action: index_js_1.AuditAction.POLICY_VIOLATED,
                        resource: 'permission',
                        details: {
                ...
                const withinAllowedTime = await rbacService_js_1.rbacService.checkTimeRestriction(req.tenantId, req.userRole);
...
const requireRole = (...allowedRoles) => {
    return (req, res, next) => {
        ...
        if (req.userRole === index_js_1.OrgRole.OWNER || allowedRoles.includes(req.userRole)) {
            next();
            return;
        }
        res.status(403).json({
...
exports.requireTenantType = requireTenantType;
// ============================================================================
// HELPER FUNCTIONS
// ============================================================================
