...
app.get('/health', async (req, res) => {
  ...
  let servicesHealth = {
    memory: { service: 'hojai-memory', status: 'unknown' as 'healthy' | 'unhealthy' | 'unknown', url: 'http://localhost:4520' },
    event: { service: 'hojai-event', status: 'unknown' as 'healthy' | 'unhealthy' | 'unknown', url: 'http://localhost:4510' },
    training: { service: 'hojai-training-pipeline', status: 'unknown' as 'healthy' | 'unhealthy' | 'unknown', url: 'http://localhost:4880' }
  };
  ...
  try {
    if (serviceConnector.isInitialized()) {
      ...
      health.forEach(h => {
        if (h.service.includes('memory')) servicesHealth.memory = { ...h, url: 'http://localhost:4520' };
        if (h.service.includes('event')) servicesHealth.event = { ...h, url: 'http://localhost:4510' };
        if (h.service.includes('training')) servicesHealth.training = { ...h, url: 'http://localhost:4880' };
      });
    }
  } catch (error) {
...
app.get('/api/brain/suggestions', async (req, res) => {
  try {
    const context = {
      conversationId: req.query.conversationId as string || 'unknown',
      tenantId: req.query.tenantId as string || 'default',
      customerId: req.query.customerId as string || 'anonymous',
      channel: (req.query.channel as any) || 'webchat',
      recentMessages: []
    };
...
export function emitOrderUpdate(orderId: string, update: any): void {
  io.to(`order:${orderId}`).emit('order:updated', update);
}

// Helper function to emit conversation updates
export function emitConversationUpdate(conversationId: string, update: any): void {
