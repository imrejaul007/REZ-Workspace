...
interface Order {
  ...
  payment: {
    method: 'upi' | 'card' | 'wallet' | 'cod' | 'netbanking';
    status: 'pending' | 'processing' | 'paid' | 'failed' | 'refunded' | 'partial';
  ...
  delivery: {
    ...
    pincode?: string;
    instructions?: string;
    status: 'pending' | 'confirmed' | 'preparing' | 'dispatched' | 'out_for_delivery' | 'delivered' | 'cancelled' | 'returned';
    estimatedTime?: string;
    dispatchedAt?: Date;
    deliveredAt?: Date;
  };

  channel: 'whatsapp' | 'instagram' | 'webchat' | 'sms' | 'email' | 'pos' | 'app';
  source?: string;

  cartId?: string;
  couponCode?: string;

  status: 'pending' | 'confirmed' | 'preparing' | 'dispatched' | 'delivered' | 'cancelled' | 'refunded';
...
router.get('/orders', requireAuth, async (req: Request, res: Response) => {
  try {
    ...
    let filtered = Array.from(orders.values()).filter(
      o => o.tenantId === authReq.tenant.tenantId
    );
...
// Get order by number
router.get('/orders/number/:orderNumber', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string || 'default';
    const order = Array.from(orders.values()).find(
      o => o.orderNumber === req.params.orderNumber && o.tenantId === tenantId
    );
...
// Update order status
router.patch('/orders/:id/status', requireAuth, requireRole('agent', 'admin'), async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const order = orders.get(req.params.id);
...
router.patch('/orders/:id/delivery', requireAuth, requireRole('agent', 'admin'), async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const order = orders.get(req.params.id);
...
router.patch('/orders/:id/payment', requireAuth, requireRole('agent', 'admin'), async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const order = orders.get(req.params.id);
...
router.post('/orders/:id/confirm-payment', async (req: Request, res: Response) => {
  try {
...
router.post('/orders/:id/refunds', requireAuth, async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const order = orders.get(req.params.id);
...
router.get('/orders/:id/refunds', requireAuth, async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const order = orders.get(req.params.id);
...
router.patch('/refunds/:id', requireAuth, requireRole('agent', 'admin'), async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
...
router.get('/orders/stats/summary', requireAuth, async (req: Request, res: Response) => {
  try {
    const authReq = req as AuthenticatedRequest;
    const tenantOrders = Array.from(orders.values()).filter(
      o => o.tenantId === authReq.tenant.tenantId
    );
