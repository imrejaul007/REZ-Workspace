...
// Seed demo products
function seedDemoProducts(tenantId) {
    const demoProducts = [
        {
            id: 'prod_1',
            tenantId,
...
router.patch('/settings', requireAuth, requireAuth, (req, res) => {
    try {
        const authReq = req;
...
router.get('/products', async (req, res) => {
    try {
        ...
        let filtered = Array.from(products.values()).filter(p => p.tenantId === tenantId);
        if (status) {
...
router.post('/cart', async (req, res) => {
    try {
        ...
        let cart = Array.from(carts.values()).find(c => c.customerId === validated.customerId &&
            c.tenantId === tenantId &&
            c.status === 'active');
        if (!cart) {
            cart = {
...
router.get('/cart/:sessionId', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        const cart = Array.from(carts.values()).find(c => c.sessionId === req.params.sessionId &&
            c.tenantId === tenantId &&
            c.status === 'active');
        if (!cart) {
...
router.get('/cart', async (req, res) => {
    try {
        ...
        const cart = Array.from(carts.values()).find(c => c.customerId === customerId &&
            c.tenantId === tenantId &&
            c.status === 'active');
        if (!cart) {
            return handleError(res, 'Cart not found', 404);
...
router.post('/cart/:id/items', async (req, res) => {
    try {
        ...
        // Find existing item
        const existingItem = cart.items.find(i => i.productId === validated.productId && i.variantId === validated.variantId);
        if (existingItem) {
...
// Update cart item
router.patch('/cart/:id/items/:itemIndex', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        const cart = carts.get(req.params.id);
        if (!cart || cart.tenantId !== tenantId) {
...
// Remove from cart
router.delete('/cart/:id/items/:itemIndex', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        const cart = carts.get(req.params.id);
        if (!cart || cart.tenantId !== tenantId) {
...
// Apply coupon
router.post('/cart/:id/coupon', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        const cart = carts.get(req.params.id);
        if (!cart || cart.tenantId !== tenantId) {
        ...
        const coupon = Array.from(coupons.values()).find(c => c.code === validated.couponCode &&
            c.tenantId === tenantId &&
            c.status === 'active');
        if (!coupon) {
...
// Remove coupon
router.delete('/cart/:id/coupon', async (req, res) => {
    try {
        const tenantId = req.headers['x-tenant-id'] || 'default';
        const cart = carts.get(req.params.id);
        if (!cart || cart.tenantId !== tenantId) {
...
router.get('/coupons', requireAuth, async (req, res) => {
    try {
        const authReq = req;
        const couponsList = Array.from(coupons.values()).filter(c => c.tenantId === authReq.tenant.tenantId);
...
function recalculateCart(cart) {
    ...
    cart.discount = 0;
    if (cart.couponCode) {
        const coupon = Array.from(coupons.values()).find(c => c.code === cart.couponCode && c.status === 'active');
        if (coupon) {
            switch (coupon.type) {
                case 'percentage':
    ...
    if (cart.couponCode) {
        const coupon = Array.from(coupons.values()).find(c => c.code === cart.couponCode && c.status === 'active' && c.type === 'free_shipping');
        if (coupon) {
            cart.deliveryFee = 0;
        }
    }
