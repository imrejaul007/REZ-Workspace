...
router.get('/tickets', requireAuth, async (req, res) => {
    try {
        const authReq = req;
        const { status, priority, category, assignedAgentId, search, limit = '50', offset = '0' } = req.query;
        let filtered = Array.from(tickets.values()).filter(t => t.tenantId === authReq.tenant.tenantId);
        if (status) {
        ...
        if (search) {
            ...
            filtered = filtered.filter(t => t.subject.toLowerCase().includes(searchLower) ||
...
router.get('/tickets/number/:ticketNumber', async (req, res) => {
    try {
        const ticket = Array.from(tickets.values()).find(t => t.ticketNumber === req.params.ticketNumber);
        if (!ticket) {
            return handleError(res, 'Ticket not found', 404);
        }
        successResponse(res, ticket);
...
// Update ticket
router.patch('/tickets/:id', requireAuth, requireRole('agent', 'admin'), async (req, res) => {
    try {
        const authReq = req;
        const ticket = tickets.get(req.params.id);
        if (!ticket || ticket.tenantId !== authReq.tenant.tenantId) {
        ...
        if (validated.assignedAgentId)
            ticket.assignedAgentId = validated.assignedAgentId;
        if (validated.assignedAgentName)
            ticket.assignedAgentName = validated.assignedAgentName;
        if (validated.team)
            ticket.team = validated.team;
...
router.post('/tickets/:id/assign', requireAuth, requireRole('agent', 'admin'), async (req, res) => {
    try {
        const authReq = req;
        const ticket = tickets.get(req.params.id);
        if (!ticket || ticket.tenantId !== authReq.tenant.tenantId) {
...
// Escalate ticket
router.post('/tickets/:id/escalate', requireAuth, requireRole('agent', 'admin'), async (req, res) => {
    try {
        const authReq = req;
        const ticket = tickets.get(req.params.id);
        if (!ticket || ticket.tenantId !== authReq.tenant.tenantId) {
...
// ============================================================================
// Resolve ticket
router.post('/tickets/:id/resolve', requireAuth, requireRole('agent', 'admin'), async (req, res) => {
    try {
        const authReq = req;
        const ticket = tickets.get(req.params.id);
        if (!ticket || ticket.tenantId !== authReq.tenant.tenantId) {
...
// Close ticket
router.post('/tickets/:id/close', requireAuth, requireRole('agent', 'admin'), async (req, res) => {
    try {
        const authReq = req;
        const ticket = tickets.get(req.params.id);
        if (!ticket || ticket.tenantId !== authReq.tenant.tenantId) {
...
router.get('/tickets/stats/summary', requireAuth, async (req, res) => {
    try {
        const authReq = req;
        const tenantTickets = Array.from(tickets.values()).filter(t => t.tenantId === authReq.tenant.tenantId);
        const stats = {
...
// ============================================================================
// Bulk assign tickets
router.post('/tickets/bulk/assign', requireAuth, requireRole('admin'), async (req, res) => {
    try {
        const authReq = req;
