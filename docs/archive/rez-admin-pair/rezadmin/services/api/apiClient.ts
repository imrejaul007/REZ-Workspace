...
class ApiClient {
  ...
  private async getHeaders(
    ...
    const headers: Record<string, string> = {
      ...
      'X-App-Version':
        Constants.expoConfig?.version ?? (String((Constants.manifest as Record<string, unknown>)?.version ?? '1.0.0')),
      ...customHeaders,
    };
  ...
  private async request<T, B = Record<string, unknown>>(
    ...
    if (typeof document !== 'undefined' && _CSRF_MUTATING_METHODS.has(method)) {
      const csrfToken = readCsrfTokenFromCookie();
      if (csrfToken) {
        headers['X-CSRF-Token'] = csrfToken;
      } else {
        // ADM-008 FIX: warn on every request so repeated failures are visible
