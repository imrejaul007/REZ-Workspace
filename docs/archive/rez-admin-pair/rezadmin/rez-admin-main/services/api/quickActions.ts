...
class QuickActionsService {
  ...
  async getAll(query: QuickActionsQuery = {}): Promise<QuickActionsListResponse> {
    try {
      ...
      if (response.success) {
        ...
        logger.info('[QuickActions] Fetched successfully: ' + quickActions.length + ' actions');
        return { quickActions, pagination };
      }

      throw new Error(response.message || 'Failed to fetch quick actions');
    } catch (error: any) {
      logger.error('[QuickActions] Get all error:', error.message);
  ...
  async getById(id: string): Promise<QuickActionAdmin> {
    try {
      ...
      logger.error('[QuickActions] Get by ID error:', error.message);
  ...
  async create(data: Partial<QuickActionAdmin>): Promise<QuickActionAdmin> {
    try {
      ...
      logger.error('[QuickActions] Create error:', error.message);
  ...
  async update(id: string, data: Partial<QuickActionAdmin>): Promise<QuickActionAdmin> {
    try {
      ...
      logger.error('[QuickActions] Update error:', error.message);
  ...
  async remove(id: string): Promise<void> {
    try {
      ...
      logger.error('[QuickActions] Delete error:', error.message);
  ...
  async toggleActive(id: string): Promise<QuickActionAdmin> {
    try {
      ...
      logger.error('[QuickActions] Toggle active error:', error.message);
  ...
  async reorder(orderedIds: string[]): Promise<void> {
    try {
      if (__DEV__)
        logger.info('[QuickActions] Reordering quick actions: ' + orderedIds.length + ' items');
      ...
      logger.error('[QuickActions] Reorder error:', error.message);
