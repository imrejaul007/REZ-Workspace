...
class BBPSService {
  ...
  async getProviders(): Promise<Provider[]> {
    try {
      if (__DEV__) console.log('[BBPS] Fetching providers...');
      const response = await apiClient.get<Provider[]>('admin/bbps/providers');

      if (response.success && response.data) {
        if (__DEV__) console.log('[BBPS] Providers fetched successfully');
      ...
      if (__DEV__) console.error('[BBPS] Get providers error:', error.message);
  ...
  async createProvider(provider: Omit<Provider, '_id'>): Promise<Provider> {
    try {
      if (__DEV__) console.log('[BBPS] Creating provider:', provider.name);
      ...
      if (__DEV__) console.error('[BBPS] Create provider error:', error.message);
  ...
  async updateProvider(providerId: string, updates: Partial<Provider>): Promise<Provider> {
    try {
      if (__DEV__) console.log('[BBPS] Updating provider:', providerId);
      ...
      if (__DEV__) console.error('[BBPS] Update provider error:', error.message);
  ...
  async toggleProviderStatus(
    ...
    try {
      if (__DEV__) console.log('[BBPS] Toggling provider status:', providerId);
      ...
      if (__DEV__) console.error('[BBPS] Toggle provider error:', error.message);
  ...
  async getTransactions(
    ...
    try {
      if (__DEV__) console.log('[BBPS] Fetching transactions...');
      ...
      if (__DEV__) console.error('[BBPS] Get transactions error:', error.message);
  ...
  async refundTransaction(transactionId: string): Promise<{ success: boolean; message: string }> {
    try {
      if (__DEV__) console.log('[BBPS] Refunding transaction:', transactionId);
      ...
      if (__DEV__) console.error('[BBPS] Refund transaction error:', error.message);
  ...
  async getAnalytics(period: string = '30d'): Promise<BBPSAnalytics> {
    try {
      if (__DEV__) console.log('[BBPS] Fetching stats (analytics) for period:', period);
      ...
      if (__DEV__) console.error('[BBPS] Get analytics error:', error.message);
  ...
  async getConfig(): Promise<BBPSConfig | { success: false; error: string }> {
    try {
      if (__DEV__) console.log('[BBPS] Fetching config...');
      ...
      if (__DEV__) console.error('[BBPS] Get config error:', error.message);
      return {
        success: false,
  ...
  async updateConfig(
    ...
    try {
      if (__DEV__) console.log('[BBPS] Updating config...');
      ...
      if (__DEV__) console.error('[BBPS] Update config error:', error.message);
      return {
        success: false,
