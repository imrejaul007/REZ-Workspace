...
export const surpriseCoinDropsService = {
  ...
  async createDrop(dropData: CreateSurpriseCoinDropRequest): Promise<SurpriseCoinDrop> {
    const res = await apiClient.post<SurpriseCoinDrop>(
  ...
  async bulkCreate(
    ...
    const res = await apiClient.post<{ created: number; skipped: number; invalidIds: number }>(
  ...
  async updateDrop(
    ...
    const res = await apiClient.put<SurpriseCoinDrop>(
