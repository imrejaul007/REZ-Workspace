...
beforeAll(async () => {
  const mod = await import('../../services/api/pos');
...
describe('POSService', () => {
  ...
  it('createBill sends bill data', async () => {
    mockPost.mockResolvedValue({ success: true, data: { _id: 'b1', total: 500 } });
    const result = await posService.createBill({
      items: [{ name: 'Coffee', price: 150, quantity: 2 }],
    });
  ...
  it('markAsPaid marks bill as paid', async () => {
    mockPut.mockResolvedValue({ success: true, data: { billId: 'b1', status: 'paid' } });
...
describe('generateUPILink', () => {
  it('generates valid UPI deep link', () => {
    const link = generateUPILink({
      ...
      name: 'Test Store',
      amount: 500,
      transactionRef: 'REZ-001',
    });
    expect(link).toContain('upi://pay');
    expect(link).toContain('pa=store@upi');
