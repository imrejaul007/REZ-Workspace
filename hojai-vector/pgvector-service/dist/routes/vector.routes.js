...
// Route Factory
// ============================================================================
export function createVectorRouter(logger) {
    const router = Router();
    const storage = getStorage();
    // Helper to create API response
    const createResponse = (data, requestId) => ({
        success: true,
        data,
        meta: {
    ...
    const createErrorResponse = (code, message, requestId, details) => ({
        success: false,
        error: {
            code,
            message,
    ...
    router.post('/vectors', async (req, res) => {
        ...
        try {
            ...
            if (!validationResult.success) {
                ...
                res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'Invalid request body', requestId, { errors: validationResult.error.errors }));
                return;
            }
            ...
            res.status(500).json(createErrorResponse('INTERNAL_ERROR', message, requestId));
        }
    });
    // ==========================================================================
    // POST /api/vectors/batch - Batch insert vectors
    // ==========================================================================
    router.post('/vectors/batch', async (req, res) => {
        const requestId = res.getHeader('X-Request-Id') || `req_${Date.now()}`;
        try {
            // Validate request body
    ...
    // ==========================================================================
    router.post('/vectors/search', async (req, res) => {
        const requestId = res.getHeader('X-Request-Id') || `req_${Date.now()}`;
        try {
            // Validate request body
    ...
    router.get('/vectors/:id', async (req, res) => {
        ...
        try {
            ...
            if (!id || !z.string().uuid().safeParse(id).success) {
                res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'Invalid vector ID format', requestId));
                return;
            }
            ...
            if (!vector) {
                ...
                res.status(404).json(createErrorResponse('NOT_FOUND', 'Vector not found', requestId));
                return;
            }
    ...
    // ==========================================================================
    router.get('/namespaces/:namespace/vectors', async (req, res) => {
        const requestId = res.getHeader('X-Request-Id') || `req_${Date.now()}`;
        const namespace = req.params['namespace'];
        if (!namespace) {
            res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'Namespace is required', requestId));
            return;
        }
        try {
            ...
            if (!queryValidation.success) {
                res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'Invalid query parameters', requestId, { errors: queryValidation.error.errors }));
                return;
            }
    ...
    // ==========================================================================
    router.get('/namespaces/:namespace/stats', async (req, res) => {
        const requestId = res.getHeader('X-Request-Id') || `req_${Date.now()}`;
        const namespace = req.params['namespace'];
        if (!namespace) {
            res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'Namespace is required', requestId));
        ...
        try {
            ...
            if (!stats) {
                res.status(404).json(createErrorResponse('NOT_FOUND', 'Namespace not found', requestId));
                return;
            }
