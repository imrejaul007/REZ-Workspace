...
const INTERNAL_TOKEN = config.integrations.internalToken;
...
export async function syncToHOJAI(entityType: string, action: string, data: any): Promise<void> {
  try {
    await axios.post(
      ...
      { entityType, action, source: 'ledgerai', data, timestamp: new Date().toISOString() },
      { headers: { 'X-Internal-Token': INTERNAL_TOKEN, 'Content-Type': 'application/json' } }
    );
