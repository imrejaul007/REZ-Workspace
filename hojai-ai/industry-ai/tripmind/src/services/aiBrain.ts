...
interface TripPreferences {
  ...
  travelers: number;
  budget?: number;
  travelStyle?: 'budget' | 'moderate' | 'luxury' | 'ultra-luxury';
  interests?: string[];
...
interface PackingList {
  essentials: string[];
  clothing: { category: string; items: string[] }[];
  toiletries: string[];
  electronics: string[];
  documents: string[];
...
interface TravelAdvisory {
  level: 'safe' | 'exercise_normal_precautions' | 'exercise_increased_caution' | 'reconsider_travel' | 'do_not_travel';
  warnings: string[];
...
class TripMindAIBrain {
  private name = 'TripMind AI Brain';
   ...
   */
  async planTrip(customerId: string, preferences: TripPreferences): Promise<AIResponse<{
    itinerary: any;
    summary: any;
    recommendations: any;
    aiInsights: string[];
  ...
  async optimizeRoute(
    ...
    preferences?: { optimizeFor?: 'time' | 'cost' | 'distance' | 'experience' }
  ): Promise<AIResponse<RouteOptimizationResult>> {
    const startTime = Date.now();
  ...
  ): Promise<AIResponse<TravelAdvisory>> {
    const startTime = Date.now();

    try {
      const prompt = `Provide travel advisory for ${destination}${travelDates ? ` from ${travelDates.start} to ${travelDates.end}` : ''}. Include safety level, health advisories, local customs, and emergency contacts.`;
   ...
   */
  async planBudget(preferences: TripPreferences): Promise<AIResponse<BudgetPlan>> {
    const startTime = Date.now();
   ...
   */
  async generatePackingList(preferences: TripPreferences): Promise<AIResponse<PackingList>> {
    const startTime = Date.now();
   ...
   */
  async query(userQuery: string, context?: any): Promise<AIResponse<{ response: string; suggestedActions?: string[] }>> {
    const startTime = Date.now();
  ...
  private buildTripPlanningPrompt(preferences: TripPreferences): string {
  ...
  private estimateTripCost(preferences: TripPreferences, days: number): number {
    const dailyRates = {
      budget: 50,
      moderate: 150,
      luxury: 400,
    ...
    const style = preferences.travelStyle || 'moderate';
    return dailyRates[style] * days * preferences.travelers;
  }

  private estimateDefaultBudget(preferences: TripPreferences): number {
    const days = this.calculateDays(preferences.startDate, preferences.endDate);
  ...
  private calculateOptimalRoute(
    ...
    const route: RoutePoint[] = locations.map((loc, idx) => ({
      order: idx + 1,
      location: loc.name,
  ...
  private generateAlternativeRoutes(locations: { name: string; coordinates?: { lat: number; lng: number } }[]): RoutePoint[][] {
  ...
  private getSuggestedTransport(index: number, total: number): string {
  ...
  private calculateBudgetBreakdown(
    ...
    style: string,
    travelers: number
  ): BudgetPlan['breakdown'] {
    const multipliers = {
      budget: { flights: 0.35, accommodation: 0.25, food: 0.15, activities: 0.10, transport: 0.08, misc: 0.07 },
      moderate: { flights: 0.30, accommodation: 0.30, food: 0.15, activities: 0.12, transport: 0.08, misc: 0.05 },
      luxury: { flights: 0.25, accommodation: 0.35, food: 0.18, activities: 0.12, transport: 0.06, misc: 0.04 },
      'ultra-luxury': { flights: 0.20, accommodation: 0.40, food: 0.20, activities: 0.12, transport: 0.05, misc: 0.03 },
    };

    const m = multipliers[style as keyof typeof multipliers] || multipliers.moderate;
    ...
    return {
      ...
      miscellaneous: Math.round(totalBudget * m.misc),
    };
  }

  private generateBudgetRecommendations(budget: number, days: number, style: string): string[] {
  ...
  private generatePackingTips(preferences: TripPreferences): string[] {
  ...
  private generateLocalTips(destination: string): string[] {
    return [
      'Learn basic local phrases',
  ...
  private getWeatherSpecificItems(destination: string): string[] {
    const dest = destination.toLowerCase();
  ...
  private getDestinationSpecificItems(destination: string): string[] {
    const dest = destination.toLowerCase();
  ...
  private extractAIInsights(response: any): string[] {
  ...
  private extractSuggestedActions(text: string): string[] {
    ...
    if (text.toLowerCase().includes('book')) actions.push('Book now');
    if (text.toLowerCase().includes('flight')) actions.push('Search flights');
    if (text.toLowerCase().includes('hotel')) actions.push('Find hotels');
    if (text.toLowerCase().includes('visa')) actions.push('Check visa requirements');
    return actions;
  }
}

// Export singleton instance
export const tripMindAIBrain = new TripMindAIBrain();
export default tripMindAIBrain;
