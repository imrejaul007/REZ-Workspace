...
import guestTwinRoutes from './routes/guest-twin.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '8443');
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'guest-twin-service',
    version: '1.0.0',
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/guest_twin';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Guest Twin Service started`, {
        port: PORT,
