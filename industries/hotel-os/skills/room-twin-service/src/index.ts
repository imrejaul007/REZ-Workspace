...
import roomTwinRoutes from './routes/room-twin.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '8444');
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'room-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/room', roomTwinRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/room_twin';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Room Twin Service started`, {
        port: PORT,
