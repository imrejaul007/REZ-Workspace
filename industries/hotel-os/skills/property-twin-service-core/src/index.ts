...
import propertyTwinRoutes from './routes/property-twin.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '8445');
...
app.get('/health', (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'property-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/property', propertyTwinRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/property_twin';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Property Twin Service started`, {
        port: PORT,
