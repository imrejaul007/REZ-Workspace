...
class MessageBroker {
  ...
  async connect(): Promise<void> {
    ...
    try {
      ...
      // Declare queues
      await this.channel.assertQueue('restaurant.twin.events', { durable: true });
      await this.channel.assertQueue('restaurant.order.events', { durable: true });
      await this.channel.assertQueue('restaurant.inventory.events', { durable: true });

      // Bind queues to exchange
      await this.channel.bindQueue('restaurant.twin.events', 'restaurant.events', 'restaurant.twin.*');
      await this.channel.bindQueue('restaurant.order.events', 'restaurant.events', 'restaurant.order.*');
      await this.channel.bindQueue('restaurant.inventory.events', 'restaurant.events', 'restaurant.inventory.*');
