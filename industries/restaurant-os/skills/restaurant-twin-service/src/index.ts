...
import restaurantTwinRoutes from './routes/restaurant-twin.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4011');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'restaurant-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/restaurant', restaurantTwinRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/restaurant_twin';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Restaurant Twin Service started`, {
        port: PORT,
