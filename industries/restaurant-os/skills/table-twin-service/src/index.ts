...
import tableTwinRoutes from './routes/table-twin.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4012');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'table-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/table', tableTwinRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/table_twin';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Table Twin Service started`, {
        port: PORT,
