...
import stafftwinserviceRoutes from './routes/stafftwinservice.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4018');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'staff-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/staff/twin/service', stafftwinserviceRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/staff_twin_service';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Staff Twin Service started`, {
        port: PORT,
