...
import kitchentwinserviceRoutes from './routes/kitchentwinservice.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4015');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'kitchen-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/kitchen/twin/service', kitchentwinserviceRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/kitchen_twin_service';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Kitchen Twin Service started`, {
        port: PORT,
