...
import customertwinserviceRoutes from './routes/customertwinservice.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4017');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'customer-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/customer/twin/service', customertwinserviceRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/customer_twin_service';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Customer Twin Service started`, {
        port: PORT,
