...
import loyaltytwinserviceRoutes from './routes/loyaltytwinservice.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4019');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'loyalty-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/loyalty/twin/service', loyaltytwinserviceRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/loyalty_twin_service';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Loyalty Twin Service started`, {
        port: PORT,
