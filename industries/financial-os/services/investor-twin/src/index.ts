...
import investorTwinRoutes from './routes/investor-twin.routes';

const app = express();
const PORT = parseInt(process.env.SERVICE_PORT || '4030');
...
app.get('/health', async (req, res) => {
  res.json({
    status: 'healthy',
    service: process.env.SERVICE_NAME || 'investor-twin-service',
    version: '1.0.0',
...
// API routes
app.use('/api/twins/investor', investorTwinRoutes);
...
async function connectDatabase(): Promise<void> {
  const mongoUri = process.env.MONGODB_URI || 'mongodb://localhost:27017/investor_twin';
...
async function start(): Promise<void> {
  try {
    ...
    app.listen(PORT, () => {
      logger.info(`Investor Twin Service started`, {
        port: PORT,
