...
import { Subscription, subscriptionStore } from '../index.js';
...
router.post('/', (req: Request, res: Response) => {
  ...
  if (!name || (!eventType && !eventPattern) || !handler) {
    return res.status(400).json(
      createErrorResponse('INVALID_REQUEST', 'name, eventType/eventPattern, and handler required')
    );
  }
...
router.get('/:id', (req: Request, res: Response) => {
  ...
  const subscription = subscriptionStore.find(
    s => s.id === id && s.tenantId === ctx.tenant_id
  );
...
router.put('/:id', (req: Request, res: Response) => {
  ...
  if (handler !== undefined) subscription.handler = handler;
  if (filter !== undefined) subscription.filter = filter;
 ...
 */
router.post('/:id/reset-stats', (req: Request, res: Response) => {
  const ctx = req.tenantContext!;
  const { id } = req.params;
