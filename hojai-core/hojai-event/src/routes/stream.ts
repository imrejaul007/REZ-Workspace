...
import { EventStream, streamStore, eventStore } from '../index.js';
...
router.post('/', (req: Request, res: Response) => {
  ...
  if (!name || !eventTypes || !Array.isArray(eventTypes)) {
    return res.status(400).json(
      createErrorResponse('INVALID_REQUEST', 'name and eventTypes array required')
    );
  }
...
router.get('/:id/events', (req: Request, res: Response) => {
  ...
  if (after) {
    streamEvents = streamEvents.filter(e => e.timestamp > (after as string));
  }

  if (before) {
    streamEvents = streamEvents.filter(e => e.timestamp < (before as string));
  }
...
router.put('/:id', (req: Request, res: Response) => {
  ...
  if (eventTypes !== undefined) stream.eventTypes = eventTypes;
  if (retentionDays !== undefined) stream.retentionDays = retentionDays;
 ...
 */
router.post('/:id/cleanup', (req: Request, res: Response) => {
  const ctx = req.tenantContext!;
  const { id } = req.params;
  ...
  const filtered = tenantEvents.filter(
    e => !stream.eventTypes.includes(e.type) || new Date(e.timestamp) > cutoff
  );
