...
import { createResponse, createErrorResponse, Workflow } from '../types/index.js';
...
router.post('/', tenantMiddleware(), (req: Request, res: Response) => {
  ...
  if (!name || !definition) {
    return res.status(400).json(
      createErrorResponse('INVALID_REQUEST', 'name and definition are required')
    );
  }
...
router.get('/:id', tenantMiddleware(), (req: Request, res: Response) => {
  ...
  if (!workflow) {
    return res.status(404).json(
      createErrorResponse('NOT_FOUND', `Workflow ${id} not found`)
    );
  }
...
router.put('/:id', tenantMiddleware(), (req: Request, res: Response) => {
  ...
  if (definition) workflow.definition = definition;
 ...
 * Activate a workflow
 */
router.post('/:id/activate', tenantMiddleware(), (req: Request, res: Response) => {
  const ctx = req.tenantContext!;
  const { id } = req.params;
  ...
  if (workflow.status !== 'draft' && workflow.status !== 'paused') {
    return res.status(400).json(
      createErrorResponse('INVALID_STATE', 'Only draft or paused workflows can be activated')
    );
  }
 ...
 */
router.post('/:id/trigger', tenantMiddleware(), (req: Request, res: Response) => {
  const ctx = req.tenantContext!;
  const { id } = req.params;
  ...
  if (workflow.status !== 'active') {
    return res.status(400).json(
      createErrorResponse('INVALID_STATE', 'Workflow must be active to trigger')
    );
  }
