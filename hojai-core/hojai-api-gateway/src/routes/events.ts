...
import { createResponse, createErrorResponse, Event } from '../types/index.js';
...
router.post('/', tenantMiddleware(), (req: Request, res: Response) => {
  ...
  if (!type || !data) {
    return res.status(400).json(
      createErrorResponse('INVALID_REQUEST', 'type and data are required')
    );
  }
...
router.get('/:id', tenantMiddleware(), (req: Request, res: Response) => {
  ...
  if (!event) {
    return res.status(404).json(
      createErrorResponse('NOT_FOUND', `Event ${id} not found`)
    );
  }
 ...
 */
router.post('/subscribe', tenantMiddleware(), (req: Request, res: Response) => {
  const ctx = req.tenantContext!;
  const { eventType, callbackUrl } = req.body;

  if (!eventType || !callbackUrl) {
    return res.status(400).json(
      createErrorResponse('INVALID_REQUEST', 'eventType and callbackUrl are required')
    );
  }
