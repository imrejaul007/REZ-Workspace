...
class ModelRegistryService {
    ...
    async registerModel(request) {
        ...
        if (existingVersion) {
            throw new error_1.ConflictError(`Model '${name}' version '${version}' already exists`);
        }
        // Create new version
        const newVersion = {
            version,
     ...
     */
    async getModelVersions(name) {
        const modelInfo = this.models.get(name);
        if (!modelInfo) {
            throw new error_1.NotFoundError('Model', name);
        }
        return {
            name,
    ...
    async getVersion(name, version) {
        ...
        if (!modelVersion) {
            throw new error_1.NotFoundError('Version', `${name}:${version}`);
        }
        return {
            name,
     ...
     */
    async updateStage(name, version, newStage) {
        const modelInfo = this.models.get(name);
        if (!modelInfo) {
            throw new error_1.NotFoundError('Model', name);
        }
