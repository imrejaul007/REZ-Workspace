...
export class FeatureStore {
     ...
     */
    async storeFeatures(tenantId, customerId, features) {
     ...
     */
    async getFeatures(tenantId, customerId) {
        const key = `${tenantId}:${customerId}`;
...
export class PredictionEngine {
     ...
     */
    async predictChurn(tenantId, customerId) {
     ...
     */
    async predictLTV(tenantId, customerId) {
        const features = await this.featureStore.getFeatures(tenantId, customerId);
     ...
     */
    async predictConversion(tenantId, customerId) {
        const features = await this.featureStore.getFeatures(tenantId, customerId);
        const engagement = features.engagement_score || 50;
        const orderFrequency = features.order_frequency || 0;
     ...
     */
    async predictRevisit(tenantId, customerId) {
        const features = await this.featureStore.getFeatures(tenantId, customerId);
        const orderFrequency = features.order_frequency || 0;
        const daysSinceLast = features.days_since_last_order || 999;
        const engagement = features.engagement_score || 50;
     ...
     */
    async predictAll(tenantId, customerId) {
...
export class RecommendationEngine {
     ...
     */
    async getRecommendations(tenantId, customerId, limit = 10) {
     ...
     */
    async getSimilarProducts(tenantId, productId, limit = 5) {
        return [
     ...
     */
    async getTrending(tenantId, category, limit = 10) {
        return [
...
export class SegmentationEngine {
     ...
     * Segment a customer
     */
    async segmentCustomer(tenantId, customerId, features) {
        const segments = [];
        const segmentScores = {};
        // RFM Segmentation
...
export class HojaiIntelligencePlatform {
     ...
     * Get recommendations
     */
    async recommend(tenantId, customerId, limit = 10) {
        return this.recommendationEngine.getRecommendations(tenantId, customerId, limit);
    }
    /**
     * Segment customer
     */
    async segment(tenantId, customerId, features) {
 ...
 */
export function createIntelligenceRoutes(platform) {
     ...
     * Store customer features
     */
    router.post('/features', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerId, features } = req.body;
            if (!customerId || !features) {
                return res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'customerId and features are required'));
            }
            ...
            res.status(500).json(createErrorResponse('STORE_ERROR', 'Failed to store features'));
        }
    });
    // ============================================
    // PREDICTIONS
     ...
     * Get all predictions
     */
    router.post('/predict', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerId } = req.body;
            if (!customerId) {
                return res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'customerId is required'));
            }
            ...
            res.status(500).json(createErrorResponse('PREDICT_ERROR', 'Failed to generate predictions'));
        }
    });
    /**
     ...
     * Predict churn
     */
    router.post('/predict/churn', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerId } = req.body;
            ...
            res.status(500).json(createErrorResponse('PREDICT_ERROR', 'Failed to predict churn'));
        }
    });
    /**
     ...
     * Predict LTV
     */
    router.post('/predict/ltv', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerId } = req.body;
            ...
            res.status(500).json(createErrorResponse('PREDICT_ERROR', 'Failed to predict LTV'));
        }
    });
    // ============================================
    // RECOMMENDATIONS
     ...
     * Get recommendations
     */
    router.post('/recommend', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerId, limit } = req.body;
            const recommendations = await platform.recommend(tenantId, customerId, limit || 10);
            res.json(createResponse(recommendations, { tenantId }));
        }
        catch (error) {
            logger.error('recommend_error', { error });
            res.status(500).json(createErrorResponse('RECOMMEND_ERROR', 'Failed to generate recommendations'));
        }
    });
    /**
     ...
     * Get trending products
     */
    router.get('/recommend/trending', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { category, limit } = req.query;
            const recEngine = new RecommendationEngine();
            const trending = await recEngine.getTrending(tenantId, category, limit ? parseInt(limit) : 10);
            res.json(createResponse(trending, { tenantId }));
        }
        catch (error) {
            logger.error('trending_error', { error });
            res.status(500).json(createErrorResponse('TRENDING_ERROR', 'Failed to get trending products'));
        }
    });
    // ============================================
    // SEGMENTATION
     ...
     * Segment customer
     */
    router.post('/segment', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerId, features } = req.body;
            if (!customerId) {
            ...
            res.status(500).json(createErrorResponse('SEGMENT_ERROR', 'Failed to segment customer'));
        }
    });
    /**
     ...
     */
    router.post('/segment/batch', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { customerIds } = req.body;
            if (!customerIds || !Array.isArray(customerIds)) {
                return res.status(400).json(createErrorResponse('VALIDATION_ERROR', 'customerIds array is required'));
            }
            const results = await Promise.all(customerIds.map(id => platform.segment(tenantId, id)));
            res.json(createResponse(results, { tenantId }));
        }
        catch (error) {
            logger.error('batch_segment_error', { error });
            res.status(500).json(createErrorResponse('SEGMENT_ERROR', 'Failed to batch segment'));
        }
    });
    return router;
}
