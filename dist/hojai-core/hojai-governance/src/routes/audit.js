...
import { createResponse, createErrorResponse, tenantMiddleware } from '../index.js';
const router = Router();
 ...
 */
router.get('/meta/actions', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    const actions = [
        // User actions
 ...
 */
router.get('/stats', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
 ...
 */
router.post('/export', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    ...
    if (format === 'csv') {
        const csv = [
            ...
            ...entries.map(e => `${e.id},${e.timestamp},${e.tenantId},${e.userId || ''},${e.action},${e.resource},${e.resourceId || ''}`)
        ].join('\n');
