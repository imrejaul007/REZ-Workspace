...
import { ROLE_DEFINITIONS, createResponse, createErrorResponse, tenantMiddleware, logAudit } from '../index.js';
const router = Router();
const userRoles = [];
/**
 * GET /api/roles
 ...
 */
router.get('/user/:userId', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
 ...
 */
router.post('/assign', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    ...
    const existing = userRoles.find(ur => ur.userId === userId && ur.roleId === roleId && ur.tenantId === ctx.tenant_id);
    if (existing) {
 ...
 */
router.delete('/revoke', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    const { userId, roleId } = req.body;
    if (!userId || !roleId) {
        return res.status(400).json(createErrorResponse('INVALID_REQUEST', 'userId and roleId required'));
    }
    const index = userRoles.findIndex(ur => ur.userId === userId && ur.roleId === roleId && ur.tenantId === ctx.tenant_id);
    if (index === -1) {
 ...
 */
router.get('/:id/users', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    const { id } = req.params;
    const role = ROLE_DEFINITIONS.find(r => r.id === id);
    if (!role) {
