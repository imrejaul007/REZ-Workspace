...
import { createResponse, createErrorResponse, tenantMiddleware, logAudit } from '../index.js';
const router = Router();
const tenantUsers = [];
/**
 * GET /api/users
...
router.post('/', tenantMiddleware(), (req, res) => {
    ...
    const existing = tenantUsers.find(u => u.email === email && u.tenantId === ctx.tenant_id);
    if (existing) {
