...
export class HojaiEventPlatform {
     ...
     */
    async publish(tenantId, type, data) {
        // Validate tenant
        if (!tenantId) {
 ...
 */
export function createEventRoutes(eventPlatform) {
    const router = express.Router();
    /**
     * POST /api/events/publish
     * Publish an event
     */
    router.post('/publish', tenantMiddleware(), async (req, res) => {
        try {
            const { type, data } = req.body;
            const tenantId = req.tenantContext.tenant_id;
            if (!type) {
                return res.status(400).json(createErrorResponse('MISSING_TYPE', 'Event type is required'));
            }
            ...
            res.status(500).json(createErrorResponse('PUBLISH_ERROR', 'Failed to publish event'));
        }
    });
    /**
     ...
     */
    router.post('/subscribe', tenantMiddleware(), async (req, res) => {
        try {
            const { eventType, callbackUrl } = req.body;
            const tenantId = req.tenantContext.tenant_id;
            if (!eventType) {
                return res.status(400).json(createErrorResponse('MISSING_EVENT_TYPE', 'Event type is required'));
            }
            // For HTTP callbacks, store the URL
            // For inline handlers, this would be different
            const subscriptionId = eventPlatform.subscribe(tenantId, eventType, async (event) => {
                if (callbackUrl) {
            ...
            res.status(500).json(createErrorResponse('SUBSCRIBE_ERROR', 'Failed to create subscription'));
        }
    });
    /**
     ...
     */
    router.delete('/subscribe/:id', tenantMiddleware(), async (req, res) => {
        try {
            const { id } = req.params;
            const tenantId = req.tenantContext.tenant_id;
            const removed = eventPlatform.unsubscribe(tenantId, id);
            if (!removed) {
                return res.status(404).json(createErrorResponse('SUBSCRIPTION_NOT_FOUND', 'Subscription not found'));
            }
            res.json(createResponse({ removed: true }, { tenantId }));
        }
        catch (error) {
            logger.error('unsubscribe_error', { error });
            res.status(500).json(createErrorResponse('UNSUBSCRIBE_ERROR', 'Failed to remove subscription'));
        }
    });
    /**
     ...
     * Get event history
     */
    router.get('/history', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            ...
            res.status(500).json(createErrorResponse('HISTORY_ERROR', 'Failed to get event history'));
        }
    });
    /**
    ...
    router.get('/stats', tenantMiddleware(), async (req, res) => {
        try {
            ...
            res.status(500).json(createErrorResponse('STATS_ERROR', 'Failed to get stats'));
        }
    });
    return router;
}
