...
import { eventStore, subscriptionStore } from '../index.js';
const router = Router();
// ============================================
// HELPERS
// ============================================
...
// ============================================
async function notifySubscribers(tenantId, event) {
    const subscriptions = subscriptionStore.filter(s => s.tenantId === tenantId &&
        s.active &&
        (s.eventType === event.type || matchesPattern(s.eventPattern || '', event.type)));
    for (const sub of subscriptions) {
        // Check filter
        if (sub.filter) {
            const passesFilter = Object.entries(sub.filter).every(([key, value]) => event.data[key] === value);
            if (!passesFilter)
                continue;
        }
        sub.stats.received++;
