...
class DashboardEngine {
     ...
     * Get dashboard
     */
    async get(tenantId, dashboardId) {
     ...
     */
    async addWidget(tenantId, dashboardId, widget) {
     ...
     */
    async refresh(tenantId, dashboardId) {
        const dashboard = await this.get(tenantId, dashboardId);
        if (!dashboard)
...
class MetricsEngine {
     ...
     * Record metric
     */
    async record(tenantId, name, value, type, unit) {
        const now = new Date().toISOString();
     ...
     * Get timeseries data
     */
    async getTimeseries(tenantId, metricName, timeRange = '7d') {
...
class MLObservabilityEngine {
     ...
     * Register model
     */
    async register(tenantId, modelName, modelType) {
        const monitor = {
            id: `model_${Date.now()}`,
            tenant_id: tenantId,
     ...
     * Get model health
     */
    async getHealth(tenantId, modelId) {
...
export class HojaiAnalyticsPlatform {
    ...
    // Dashboards
    async createDashboard(tenantId, createdBy, data) {
        return this.dashboardEngine.create(tenantId, createdBy, data);
    }
    async getDashboard(tenantId, dashboardId) {
    ...
    // Reports
    async createReport(tenantId, createdBy, data) {
        return this.reportEngine.create(tenantId, createdBy, data);
    }
    async runReport(tenantId, reportId) {
        return this.reportEngine.run(tenantId, reportId);
    }
    async getReportRun(tenantId, runId) {
        return this.reportEngine.getRun(tenantId, runId);
    }
    // Metrics
    async recordMetric(tenantId, name, value, type) {
    ...
    async getMetricTimeseries(tenantId, metricName, timeRange) {
        return this.metricsEngine.getTimeseries(tenantId, metricName, timeRange);
    }
    // ML Observability
    async registerModel(tenantId, modelName, modelType) {
        return this.mlEngine.register(tenantId, modelName, modelType);
    }
    async getModelHealth(tenantId, modelId) {
...
// EXPRESS ROUTES
// ============================================
export function createAnalyticsRoutes(platform) {
    const router = express.Router();
    // ========== DASHBOARDS ==========
    router.post('/dashboards', tenantMiddleware(), async (req, res) => {
        try {
    ...
    router.get('/dashboards', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/dashboards/:id', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/dashboards/:id/data', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    // ========== REPORTS ==========
    router.post('/reports', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.post('/reports/:id/run', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/reports/runs/:runId', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    // ========== METRICS ==========
    router.post('/metrics', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/metrics', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/metrics/:name/timeseries', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
            const { time_range } = req.query;
            const timeseries = await platform.getMetricTimeseries(tenantId, req.params.name, time_range);
    ...
    // ========== ML OBSERVABILITY ==========
    router.post('/ml/models', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/ml/models', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
    ...
    router.get('/ml/models/:id/health', tenantMiddleware(), async (req, res) => {
        try {
            const tenantId = req.tenantContext.tenant_id;
