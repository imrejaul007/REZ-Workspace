...
export class HojaiDataService extends BaseService {
    ...
    setupRoutes() {
        // Customer routes
        this.app.get('/api/customers', tenantMiddleware(), async (req, res) => {
            const repo = this.getCustomerRepository(req.tenantContext.tenant_id);
            const customers = await repo.findMany({});
            res.json({ success: true, data: customers });
        });
        this.app.get('/api/customers/:id', tenantMiddleware(), async (req, res) => {
            const repo = this.getCustomerRepository(req.tenantContext.tenant_id);
        ...
        // Order routes
        this.app.get('/api/orders', tenantMiddleware(), async (req, res) => {
            const repo = this.getOrderRepository(req.tenantContext.tenant_id);
            const orders = await repo.findMany({});
            res.json({ success: true, data: orders });
        });
        this.app.get('/api/orders/:id', tenantMiddleware(), async (req, res) => {
            const repo = this.getOrderRepository(req.tenantContext.tenant_id);
