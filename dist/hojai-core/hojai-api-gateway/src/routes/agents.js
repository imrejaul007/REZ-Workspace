...
router.post('/', tenantMiddleware(), (req, res) => {
    ...
    if (!name || !type || !capabilities) {
        return res.status(400).json(createErrorResponse('INVALID_REQUEST', 'name, type, and capabilities are required'));
    }
    const agent = {
        id: uuidv4(),
...
router.get('/', tenantMiddleware(), (req, res) => {
    ...
    tenantAgents.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
 ...
 */
router.get('/:id/executions', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    const { id } = req.params;
 ...
 * Get execution by ID
 */
router.get('/executions/:execId', tenantMiddleware(), (req, res) => {
    const ctx = req.tenantContext;
    const { execId } = req.params;
    const tenantExecutions = executionStore.get(ctx.tenant_id) || [];
    const execution = tenantExecutions.find(e => e.id === execId);
    if (!execution) {
        return res.status(404).json(createErrorResponse('NOT_FOUND', `Execution ${execId} not found`));
    }
