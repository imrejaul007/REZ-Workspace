...
class HojaiAPIGateway {
    ...
    setupRoutes() {
        ...
        this.app.use((err, req, res, next) => {
            ...
            res.status(500).json(createErrorResponse('GATEWAY_ERROR', 'An error occurred'));
        });
        // 404 handler
    ...
    startHealthChecks() {
        setInterval(async () => {
            for (const [key, service] of Object.entries(SERVICES)) {
                try {
                    const response = await fetch(`http://${service.baseUrl}:${service.port}${service.healthPath}`, { timeout: 5000 });
