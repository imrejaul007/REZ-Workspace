...
export const OnboardingModel = mongoose.model('OnboardingSession', OnboardingSchema);
// ============================================================================
// ONBOARDING SERVICE
// ============================================================================
export class OnboardingService {
     ...
     * Save business info
     */
    async saveBusinessInfo(sessionId, params) {
        const session = await OnboardingModel.findById(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
     ...
     * Connect WhatsApp
     */
    async connectWhatsApp(sessionId, params) {
        const session = await OnboardingModel.findById(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
     ...
     */
    async addInitialKnowledge(sessionId, items) {
        const session = await OnboardingModel.findById(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
     ...
     */
    async createSubscription(sessionId, plan) {
        const session = await OnboardingModel.findById(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
     ...
     * Resend email OTP
     */
    async resendEmailOtp(sessionId) {
        const session = await OnboardingModel.findById(sessionId);
        if (!session) {
            throw new Error('Session not found');
        }
    ...
    getNextStep(status, steps) {
        ...
        if (!steps.knowledgeAdded)
            return 'add_knowledge';
        if (!steps.paymentDone)
            return 'payment';
        return 'completed';
    }
    /**
     ...
     */
    getKnowledgeTemplates(businessType) {
        const templates = {
            salon: [
