...
export class PaymentService {
    ...
    async cancelSubscription(tenantId) {
        const result = await SubscriptionModel.updateOne({ tenantId }, { $set: { status: 'cancelled' } });
    ...
    async hasActiveSubscription(tenantId) {
        ...
        if (subscription.currentPeriodEnd < new Date()) {
            await SubscriptionModel.updateOne({ tenantId }, { $set: { status: 'expired' } });
            return false;
        }
        return true;
    }
     ...
     * Check usage limits
     */
    async checkLimits(tenantId, type) {
        const subscription = await SubscriptionModel.findOne({
            tenantId,
            status: 'active'
        });
    ...
    async handleWebhook(event, payload) {
        switch (event) {
            case 'subscription.activated':
                await SubscriptionModel.updateOne({ razorpaySubscriptionId: payload.subscription_id }, { $set: { status: 'active' } });
                break;
            case 'subscription.cancelled':
                await SubscriptionModel.updateOne({ razorpaySubscriptionId: payload.subscription_id }, { $set: { status: 'cancelled' } });
                break;
            case 'subscription.expired':
                await SubscriptionModel.updateOne({ razorpaySubscriptionId: payload.subscription_id }, { $set: { status: 'expired' } });
                break;
            case 'subscription.past_due':
                await SubscriptionModel.updateOne({ razorpaySubscriptionId: payload.subscription_id }, { $set: { status: 'past_due' } });
                break;
        }
    }
}
