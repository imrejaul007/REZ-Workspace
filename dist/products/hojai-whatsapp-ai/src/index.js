...
app.get('/webhook/whatsapp', (req, res) => {
    ...
    const result = verifyWebhookChallenge(String(mode || ''), String(token || ''), String(challenge || ''), WHATSAPP_VERIFY_TOKEN);
    if (result.verified) {
...
// MESSAGE PROCESSING
// ============================================================================
async function processMessage(phoneNumberId, message, value) {
    const from = message.from;
    const customerName = value?.contacts?.[0]?.profile?.name || 'Customer';
    const messageContent = message.text?.body || '';
    // Get merchant
    const merchant = await merchantService.getMerchantByTenantId(process.env.TENANT_ID || 'demo');
    if (!merchant) {
        console.error('[Process] No merchant configured');
        return;
    }
    // Create/get conversation
    const conversation = await conversationService.getOrCreateConversation(merchant.tenantId, merchant.id, from, from, customerName);
...
// ============================================================================
async function emitSignals(merchant, userId, phone, aiResponse) {
    try {
