...
async function auth(req, res, next) {
    ...
    try {
        ...
        req.orgId = decoded.orgId || decoded.tenantId;
        next();
    }
    catch {
...
app.get('/api/org', auth, async (req, res) => {
    try {
        const org = await Org.findOne({ tenantId: req.tenantId });
        if (!org)
...
app.post('/api/org', auth, async (req, res) => {
    try {
        ...
        const org = new Org({ orgId, tenantId: req.tenantId, name, industry, size });
        await org.save();
...
// ORG CHART
// ============================================
app.get('/api/orgchart', auth, async (req, res) => {
    try {
...
// HUMANS
// ============================================
app.get('/api/humans', auth, async (req, res) => {
    try {
...
app.post('/api/humans', auth, async (req, res) => {
    try {
...
app.patch('/api/humans/:id', auth, async (req, res) => {
    try {
        const human = await Human.findOneAndUpdate({ employeeId: req.params.id, tenantId: req.tenantId }, req.body, { new: true });
        if (!human)
...
// AI EMPLOYEES
// ============================================
app.get('/api/ai-employees', auth, async (req, res) => {
    try {
...
app.post('/api/ai-employees', auth, async (req, res) => {
    try {
...
app.patch('/api/ai-employees/:id', auth, async (req, res) => {
    try {
        const ai = await AIEmployee.findOneAndUpdate({ employeeId: req.params.id, tenantId: req.tenantId }, req.body, { new: true });
        if (!ai)
            return res.status(404).json({ error: 'NOT_FOUND' });
...
app.get('/api/workflows', auth, async (req, res) => {
    try {
        const workflows = await WorkflowStatus.find({ tenantId: req.tenantId });
...
app.patch('/api/alerts/:id/acknowledge', auth, async (req, res) => {
    try {
        const alert = await Alert.findOneAndUpdate({ alertId: req.params.id, tenantId: req.tenantId }, { acknowledged: true }, { new: true });
        if (!alert)
            return res.status(404).json({ error: 'NOT_FOUND' });
...
app.get('/api/dashboard', auth, async (req, res) => {
    try {
        ...
        const deptBreakdown = await Promise.all(departments.map(async (d) => ({
            name: d.name,
            type: d.type,
