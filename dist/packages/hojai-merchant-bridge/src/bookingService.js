...
export class BookingBridgeService {
    ...
    async create(input) {
        try {
            const res = await axios.post(`${this.baseUrl}/api/bookings`, input, { headers: this.headers() });
    ...
    async get(bookingId) {
        try {
            const res = await axios.get(`${this.baseUrl}/api/bookings/${bookingId}`, { headers: this.headers() });
            return this.transformBooking(res.data);
        }
        catch (error) {
    ...
    async update(bookingId, updates) {
        try {
            await axios.patch(`${this.baseUrl}/api/bookings/${bookingId}`, updates, { headers: this.headers() });
            return true;
        }
        catch (error) {
    ...
    async cancel(bookingId) {
        try {
            await axios.post(`${this.baseUrl}/api/bookings/${bookingId}/cancel`, {}, { headers: this.headers() });
            return true;
        }
        catch (error) {
    ...
    async getAvailableSlots(merchantId, date, guests, type) {
        try {
            const res = await axios.get(`${this.baseUrl}/api/bookings/slots?merchantId=${merchantId}&date=${date}&guests=${guests}&type=${type}`, { headers: this.headers() });
    ...
    async getByMerchant(merchantId, date) {
        try {
            ...
            const res = await axios.get(`${this.baseUrl}/api/bookings/merchant/${merchantId}${params}`, { headers: this.headers() });
    ...
    async getByCustomer(customerId) {
        try {
            const res = await axios.get(`${this.baseUrl}/api/bookings/customer/${customerId}`, { headers: this.headers() });
            return (res.data.bookings || []).map(this.transformBooking);
        }
        catch (error) {
    ...
    async confirm(bookingId) {
        try {
            await axios.post(`${this.baseUrl}/api/bookings/${bookingId}/confirm`, {}, { headers: this.headers() });
            return true;
        }
        catch (error) {
    ...
    async sendReminder(bookingId) {
        try {
            await axios.post(`${this.baseUrl}/api/bookings/${bookingId}/remind`, {}, { headers: this.headers() });
            return true;
        }
        catch (error) {
