...
export class InventoryBridgeService {
    ...
    async getItems(merchantId) {
        try {
            const res = await axios.get(`${this.baseUrl}/api/inventory/${merchantId}`, { headers: this.headers() });
    ...
    async getLowStock(merchantId) {
        try {
            const res = await axios.get(`${this.baseUrl}/api/inventory/${merchantId}/low-stock`, { headers: this.headers() });
            return res.data.items || [];
        }
        catch (error) {
    ...
    async checkAvailability(merchantId, itemId, quantity) {
        try {
            const res = await axios.get(`${this.baseUrl}/api/inventory/${merchantId}/check/${itemId}?qty=${quantity}`, { headers: this.headers() });
    ...
    async updateStock(merchantId, itemId, quantity) {
        try {
            await axios.patch(`${this.baseUrl}/api/inventory/${merchantId}/${itemId}`, { quantity }, { headers: this.headers() });
            return true;
        }
        catch (error) {
    ...
    async deductStock(merchantId, itemId, quantity) {
        try {
            await axios.post(`${this.baseUrl}/api/inventory/${merchantId}/${itemId}/deduct`, { quantity }, { headers: this.headers() });
            return true;
        }
        catch (error) {
