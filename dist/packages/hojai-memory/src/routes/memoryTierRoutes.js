...
router.get('/by-tier/:tier', async (req, res, next) => {
    try {
        ...
        const memories = await MemoryTierModel.find(filter)
            ...
            .limit(limit ? parseInt(limit) : MEMORY_TIER_CONFIG[tier].maxItems);
...
router.get('/context', async (req, res, next) => {
    try {
        ...
        const includeTiers = tiers
            ? tiers.split(',').filter(t => Object.values(MemoryTier).includes(t))
            : Object.values(MemoryTier);
        const byTier = {};
        const allMemories = [];
        // Fetch from each tier in priority order (L1 first)
        for (const tier of includeTiers.sort((a, b) => MEMORY_TIER_CONFIG[a].priority - MEMORY_TIER_CONFIG[b].priority)) {
            const filter = { tenantId, userId: userId, tier };
            if (entityId)
                filter.entityId = entityId;
            const memories = await MemoryTierModel.find(filter)
                .sort({ importance: -1, createdAt: -1 })
                .limit(maxItemsPerTier ? parseInt(maxItemsPerTier) : MEMORY_TIER_CONFIG[tier].maxItems);
