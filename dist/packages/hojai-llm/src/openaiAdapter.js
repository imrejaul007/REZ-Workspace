...
import { OpenAIModel, LLMProvider, MessageRole } from './types/index.js';
import { AuthenticationError, RateLimitError, TokenLimitError, EmptyResponseError, isRetryableError, sleep } from './errors.js';
        ...
        for (let attempt = 1; attempt <= this.config.maxRetries; attempt++) {
            try {
                ...
                if (lastError instanceof OpenAI.RateLimitError) {
                    throw new RateLimitError(`OpenAI rate limit exceeded: ${lastError.message}`, { limitType: 'requests', originalError: lastError });
                }
                // Authentication error
                if (lastError instanceof OpenAI.AuthenticationError) {
                    throw new AuthenticationError(`OpenAI authentication failed: ${lastError.message}`, lastError);
                }
                // Check if retryable
...
// DEFAULT EXPORT
// ============================================================================
export function createOpenAIAdapter(config, logger) {
