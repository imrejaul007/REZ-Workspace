...
app.post('/api/subscriptions/cancel', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const subscription = await Subscription.findOneAndUpdate({ tenantId, status: 'active' }, { cancelAtPeriodEnd: true }, { new: true });
        if (!subscription) {
...
// Record Usage Batch
app.post('/api/usage/batch', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
app.post('/api/ai-employees/billing', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
app.patch('/api/ai-employees/:employeeId/billing', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const { level, status, basePrice } = req.body;
        const billing = await AIEmployeeBilling.findOneAndUpdate({ tenantId, employeeId: req.params.employeeId }, {
            $set: {
                ...(level && { level }),
                ...(status && { status }),
...
app.get('/api/ai-employees/billing/summary', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        ...
        const byRole = billings.reduce((acc, b) => {
            if (!acc[b.role])
                acc[b.role] = { count: 0, cost: 0 };
            acc[b.role].count++;
...
app.get('/api/royalties', authMiddleware, async (req, res) => {
    try {
        ...
        const totalEarnings = royalties.reduce((sum, r) => sum + r.total, 0);
        res.json({
            success: true,
            data: {
                totalEarnings,
...
app.get('/api/dashboard', authMiddleware, async (req, res) => {
    try {
        ...
        const usageSummary = usage.reduce((acc, u) => { acc[u._id] = u; return acc; }, {});
        // AI employee billing
