...
import { intentService, IntentType } from './intentService.js';
...
app.post('/api/voice/transcribe', async (req, res) => {
    try {
        ...
        const transcript = await voiceService.recognizeSpeech(Buffer.from(audio, 'base64'), language);
 ...
 */
app.post('/api/voice/vad', async (req, res) => {
    try {
        const { audio } = req.body;
        if (!audio) {
            return res.status(400).json({ error: 'Audio is required' });
        }
        const segments = await voiceService.detectVoiceActivity(Buffer.from(audio, 'base64'));
 ...
 */
app.post('/api/memory/prefetch', async (req, res) => {
    try {
