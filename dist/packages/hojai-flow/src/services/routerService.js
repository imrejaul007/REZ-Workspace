...
export class RouterService {
     ...
     * Generate execution plan
     */
    generateExecutionPlan(routerType, intent, context) {
        const steps = [];
        switch (routerType) {
            case 'DICTATION':
     ...
     */
    generateDictationPlan(intent, context) {
        const steps = [];
        ...
        const dictationAction = DICTATION_ACTIONS[suggestedAction];
        steps.push({
            order: 3,
     ...
     */
    generateKnowledgePlan(intent, context) {
        const steps = [];
     ...
     */
    generateWorkflowPlan(intent, context) {
        const steps = [];
        ...
        const workflowAction = WORKFLOW_ACTIONS[intent.suggestedAction];
        steps.push({
            order: 3,
     ...
     */
    generateAgentPlan(intent, context) {
        const steps = [];
        ...
        const agentAction = AGENT_ACTIONS[intent.suggestedAction];
        steps.push({
            order: 3,
     ...
     */
    generateMultiAgentPlan(intent, context) {
        const steps = [];
        ...
        const multiAction = MULTI_AGENT_ACTIONS[intent.suggestedAction];
        steps.push({
            order: 3,
     ...
     */
    estimateLatency(routerType, steps) {
        const baseLatency = {
        ...
        const stepLatency = steps.reduce((sum, step) => {
            if (step.service === 'memory')
                return sum + 10;
            if (step.service === 'knowledge')
                return sum + 50;
            if (step.service === 'workflow')
                return sum + 100;
            if (step.service === 'agent')
                return sum + 200;
            return sum + 50;
        }, 0);
    ...
    getStatus() {
        ...
        if (this.config.enableDictation)
            enabledRouters.push('DICTATION');
        if (this.config.enableKnowledge)
            enabledRouters.push('KNOWLEDGE');
        if (this.config.enableWorkflow)
            enabledRouters.push('WORKFLOW');
        if (this.config.enableAgent)
            enabledRouters.push('AGENT');
        if (this.config.enableMultiAgent)
            enabledRouters.push('MULTI_AGENT');
        return {
            enabledRouters,
            config: this.config,
            stats: {
