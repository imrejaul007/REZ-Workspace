...
async function proxyToService(serviceUrl, req, res) {
    try {
        ...
        res.status(error.response?.status || 500).json(error.response?.data || { error: 'Service unavailable', code: 'SERVICE_ERROR' });
    }
}
// Health endpoints
...
app.get('/api/unified/search', authMiddleware, async (req, res) => {
...
// Unified profile
app.get('/api/unified/profile/:userId', authMiddleware, async (req, res) => {
...
app.post('/api/unified/track', authMiddleware, async (req, res) => {
