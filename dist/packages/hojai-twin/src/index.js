...
app.post('/api/employee/:employeeId', auth, async (req, res) => {
    try {
        const { employeeId } = req.params;
        const tenantId = req.tenantId;
        const twinId = `emp_twin_${uuid().slice(0, 8)}`;
        const twin = await EmployeeTwin.findOneAndUpdate({ employeeId, tenantId }, { ...req.body, twinId, tenantId }, { upsert: true, new: true });
...
// Get Employee Twin
app.get('/api/employee/:employeeId', auth, async (req, res) => {
    try {
        ...
        if (!twin)
            return res.status(404).json({ error: 'TWIN_NOT_FOUND' });
        res.json({ success: true, data: twin });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
// Get Predictions
app.get('/api/employee/:employeeId/predictions', auth, async (req, res) => {
    try {
        const twin = await EmployeeTwin.findOne({
            employeeId: req.params.employeeId,
            tenantId: req.tenantId
...
app.post('/api/customer/:customerId', auth, async (req, res) => {
    try {
        const { customerId } = req.params;
        const tenantId = req.tenantId;
        const twinId = `cust_twin_${uuid().slice(0, 8)}`;
        const twin = await CustomerTwin.findOneAndUpdate({ customerId, tenantId }, { ...req.body, twinId, tenantId }, { upsert: true, new: true });
        res.json({ success: true, data: twin });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
// Get Customer Twin
app.get('/api/customer/:customerId', auth, async (req, res) => {
    try {
...
// Get Recommendations
app.get('/api/customer/:customerId/recommendations', auth, async (req, res) => {
    try {
        const twin = await CustomerTwin.findOne({
            customerId: req.params.customerId,
            tenantId: req.tenantId
...
app.post('/api/company/:companyId', auth, async (req, res) => {
    try {
        const { companyId } = req.params;
        const tenantId = req.tenantId;
        const twinId = `comp_twin_${uuid().slice(0, 8)}`;
        const twin = await CompanyTwin.findOneAndUpdate({ companyId, tenantId }, { ...req.body, twinId, tenantId }, { upsert: true, new: true });
        res.json({ success: true, data: twin });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
app.get('/api/company/:companyId/insights', auth, async (req, res) => {
    try {
        const twin = await CompanyTwin.findOne({
            companyId: req.params.companyId,
            tenantId: req.tenantId
...
app.post('/api/merchant/:merchantId', auth, async (req, res) => {
    try {
        const { merchantId } = req.params;
        const tenantId = req.tenantId;
        const twinId = `merch_twin_${uuid().slice(0, 8)}`;
        const twin = await MerchantTwin.findOneAndUpdate({ merchantId, tenantId }, { ...req.body, twinId, tenantId }, { upsert: true, new: true });
        res.json({ success: true, data: twin });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
// Get Merchant Twin
app.get('/api/merchant/:merchantId', auth, async (req, res) => {
    try {
...
function generateCompanyRecommendations(twin) {
    const recs = [];
