...
app.post('/api/signal/validate/batch', async (req, res) => {
    const tenantId = req.headers['x-tenant-id'];
    const results = await Promise.all(req.body.events.map((e) => signalService.processEvent(tenantId, e)));
    res.status(201).json({ success: true, data: { processed: results.length, results } });
});
// Quality metrics
app.get('/api/signal/metrics', async (req, res) => {
    const tenantId = req.headers['x-tenant-id'];
    const metrics = await signalService.getQualityMetrics(tenantId, req.query.period || 'day');
