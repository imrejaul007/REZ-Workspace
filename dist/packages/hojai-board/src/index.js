...
app.get('/api/board', auth, async (req, res) => {
    try {
        const members = await BoardMember.find({ tenantId: req.tenantId, status: 'active' });
...
// Add Board Member
app.post('/api/board/members', auth, async (req, res) => {
    try {
        ...
        const member = new BoardMember({
            ...
            role,
            name: name || `${role} AI`,
            expertise: expertise || BOARD_RESPONSES[role]?.expertise || []
        });
        await member.save();
...
// Remove Board Member
app.delete('/api/board/members/:id', auth, async (req, res) => {
    try {
        await BoardMember.findOneAndUpdate({ memberId: req.params.id, tenantId: req.tenantId }, { status: 'inactive' });
...
// ============================================
// Ask Question
app.post('/api/board/ask', auth, async (req, res) => {
    try {
        ...
        const responses = selectedMembers.map(member => {
            const boardProfile = BOARD_RESPONSES[member.role];
        ...
        await BoardMember.updateMany({ memberId: { $in: selectedMembers.map(m => m.memberId) } }, { $inc: { decisions: 1 } });
        res.status(201).json({
            success: true,
            data: {
                meetingId,
...
// Get Meeting
app.get('/api/board/meetings/:id', auth, async (req, res) => {
    try {
...
// List Meetings
app.get('/api/board/meetings', auth, async (req, res) => {
    try {
...
// Approve Decision
app.post('/api/decisions/:id/approve', auth, async (req, res) => {
    try {
        const decision = await Decision.findOneAndUpdate({ decisionId: req.params.id, tenantId: req.tenantId }, { status: 'approved', approvedBy: req.userId }, { new: true });
        if (!decision)
            return res.status(404).json({ error: 'NOT_FOUND' });
...
// Implement Decision
app.post('/api/decisions/:id/implement', auth, async (req, res) => {
    try {
        const decision = await Decision.findOneAndUpdate({ decisionId: req.params.id, tenantId: req.tenantId, status: 'approved' }, { status: 'implemented', implementedAt: new Date() }, { new: true });
        if (!decision)
            return res.status(404).json({ error: 'NOT_FOUND' });
        res.json({ success: true, data: decision });
    }
...
app.post('/api/board/strategic/market-expansion', auth, async (req, res) => {
    try {
        const { market, timeline, investment } = req.body;
        const response = await askStrategicQuestion(req.tenantId, `Should we expand to ${market}?`, 'market_expansion', { market, timeline, investment });
...
// Should we hire?
app.post('/api/board/strategic/hiring', auth, async (req, res) => {
    try {
        const { role, department, salary, urgency } = req.body;
        const response = await askStrategicQuestion(req.tenantId, `Should we hire ${role} for ${department}?`, 'hiring', { role, department, salary, urgency });
        res.json({ success: true, data: response });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
...
// Pricing decision
app.post('/api/board/strategic/pricing', auth, async (req, res) => {
    try {
        const { product, currentPrice, newPrice, reason } = req.body;
        const response = await askStrategicQuestion(req.tenantId, `Should we change ${product} price from ${currentPrice} to ${newPrice}?`, 'pricing', { product, currentPrice, newPrice, reason });
        res.json({ success: true, data: response });
    }
    catch (e) {
        res.status(500).json({ error: e.message });
        ...
        res.json({
            ...
            data: {
                board: {
                    ...
                    byRole: boardMembers.reduce((acc, m) => {
...
// HELPERS
// ============================================
function generateResponse(member, question, topic, context, profile) {
...
function getPositionForQuestion(role, question, topic, profile) {
...
function generateReasoning(role, question, position) {
    const reasonings = {
        CEO: [
...
function generatePros(role, question) {
    const generic = [
        'Accelerates growth',
...
function generateCons(role, question) {
    return [
        'Initial investment required',
        'Execution complexity'
    ];
}
function generateMetrics(role) {
    return {
...
function generateConsensus(responses, question) {
    const positions = responses.map((r) => r.position);
    const yesVotes = positions.filter((p) => p.includes('yes') || p.includes('positive')).length;
...
async function askStrategicQuestion(tenantId, question, type, context) {
    const members = await BoardMember.find({ tenantId, status: 'active' });
    const responses = members.map(member => {
        const profile = BOARD_RESPONSES[member.role];
