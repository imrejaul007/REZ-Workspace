...
// Batch create nodes
app.post('/api/nodes/batch', authMiddleware, async (req, res) => {
    try {
...
// Get node
app.get('/api/nodes/:entityType/:entityId', authMiddleware, async (req, res) => {
    try {
...
// Update node
app.patch('/api/nodes/:entityType/:entityId', authMiddleware, async (req, res) => {
    try {
        const { entityType, entityId } = req.params;
        const tenantId = req.tenantId;
        const { name, properties, metadata } = req.body;
        const node = await Node.findOneAndUpdate({ tenantId, entityType, entityId }, { $set: { ...(name && { name }), ...(properties && { properties }), ...(metadata && { metadata }), updatedAt: new Date() } }, { new: true });
        if (!node) {
            return res.status(404).json({ error: 'Entity not found', code: 'NOT_FOUND' });
        }
        res.json({ success: true, data: node });
...
// Delete node
app.delete('/api/nodes/:entityType/:entityId', authMiddleware, async (req, res) => {
    try {
        const { entityType, entityId } = req.params;
        const tenantId = req.tenantId;
...
// ============================================
// Create relationship
app.post('/api/relationships', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/relationships/batch', authMiddleware, async (req, res) => {
    try {
...
app.get('/api/relationships/:entityType/:entityId', authMiddleware, async (req, res) => {
    try {
        const { entityType, entityId } = req.params;
...
// Delete relationship
app.delete('/api/relationships/:edgeId', authMiddleware, async (req, res) => {
    try {
...
app.post('/api/graph/traverse', authMiddleware, async (req, res) => {
    try {
...
app.get('/api/graph/ego/:entityType/:entityId', authMiddleware, async (req, res) => {
    try {
        const { entityType, entityId } = req.params;
        ...
        // Get connected nodes
        const connectedIds = edges.map(e => e.sourceId === entityId ? { id: e.targetId, type: e.targetType } : { id: e.sourceId, type: e.sourceType });
...
app.post('/api/ai-employees', authMiddleware, async (req, res) => {
    try {
...
// Get AI team
app.get('/api/ai-employees/team/:department', authMiddleware, async (req, res) => {
    try {
