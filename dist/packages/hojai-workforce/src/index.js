...
// Get AI Employee
app.get('/api/employees/:employeeId', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
// Update AI Employee
app.patch('/api/employees/:employeeId', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const { name, role, department, status, level, skills, capabilities, managerId, teamId, departmentId, pricing } = req.body;
        const employee = await AIEmployee.findOneAndUpdate({ tenantId, employeeId: req.params.employeeId }, {
            $set: {
                ...(name && { name }),
                ...(role && { role }),
...
app.post('/api/employees/:employeeId/metrics', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const { metric, value, context } = req.body;
        // Update employee metrics
        const updatePath = `metrics.${metric}`;
        const employee = await AIEmployee.findOneAndUpdate({ tenantId, employeeId: req.params.employeeId }, { $set: { [updatePath]: value, updatedAt: new Date() } }, { new: true });
        if (!employee) {
            return res.status(404).json({ error: 'Employee not found', code: 'NOT_FOUND' });
        }
        // Log performance
        ...
        if (metric === 'xp' && value >= employee.level * 1000) {
            await AIEmployee.updateOne({ tenantId, employeeId: req.params.employeeId }, { $inc: { level: 1 }, $set: { xp: 0 } });
        }
        res.json({ success: true, data: employee });
    }
    catch (error) {
...
app.post('/api/teams/:teamId/members', authMiddleware, async (req, res) => {
    try {
        ...
        // Add to team
        await Team.updateOne({ tenantId, teamId: req.params.teamId }, { $addToSet: { memberIds: employeeId } });
        // Update employee
        await AIEmployee.updateOne({ tenantId, employeeId }, { $set: { teamId: req.params.teamId } });
...
// Get Department
app.get('/api/departments/:departmentId', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
...
app.get('/api/marketplace', async (req, res) => {
    try {
        ...
        let sortOption = { 'stats.rating': -1 };
        if (sort === 'popular')
            sortOption = { 'stats.installs': -1 };
        if (sort === 'newest')
            sortOption = { createdAt: -1 };
        if (sort === 'price_low')
            sortOption = { 'pricing.monthly': 1 };
        if (sort === 'price_high')
            sortOption = { 'pricing.monthly': -1 };
...
// Install AI Employee
app.post('/api/marketplace/listings/:listingId/install', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        ...
        await Listing.updateOne({ listingId: req.params.listingId }, { $inc: { 'stats.installs': 1 } });
        res.status(201).json({
            success: true,
            data: {
                employee,
...
app.get('/api/analytics/workforce', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const [totalEmployees, activeEmployees, departmentCount, teamCount] = await Promise.all([
...
app.get('/api/employees/:employeeId/performance', authMiddleware, async (req, res) => {
    try {
        const tenantId = req.tenantId;
