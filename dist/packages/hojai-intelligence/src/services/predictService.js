...
import { PredictionType, PredictionRisk, RFMTier } from '../types/index.js';
...
export class PredictService {
    ...
    async computeRFM(params) {
        ...
        await RFMModel.findOneAndUpdate({ tenantId: params.tenantId, userId: params.userId }, rfm, { upsert: true });
        return rfm;
    }
    // =========================================================================
    // HELPER METHODS
    ...
    getChurnRisk(probability) {
        if (probability >= 0.8)
            return PredictionRisk.CRITICAL;
        if (probability >= 0.6)
    ...
    getRFMTier(r, f, m) {
        const score = r + f + m;
        if (score >= 13)
            return RFMTier.CHAMPIONS;
        if (score >= 10 && r >= 4 && f >= 4)
            return RFMTier.LOYAL;
        if (score >= 8 && r >= 3)
            return RFMTier.POTENTIAL;
        if (score >= 6 || r <= 2)
            return RFMTier.AT_RISK;
        return RFMTier.LOST;
    }
    getRFMSegment(tier) {
        const segments = {
