...
import { MessageDirection, MessageType } from '../types/index.js';
class MessageRouter {
     ...
     */
    async routeToEmployee(tenantId, intent, preferredEmployeeId) {
        ...
        if (availableEmployees.length > 0) {
            ...
            const selected = availableEmployees.sort((a, b) => new Date(a.updatedAt).getTime() - new Date(b.updatedAt).getTime())[0];
            return {
    ...
    async processMessage(context) {
        try {
            ...
            const response = await this.generateAIResponse(tenantId, routing.employee, conversation.id, message, intent);
            // Send response
     ...
     */
    async generateAIResponse(tenantId, employee, conversationId, userMessage, intent) {
     ...
     */
    async handleNoEmployeeAvailable(tenantId, conversationId, userId, source) {
     ...
     */
    async forwardToEmployee(tenantId, conversationId, employeeId, message, source, metadata) {
        try {
