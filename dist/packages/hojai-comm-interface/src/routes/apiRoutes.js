...
import { authenticateAny, tenantContext, generateToken } from '../middleware/auth.js';
 ...
 */
router.post('/api/chat', authenticateAny, async (req, res) => {
    try {
 ...
 */
router.get('/api/employees', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        if (!tenantId) {
            return res.status(400).json({
        ...
        let employees;
        if (status) {
            employees = await employeeRegistry.findByStatus(tenantId, status);
        }
        else if (role) {
            employees = await employeeRegistry.findByRole(tenantId, role);
        }
        else if (capability) {
            employees = await employeeRegistry.findByCapability(tenantId, capability);
        }
        else {
 ...
 */
router.post('/api/employees', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        if (!tenantId) {
            return res.status(400).json({
 ...
 * Get employee by ID
 */
router.get('/api/employees/:id', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        const { id } = req.params;
        if (!tenantId) {
 ...
 * Update an employee
 */
router.patch('/api/employees/:id', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        const { id } = req.params;
        if (!tenantId) {
 ...
 */
router.post('/api/employees/:id/status', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        const { id } = req.params;
        if (!tenantId) {
 ...
 * Delete an employee
 */
router.delete('/api/employees/:id', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        const { id } = req.params;
        if (!tenantId) {
 ...
 */
router.post('/api/employees/:id/assign', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        ...
        const conversation = await conversationManager.assignToEmployee(conversationId, tenantId, employeeId);
        if (!conversation) {
            return res.status(404).json({
                success: false,
 ...
 */
router.get('/api/conversations/:userId', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        ...
        if (conversationId) {
            ...
            const messages = await conversationManager.getMessageHistory(conversationId, tenantId, parseInt(limit, 10));
        ...
        const conversations = await conversationManager.listByUser(tenantId, userId, parseInt(limit, 10));
 ...
 */
router.post('/api/conversations/:conversationId/close', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
 ...
 */
router.post('/api/conversations/:conversationId/read', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        const { conversationId } = req.params;
        if (!tenantId) {
...
router.get('/api/webhooks/whatsapp', (req, res) => {
    ...
    if (whatsAppBridge.verifyWebhook(mode, token, challenge)) {
        res.send(challenge);
    }
    else {
        res.sendStatus(403);
 ...
 */
router.get('/api/stats', tenantContext, async (req, res) => {
    try {
        const tenantId = req.tenantContext?.tenantId;
        if (!tenantId) {
            return res.status(400).json({
