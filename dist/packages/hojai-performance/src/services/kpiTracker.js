...
import { KPI_THRESHOLDS } from '../types/index.js';
...
export class KPITracker {
     ...
     */
    async initializeKPI(employeeId, tenantId, period) {
     ...
     */
    async getKPI(employeeId, tenantId, period) {
        const periodLabel = period || format(new Date(), 'yyyy-MM');
     ...
     * Update KPI metrics
     */
    async updateKPI(employeeId, tenantId, updates, period) {
        const periodLabel = period || format(new Date(), 'yyyy-MM');
        ...
        if (updates.responseTime !== undefined) {
            const newAvg = kpi.totalTasksAttempted === 0
     ...
     */
    async incrementTaskCompleted(employeeId, tenantId, responseTime, customerSatisfaction) {
     ...
     * Record error
     */
    async recordError(employeeId, tenantId) {
        const periodLabel = format(new Date(), 'yyyy-MM');
        let kpi = await this.initializeKPI(employeeId, tenantId, periodLabel);
        kpi.tasksFailed += 1;
        kpi.totalTasksAttempted += 1;
     ...
     * Record escalation
     */
    async recordEscalation(employeeId, tenantId) {
        const periodLabel = format(new Date(), 'yyyy-MM');
        let kpi = await this.initializeKPI(employeeId, tenantId, periodLabel);
        kpi.escalationCount += 1;
     ...
     */
    async recordCustomerSatisfaction(employeeId, tenantId, rating) {
        const periodLabel = format(new Date(), 'yyyy-MM');
        let kpi = await this.initializeKPI(employeeId, tenantId, periodLabel);
     ...
     * Record revenue
     */
    async recordRevenue(employeeId, tenantId, amount) {
        const periodLabel = format(new Date(), 'yyyy-MM');
        let kpi = await this.initializeKPI(employeeId, tenantId, periodLabel);
     ...
     */
    async recalculateDerivedMetrics(kpi) {
        ...
        kpi.qualityScore = qualityComponents.reduce((sum, c) => sum + c.value * c.weight, 0) * 100;
     ...
     * Record KPI history
     */
    async recordHistory(kpi, changes, timestamp) {
     ...
     * Get KPI history
     */
    async getHistory(employeeId, tenantId, startDate, endDate) {
     ...
     */
    async getKPITrends(employeeId, tenantId, months = 6) {
     ...
     */
    async getTeamKPIs(tenantId, department, period) {
     ...
     */
    async getKPISummary(tenantId, period) {
        const periodLabel = period || format(new Date(), 'yyyy-MM');
     ...
     */
    getPerformanceRating(kpi) {
        if (kpi.qualityScore >= KPI_THRESHOLDS.excellent)
            return 'Excellent';
        if (kpi.qualityScore >= KPI_THRESHOLDS.good)
            return 'Good';
        if (kpi.qualityScore >= KPI_THRESHOLDS.average)
            return 'Average';
        if (kpi.qualityScore >= KPI_THRESHOLDS.poor)
     ...
     */
    checkKPIMetThreshold(kpi, metric) {
