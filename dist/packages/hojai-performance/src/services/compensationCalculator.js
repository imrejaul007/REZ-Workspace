...
import { DEFAULT_COMPENSATION_CONFIG, } from '../types/index.js';
import { Compensation, Evaluation, KPI, EmployeeProfile, } from '../models/performanceModel.js';
...
export class CompensationCalculator {
     ...
     */
    async calculateCompensation(employeeId, tenantId, period, overrideBase) {
        ...
        const breakdown = this.calculateBreakdown(employee, kpi, overallScore, baseAmount, daysWorked, config);
     ...
     */
    calculateBreakdown(employee, kpi, overallScore, baseAmount, daysWorked, config) {
     ...
     */
    calculateQualityMultiplier(qualityScore, threshold) {
        if (qualityScore >= 90)
            return 1.5;
        if (qualityScore >= 80)
            return 1.25;
     ...
     */
    calculatePerformanceMultiplier(overallScore) {
        if (overallScore >= 95)
            return 500;
        if (overallScore >= 90)
            return 400;
     ...
     */
    async getCompensation(employeeId, tenantId, period) {
     ...
     */
    async getCompensationHistory(employeeId, tenantId, limit = 12) {
     ...
     */
    async approveCompensation(compensationId, approverId) {
     ...
     */
    async markAsPaid(compensationId) {
        const compensation = await Compensation.findOne({ compensationId });
        if (!compensation) {
            throw new Error(`Compensation ${compensationId} not found`);
        }
     ...
     */
    async processPayment(compensationId) {
        const compensation = await Compensation.findOne({ compensationId });
        if (!compensation) {
            throw new Error(`Compensation ${compensationId} not found`);
        }
     ...
     */
    async getPayrollSummary(tenantId, period) {
     ...
     */
    async getCompensationStats(tenantId, period) {
     ...
     */
    async getBonusBreakdown(employeeId, tenantId, period) {
        const evaluation = await Evaluation.findOne({ employeeId, tenantId, period });
        const kpi = await KPI.findOne({ employeeId, tenantId, period });
        if (!evaluation || !kpi) {
     ...
     */
    async generateForecast(tenantId, months = 3) {
        const forecasts = [];
...
//# sourceMappingURL=compensationCalculator.js.map