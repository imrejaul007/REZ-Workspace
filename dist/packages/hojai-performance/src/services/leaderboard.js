...
import { Leaderboard, KPI, Evaluation, EmployeeProfile, } from '../models/performanceModel.js';
export class LeaderboardService {
    /**
     * Generate leaderboard for a specific metric and period
     */
    async generateLeaderboard(tenantId, metric = 'overall', period = 'monthly', department) {
     ...
     * Get leaderboard
     */
    async getLeaderboard(tenantId, metric = 'overall', period = 'monthly', limit = 10, offset = 0) {
        const periodLabel = this.getPeriodLabel(period);
     ...
     */
    async getEmployeeRank(employeeId, tenantId, metric = 'overall', period = 'monthly') {
        const periodLabel = this.getPeriodLabel(period);
        const leaderboard = await Leaderboard.findOne({
            tenantId,
            metric,
     ...
     * Get top performers
     */
    async getTopPerformers(tenantId, period = 'monthly', limit = 5) {
        const periodLabel = this.getPeriodLabel(period);
        const leaderboard = await Leaderboard.findOne({
            tenantId,
            metric: 'overall',
     ...
     */
    async getMostImproved(tenantId, period = 'monthly', limit = 5) {
        const periodLabel = this.getPeriodLabel(period);
        const leaderboard = await Leaderboard.findOne({
            tenantId,
            metric: 'overall',
     ...
     */
    async getEmployeeLeaderboardHistory(employeeId, tenantId, metric = 'overall', limit = 6) {
     ...
     */
    async compareEmployees(employee1Id, employee2Id, tenantId, period = 'monthly') {
        const periodLabel = this.getPeriodLabel(period);
        ...
        const metrics = ['overall', 'productivity', 'quality', 'revenue', 'efficiency'];
        const comparison = {
            employees: {
     ...
     */
    async getDepartmentLeaderboard(tenantId, period = 'monthly') {
        const periodLabel = this.getPeriodLabel(period);
    ...
    async getEmployees(tenantId, department) {
    ...
    async calculateEntries(tenantId, employees, metric, period) {
        const periodLabel = this.getPeriodLabel(period);
        const entries = [];
        for (const employee of employees) {
            const kpi = await KPI.findOne({
            ...
            const prevEntry = prevLeaderboard?.entries.find((e) => e.employeeId === employee.employeeId);
    ...
    calculateScore(metric, kpi, evaluation) {
        switch (metric) {
            case 'overall':
    ...
    assignBadge(entry, position, total) {
        if (position === 0)
        ...
        if (entry.rankChange >= 3)
            return 'most_improved';
        return undefined;
    }
    determineWinner(metric, score1, score2) {
        if (score1 > score2)
            return 'employee1';
        if (score2 > score1)
            return 'employee2';
    ...
    async aggregateByDepartment(tenantId, period, periodLabel) {
