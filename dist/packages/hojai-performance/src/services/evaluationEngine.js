...
import { DEFAULT_EVALUATION_WEIGHTS, EVALUATION_GRADES, } from '../types/index.js';
import { Evaluation, EvaluationConfig, KPI, EmployeeProfile, } from '../models/performanceModel.js';
export class EvaluationEngine {
    /**
     * Generate evaluation for an employee
     */
    async evaluateEmployee(employeeId, tenantId, periodType = 'monthly', evaluatorId = 'system', customWeights) {
        ...
        const overallScore = this.calculateOverallScore({ qualityScore, productivityScore, reliabilityScore, collaborationScore, growthScore }, weights);
        const { percentileRank, tenantPercentileRank } = await this.calculatePercentileRanks(employeeId, tenantId, overallScore, period);
     ...
     */
    async getEvaluation(employeeId, tenantId, period, periodType) {
     ...
     */
    async getEvaluationHistory(employeeId, tenantId, limit = 12) {
     ...
     */
    calculateCollaborationScore(kpi) {
     ...
     */
    async calculateGrowthScore(kpi, tenantId) {
        ...
        return Math.round(Math.max(0, Math.min(100, components.reduce((sum, c) => sum + c.value * c.weight, 0))));
    }
    /**
     ...
     */
    async calculatePercentileRanks(employeeId, tenantId, overallScore, period) {
     ...
     */
    generateStrengths(kpi, qualityScore, productivityScore) {
        const strengths = [];
        if (qualityScore >= 85) {
     ...
     */
    generateImprovements(kpi, qualityScore, reliabilityScore) {
        const improvements = [];
        if (qualityScore < 70) {
     ...
     */
    generateRecommendations(overallScore, kpi) {
     ...
     */
    async getOrCreateConfig(tenantId, customWeights) {
    ...
    async updateConfig(tenantId, updates) {
        ...
        if (!config) {
            ...
            if (updates.autoEvaluation !== undefined)
                config.autoEvaluation = updates.autoEvaluation;
            if (updates.evaluationFrequency)
                config.evaluationFrequency = updates.evaluationFrequency;
        }
        await config.save();
        return config;
    }
     ...
     */
    getEvaluationGrade(overallScore) {
     ...
     */
    async batchEvaluate(tenantId, employeeIds, periodType = 'monthly') {
     ...
     */
    async getEvaluationStats(tenantId, period) {
