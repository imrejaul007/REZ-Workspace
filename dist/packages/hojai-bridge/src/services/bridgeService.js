...
import { TenantType, DataSensitivity } from '../types/index.js';
// ============================================================================
// MODELS
// ============================================================================
...
export class BridgeService {
    ...
    async configureBridge(config) {
        const doc = await BridgeConfigModel.findOneAndUpdate({ tenantId: config.tenantId }, config, { upsert: true, new: true });
        return doc.toObject();
    }
    /**
    ...
    async forwardToRez(event, sourceService, sourceApp) {
        try {
            await axios.post(`${this.REZ_EVENT_BUS_URL}/api/events`, {
                type: event.type,
    ...
    async linkCrossAppIdentity(params) {
        ...
        const identity = await CrossAppIdentityModel.findOneAndUpdate({ tenantId, rezUserId }, {
            tenantId,
            rezUserId,
    ...
    async getUnifiedProfile(params) {
        ...
        try {
            ...
            const response = await axios.get(`${this.REZ_IDENTITY_URL}/api/unified/${entityType}/${entityId}`, {
                headers: {
                    'X-Internal-Token': this.INTERNAL_TOKEN,
    ...
    async forwardIntelligenceToRez(params) {
        try {
            await axios.post(`${this.REZ_INTELLIGENCE_URL}/api/intelligence/share`, {
                type: params.type,
    ...
    async syncAudience(params) {
        ...
        if (direction === 'hojai_to_rez') {
            await axios.post(`${this.REZ_INTELLIGENCE_URL}/api/audiences/${audienceId}/sync`, { users }, {
                headers: {
                    'X-Internal-Token': this.INTERNAL_TOKEN,
                    'X-Source': 'hojai'
                }
    ...
    async getBehavioralSignals(tenantId, userId) {
        ...
        try {
            const response = await axios.get(`${this.REZ_INTELLIGENCE_URL}/api/signals/user/${userId}`, {
                headers: {
                    'X-Internal-Token': this.INTERNAL_TOKEN,
                    'X-Tenant-ID': tenantId
                }
    ...
    async getCrossAppTouchpoints(tenantId, sessionId) {
        ...
        try {
            const response = await axios.get(`${this.REZ_INTELLIGENCE_URL}/api/attribution/touchpoints/${sessionId}`, {
                headers: {
                    'X-Internal-Token': this.INTERNAL_TOKEN,
                    'X-Tenant-ID': tenantId
                }
