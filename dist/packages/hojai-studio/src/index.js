...
app.post('/api/dna', auth, async (req, res) => {
    try {
        ...
        const dna = await CompanyDna.findOneAndUpdate({ tenantId }, { ...data, dnaId, tenantId }, { upsert: true, new: true });
        res.json({ success: true, data: dna });
    }
    catch (e) {
        res.status(400).json({ error: e.message });
...
app.post('/api/dna/apply', auth, async (req, res) => {
    try {
        ...
        const dna = await CompanyDna.findOne({ tenantId });
        if (!dna)
            return res.status(404).json({ error: 'DNA_NOT_CONFIGURED' });
        const employee = await AIEmployeeInstance.findOneAndUpdate({ tenantId, instanceId: employeeId }, { $set: { dna: dna.toObject() } }, { new: true });
        if (!employee)
...
app.post('/api/employees/:instanceId/activate', auth, async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const employee = await AIEmployeeInstance.findOneAndUpdate({ tenantId, instanceId: req.params.instanceId }, { status: 'active' }, { new: true });
        if (!employee)
            return res.status(404).json({ error: 'NOT_FOUND' });
        res.json({ success: true, data: employee });
    }
