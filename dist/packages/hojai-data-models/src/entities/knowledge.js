...
// ============================================
export const KNOWLEDGE_RELATIONSHIP_LABELS = {
    is_a: 'is a type of',
    part_of: 'is part of',
 ...
 */
export function createKnowledgeNode(tenantId, data) {
 ...
 */
export function createKnowledgeEdge(tenantId, data) {
    const now = new Date().toISOString();
    return {
 ...
 */
export function createKnowledgeCollection(tenantId, data) {
    const now = new Date().toISOString();
    return {
 ...
 */
export function markNodeHelpful(node) {
    return {
        ...node,
 ...
 */
export function markNodeNotHelpful(node) {
    return {
        ...node,
 ...
 */
export function updateNodeQualityScore(node, helpfulRate) {
 ...
 */
export function verifyNode(node, verifiedBy) {
    const now = new Date().toISOString();
    return {
        ...node,
        verified: true,
 ...
 */
export function incrementNodeUsage(node) {
    return {
        ...node,
 ...
 * Archive a node
 */
export function archiveNode(node) {
    return {
        ...node,
        status: 'archived',
        is_published: false,
 ...
 */
export function findConnectedNodes(edges, nodeId, maxHops = 2) {
    const visited = new Set();
    const results = [];
    function traverse(currentId, hops) {
        if (hops > maxHops || visited.has(currentId))
            return;
