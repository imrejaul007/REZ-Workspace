 ...
 */
export const CONSENT_TEXT_TEMPLATES = {
 ...
 * Deny consent
 */
export function denyConsent(tenantId, data) {
    const now = new Date().toISOString();
    return {
        id: `consent_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        tenant_id: tenantId,
 ...
 * Withdraw consent
 */
export function withdrawConsent(consent, reason) {
    const now = new Date().toISOString();
    return {
        ...consent,
        status: 'withdrawn',
 ...
 */
export function getCustomerConsentPreference(customerId, tenantId, consents) {
    const customerConsents = consents.filter(c => c.customer_id === customerId);
    const marketingOptOut = customerConsents.some(c => c.purpose === 'marketing_communication' && c.status === 'withdrawn');
