...
import { EntityType, TrustLevel } from '../types/index.js';
...
export class TrustService {
    ...
    async calculateScore(params) {
        ...
        const score = await TrustScoreModel.findOneAndUpdate({ tenantId, entityType, entityId }, scoreData, { upsert: true, new: true });
     ...
     */
    async addEdge(edge) {
        const doc = await TrustEdgeModel.findOneAndUpdate({ tenantId: edge.tenantId, sourceId: edge.sourceId, targetId: edge.targetId }, { ...edge, id: uuid() }, { upsert: true, new: true });
        return doc.toObject();
    }
    /**
