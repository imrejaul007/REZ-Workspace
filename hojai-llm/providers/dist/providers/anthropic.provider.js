...
export class AnthropicProvider {
    ...
    async embed(_text) {
        ...
        throw new LLMProviderError('Anthropic does not support embeddings. Use OpenAI provider instead.', 'PROVIDER_NOT_SUPPORTED', 400, this.name, 'embed');
    }
    /**
...
Respond with ONLY the category name that best matches. Do not include any explanation.`;
            ...
            const matchedLabel = labels.find((l) => l.toLowerCase() === result.toLowerCase() || result.toLowerCase().includes(l.toLowerCase()));
            const duration = Date.now() - startTime;
    ...
    mapError(error, model) {
        if (error instanceof Anthropic.APIError) {
            ...
            return new LLMProviderError(errorMessage, code, statusCode, this.name, model, retryable);
        }
        if (error instanceof Error) {
            if (error.message.includes('timeout')) {
                return new LLMProviderError('Request timed out', LLMErrorCodes.TIMEOUT, 408, this.name, model, true);
            }
            if (error.message.includes('ECONNREFUSED') || error.message.includes('ENOTFOUND')) {
                return new LLMProviderError('Network error connecting to Anthropic', LLMErrorCodes.NETWORK_ERROR, 503, this.name, model, true);
            }
        }
        return new LLMProviderError(error instanceof Error ? error.message : 'Unknown error', LLMErrorCodes.UNKNOWN_ERROR, 500, this.name, model, false);
    }
}
/**
