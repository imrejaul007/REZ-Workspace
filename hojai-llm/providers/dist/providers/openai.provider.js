...
export class OpenAIProvider {
        ...
        try {
            ...
            if (options.functions && options.functions.length > 0) {
                ...
                if (options.functionCall) {
                    requestOptions.tool_choice = options.functionCall;
                }
            }
     ...
     */
    async embed(text, embeddingModel) {
...
Respond with ONLY the category name that best matches. Do not include any explanation.`,
    ...
    mapError(error, model) {
        if (error instanceof OpenAI.APIError) {
            ...
            return new LLMProviderError(error.message || 'OpenAI API error', code, statusCode, this.name, model, retryable);
        }
        if (error instanceof Error) {
            ...
            if (error.message.includes('ECONNREFUSED') || error.message.includes('ENOTFOUND')) {
                return new LLMProviderError('Network error connecting to OpenAI', LLMErrorCodes.NETWORK_ERROR, 503, this.name, model, true);
            }
        }
