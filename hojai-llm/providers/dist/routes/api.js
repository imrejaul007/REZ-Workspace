...
import { ChatRequestSchema, EmbedRequestSchema, ClassifyRequestSchema, validateRequest, formatZodError, } from '../middleware/validation.js';
...
export function createAPIRouter(deps) {
     ...
     */
    apiRouter.post('/embed', async (req, res, next) => {
        const requestId = `req_${uuidv4()}`;
        const startTime = Date.now();
        ...
        try {
            ...
            const validation = validateRequest(isBatch ? EmbedRequestSchema : EmbedRequestSchema, req.body);
            if (!validation.success) {
                const response = {
                    success: false,
                    error: {
            ...
            const embedRequest = validation.data;
     ...
     */
    apiRouter.post('/classify', async (req, res, next) => {
        const requestId = `req_${uuidv4()}`;
        const startTime = Date.now();
        ...
        try {
            ...
            const classifyRequest = validation.data;
            // Classify
            const result = await llmRouter.classify(classifyRequest.text, classifyRequest.labels, classifyRequest.provider);
            const response = {
                success: true,
                data: {
                    label: result,
