/**
 * GENIE Household Service - Main Entry Point
...
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/genie-household';
...
app.use((err: AppError, _req: Request, res: Response, _next: NextFunction) => {
  ...
  if (err.name === 'ValidationError' || err.code === 'VALIDATION_ERROR') {
    res.status(400).json({ success: false, error: { code: 'VALIDATION_ERROR', message: err.message }, meta: { timestamp: new Date().toISOString() } });
    return;
  }
  res.status(err.statusCode || 500).json({ success: false, error: { code: err.code || 'INTERNAL_ERROR', message: NODE_ENV === 'production' ? 'An error occurred' : err.message }, meta: { timestamp: new Date().toISOString() } });
});
